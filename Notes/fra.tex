...
A \defin{source-free electromagnetic field}\index{source-free!electromagnetic field}\index{electromagnetic field!source-free --} is a 2-form $\w{F}$ on spacetime
that obeys the \defin{source-free Maxwell equations}:
\be
    \dd \w{F} = 0 \qand  \wnab\cdot\vvw{F}  = 0
\ee
where $\dd\w{F}$ is the exterior derivative of $\w{F}$ (cf. Sec.~\ref{s:bas:ext_deriv})
and $\vvw{F}$ in the type-$(2,0)$ antisymmetric tensor (bivector) associated by metric duality
to $\w{F}$ [cf. Eqs.~(\ref{e:bas:arrow_double}) and (\ref{e:bas:arrow_double_index})].

The \defin{electrovacuum Einstein equation}\index{electrovacuum!Einstein equation} is the Einstein equation (\ref{e:fra:Einstein_eq})
...
\ee
where $\mu_0$ is a constant called the \defin{vacuum permeability}\index{vacuum!permeability}.
...
Einstein equation (\ref{e:fra:Einstein_eq}) with $\w{T}$ given by Eq.~(\ref{e:fra:T_em}).
