...
A timelike vector is neither orthogonal to a timelike vector nor
...
the metric $\w{g}$ obeys \defin{Einstein equation}\index{Einstein!equation}:
...
$\w{R} - R/2\, \w{g}$ is identically zero for any metric tensor $\w{g}$ if $n=2$
(consider the trace of Eq.~(\ref{e:bas:Riem_n_2}) in Appendix~\ref{s:bas}).
The exclusion of $n=2$ follows also by noticing that
the Einstein-Hilbert action\index{Einstein-Hilbert action},
which gives birth to Eq.~(\ref{e:fra:Einstein_eq})
for $n\geq 3$, reduces (via the Gauss-Bonnet formula\index{Gauss-Bonnet formula}) to the
...
(\ref{e:fra:Einstein_eq_n}) would have no mathematical meaning for $n=2$
