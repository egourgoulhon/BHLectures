...
that several coordinate systems are required to cover $\M$, while a single one is obviously
sufficient for $\R^n$.
...
\begin{tlemma}{scalar product of a timelike vector with a causal vector}{fra:lem1}
Let $(\M,\w{g})$ be a time-orientable spacetime and $\w{u}$ a future-directed
timelike vector. For any null or timelike (nonzero) vector $\w{v}$, one has necessarily
$\w{g}(\w{u},\w{v}) \neq 0$ and
\begin{subequations}
\begin{align}
...
\begin{tlemma}{scalar product of a null vector with a causal vector}{fra:lem2}
Let $(\M,\w{g})$ be a time-orientable spacetime and $\w{u}$ a future-directed
null vector. For any null or timelike vector $\w{v}$, one has
\begin{subequations}
\begin{align}
...
\begin{tprop}{no causal orthogonality to a timelike vector}{fra:corol1}
A timelike vector is never orthogonal to a timelike vector or to a null vector.
\end{tprop}

From the part (\ref{e:fra:orient_null2}) of Lemma~\ref{p:fra:lem2}, we get

\begin{tprop}{orthogonality of two null vectors}{fra:corol2}
...
\be \label{e:fra:def_mass}
   m := \sqrt{- \w{p}\cdot\w{p}} .
\ee
Of course, for a massless particle, Eq.~\eqref{e:fra:def_mass} leads to $m=0$.
...
Since the Euler characteristic is a topological invariant independent of $\w{g}$,
the Einstein-Hilbert action is a constant and therefore its extremization is trivial
and cannot give rise to any differential equation ruling $\w{g}$.
\end{remark}
...
\ee
In the mathematical literature, a solution $\w{g}$ to Eq.~(\ref{e:fra:vac_Einstein_Lambda})
...
\defin{vacuum Einstein equation}\index{Einstein!equation!vacuum --}\index{vacuum!Einstein equation}:
 \be \label{e:fra:vac_Einstein}
    \encadre{ \w{R}  = 0 }.
\ee
It thus corresponds to the vanishing of the Ricci tensor. Solutions $\w{g}$ to
