...
one has necessarily $\weps(\w{e}_0,\ldots, \w{e}_{n-1}) \not =0$
...
iff $\weps(\w{e}_0,\ldots, \w{e}_{n-1}) > 0$ or $<0$, respectively.
The components of $\weps$ with
...
it is applicable only to a specific kind of tensor fields: the differential forms (Sec.~\ref{s:bas:ext_deriv}).
...
One can use the flow $\w{u}$ to transport the vector $\w{v}$ from one point $p$ to
...
$\Phi_{\veps*} \left. \w{v} \right| _p$ as defined in Sec.~\ref{s:bas:push_pull}.
...
$\left. \w{v} \right| _q$ in $T_p\M$ via the pushforward by $\Phi_\veps^{-1}$ and comparing it with
...
readily the so-called \defin{Killing expression}\index{Killing!expression}
...
$(p+1)$-form that is denoted $\dd\w{\omega}$ and whose components
...
\end{align}
\end{subequations}
It can be easily checked that these formulas, although expressed in terms of
...
all partial derivatives in formulas
...
The exterior derivative enters in the well known \defin{Stokes' theorem}\index{Stokes!theorem}:
if $\mathscr{D}$ is a $d$-dimensional submanifold with boundary of $\M$, then for any
$(d-1)$-form $\w{\omega}$,
\be \label{e:bas:Stokes}
   \encadre{ \int_{\partial\mathscr{D}} \w{\omega} =
    \int_{\mathscr{D}} \dd\w{\omega} },
\ee
where $\partial\mathscr{D}$ is the boundary of $\mathscr{D}$ (cf. Sec.~\ref{s:bas:manif_boundary}).
Note that $\partial\mathscr{D}$ is a manifold of dimension $d-1$ and
...
Given a vector field $\w{v}$ on a pseudo-Riemannian manifold $(\M,\w{g})$ of dimension $n$,
...
is denoted\footnote{More generally, the dot notation stands for the contraction on adjacent indices; here: $(\w{v}\cdot\weps)_{\alpha_1\cdots\alpha_{n-1}} := v^\mu \eps_{\mu\alpha_1\cdots\alpha_{n-1}}$.}
$\w{v}\cdot\weps$ and is a $(n-1)$-form. Its exterior
...
The Ricci tensor naturally appears in the \defin{contracted Ricci identity}\index{Ricci!identity!contracted --}:
