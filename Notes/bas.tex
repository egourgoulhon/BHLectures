...
\cite{Lafon15,Lee13,Lee18,ONeil83,Berge03,ChoquDD82,Colli26,Eschr11}, as well as
...
according to standard usage, the symbol for the vector $\w{e}_\alpha$ and that for the linear form $\w{e}^\alpha$ differ only by the position of the index $\alpha$.}
...
$\forall \w{v}\in{T_p\M},\ \w{g}(\w{u},\w{v}) = 0$ is necessarily the zero vector.
\end{enumerate}
...
Moreover, when there is no ambiguity on the metric (usually assumed to be the spacetime metric), we are
...
and  linear forms, following standard usage:
...
is given by\footnote{See e.g. Eq.~(14.29) in Ref.~\cite{Lee13}.}
\begin{align}
...
by covariant derivatives taken with respect to the Levi-Civita connection
of $\w{g}$, namely $\wnab$,
...
the Riemann tensor.
\begin{remark}
The decomposition (\ref{e:bas:Weyl}) is valid for $n=3$ as well; however,
