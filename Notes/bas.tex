...
(cf.~Remark~\ref{r:bas:topol_manif}), meaning that one has the notion of continuity, but not of differentiability. To get the latter, one should rely on the differentiable structure of $\R^n$, via the atlases:
a \defin{differentiable manifold}\index{differentiable!manifold}\index{manifold!differentiable --}
or \defin{smooth manifold}\index{smooth!manifold}\index{manifold!smooth --},
...
$\phi : \M \rightarrow \M'$ is \defin{differentiable map}\index{differentiable!map} iff in some (and hence all, thanks to the smoothness of (\ref{e:bas:transition_map})) coordinate systems
...
it is bijective and both $\phi$ and $\phi^{-1}$ are differentiable. This implies $n=n'$.

\begin{remark}
Strictly speaking a differentiable manifold is a pair $(\M,\mathcal{A})$  where
...
the \defin{infinitesimal displacement vector from $p$ to $q$}\index{infinitesimal displacement vector} as the vector $\D\w{x}\in{{T_p\M}}$ whose action on a scalar field $f$ is
...
Let $\M$ and $\mathscr{N}$ be two differentiable manifolds
and
\be
    \Phi:\ \M \longrightarrow \mathscr{N}
\ee
be a differentiable map (cf. Sec.~\ref{s:bas:def_manif}).
...
The differentiable map $\Phi$ is called an \defin{immersion}\index{immersion} iff
...
$\mathscr{S}$ has a differentiable structure with respect to which
...
One has necessarilly $\dim \mathscr{S} \leq \dim \M$. The non-negative integer
...
A \defin{pseudo-Riemannian manifold}\index{pseudo-Riemannian manifold}\index{manifold!pseudo-Riemannian} is a pair $(\M,\w{g})$ where $\M$ is a differentiable manifold
...
The vector field $\w{n}$ is called a \defin{normal}\index{normal!to a hypersurface}
 to $\mathscr{S}$. All normal vectors are colinear to each other.
...
The third one depends only on the differentiable-manifold structure, i.e.
