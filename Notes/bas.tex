...
Contrary to what happens for an affine space, one cannot, in general, define a vector connecting two distinct points $p$ and $q$ on a manifold, except if $p$ and $q$ are infinitesimally close to each other. Indeed, in the latter case, one defines
...
consider that $\w{v}$ is the tangent vector
to a curve $\Li$ through $p$ associated to some parameter $\lambda$ and let $q$ be
...
\be
    ...
    \left. \Phi^* \w{T} \right| _p := \left. \w{T} \right| _{\Phi(p)}
...
(essentially because it carries curves in $\M$ to curves in $\mathscr{N}$, as discussed above),
while it carries covariant tensors, among which linear forms, in the reverse way, i.e. from
...
pushforward extends to vector fields is when $\Phi$ is a diffeomorphism.
\end{remark}
...
The third one, called the \emph{exterior derivative} $\dd$, depends only on the smooth-manifold structure, i.e.
it is independent of any (metric or vector) field; on the other side,
it is applicable only to a specific kind of tensor fields: the $p$-forms (Sec.~\ref{s:bas:ext_deriv}).
...
Sec.~\ref{s:bas:affine_connect} is an affine connection, possibly (by not necessarily)
provided by a metric tensor (case of the Levi-Civita connection).
...
value via $\w{u}$. More precisely (cf. Fig.~\ref{f:bas:deriv}), given an infinitesimal
parameter $\veps$, let us first define the \defin{flow map along}\index{flow!map} $\w{u}$
as the map $\Phi_\veps: \M \to \M$ such that
$\Phi_\varepsilon(p)=q$, where $q$ is the point connected to $p$ by the infinitesimal
displacement vector
$\overrightarrow{pq}=\varepsilon \left. \w{u} \right| _p$ (cf. Sec.~\ref{s:bas:vectors}).
The natural reference vector to compare $\left. \w{v} \right| _q$ with is then the pushforward of
$\left. \w{v} \right| _p$ by $\Phi_\varepsilon$

...

%@@@@@@
If we multiply the vector $\w{v}(p)$ by
...
infinitesimal vector at $q$, which, up to a scale factor $\lambda$,
can be interpreted as the vector $\w{v}(p)$ transported along the field lines
of $\w{u}$ by the distance $\varepsilon$:
...
by the flow map $\Phi_\varepsilon$. Actually it is nothing but
...
actual value of the field $\w{v}$ at $q=\Phi_\varepsilon(p)$ since both
vectors belong to the same vector space $T_q\M$;
this leads to the definition of the
\defin{Lie derivative}\index{Lie!derivative}\index{derivative!Lie --}
of $\w{v}$ along $\w{u}$ at the point $p$:
\be \label{e:bas:def_Lie_der}
    \encadre{
    \left. \Lie{u} \w{v} \right| _p := \lim_{\varepsilon\rightarrow 0} \frac{1}{\varepsilon}
...
\begin{remark}
The term $\w{v}\left(\Phi_\varepsilon(p)\right) - \Phi_{\veps*}\left(\w{v}(p)\right)$
...
only because $q = \Phi_\veps(p) \to p$ in the limit $\veps \to 0$. Some authors (e.g. \cite{Lee13,Wald84}) prefer to define the Lie derivative from a difference of vectors of $T_p\M$, by carrying $\w{v}(q)$
in $T_p\M$ via the pushforward by $\Phi_\eps^{-1}$ and comparing it with $\w{v}(p)$,
which results in the formula
\be \label{e:bas:def_Lie_der_alt}
    \left. \Lie{u} \w{v} \right| _p := \lim_{\varepsilon\rightarrow 0} \frac{1}{\varepsilon}
    \left[ (\Phi_\veps^{-1})_* (\w{v}\left(\Phi_\varepsilon(p)\right)) - \w{v}(p) \right] .
\ee
...
to the geometrical construction of Fig.~\ref{f:bas:deriv}.
\end{remark}
...
Given the definitions of points $q$, $p'$ and $q'$, we have
\[
  x^\alpha(q) = x^\alpha(p)+\varepsilon \delta^\alpha_{\ \ 0} ,\quad
  x^\alpha(p')= x^\alpha(p)+ \lambda v^\alpha(p) , \quad
...
\ee
The \defin{Lie derivative}\index{Lie!derivative}\index{derivative!Lie --} of $\w{T}$ along $\w{u}$ is then defined
