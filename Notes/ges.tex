...
\[
    ...
    \end{cases}
\]
We thus perform the change of variable $x=2u + 1/3$.
For the case $u<1/3$ ($\iff r> 3m$), which implies $x\in (0,1)$, we arrive
at
...
Fig.~\ref{f:ges:null_b_crit_from_inf} shows such a null geodesic with
$\ph_\infty=0$, which implies $\ph_0 = - \ph_*$ and $\ph>0$.
This geodesic spirals onto the photon
circular orbit at $r=3m$ when $\ph\rightarrow +\infty$,
which corresponds to the horizontal asymptote in
the right part of Fig.~\ref{f:ges:null_r_phi_bcrit}.
Note that Fig.~\ref{f:ges:null_b_crit_from_inf} describes a
geodesic with $L>0$ ($\ph$ increasing towards the future) arising from $r\rightarrow + \infty$
and spiraling inwards ($r$ decaying towards the future) to the photon circular orbit
as well as a geodesic with $L<0$ ($\ph$ decaying towards the future)
and spiraling outwards ($r$ increasing to the future), escaping to $r\rightarrow +\infty$.
...
\begin{figure}
\centerline{\includegraphics[width=0.7\textwidth]{ges_null_b_crit_from_inf}}
\caption[]{\label{f:ges:null_b_crit_from_inf} \footnotesize
Trace in the equatorial plane of a
null geodesic in the region $r>3m$ with an impact parameter
equal to the critical one: $b = 3\sqrt{3} \, m \simeq 5.196 m$ and $\ph_0 = -\ph_*$.}
\end{figure}


Similarly, for $u>1/3$ ($\iff r< 3m$), which implies $x\in (1,+\infty)$, we
get
\be \label{e:ges:null_r_phi_coth}
    r = \frac{2m}{\coth^2 \left(\frac{\ph - \ph_0}{2} \right) - \frac{1}{3}} .
\ee
Again, $\ph_0$ is an integration constant. It cannot be determined by the
value of $\ph$ when $r\rightarrow +\infty$ since $r<3m$.
