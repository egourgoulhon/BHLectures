...
We note that Eq.~(\ref{e:ges:1d_motion_timelike}) takes the shape of
the 1-dimensional motion of a non-relativist particle in the potential
$V_{\ell}$, the term $1/2 \, (\D r/\D\tau)^2$ being interpreted as
...
quadratic equation in $r$ admits real roots iff $|\ell| \geq \ell_{\rm crit}$,
with $\ell_{\rm crit} = 2\sqrt{3}\, m$ [Eq.~(\ref{e:ges:ell_crit})].
The two roots are then
\be \label{e:ges:r_circ_ell}
    \encadre{ r_{\rm circ}^\pm(\ell) = \frac{\ell^2}{2m} \left(
     1 \pm \sqrt{1 - \left( \frac{\ell_{\rm crit}}{\ell} \right) ^2 } \right) }.
\ee
