...
\subsection{Equations to be solved} \label{s:ges:eq_to_be_solved}
...
(Sgr A*) \cite{GenzeEG10,Johan16} and the massive black hole in the nucleus of the galaxy
M87 \cite{Gebha_al11}.}
\end{table}
...
dimension of $L$, i.e. a squared length, given that $\lambda$ is dimensionless
...
Indeed, let us consider that the massless
particle $\mathscr{P}$ arises from a point of coordinates $\theta=\pi/2$,
$r=r_0 \gg m$ and $\ph = \ph_0$.  Let us introduce in the plane $\theta=\pi/2$
Cartesian coordinates $(x,y)$ in the standard way:
...
location of the event horizon. The horizontal line marked ``1'' (resp. ``2'')
corresponds to the $r$-trajectory of a massless particle coming from infinity
with a small (resp. large) impact parameter.}
\end{figure}
...
orbit at $r=3m$ is an unstable orbit. So one should not imaginate that a
...
particles, like, e.g., neutrinos.

We infer from the effective first integral (\ref{e:ges:dot_r_square_null_b})
that a photon coming from infinity with an impact parameter
$b$ small enough to fulfil $b^{-2} > U_{\rm max}$ (cf. trajectory
no.~1 in Fig.~\ref{f:ges:eff_pot_null}) passes above the potential barrier
constituted by $U(r)$. It then reaches arbitrary small values of $r$ and
is eventually absorbed by the black hole ($r < 2m$).
On the contrary, a photon coming from infinity with an impact parameter large enough
to guarantee $b^{-2} < U_{\rm max}$ (cf. trajectory
no.~2 in Fig.~\ref{f:ges:eff_pot_null}) ``bounces'' on the potential barrier
at some minimal value  $r_{\rm per}$ of $r$ --- the \defin{periastron}\index{periastron},
which is given by  $U(r_{\rm per}) = b^{-2}$. Equation~(\ref{e:ges:dot_r_square_null_b})
implies then
\be
    \left. \frac{\D r}{\D\tilde{\lambda}} \right| _{r = r_{\rm per}} = 0 .
\ee
Actually, $\D r / \D\tilde{\lambda}$ changes sign at $r = r_{\rm per}$ and
the photon goes back to
infinity (generically in a direction different from the incoming one): the
particle is scattered by the black hole.

The critical value of the impact parameter distinguishing the absorption
by the black hole from the scattering off it is given by $b_{\rm crit}^{-2} = U_{\rm max}$.
Given the value (\ref{e:ges:U_max}) of $U_{\rm max}$, we get
\be
    \encadre{b_{\rm crit} = 3\sqrt{3} \, m }.
\ee
