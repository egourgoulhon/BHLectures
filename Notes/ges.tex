\chapter{Generic and timelike geodesics in Schwarzschild spacetime}
\label{s:ges}
...
\subsection{Trajectories in the orbital plane}
...
\be \label{e:ges:DuDph_trajectories}
    \encadre{
    \frac{\D u}{\D\ph}  = \pm \sqrt{ 2 u^3 - u^2 + 2 \left( \frac{m\mu}{L} \right)^2 u
...
\ee
which is obviously of the type (\ref{e:ges:Weierstrass_eq}). The solution is
thus
\be
    u = \wp\left( \frac{\ph}{2} + C; \omega_1, \omega_2 \right) + \frac{1}{6} ,
\ee
where $C\in \mathbb{C}$ is a contant and $\omega_1$ and $\omega_2$ are
determined by $m$, $\mu$, $E$ and $L$.
\item The second approach amounts to write the solution of
Eq.~(\ref{e:ges:DuDph_trajectories}) as
\be
...
We shall detail such a method for null geodesics in Sec.~\ref{s:gis:polar_equations}.
Note that this approach leads to $\ph = \ph(r)$, whereas the method based of the
Weierstrass function leads to $r = r(\ph)$. It is possible though to
get the latter form by invoking the inverses of elliptic integrals, namely the
...
(Sec.~\ref{s:ges:timelike}) and for null ones (Chap.~\ref{s:gis}).
...
extrema: a mimimum $r_{\rm per}$, for \defin{periastron}\index{periastron} (or \defin{pericenter}\index{pericenter} or \defin{periapsis}\index{periapsis}), and
a maximum
