...
has a double zero: $u=1/3$ and a negative zero: $u=-1/6$; only the first
one is physical and correspond to $r=3m$.
\item If $\Delta>0$ $\iff$ $|b| > b_{\rm crit}$, $P_b(u)$ has three distinct
real zeros; one of them, $u_{\rm neg}$ say, is
...
\bea
  ...
  & = & 2 \int_0^{u_{\rm per}} \frac{\D u}{\sqrt{2 u^3 - u^2 + (m/b)^2}} .
\eea
...
$u\in(0,u_{\rm per})$. For $b>b_{\rm crit}$, $u_{\rm apo}>u_{\rm per}$ and
...
\]
is finite, being equal to $2\sqrt{u_{\rm per}}$. When $b\to b_{\rm crit}$,
...
\]
which is diverging. We recover the behavior obtained for
the external critical null geodesic ($b=b_{\rm crit}$, $r>3m$):
