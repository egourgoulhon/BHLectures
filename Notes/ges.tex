...
screen. This is particularly true for image 3 and this agrees with the law
(\ref{e:ges:b_bc_exp_Theta}): geodesics that perform $n$ turns around the
black hole have an impact parameter exponentially close to $b_{\rm crit}$,
by a factor $\mathrm{e}^{- 2\pi n}$, with $\mathrm{e}^{- 2\pi}\simeq 1.9\times 10^{-3}$
(see also Fig.~\ref{f:ges:null_b2}).

For the sake of clarity, only three images of the source $S$ have been
depiced in Fig.~\ref{f:ges:mult_images}, but there are actually an infinite
number of them: two per value of $n$ (the number of turns around the black),
one with $L>0$ and the second one with $L<0$. The selected geodesics in
Fig.~\ref{f:ges:mult_images} have $n=0$ and $L<0$ (image 1), $n=0$ and $L>0$
(image 2) and $n=1$ and $L<0$ (image 3).
