...
Proper time elapsed to reach the event horizon ($\tau_{\rm h}$, dashed curve)
...
flat region $r\gg 2m$, since $\veps \geq \mu$ there, which implies $\bar{\veps} \geq 1$.
For this reason, the circular orbit at $r=4m$ is called the
\defin{marginally bound orbit}\index{marginally bound!orbit}\index{orbit!marginally bound --}. Not that the marginally bound orbit is unstable, since it has $r<6m$.
