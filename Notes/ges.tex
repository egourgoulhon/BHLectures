...
\ee
correspond to points for which $\D\ph/\D u \rightarrow \pm \infty$,
or equivalently $\D u /\D\ph \rightarrow 0$. They are thus stationary points
...
Mathematically, the zeros of $P_b(u)$ are governed by its
...
for physical values of $u$ ($u\in\mathbb{R}^+$).
\item If $\Delta=0$ $\iff$ $|b| = b_{\rm crit}$, $P_b(u)$
has a double zero $u=1/3$ and a negative zero: $u=-1/6$.
\item If $\Delta>0$ $\iff$ $|b| > b_{\rm crit}$, $P_b(u)$ has three distinct
zeros; one of them, $u_{\rm neg}$ say, is
...
two cases : $u<1/3 \iff x\in (0,1)$ and $u>1/3 \iff x\in (1,+\infty)$.
...
\ee
Moving back to $r = m/u$, we obtain the equation of the null geodesic in polar form:
\be \label{e:ges:null_r_phi_tanh}
    r = \frac{2m}{\tanh^2 \left(\frac{\ph - \ph_0}{2} \right) - \frac{1}{3}} .
\ee
The constant $\ph_0$ can be related to the asymptotic value $\ph_\infty$ of $\ph$
when $r\rightarrow +\infty$ by setting $u=0$ in Eq.~(\ref{e:ges:u_ph_crit_null_geod});
...
When $\ph\rightarrow +\infty$, the geodesic rolls up indefinitely onto the photon
...
Note that Fig.~\ref{f:ges:null_b_crit_from_inf_L_pos} describes both (i) the trace of a
geodesic with $L>0$ (so that $\ph$ increases towards the future, cf. Eq.~(\ref{e:ges:dot_ph_null})) arising from $r\rightarrow + \infty$
...
and spiraling outwards, escaping to $r\rightarrow +\infty$ as $\ph\rightarrow 0$.
...
Fig.~\ref{f:ges:null_b_crit_from_inf_L_neg} is the trace of two distinct
geodesics: one with $L<0$ arising from $r\rightarrow + \infty$
and spiraling inwards to the photon orbit when $\ph\rightarrow -\infty$
...
which implies $x\in (1,+\infty)$ in Eq.~(\ref{e:ges:primitive_crit_geod}).
Equation~\eqref{e:ges:dphi_du_null_bcrit} is then integrated to
\be \label{e:ges:null_r_phi_coth}
    r = \frac{2m}{\coth^2 \left(\frac{\ph - \ph_0}{2} \right) - \frac{1}{3}} .
\ee
Again, $\ph_0$ is an integration constant. But this time, it cannot be determined by the
value of $\ph$ when $r\rightarrow +\infty$ since we are in the case $r<3m$.
...
For $\ph\rightarrow -\infty$, it rolls up indefinitely onto the photon
orbit ($r=3m$) from below.
The plotted curve actually represents the trace in the equatorial plane of two
kinds of geodesics: (i) a geodesic with $L>0$ arising
from $r=r_{\rm em} < 3 m$, $\ph=\ph_{\rm em}<0$ and inspiralling to the
central singularity as $\ph\rightarrow 0$ and (ii) a geodesic with $L<0$ arising from
$r = r_{\rm em} \in (2m,  3 m)$, $\ph=\ph_{\rm em}<0$ and spiralling outward
to the photon orbit as $\ph\rightarrow -\infty$. Note that for (ii), the
...
central singularity as $\ph\rightarrow 0$ and (ii) a geodesic with $L>0$ arising from
$r = r_{\rm em} \in (2m,  3 m)$, $\ph=\ph_{\rm em}>0$ and spiralling outward
to the photon orbit as $\ph\rightarrow +\infty$.
...
Figures~\ref{f:ges:null_b1}-\ref{f:ges:null_b2} show traces in the equatorial plane from null
...
\bea
    \Delta \ph &=& - \int_{+\infty}^{r_{\rm per}} \frac{1}{r^2} \left( \frac{1}{b^2}
        - \frac{1}{r^2} +  \frac{2m}{r^3} \right) ^{-1/2} \D r
