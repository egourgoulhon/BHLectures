...
so that $\Li$ lies in the plane $\th=\pi/2$.
...
does define a new affine parameter. In terms of it,
...
The null geodesics are thus entirely determined by the parameter $b$. The latter
has the dimension of a length and can be interpreted as the
impact parameter\index{impact parameter} in the case of particle arising from
infinity. Indeed, let us consider that the massless
particle $\mathscr{P}$ arises from a point of coordinates $\theta=\pi/2$,
$r=r_0 \gg m$ and $\ph = \ph_0 \gg 1$. In Cartesian coordinates
