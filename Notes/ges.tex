...
if the turning point corresponds to a minimum of $r$ (which can occur
only for $r>3m$), the null geodesic escapes to infinity, while if the turning
point corresponds to a maximum of $r$ (which can occur only for $r<3m$), the
null geodesic terminates into the black hole.
\end{greybox}
...
Figures~\ref{f:ges:null_b1}-\ref{f:ges:null_b2} show some null geodesics in
the equatorial plane $\theta=\pi/2$ coming from infinity along the
direction $\ph=0$, for various values of the
impact parameter $b$ decaying to $b_{\rm crit}$.
