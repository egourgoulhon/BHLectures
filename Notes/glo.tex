...
while $(\theta,\phi)$ are standard polar coordinates on $\mathbb{S}^2$:
...
and $\w{T}=0$ (vacuum). Using coordinates $(x^\alpha)=(\tau,\chi,\theta,\phi)$ with
...
constant, as follows:
\begin{greybox}
If $\w{g}$ is a solution of Einstein equation with
...
involves a subcategory of conformal completions:
...
whose components in the so-called \emph{global coordinates}
$(x^\alpha)=(t,\chi,\theta,\phi)$ are given by
\be \label{e:glo:dS4}
    g_{\mu\nu} \D x^\mu \D x^\nu = \ell^2 \left[ -  \D t^2
    + \cosh^2 t \left(
    \D \chi^2 + \sin^2\chi \left( \D\th^2 + \sin^2\th \, \D\ph^2 \right) \right) \right] ,
\ee
where $\ell$ is a positive constant. Note that $t$ spans $\mathbb{R}$
while $(\chi,\theta,\phi)$ are standard polar coordinates on $\mathbb{S}^3$:
$\chi\in(0,\pi)$, $\theta\in(0,\pi)$ and $\phi\in(0,2\pi)$.
...
\be
    g_{\mu\nu} \D x^\mu \D x^\nu = \frac{\ell^2}{\cos^2\tau} \left[ - \D \tau^2
    + \D \chi^2 + \sin^2\chi \left( \D\th^2 + \sin^2\th \, \D\ph^2 \right) \right] .
\ee
Defining $\Omega := \ell^{-1}\cos\tau = (\ell\cosh t)^{-1}$, we notice that
a conformal completion of $(\M,\w{g})$ is $(\tilde{\M},\w{\tilde{g}})$
where (i) $\tilde{\M}$ is the part $-\pi/2\leq \tau \leq \pi/2$ of the Einstein cylinder\footnote{Recall that on the
Einstein cylinder the range of $\tau$ is $(-\infty,+\infty)$, cf. Eq.~(\ref{e:glo:range_tau_chi}).}
introduced in Sec.~\ref{s:glo:conf_complet_Mink}
and (ii)  $\w{\tilde{g}}$ is the metric (\ref{e:glo:tg_Einstein}).
