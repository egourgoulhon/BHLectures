...
a rigourous proof can be found in \cite{ChrusDGH01}),
...
\subsection{Null stationary Killing field on $\Hor$}
...
hypersurface-orthogonal everywhere, i.e. that the spacetime $(\M,\w{g})$ is \defin{static}\index{static spacetime}. One can then
apply a uniqueness theorem by W.~Israel (1967) \cite{Israe67}
stating that every static vacuum solution
of the Einstein equation is the Schwarzschild solution\index{Schwarzschild!black hole} (or the Reissner-Nordström\index{Reissner-Nordström black hole}
one in the electro-vacuum case \cite{Israe68}).
A demonstration of Israel's theorem can be found in Straumann's textbook \cite{Strau04}.

\subsection{Spacelike stationary Killing field on $\Hor$}

When $\w{\xi}$ is spacelike on $\Hor$, it obviously cannot be collinear to
any null normal $\wl$, so that one cannot say a priori that
$\Hor$ is a Killing horizon. When $n=4$, it turns out
that this is indeed the case, thanks to a famous result by S.W.~Hawking (1972)
\cite{Hawki72}, known as the
\emph{strong rigidity theorem}\index{strong!rigidity theorem}\index{rigidity theorem!strong --}.
By means of the Einstein equation
Hawking was able to show that there exists a second Killing vector field,
$\w{\chi}$ say, which is null on $\Hor$, so that $\Hor$ is a Killing horizon
