...
whose components in the global hyperbolic coordinates $(x^\alpha)=(\tau,\rho,\theta,\phi)$
are given by
\be \label{e:glo:AdS4}
...
\ee
Defining $\Omega := \cos\chi / \ell = 1/(\ell\cosh\rho)$, we notice that
...
of the Einstein cylinder; $\scri$ is spanned by the coordinates $(\tau,\th,\ph)$
and it topology is that of a 3-dimensional cylinder: $\scri \simeq \mathbb{R}\times\mathbb{S}^2$.
...
a cosmological constant $\Lambda$ [Eq.~(\ref{e:fra:Einstein_eq})], then
\begin{itemize}
