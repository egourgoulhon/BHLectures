...
The SageMath notebook (cf.~Appendix~\ref{s:sam}) leading to this figure
is available at {\scriptsize \url{http://nbviewer.jupyter.org/github/sagemanifolds/SageManifolds/blob/master/Worksheets/v1.0/SM_AdS.ipynb}}.}
\end{figure}

\begin{example}[conformal completion of AdS$_{4}$ spacetime]
The 4-dimensional \defin{anti-de Sitter spacetime}\index{anti-de Sitter spacetime} is
$(\M,\w{g})$ with $\M\simeq \mathbb{R}^4$ and $\w{g}$ is the metric
whose components in the so-called \emph{global coordinates}
$(x^\alpha)=(\tau,\rho,\theta,\phi)$ are given by
\be \label{e:glo:AdS4}
    g_{\mu\nu} \D x^\mu \D x^\nu = \ell^2 \left[ - \cosh^2\rho \, \D \tau^2
    + \D \rho^2 + \sinh^2\rho \left( \D\th^2 + \sin^2\th \, \D\ph^2 \right) \right] ,
\ee
where $\ell$ is a positive constant related to the cosmological constant $\Lambda$
by $\Lambda = - 3/\ell^2$. The metric (\ref{e:glo:AdS4}) is then a solution
of the Einstein equation (\ref{e:fra:Einstein_eq}) with $\w{T}=0$
(vacuum). Using coordinates $(x^\alpha)=(\tau,\chi,\theta,\phi)$ with
$\chi = \mathrm{atan}(\sinh\rho) \in (0,\pi/2)$, one gets
\be
    g_{\mu\nu} \D x^\mu \D x^\nu = \frac{\ell^2}{\cos^2\chi} \left[ - \D \tau^2
    + \D \chi^2 + \sin^2\chi \left( \D\th^2 + \sin^2\th \, \D\ph^2 \right) \right] .
\ee
...
More generally, if $\w{g}$ is a solution of Einstein equation with
a cosmological constant $\Lambda$ [Eq.~(\ref{e:fra:Einstein_eq})]
and an energy-momentum tensor decaying to zero at $\scri$,
then
\begin{itemize}
...
\end{itemize}
\begin{proof}
It follows from $\w{\tilde{g}} = \Omega^2 \w{g}$ that, on $\M$, the Ricci scalars $\tilde{R}$
...
\[
    ...
    \right)
\]
This equation is a priori valid in $\M$ only. Taking the limit $\Omega\rightarrow 0$ and
...
the left-hand side of this equation is nothing but the scalar square, with respect
to the conformal metric, of the vector $\w{n}$ normal to $\scri$,
whose components are $n^\alpha = \tilde{g}^{\alpha\mu} \partial_\mu \Omega$
(remember that by hypothesis 4 in the definition
of a conformal completion, $\dd\scri$ is not vanishing on $\scri$, so that
$\w{n}$, which is its dual via $\w{\tilde{g}}$, is a valid normal
...
Penrose\index{Penrose, R.} \cite{Penro64,Penro68} has defined
...
Penrose\index{Penrose, R.} \cite{Penro68} has introduced the following definition:
...
\begin{hist}
The term \emph{event horizon} has been introduced by Wolfgang Rindler\index{Rindler, W.} in 1956 \cite{Rindl56}
...
\subsubsection{Property 3 (Penrose\index{Penrose, R.} 1968 \cite{Penro68}):}
\begin{greybox}
