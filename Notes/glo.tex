...
Far from the black hole region, i.e. near $\scri^+$ and $\scri^-$, the spacetime is close to the de Sitter spacetime\index{de Sitter spacetime}, discussed briefly in Example~\ref{x:glo:dS4} on p.~\pageref{x:glo:dS4}. The exists two values, $r_{\rm h}$ and $r_{\rm c}$ say, of $r$ such that the hypersurfaces $r=r_{\rm h}$ or $r=r_{\rm c}$ are null. According to the criterium~(\ref{e:bas:char_null_coord}),
they are obtained as the two zeros $r_{\rm h}$ and $r_{\rm c}$ of the function
$g^{rr} = 1 - \frac{2 m}{r} - \frac{\Lambda}{3} \,  r^2$,
which is deduced from Eq.~\ref{e:sch:Kottler_metric}.
\end{example}
