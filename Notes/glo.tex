...
namely it offers a compactified view of the entire spacetime, with each point being actually a sphere $\mathbb{S}^2$ of
metric area $4\pi r^2$ (some hypersurfaces of constant $r$ are indicated in Fig.~\ref{f:glo:conf_Mink_null}) and the radial null geodesics are straight lines inclined
at $\pm 45^\circ$, so that the metric null cones appear everywhere as vert....
...
The left and right boundaries marked by dashed lines
must be identified.}
\end{figure}
...
whose Carter-Penrose diagram in shown in Fig.~\ref{f:glo:de_Sitter_BH}.
More precisely, Fig.~\ref{f:glo:de_Sitter_BH} represents one of the many maximal analytic extensions
...
obtained by attaching an arbitrary number (possibly infinite) of copies of the spacetime shown in Fig.~\ref{f:glo:de_Sitter_BH} along these edges, with the most extremal edges being identified for a finite
number of copies (see Sec.~9.4 of Ref.~\cite{GriffP09} and references therein).
In areal coordinates, the metric is given by Eq.~(\ref{e:sch:Kottler_metric}) with $0 < \Lambda < (9m^2)^{-1}$.
Far from the black hole region, i.e. near $\scri^+$ and $\scri^-$, the spacetime approaches the de Sitter spacetime\index{de Sitter spacetime}, discussed briefly in Example~\ref{x:glo:dS4} on p.~\pageref{x:glo:dS4}.
...
A black hole example in a spacetime with a timelike $\scri^+$ is provided by the
\defin{Schwarzschild-anti de Sitter black hole}\index{Schwarzschild-anti de Sitter!black hole},
cf. Fig.~\ref{f:glo:anti_dS_BH}. TBC.
\end{example}
