...
of the future null infinity $\scri^+$ that lies in $\M$.
...
contains some points (the caustics defined below) at which it is not differentiable.
\end{remark}
...
inside $\M$, the event horizon $\Hor$ must therefore be invariant under the group action.
Note that this means that $\Hor$ is invariant \emph{as a whole}, not that
each point of $\Hor$ is invariant under the group action. Equivalently, this
means that the generator $\w{\xi}$ of the isometry group is tangent to $\Hor$.
...
\begin{quote}
The Killing vector $\w{\xi}$ field is either null or spacelike on
...
In dimension $n=4$ and using Einstein equation,
D.~Sudarski and R.M.~Wald \cite{SudarW92} have then proven that $\w{\xi}$ must be
...
One can then apply a uniqueness theorem by W.~Israel (1967) \cite{Israe67},
and improved latter by many authors (in particular by Chru\'sciel \& Galloway \cite{ChrusG10}
to remove the hypothesis of analyticity), which
...
So basically, in dimension $n=4$ (i.e. when the staticity theorem applies) all stationary black holes with the stationary Killing field $\xi$ null
...
and J.~Isenberg (2008) \cite{MoncrI08}, under the hypothesis that $\Hor$
has compact cross-sections transverse to $\w{\xi}$ (see also
...
A linear combination of Killing vectors with constant coefficients being a
Killing vector, the vector field $\w{\eta}$ defined by
\be
    \w{\eta} = \frac{1}{\Omega_H} \left( \w{\chi} - \w{\xi} \right)
    \quad\iff\quad
    \w{\chi} = \w{\xi} + \Omega_H \w{\eta} ,
\ee
where $\Omega_H$ is a non-zero constant, is a Killing vector field on $\M$.
One can show (see e.g. \cite{Chrus97} for a rigorous proof) that $\Omega_H$
and some constant scale factor of $\w{\chi}$
can be chosen so that $\w{\eta}$ is a spacelike vector field with closed
field lines, with $2\pi$-periodicity in terms of the parameter $\ph$
...
called the \defin{ergoregion}\index{ergoregion} and its boundary $\partial\mathscr{E}$
...
black hole spacetime $(\M,\w{g})$
solution of the Einstein vacuum equation with a connected regular
...
\begin{remark}
In their original work, Carter and Robinson assumed that $\Hor$ is a
\emph{non-degenerate}
...
By combining the Israel theorem, the strong rigidity theorem and the Carter-Robinson theorem,
one arrives at the \defin{no-hair theorem}\index{no-hair theorem}:
\begin{quote}
Any stationary 4-dimensional asymptotically flat
black hole spacetime $(\M,\w{g})$
solution of the Einstein vacuum equation with a connected regular
event horizon $\Hor$ has a domain of outer communications that is isometric
to the domain of outer communications of the Kerr spacetime.
