...
\defin{Einstein static universe}\index{Einstein!static universe}\index{static!universe (Einstein)}, a static solution of the Einstein equation (\ref{e:bas:Einstein_eq})
...
Let $\tilde{\M}$ be the topological closure of $\M$ in $\mathscr{E}$.
$\tilde\M$ is a manifold with boundary. Denoting its boundary by $\scri$, we have
then:
\be
    \tilde{\M} = \M \cup \scri .
\ee
It is customary to pronounce $\scri$ as ``scri'', for \emph{script i}.
$\scri$ can be decomposed in five disjoint parts
(cf. Fig.~\ref{f:glo:glo_conf_diag_Mink}):
\be
    \scri = \scri^+ \cup \scri^- \cup \left\{ i^0 \right\} \cup
...
\item $i^-$ is the point of $\mathscr{E}$ defined by $\tau=-\pi$ and $\chi=0$.
\end{itemize}
Note that on $\mathbb{S}^3$, the hyperspherical coordinates $(\chi,\th,\ph)$
...
Note also that the vertical left boundary of the daimond drown in
Fig.~\ref{f:glo:glo_conf_diag_Mink}, i.e. the segment defined by
$\tau\in(-\pi,\pi)$ and $\chi=0$, is \emph{not} a part of the boundary $\scri$,
but merely reflect the coordinate singularity at $\chi=0$, in the same
