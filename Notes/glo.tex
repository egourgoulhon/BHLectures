...
(cf. Sec.~\ref{s:glo:causal_struct}), lies entirely in $J^-(\scri^+)$.
...
$I^+(q)$ is an open neighborhood of $p$. Given that $p$ lies on the boundary
...
$\scri^+$, hence a future-directed causal curve from $q$ to $\scri^+$, which proves that  $q\in J^-(\scri^+)$. We conclude that $I^-(p)\subset J^-(\scri^+)$.
On the other hand, let us show that the chronological future of $p$, $I^+(p)$,
lies entirely in $\mathscr{B}$. For any $q\in I^+(p)$,  we have $p\in I^-(q)$. Since $I^-(q)$ is open, it constitutes an open neighborhood of $p$. Given that $p\in\partial\mathscr{B}$, we have
...
For $p\in\Hor$, let us consider an open neighborhood $U$ of $p$ and a coordinate chart
$\Phi: U \to (-1,1)\times V$, $q\mapsto (x^0,x^1,\ldots,x^{n-1})$ where $V$ is an open subset of $\R^{n-1}$, such that $x^0(p) = 0$ and the vector $\wpar_0$ is future-directed timelike.
By choosing $U$ sufficiently small, we may ensure that the slice $x^0=-1/2$ lies in $I^-(p)$
and the slice $x^0=1/2$ lies in $I^+(p)$ (cf. Fig.~??). Any coordinate line defined by
...
the point $(x^\alpha)=(1/2,a_1,\ldots,a_{n-1})$ of $I^+(p)$. Given that $I^-(p)\subset \M\setminus\mathscr{B}$, $I^+(p)\subset\mathscr{B}$ and $\Hor = \partial\mathscr{B}$,
such a curve necessarily intersects  $\Hor$. Moreover the intersection is reduced
to a single point $p'\in\Hor$ for the curve is timelike and $\Hor$ is
achronal (Property~\ref{p:glo:prop1}).
Let us then give the coordinates $(y^i) = (a_1,\ldots,a_{n-1})$ to $p'$.
