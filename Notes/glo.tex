...
as\footnote{See also Sec.~\ref{s:sam:notebooks} for the computation
with SageManifolds.}
\be \label{e:glo:g_UV}
...
Penrose\index{Penrose, R.} has also introduced the following definition \cite{Penro68}:
...
story is that it has been coined by John~A.~Wheeler\index{Wheeler, J.A.} in the end of 1967,
...
Robert Dicke\index{Dicke, R.} in some lecture given in 1961.
...
by a causal curve with a timelike segment. By Lemma~2, this curve can
deformed into a timelike curve from $p$ to $q$, which
which contradicts
