...
Having in mind to define a black hole in full generality, we shall start by a naive definition
and some reflections about what the black hole boundary could be (Sec.~\ref{s:def:BH_null_hypsurf}).
We shall then convince ourselves that a rigorous definition of a black hole
requires taking into account the global structure of spacetime, in particular its
asymptotics.  This far-away structure is best
described in terms of the so-called \emph{conformal completion}, which we
discuss first for Minkowski spacetime (Sec.~\ref{s:glo:conf_Mink}),
before considering it for general spacetimes (Sec.~\ref{s:glo:conf_compl}).
This allows us to state the rigorous definition of a black hole (Sec.~\ref{s:glo:BH}),
and to study the main properties that directly follow from it (Sec.~\ref{s:glo:properties_H}),
independently of any gravity theory.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Preliminary reflections} \label{s:def:BH_null_hypsurf}

\subsection{A naive definition} \label{s:def:first_defin}
...
\begin{tdef}{``naive'' black hole}{glo:naive_BH}
...
a \emph{hypersurface} of the spacetime manifold $\M$, for it has to divide $\M$ in two regions:
the interior (the black hole itself) and the exterior.
By \defin{hypersurface}\index{hypersurface}, it is meant an
embedded submanifold of $\M$ of dimension $n-1$, or equivalently of
codimension 1
...
\subsection{Hypersurfaces in spacetime and black hole boundaries} \label{s:def:hor_as_null}
...
Hence the black hole boundary can either be a null hypersurface or a spacelike one.
Actually, we shall see in Sec.~\ref{s:glo:properties_H} that, wherever it is smooth,
the event horizon is a null hypersurface  (Property~\ref{p:glo:prop4}).
However we shall also see in Chap.~\ref{s:loc} that spacelike hypersurfaces,
called \emph{dynamical
horizons}\index{dynamical!horizon}\index{horizon!dynamical --}, are involved
in quasi-local approaches to black holes. So both types ---null and spacelike--- of one-way membranes
are useful in the theory of black holes!

\subsection{Reaching or not infinitely remote regions}

Of course, any region bounded by a null hypersurface cannot be qualified as
a ``black hole''. For instance, the interior of a future light cone in Minkowski
spacetime is such a region. It is true that any particle moving inside it is
trapped: it cannot cross the light-cone boundary. However the particle can travel
infinitely far from any ``center'' in that region. This is where the ``localized''
facet of the naive definition given in Sec.~\ref{s:def:first_defin} enters:
it forces the black hole region to have some ``finite spatial extent'',
so that particles inside it cannot travel to infinitely remote regions.
To formalize the concept of ``infinitely remote region'' in a coordinate independent way, one
introduces a so-called \emph{conformal completion} of the spacetime $(\M,\w{g})$, which brings
the infinity(ies) in $(\M,\w{g})$ to a finite distance in another manifold.
This technique has been initiated by Roger Penrose\index[pers]{Penrose, R.} \cite{Penro63,Penro64} (see
Refs.~\cite{Fraue04} and \cite{Nicol17} for reviews). We present it now, by
starting by the simplest
...
Since we would like to deal with the ``far-away'' region, it is natural to introduce
...
\begin{tprop}{null geodesics preserved by conformal transformations}{}
...
\begin{tprop}{}{glo:Mink_scri_null}
...
in 1963 \cite{Penro63} and expanded in 1964 in the seminal article \cite{Penro64},
where Penrose constructed the
conformal completion of Minkowski spacetime as a part of the Einstein cylinder.
In particular, Fig.~3 in Ref.~\cite{Penro64} is equivalent to Fig.~\ref{f:glo:Einstcyl_Mink}.
\end{hist}
...
\begin{tdef}{conformal completion at infinity}{glo:conformal_completion}
...
\end{enumerate}
$\scri$ is called the \defin{conformal infinity}\index{conformal!infinity}\index{infinity!conformal --},
or the \defin{conformal boundary}\index{conformal!boundary}\index{boundary!conformal --},
of $(\M,\w{g})$ within
the conformal completion $(\tilde{\M},\w{\tilde{g}})$.
\end{tdef}
Condition~1 expresses that $\M$ has been endowed with some boundary.
A rigorous formulation of it would be that there exists an
embedding $\Phi:\M \rightarrow \tilde{\M}$,
as in Eq.~(\ref{e:glo:embed_Mink_Einst}), such that
...
\]
with condition~3 telling $1/\Omega^2 \rightarrow +\infty$ as one approaches $\scri$.
...
\begin{remark}
The conformal infinity $\scri$ is \emph{not} part of the physical spacetime
$\M$. In other words, $\scri$ and $\M$ are both subsets of $\tilde{\M}$
that obeys $\scri \cap \M = \varnothing$.
\end{remark}
...
The conformal infinity $\scri$ is shown in yellow, red lines are lines
...
\begin{texample}{conformal completion of AdS$_{4}$}{glo:AdS}
The 4-dimensional \defin{anti-de Sitter spacetime}\index{anti-de Sitter spacetime}\index{AdS} (AdS$_{4}$)
...
Note that $\tau$ and $r$ are dimensionless, $\tau$ spans $\mathbb{R}$ and $r$ spans $(0,+\infty)$,
while $(\th,\ph)$ are standard spherical coordinates on $\mathbb{S}^2$:
$\th\in(0,\pi)$ and $\ph\in(0,2\pi)$.
The metric (\ref{e:glo:AdS4}) is a solution to
...
\be \label{e:glo:metrix_AdS_conformal}
   \w{g} = \frac{\ell^2}{\cos^2\chi} \left[ - \dd \tau^2
    + \dd \chi^2 + \sin^2\chi \left( \dd\th^2 + \sin^2\th \, \dd\ph^2 \right) \right] .
\ee
Note that, since $\chi < \pi/2$ (and not $\pi$), $(\chi,\th,\ph)$ spans only one half of the hypersphere\index{hypersphere} $\mathbb{S}^3$.
...
We notice that conditions 3 and 4 of Definition~\ref{d:glo:conformal_completion}
...
constant, as follows:
\begin{tprop}{causal type of $\scri$ and sign of the cosmological constant}{glo:type_scri_sign_Lambda}
...
(remember that by hypothesis 4 in Definition~\ref{d:glo:conformal_completion}, $\dd\Omega$ is non-vanishing on $\scri$, so that
...
One may distinguish two subparts of the conformal infinity:

\begin{tdef}{future infinity and past infinity}{glo:fut_past_inf}
Let $(\M,\w{g})$ be a time-orientable\footnote{Cf. Sec.~\ref{s:fra:time_orientation}.} spacetime admitting a conformal completion at infinity $(\tilde{\M},\w{\tilde{g}})$, with conformal infinity $\scri$.
...
\begin{texample}{future and past infinities of AdS$_{4}$}{}
...
\begin{texample}{conformal completion of dS$_{4}$}{glo:dS4}
...
\ee
where $\ell$ is a positive constant. Note that $t$ is dimensionless and spans $\mathbb{R}$,
...
We notice that conditions 3 and 4 in Definition~\ref{d:glo:conformal_completion}
...
matches Definition~\ref{d:glo:fut_past_inf} of a future (resp. past) infinity.
...
there exist an open subset $\mathscr{U}$ of $\M$ and
...
\begin{texample}{asymptotics of dS$_4$ and AdS$_4$}{}
...
\begin{tprop}{null $\scri$ for asymptotically flat spacetimes}{glo:null_scri}
The conformal infinity $\scri$ of an asymptotically flat spacetime $(\M,\w{g})$
...
\section{Black hole definition} \label{s:glo:BH}
...
consequence of Lemma~\ref{p:glo:lem1} below.} of $\M$, while
...
that both sets share the same (topological) boundary\footnote{\label{ft:glo:top_boundary}Let us recall that the \defin{topological
boundary}\index{boundary!topological --} of a subset $S\subset \M$ is
$\partial S := \overline{S} \setminus \mathrm{int}\, S$, where $\overline{S}$ is the closure
of $S$ and $\mathrm{int}\, S$ its interior.}:
...
If the spacetime $(\M,\w{g})$ admits a conformal completion at infinity $(\tilde{\M},\w{\tilde{g}})$
with conformal infinity $\scri$, it follows immediately from the above definitions
of causal future and past, combined with the definitions of future and past
infinities given in Sec.~\ref{s:glo:def_conf_compl}, that the future and past
infinity of $\M$ are expressible as
\be
    \scri^+ = \scri \cap J^+(\M) \qand
    \scri^- = \scri \cap J^-(\M) .
\ee
Note that in the above relations, $J^+(\M)$ (resp. $J^-(\M)$) is the causal future (resp. past)
of $\M$ \emph{considered as a subset of $\tilde{\M}$}.

\subsection{Definition and examples} \label{s:glo:def_BH}
...
(cf. Property~\ref{p:glo:null_scri}),
...
such as black holes in Minkowski spacetime (cf. Remark~\ref{s:glo:spurious_bh} below).
...
\begin{tdef}{black hole region}{glo:BH}
...
The (topological) boundary\footnote{See footnote~\ref{ft:glo:top_boundary} on p.~\pageref{ft:glo:top_boundary}.} $\Hor$ of the black hole region
is then called the \defin{future event horizon}\index{future!event horizon}
(or simply the \defin{event horizon}\index{event!horizon}
when no ambiguity may arise):
\be \label{e:glo:def_horizon}
    \encadre{\Hor := \partial \mathscr{B}}.
\ee
\end{tdef}
From Eq.~(\ref{e:glo:def_BH}), one sees that the black hole region is the set of points of $\M$
...
\overline{\mathscr{B}} \cap \overline{\M\setminus \mathscr{B}}$,
we can rewrite Eq.~(\ref{e:glo:def_horizon}) as
\[
...
Let us illustrate Definition~\ref{d:glo:BH} by a few examples, starting
from a trivial one:

\begin{texample}{no black hole in Minkowski spacetime}{}
The Minkowski spacetime contains no black hole, since all future-directed null geodesics
terminate at $\scri^+$ (cf. Fig.~\ref{f:glo:conf_Mink_null}).
Indeed, the later property implies $\M\subset J^-(\scri^+)$, so that Eq.~(\ref{e:glo:def_BH})
results in $\mathscr{B} = \varnothing$.
...
\begin{figure}
\centerline{\includegraphics[width=0.7\textwidth]{glo_Schwarz_BH.pdf}}
\caption[]{\label{f:glo:Schwarz_BH} \footnotesize
Maximally extended Schwarzschild spacetime, depicted in a Carter-Penrose diagram;
the pale yellow area corresponds to the causal past of the future null infinity, $J^-(\scri^+)$,
which is divided in three parts delimited by $r=2m$ hypersurfaces:
$\M_{\rm I}$, $\M_{\rm III}$ and $\mathscr{W}$.
The grey area is the black hole region $\mathscr{B}$. Green lines are radial null geodesics and
the orange zigzag lines at $r=0$ denote curvature singularities.}
\end{figure}

Then, let us move to three basic examples of spherically symmetric black holes.
All of them belong to the family of Schwarzschild-(anti)-(de Sitter) solutions
to the vacuum Einstein equation with a cosmological constant $\Lambda$,
which will be studied in Sec.~\ref{s:sch:Sch_sol_lamb}.
Their metric tensor is given by Eq.~(\ref{e:sch:Kottler_metric}). These three
examples show various causal character of the future infinity (arising
from various values of $\Lambda$, in agreement with Property~\ref{p:glo:type_scri_sign_Lambda}):
$\scri^+$ null ($\Lambda = 0$, Example~\ref{x:glo:Schwarzschild_BH}),
$\scri^+$ spacelike ($\Lambda > 0$, Example~\ref{x:glo:de_Sitter_BH})
and $\scri^+$ timelike ($\Lambda < 0$, Example~\ref{x:glo:anti_de_Sitter_BH}).

\begin{texample}{Schwarzschild black hole}{glo:Schwarzschild_BH}
The prototype of a black hole is the \emph{Schwarzschild black hole}\index{Schwarzschild!black hole}.
It will be studied in full details in Part~\ref{P:sch}. In particular,
it will be shown in Sec.~\ref{s:sch:BH} that the Schwarzschild spacetime contains
a region $\mathscr{B}$ that fulfills Definition~\ref{d:glo:BH} of a black hole region.
Here we simply illustrate graphically Definition~\ref{d:glo:BH} by
presenting in Fig.~\ref{f:glo:Schwarz_BH}
the conformal diagram of the maximally extended Schwarzschild spacetime.
Such a diagram, called a \emph{Carter-Penrose diagram}\index{Carter-Penrose diagram},
will be constructed rigorously in Sec.~\ref{s:max:Carter-Penrose}
(cf. Fig.~\ref{f:max:carter-penrose-std}). For the present
purpose, it is sufficient to mention that it
shares the same properties as the conformal diagram of Minkowski spacetime
depicted in Figs.~\ref{f:glo:conf_diag_Mink} and \ref{f:glo:conf_Mink_null},
namely it offers a compactified view of the entire spacetime, with each point being actually a 2-sphere of
metric area $4\pi r^2$ (some hypersurfaces of constant $r$ are indicated in Fig.~\ref{f:glo:Schwarz_BH}), all radial null geodesics being straight lines inclined
at $\pm 45^\circ$ and the metric null cones having everywhere a vertical axis and a
$90^\circ$ opening angle.
The extended Schwarzschild spacetime has two (causally disconnected) regions where $2m < r < +\infty$,
which are denoted by $\M_{\rm I}$ and $\M_{\rm III}$ in Fig.~\ref{f:glo:Schwarz_BH}.
Both are asymptotically flat: note that their large $r$ parts
resemble the Minkowski diagram of Fig.~\ref{f:glo:conf_Mink_null}.
In agreement with Property~\ref{p:glo:type_scri_sign_Lambda},
their future conformal boundaries $\scri^+_{\rm I}$ and $\scri^+_{\rm III}$
are null hypersurfaces of $(\tilde{\M},\w{\tilde{g}})$ and therefore
are drawn with a inclination of $\pm 45^\circ$ in Fig.~\ref{f:glo:Schwarz_BH}.
The full future infinity $\scri^+ = \scri^+_{\rm I} \cup \scri^+_{\rm III}$
is thus a null hypersurface with two connected components.
From the diagram of Fig.~\ref{f:glo:Schwarz_BH}, it is clear that
$J^-(\scri^+) \cap \M$ does not entirely cover $\M$, hence
there is a black hole region $\mathscr{B}$. Its boundary,
the event horizon $\Hor$, is a hypersurface $r=2m$, where $m$ is the mass parameter
of the Schwarzschild solution.
\end{texample}


\begin{figure}
\centerline{\includegraphics[width=0.7\textwidth]{glo_de_Sitter_BH.pdf}}
\caption[]{\label{f:glo:de_Sitter_BH} \footnotesize
Schwarzschild-de Sitter spacetime, depicted in a Carter-Penrose diagram;
the pale yellow area corresponds to the causal past of the future infinity, $J^-(\scri^+)$,
while the grey area is the black hole region $\mathscr{B}$. As in Fig.~\ref{f:glo:Schwarz_BH},
green lines are radial null geodesics and
the orange zigzag lines at $r=0$ denote curvature singularities.
The vertical red line is a timelike curve, which shows that $\scri^- \subset J^-(\scri^+)$.
The left and right edges marked by dashed lines
must be identified.}
\end{figure}

\begin{texample}{Schwarzschild-de Sitter black hole}{glo:de_Sitter_BH}
An example of black hole spacetime with a spacelike future infinity $\scri^+$ is provided by the
\defin{Schwarzschild-de Sitter black hole}\index{Schwarzschild-de Sitter!black hole},
whose Carter-Penrose diagram\footnote{More precisely, Fig.~\ref{f:glo:de_Sitter_BH} represents one of the many maximal analytic extensions
of the Schwarzschild-de Sitter spacetime: that obtained by identifying the two timelike edges
marked as dashed vertical lines in Fig.~\ref{f:glo:de_Sitter_BH}. The other extensions are
obtained by attaching an arbitrary number $\mathcal{N}_{\rm c}$ (possibly infinite) of copies of the spacetime shown in Fig.~\ref{f:glo:de_Sitter_BH} along these edges,
with the most extremal edges identified for $\mathcal{N}_{\rm c}<+\infty$ (see Refs.~\cite{GibboH77,Geyer80,BicakP95}, Sec.~9.4 of Ref.~\cite{GriffP09} or Sec.~3 of Ref.~\cite{GaspeV17}).} is shown in Fig.~\ref{f:glo:de_Sitter_BH}.
In areal coordinates, the metric is given by Eq.~(\ref{e:sch:Kottler_metric}) with $0 < \Lambda < (9m^2)^{-1}$.
Far from the black hole region, i.e. near $\scri^+$ and $\scri^-$, the spacetime approaches the de Sitter spacetime\index{de Sitter spacetime}, for which $\scri^+$ and $\scri^-$
have been shown to be spacelike (cf. Example~\ref{x:glo:dS4}).
It is clear from Fig.~\ref{f:glo:de_Sitter_BH} that
$J^-(\scri^+) \cap \M$ fails to cover all of $\M$, hence
there is a black hole region $\mathscr{B}$.
There exist two values of the areal radius $r$,  $r_{\rm h}$ and $r_{\rm c}$ say,
such that the hypersurfaces $r=r_{\rm h}$ or $r=r_{\rm c}$ are null. According to the criterion~(\ref{e:bas:char_null_coord}),
they are obtained as the two positive zeros $r_{\rm h}$ and $r_{\rm c}$, ordered as $r_{\rm h} < r_{\rm c}$,
of the function
$g^{rr} = 1 - \frac{2 m}{r} - \frac{\Lambda}{3} \,  r^2$ (this value of $g^{rr}$
follows easily from the diagonal form (\ref{e:sch:Kottler_metric}) of the metric). The black hole event horizon $\Hor$ is located at $r = r_{\rm h}$, while null hypersurfaces $r=r_{\rm c}$
correspond to \emph{cosmological horizons}\index{cosmological!horizon}\index{horizon!cosmological --}
in de Sitter spacetime (see e.g. Ref.~\cite{GriffP09}).
\end{texample}

\begin{figure}
\centerline{\includegraphics[width=0.5\textwidth]{glo_anti_dS_BH.pdf}}
\caption[]{\label{f:glo:anti_dS_BH} \footnotesize
Schwarzschild-anti-de Sitter spacetime, depicted in a Carter-Penrose diagram;
the pale yellow area corresponds to the causal past of the future infinity, $J^-(\scri^+)$,
while the grey area is the black hole region $\mathscr{B}$.
As in Figs.~\ref{f:glo:Schwarz_BH} and \ref{f:glo:de_Sitter_BH},
green lines are radial null geodesics and
the orange zig-zag curves at $r=0$ denote curvature singularities.}
\end{figure}

\begin{texample}{Schwarzschild-anti de Sitter black hole}{glo:anti_de_Sitter_BH}
Finally, an example of black hole in a spacetime with a timelike $\scri^+$ is provided by the
\defin{Schwarzschild-anti de Sitter black hole}\index{Schwarzschild-anti de Sitter!black hole},
whose Carter-Penrose diagram is shown in Fig.~\ref{f:glo:anti_dS_BH}
(see Refs.~\cite{LakeR77} and \cite{FidkoHKS04} for the construction of this
diagram\footnote{In particular, it is argued in Ref.~\cite{FidkoHKS04} that the conformal infinity
$\scri$ and the curvature singularity $r=0$ cannot be both represented as straight segments
in the conformal diagram. Hence if one choose a straight segment for $\scri^+ (= \scri)$,
as in Fig.~\ref{f:glo:anti_dS_BH}, then $r=0$ has to be bowed in.}).
In areal coordinates, the metric is given by Eq.~(\ref{e:sch:Kottler_metric}) with $\Lambda < 0$.
For large values of the areal radius $r$, the spacetime approaches the anti-de Sitter spacetime\index{anti-de Sitter spacetime}, for which $\scri^+ = \scri^- = \scri$ and $\scri^+$
is a timelike hypersurface of $(\tilde{\M},\w{\tilde{g}})$, as shown in Example~\ref{x:glo:AdS}.
It is clear from Fig.~\ref{f:glo:anti_dS_BH} that
$J^-(\scri^+) \cap \M$ fails to cover all of $\M$, hence
there is a black hole region $\mathscr{B}$. The event horizon $\Hor$ is located
at $r=r_{\rm h}$, where $r_{\rm h}$ is the only value of $r$ for which
the constant-$r$ hypersurfaces are null, being the only positive zero of
$g^{rr}$ for $\Lambda < 0$ (cf. the discussion in Example~\ref{x:glo:de_Sitter_BH}).
\end{texample}
...
If one releases the assumption of $\scri^+$-completeness in
Definition~\ref{d:glo:BH},
one may end up with unphysical or ``spurious'' black holes \cite{GerocH78}.
...
The black hole region in a given spacetime $(\M,\w{g})$ with a disconnected $\scri^+$
may depend on the choice of connected components of $\scri^+$,
even if each component is assumed complete.
For instance let us consider the maximally extended Schwarzschild spacetime,
as depicted in Fig.~\ref{f:glo:Schwarz_BH}. The regions $\M_{\rm I}$ and
$\M_{\rm III}$, in which $2m < r < +\infty$, are both asymptotically flat.
They are causally disconnected: no causal curve can travel from $\M_{\rm I}$ to
$\M_{\rm III}$ or vice versa. From an astrophysical point of view, it then makes
sense to define $\M_{\rm I}$ as ``our world''  and to consider
a conformal completion $(\tilde{\M},\w{\tilde{g}})$ of $(\M,\w{g})$
such that $\scri = \scri_{\rm I}$,
where $\scri_{\rm I}$ is the (assumed complete) conformal infinity of a
conformal completion of $(\M_{\rm I},\w{g})$, cf. Fig.~\ref{f:glo:Schwarz_BH_alt}.
This defines a perfectly valid
conformal completion at infinity of the whole spacetime $(\M,\w{g})$, i.e. it obeys all
the requirements stated in Sec.~\ref{s:glo:def_conf_compl}. The corresponding
black hole region $\mathscr{B}$ obtained in this way is then larger than that obtained
in Example~\ref{x:glo:Schwarzschild_BH}, since it now encompass $\M_{\rm III}$: compare Fig.~\ref{f:glo:Schwarz_BH_alt} to Fig.~\ref{f:glo:Schwarz_BH}. There is no physical
issue with these different black hole regions in the same spacetime
$(\M,\w{g})$, given that no particle can travel from $\M_{\rm III}$ to
$\M_{\rm I}$. In other words, from the point of view of observers
in $\M_{\rm I}$, it makes no difference to consider that $\M_{\rm III}$ lies in the black hole region or not.


\begin{figure}
\centerline{\includegraphics[width=0.7\textwidth]{glo_Schwarz_BH_alt.pdf}}
\caption[]{\label{f:glo:Schwarz_BH_alt} \footnotesize
Maximally extended Schwarzschild spacetime,
as in Fig.~\ref{f:glo:Schwarz_BH}, but with a different conformal completion,
so that $\scri^+$ lies only at the boundary of the asymptotically flat region $\M_{\rm I}$.}
\end{figure}
...
According to such a definition, if $n = \dim \M$,
a black hole is a $(n-1)$-dimensional object,
while the black hole region $\mathscr{B}$ in Definition~\ref{d:glo:BH} is a $n$-dimensional object.
\end{remark}

\begin{remark}
All the black holes examples~\ref{x:glo:Schwarzschild_BH} to \ref{x:glo:anti_de_Sitter_BH} exhibit a curvature singularity\footnote{A \emph{curvature singularity} expresses some divergence of the Riemann curvature tensor; this concept will be discussed in more detail in Sec.~\ref{s:sch:singularities}.}\index{curvature!singularity}\index{singularity!curvature --} in the future of the black hole region (the top orange zigzag line in
Figs.~\ref{f:glo:Schwarz_BH}--\ref{f:glo:anti_dS_BH} and \ref{f:glo:Schwarz_BH_alt}).
However, it is worth to stress that the black hole definition (\ref{e:glo:def_BH}) does not involve
any curvature singularity, whatsoever. Actually, there exist black hole solutions to the Einstein equation, albeit with some quite exotic matter fields, which do not harbor any curvature singularity.
Such objects are called \defin{regular black holes}\index{regular black hole}\index{black!hole!regular --} (see e.g. Ref.~\cite{Bambi23}).
A famous example is the \emph{Bardeen black hole}\index{Bardeen black hole}, which will be discussed in
Example~\ref{x:loc:Bardeen_BH}.
\end{remark}

\subsection{White holes}

By inverting past and future in Definition~\ref{d:glo:BH}, one defines
...
that cannot be reached by a (future-directed) causal curve emanating
from $\scri^-$.
...
one is expelled, rather than absorbed, by a fountain.
\end{remark}

\begin{texample}{Schwarzschild white hole}{glo:Schwatz_white_hole}
In the maximally extended Schwarzschild spacetime depicted in Fig.~\ref{f:glo:Schwarz_BH}
and described in Example~\ref{x:glo:Schwarzschild_BH}, the region marked $\mathscr{W}$ is
a white hole\index{Schwarzschild!white hole}, with the past event horizon $\Hor^-$ being a null hypersurface $r=2m$. Similarly, the regions marked by $\mathscr{W}$ in the
Carter-Penrose diagrams of the Schwarzschild-de Sitter and Schwarzschild-anti-de Sitter
spacetimes in Figs.~\ref{f:glo:de_Sitter_BH} and \ref{f:glo:anti_dS_BH}
are white holes.
\end{texample}
...
the term \emph{black hole} rapidly superseded previous names such as
...
\section{Properties of the future event horizon} \label{s:glo:properties_H}
...
\subsection{Achronal character}

\begin{tprop}{the event horizon as an achronal set}{glo:prop1}
...
\begin{tlemma}{stability of timelike curves with respect to their ends}{glo:lem1}
...
\subsection{Topological manifold ruled by never leaving null geodesics}

As the boundary between the interior and the exterior of the black hole,
one may expect that the event horizon $\Hor$ is a hypersurface
of spacetime, i.e. a submanifold of codimension 1 (cf. the discussion in Sec.~\ref{s:def:hor_as_null}).
However, $\Hor$ is not necessarily a \emph{smooth} manifold: there may exist some points where
$\Hor$ is not differentiable. Only the continuous structure of the manifold holds
everywhere, i.e. $\Hor$ is guaranteed to be a \emph{topological} manifold, if not
a smooth one:

\begin{tprop}{the event horizon as a topological manifold of codimension 1}{glo:prop2}
A black hole event horizon $\Hor$ in a $n$-dimensional spacetime
is a topological manifold of dimension $n-1$.
\end{tprop}
...
By definition, there exist a future-directed timelike curve from $q$ to $a$ and a future-directed causal curve from $a$ to
...
\begin{tprop}{event horizon ruled by never leaving null geodesics \textnormal{(Penrose\index[pers]{Penrose, R.} 1968)}}{glo:prop3}
...
in $\Hor$ or never leave $\Hor$ when followed to the future from the
...
\cite{Siino98a,Siino98b,Brill14,GadioR23}.
...
\begin{remark}
\label{r:glo:ruled}
The qualifier ``ruled''\index{ruled!horizon} is used in Property~\ref{p:glo:prop3} by analogy with
a \defin{ruled surface}\index{ruled!surface}, namely a surface $\Sp$ in the Euclidean space such that there is a straight line lying in $\Sp$ through each point of $\Sp$ (for example a
cylinder, a cone, or a hyperboloid of one sheet). The analogy holds
because straight lines are the geodesics of the Euclidean space.
\end{remark}

Penrose's original proof \cite{Penro68} of Property~\ref{p:glo:prop3} is discussed briefly
in the historical note on p.~\pageref{h:glo:BH_properties}. Here we provide a proof
adapted from that given in Box~34.1 of MTW \cite{MisneTW73}.
In addition of Lemma~\ref{p:glo:lem1}, it relies on the following lemma.

\begin{tlemma}{}{glo:lem2}
...
\begin{texample}{head-on binary black hole merger}{}
...
\begin{texample}{inspiralling binary black hole merger}{}
Figure~\ref{f:glo:EH_binspir} shows a cross-section
...
subset of $\Hor$ \cite{Siino98a,Siino98b,HusaW99,CohenKS12,GadioR23}.


\begin{tprop}{the event horizon as a null hypersurface}{glo:prop4}
Wherever it is smooth, $\Hor$ is a null hypersurface.
\end{tprop}

\begin{proof}
Let us assume that $\Hor$ is smooth in some open subset $U$ of $\M$.
...
\begin{texample}{Schwarzschild and Schwarzschild-(anti-)de Sitter horizons}{}
It is clear from the Carter-Penrose diagrams of Figs.~\ref{f:glo:Schwarz_BH}--\ref{f:glo:anti_dS_BH} that the horizons of the Schwarzschild, Schwarzschild-de Sitter
and Schwarzschild-anti de Sitter black holes (Examples~\ref{x:glo:Schwarzschild_BH}--\ref{x:glo:anti_de_Sitter_BH}) are null hypersurfaces, since they appear
as lines with an inclination of $\pm 45^\circ$ in these conformal diagrams.
\end{texample}
...
seems that the latter has been introduced by Stephen Hawking\index[pers]{Hawking, S.W.} and George Ellis\index[pers]{Ellis, G.F.R.} in their famous 1973 textbook~\cite{HawkiE73}.
\end{hist}
