...
namely each point in the diagram is actually a sphere $\mathbb{S}^2$ of
...
at $\pm 45^\circ$.
Note that the Schwarzschild spacetime is asymptotically flat, so that
$\scri^+$ is a null hypersurface of $(\tilde{\M},\w{\tilde{g}})$ and hence
drawn with a inclination of $\pm 45^\circ$ in Fig.~\ref{f:glo:Schwarz_BH}.
...
the orange zig-zag lines at $r=0$ denote curvature singularities.
The vertical red line is a timelike curve. The left and right boundaries marked by dashed lines
must be identified.}
\end{figure}
...
they are obtained as the two positive zeros $r_{\rm h}$ and $r_{\rm c}$ of the function
$g^{rr} = 1 - \frac{2 m}{r} - \frac{\Lambda}{3} \,  r^2$, the value of $g^{rr}$
following easily from the diagonal form (\ref{e:sch:Kottler_metric}) of the metric. The black hole event horizon $\Hor$ is located at $r = r_{\rm h}$, while null hypersurfaces $r=r_{\rm c}$
correspond to \emph{cosmological horizon}\index{cosmological!horizon}\index{horizon!cosmological --}
in de Sitter spacetime (see e.g. Ref.~\cite{GriffP09}).
%It is worth to note that Fig.~\ref{f:glo:de_Sitter_BH} is the
%maximal extension of the Schwarzschild-de Sitter spacetime obtained by identifying
%the timelike hypersurfaces
\end{example}
