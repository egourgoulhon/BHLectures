\chapter{Geometry of null hypersurfaces}
\label{s:def}
...
Having established in the preceding chapter that (the smooth part of) the event horizon of
a black hole in a null hypersurface (Property~\ref{p:glo:prop4}),
we examine here the basic properties of this type of hypersurfaces.
Null hypersurfaces have been defined in Sec.~\ref{s:def:hor_as_null}
as hypersurfaces of spacetime
for which the induced metric is degenerate, or equivalently, for which
any normal vector is null. They can also be characterized as having their normal
vectors tangent to them. Here, we shall show that the integral lines
of the normal vectors form a congruence of null geodesics on the hypersurface
(Sec.~\ref{s:def:geom_null_hypsurf}). Then, in Sec.~\ref{s:nul:cross_sections}, we discuss the
surfaces transverse to this geodesic congruence, called the
\emph{cross-sections}, and study their
properties. This allows us
to introduce the concepts of expansion and shear along the null normals.
Finally, in Sec.~\ref{s:def:null_raychaud},
we derive the null version of the famous Raychaudhuri equation
and draw from it a law of non-increase of the expansion.
This law will be of great importance for black hole theory.

The results established in this chapter are fully generic, i.e. they are valid for any
null hypersurface, not necessarily a black hole horizon. They are also fully
geometric, i.e. independent of any theory of gravity.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Geodesic generators of a null hypersurface} \label{s:def:geom_null_hypsurf}

\subsection{Hypersurfaces as level sets} \label{s:def:hypsurf_level}

Let $\Hor$ be a null hypersurface in a $n$-dimensional spacetime $(\M,\w{g})$.
...
\begin{texample}{null hyperplane}{def:null_hyp}
...
\begin{texample}{light cone}{def:light_cone}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor}
...
and have the advantage over the Schwarzschild-Droste ones to be regular on the event horizon,
...
from Eq.~(\ref{e:def:Schw_metric_inv}), $g^{\mu\nu} \partial_\mu r \, \partial_\nu r = g^{rr}=0$ on $\Hor$.} that $\Hor$ is indeed a null hypersurface. We shall call it the
\defin{Schwarzschild horizon}\index{Schwarzschild!horizon} for it is the event
horizon of the Schwarzschild black hole, as we shall see in Sec.~\ref{s:sch:BH}.
\end{texample}
...
\begin{texample}{families of null hypersurfaces in previous examples}{}
...
\begin{texample}{normal to the null hyperplane}{def:null_hyp2}
We deduce from
...
\begin{texample}{normal to the light cone}{def:light_cone2}
...
\begin{texample}{normal to the Schwarzschild horizon}{def:Schw_hor2}
...
\begin{tprop}{geodesic generators of a null hypersurface}{def:null_geod_generators}
Any null hypersurface $\Hor$ is ruled\index{ruled!null hypersurface}\footnote{See Remark~\ref{r:glo:ruled} on p.~\pageref{r:glo:ruled} for the justification of the term ``ruled''.}
by a family of null geodesics, called the
...
tangent to these null geodesics. The null geodesic generators
form a congruence\footnote{Let us recall that a \defin{congruence}\index{congruence} on a
submanifold $\Sigma$ of $\M$ is a family of curves tangent to $\Sigma$, one, and exactly one, through
each point of $\Sigma$.}
in $\Hor$.
\end{tprop}
...
vector field $\wl$ are null curves tangent to $\Hor$, one must
...
For a smooth black hole event horizon, which is a null hypersurface (Property~\ref{p:glo:prop4}),
the \emph{null geodesic generators} defined
by Property~\ref{p:def:null_geod_generators} are the same as those
defined in Property~\ref{p:glo:prop3}.

\begin{texample}{null hyperplane}{def:null_hyp3}
...
\begin{texample}{light cone}{def:light_cone3}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor3}
...
\section{Cross-sections of a null hypersuface} \label{s:nul:cross_sections}

\subsection{Definition and first properties} \label{s:def:spacelike_sections}

\begin{tdef}{cross-section}{nul:cross_section}
...
At this stage, we put no restriction on the topology of cross-sections.
We simply note that if the null hypersurface $\Hor$ is a black hole event
horizon, the complete cross-sections are generically
\defin{closed manifolds}\index{closed!manifold}, i.e.
...
But for $n > 4$, other
compact topologies, like the torus' one, are allowed for
cross-sections of the event horizon of a stationary black hole, as we
shall discuss in Sec.~\ref{s:sta:topology}.

A first important property of cross-sections is:

\begin{tprop}{spacelike character of cross-sections}{def:spacelike_cs}
...
\begin{tlemma}{tangent vectors to a null hypersurface}{def:tangent_to_null_hyp}
...
that $\w{v}$ cannot be timelike.
Besides, if $\w{v}$ is null, Property~\ref{p:fra:corol2}
...
which is not allowed, given Definition~\ref{d:nul:cross_section} of a cross-section. We conclude
that $\w{v}$ is necessarily spacelike, which proves that $\Sp$ is a spacelike
submanifold.
\end{proof}

\begin{texample}{light cone}{def:light_cone4}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor4}
...
\begin{texample}{binary black hole}{}
...
\begin{tprop}{Riemannian metric on cross-sections}{}
The symmetric bilinear form $\w{q}$ defined at any point $p$ of a cross-section $\Sp$
by
\be \label{e:def:def_q_S}
    \forall (\w{u},\w{v})\in T_p\Sp\times T_p\Sp, \quad
     \w{q}(\w{u},\w{v}) := \w{g}(\w{u},\w{v})
\ee
is called the \defin{metric induced by}\index{induced!metric}\index{metric!induced --}
$\w{g}$ on $\Sp$.
It is a Riemannian metric\index{Riemannian!metric}, i.e. $\w{q}$ is positive definite: $\w{q}(\w{v},\w{v}) \geq 0$ for all
$\w{v}\in T_p\Sp$, with $\w{q}(\w{v},\w{v}) = 0$ $\iff$ $\w{v} = 0$.
\end{tprop}
\begin{proof}
That $\w{q}$ is Riemannian follows immediately from the spacelike character of $\Sp$ (Property~\ref{p:def:spacelike_cs})
\end{proof}

\begin{texample}{Schwarzschild horizon}{def:Schw_hor4a}
...
\begin{tprop}{orthogonal complement of a cross-section tangent space}{}
...
\begin{tprop}{complement null normal to a cross-section}{}
At each point $p$ of a cross-section $\Sp$ of a null hypersurface $\Hor$
...
\begin{tprop}{extension of $\w{q}$}{}
The bilinear form $\w{q}$, which has been defined only on $T_p\Sp$ via (\ref{e:def:def_q_S}),
can be extended to all vectors of $T_p\M$ for $p\in\Sp$ by setting
...
\be \label{e:def:q_all_vectors}
    \forall (\w{u},\w{v})\in T_p\M\times T_p\M, \quad
     \w{q}(\w{u},\w{v}) = \w{g}(\w{u}^\parallel,\w{v}^\parallel) ,
\ee
...
\begin{texample}{light cone}{def:light_cone5}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor5}
...
\begin{tprop}{orthogonal projector onto a cross-section}{}
...
\subsection{Expansion along a null normal} \label{s:def:def_expansion}

One defines the expansion of the cross-section $\Sp$ along the vector
field $\wl$ as follows.

\begin{tdef}{expansion along a null normal}{}
...
\begin{tprop}{area $(n-2)$-form of a cross-section}{}
...
\begin{texample}{light cone}{def:light_cone6}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor6}
...
\begin{texample}{light cone}{def:light_cone7}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor7}
...
We conclude:
\begin{tprop}{independence of the expansion from the cross-section}{def:expansion_indep_cs}
...
\begin{tprop}{rescaling of the expansion upon a change of null normal}{def:rescaling_expansion}
...
\begin{texample}{light cone}{def:light_cone8}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor8}
...
\begin{tprop}{rescaling of the deformation rate and shear upon a change of null normal}{}
Two null normals $\wl$ and $\wl'$ to a null hypersurface $\Hor$ are necessarily collinear:
$\wl' = \alpha \wl$, where $\alpha$ is a non-vanishing scalar field on $\Hor$.
...
\begin{tprop}{null Raychaudhuri equation}{}
...
\begin{texample}{Schwarzschild horizon}{def:Schw_hor9}
...
\begin{tprop}{non-increase of the expansion}{def:non_inc_expans}
