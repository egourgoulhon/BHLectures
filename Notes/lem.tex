...
\begin{example}
The trapped surface $\Sp$ corresponding tp $(\eta,\chi) = (11\pi/16, 3\pi/16)$
in the Oppen\-heimer-Snyder collapse with $r_0 = 4 m_0$
...
$r_{\Sp} = 2\sqrt{2} \sin(3\pi/16) (1 + \cos(11\pi/16)) \simeq 0.70 m_0$.
...
that both geodesics encounter the curve $r=0.65 m_0$ in the immediate future of $\Sp$.
...
Let us consider a remote static observer $\Obs$ that receives electromagnetic
...
Let us suppose that, at the comoving proper time $\tau_1$, an electromagnetic
signal (``flash'') is emitted in the radial direction from
the surface of the collapsing star and that a similar signal is emitted
subsequently at the proper time $\tau_2$.
Since each signal is carried by a outgoing radial null geodesic (the green curves
...
\[
    \ti^{\rm rec}_i - r_{\Obs} - 4 m_0 \ln \left( \frac{r_{\Obs}}{2 m_0} - 1 \right) =
    \ti^{\rm em}(\tau_i) - r_{\rm s}(\tau_i) - 4 m_0 \ln \left( \frac{r_{\rm s}(\tau_i)}{2 m_0} - 1 \right),
    \quad i\in\{1,2\} ,
\]
...
the proper time $\tau_i$;  $\ti^{\rm em}(\tau_i)$ is given by Eq.~(\ref{e:lem:OS:ti_star_surf})
with $\eta$ being the function of $\tau_i$ defined implicitly by Eq.~(\ref{e:lem:OS:evol_star_surf}).
We deduce from the above relation the lapse in $\ti$
separating the reception of each signal by $\Obs$:
\be \label{e:lem:OS:delay}
    \ti^{\rm rec}_2 - \ti^{\rm rec}_1 =
    \ti^{\rm em}(\tau_2) - \ti^{\rm em}(\tau_1)
    + r_{\rm s}(\tau_1) - r_{\rm s}(\tau_2)
    + 4 m_0 \ln \left( \frac{r_{\rm s}(\tau_1) - 2 m_0}{r_{\rm s}(\tau_2) - 2 m_0} \right) .
\ee
If $\Obs$ is asymptotically far, i.e. if $r_\Obs \gg m_0$, $\ti^{\rm rec}_2 - \ti^{\rm rec}_1$
...
are always finite but when $\tau_2$ approaches $\tau_*$ [cf. Eq.~(\ref{e:lem:OS:tau_star})], i.e. when $r_{\rm s}(\tau_2)$ approaches $2 m_0$, the logarithm term diverges, so that
\be
...
proceeds. The delay between two successive signals becoming infinite, so that, among the emitted sequence, there exist a very
last received by $\Obs$. All subsequent signals are captured by the black hole.
This is illustrated by Fig.~\ref{f:lem:OS:diag_delay}, which regards the Oppenheimer-Snyder collapse
with $r_0 = 4 m_0 \iff \chi_{\rm s} = \pi/4$. Starting from $\tau=0$, a sequence of 8 signals is emitted from the star's surface, the separation between two successive signals being
$\delta\tau = \tau_{\rm end}/8 = \pi\sqrt{2} m_0 / 4$. One sees graphically that the signals are received by $\Obs$
...
\subsubsection{Infinite redshift}

The time dilation effect described above manifests itself on the frequencies of photons
received by the remote observer from the surface of the collapsing star.
The inverses of the observed frequencies, i.e. the observed electromagnetic periods, are expected to increase without any bound during the collapsing, leading to some infinite redshift.
...
the radial null geodesics propagate to the remote observer $\Obs$ and
which make computations slightly easier than with IEF coordinates.
Given expression (\ref{e:sch:outgoing_null_geod})
for the outgoing radial null geodesics and the fact that $r$ is an affine
parameter along them (cf. Sec.~\ref{s:sch:rad_null_geod}), so that $r = \alpha \lambda$
for some constant $\alpha$, we get
\[
    \w{p} = \derd{x^\mu}{\lambda} \wpar_\mu = \alpha \derd{x^\mu}{r} \wpar_\mu
          = \alpha \left[ \left( 1  - \frac{2m_0}{r} \right) ^{-1} \wpar_t + \wpar_r \right] .
\]
The constant $\alpha$ is set by considering the energy measured at infinity by
$\Obs$: $E_\infty = -\w{g}(\w{u}_\Obs, \w{p})$. Since $\w{u}_\Obs = \wpar_t$,
we have $E_\infty = - g_{t\mu} p^\mu$. In view of the
Schwarzschild-Droste components (\ref{e:sch:Schwarz_metric_SD}) of the metric tensor,
we get $E_\infty = \alpha$. Hence
\be \label{e:lem:OS:p_photon}
     \w{p} = E_\infty  \left[ \left( 1  - \frac{2m_0}{r} \right) ^{-1} \wpar_t + \wpar_r \right] .
\ee
The 4-velocity of the comoving observer at the surface of the collapsing star
is $\w{u} = u^t \wpar_t + u^r \wpar_r$. Since this observer is in radial free fall
from rest,
$u^t=\D t / \D\tau$ is given by Eq.~(\ref{e:ges:Dt_Dtau})
with $\veps = \sqrt{1 - 2m_0/r_0}$ [Eq.~(\ref{e:ges:beps2})]
and $u^r = \D r /\D\tau$ is given by
Eq.~(\ref{e:ges:radial_motion}) with $\veps^2 - 1 = - 2m_0/r_0$.
Hence we get
\be \label{e:lem:OS:u_comoving}
    \w{u} = \sqrt{1 - \frac{2m_0}{r_0} }
         \left(1 - \frac{2m_0}{r_{\rm s}(\tau)} \right)^{-1} \, \wpar_t
        - \sqrt{\frac{2m_0}{r_{\rm s}(\tau)} - \frac{2m_0}{r_0}} \,
        \wpar_r .
\ee
The energy of the emitted photon measured by the comoving observer
is $E_{\rm em} = -\w{g}(\w{u},\w{p})$. With the components of $\w{g}$,
$\w{u}$ and $\w{p}$ given by respectively Eqs.~(\ref{e:sch:Schwarz_metric_SD}),
(\ref{e:lem:OS:u_comoving}) and (\ref{e:lem:OS:p_photon}), we get
\be
    E_{\rm em} = E_\infty
    \left(1 - \frac{2m_0}{r_{\rm s}(\tau)} \right)^{-1}
    \left( \sqrt{1 - \frac{2m_0}{r_0} } + \sqrt{\frac{2m_0}{r_{\rm s}(\tau)} - \frac{2m_0}{r_0}}
    \right) .
\ee
The photon's redshift\index{redshift} when received by $\Obs$ is
$z := (\lambda_\infty - \lambda_{\rm em})/\lambda_{\rm em} = E_{\rm em}/E_\infty - 1$.
Hence
\be \label{e:lem:OS:redshift}
    \encadre{ z = \left(1 - \frac{2m_0}{r_{\rm s}(\tau)} \right)^{-1}
    \left( \sqrt{1 - \frac{2m_0}{r_0} } + \sqrt{\frac{2m_0}{r_{\rm s}(\tau)} - \frac{2m_0}{r_0}}
    \right)  - 1 } .
\ee
...
$z = (1 - 2m_0/r_0)^{-1/2} - 1$, which is the standard expression for the redshift of
a photon emitted by a static observer at $r=r_0$ in the Schwarzschild metric. This is
not surprising since the star is momentarily static at $\tau=0$, the collapse starting
from rest.

For $\tau\to \tau_*$, $r_{\rm s}(\tau) \to 2 m_0$ and formula~(\ref{e:lem:OS:redshift}) yields
\be \label{e:lem:OS:redshift_sim}
    z \underset{\tau\to\tau_*}{\sim}  2 \sqrt{1 - \frac{2m_0}{r_0} }
     \left(1 - \frac{2m_0}{r_{\rm s}(\tau)} \right)^{-1} ,
\ee
which implies $z \to +\infty$ for $\tau \to \tau_*$.

Let us denote by $\Delta t_\Obs$ the proper time of observer $\Obs$ elapsed
between the reception of the photon emitted
at $\tau = 0$ (start of the collapse) and that of the photon emitted at $\tau > 0$.
Since $\ti^{\rm rec}$ is $\Obs$'s proper time,
...
\[
    ...
    + 4 m_0 \ln \left( \frac{r_0}{2m_0} - 1 \right)}_{A}
    -  4 m_0 \ln \left( \frac{r_{\rm s}(\tau)}{2 m_0}  - 1 \right) .
\]
Since the terms denoted by $A$ are bounded when $\tau\to \tau_*$, we get
\[
    \Delta t_\Obs  \underset{\tau\to\tau_*}{\sim} -  4 m_0 \ln \left( \frac{r_{\rm s}(\tau)}{2 m_0}  - 1 \right)
    \iff 1 - \frac{2m_0}{r_{\rm s}(\tau)} \underset{\tau\to\tau_*}{\sim} \mathrm{e}^{-\Delta t_\Obs/(4 m_0)} .
\]
...
\be
   \encadre{ z \underset{\tau\to\tau_*}{\sim}  2 \sqrt{1 - \frac{2m_0}{r_0} }
     \; \mathrm{e}^{\Delta t_\Obs/(4 m_0)} } .
\ee
Hence we conclude:
\begin{greybox}
The radiation from the collapsing star's surface is received by the remote static observer $\Obs$
with an increasing redshift. At late times, the redshift grows exponentially
with a characteristic time scale $4 m_0$. Equivalently, the energy of the received photons
