...
\be
    \tau_* = m_0 \frac{\pi - 2 \chi_{\rm s} + \sin(2\chi_{\rm s})}{\sin^3\chi_{\rm s}}
     = \tau_{\rm end} \left( 1 - \frac{2\chi_{\rm s} - \sin(2\chi_{\rm s})}{\pi} \right) .
\ee


For $\tau < \tau_*$, the event horizon must be located inside the collapsing star.
To determine it, let us consider the outgoing radial null geodesics, which are drawn as solid green curves
in Fig.~\ref{f:lem:OS:diag_int_EF}. Their equations are pretty simple in terms of
the coordinates $(\eta,\chi)$:
