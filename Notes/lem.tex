...
would lead to the areal radius $r$ being a decaying function of $\chi$ near $\chi_{\rm s}$,
...
\be
    \tau_{\rm end} = \frac{\pi}{2} a_0 = \frac{\pi}{2} \sqrt{\frac{r_0^3}{2 m_0}}
...
so that $\eta_* = \pi/2$ and $\tau_* = \sqrt{2}(\pi + 2) m_0 \simeq 7.27 m_0$.
\end{example}
...
to infinity, while those for which $\eta_{\rm s} > \eta_*$ emerge in the
...
\be
    \eta_{\rm hor} = \pi - 3 \chi_{\rm s}
\ee
or equivalently at the matter proper time
\be \label{e:lem:OS:tau_hor}
  \tau_{\rm hor} = \frac{\pi - 3 \chi_{\rm s} + \sin(3\chi_{\rm s})}{\sin^3\chi_{\rm s}} m_0
     =  \left( 1 - \frac{3\chi_{\rm s} - \sin(3\chi_{\rm s})}{\pi} \right) \tau_{\rm end} .
\ee
It then expands according to Eq.~(\ref{e:lem:OS:Hor_eta_chi}) until it reaches the surface
...
we get $\eta_{\rm hor} = \pi/4$, which corresponds to the origin of the
black straight segment drawn in Fig.~\ref{f:lem:OS:diag_int_EF}, left.
Equation~(\ref{e:lem:OS:tau_hor}) gives
$\tau_{\rm hor} = (2 + \pi/\sqrt{2}) m_0 \simeq 4.22 m_0$.
\end{example}
