...
$\chi_{\rm s}$ the value of $\chi$ at the stellar surface. The
matching of the surface areal radii given by Eqs.~(\ref{e:lem:OS:evol_star_surf})
and (\ref{s:lem:OS:tau_r_eta}) implies $r_0 = \sqrt{r_0^3/2m_0}\sin\chi_{\rm s}$,
...
virtue of Eq.~(\ref{s:lem:OS:tau_r_eta}).
...
from expression (\ref{s:lem:OS:tau_r_eta}); we get a function of $\tau$
only, which we rewrite as $a(\tau)$:
\be
    a(\tau) = \frac{a_0}{2} \left( 1 + \cos\eta \right) .
\ee
We may then rewrite the second equation in (\ref{s:lem:OS:tau_r_eta}) as
...
\be \label{e:lem:OS:g_interior_tau}
   \w{g} = - \dd\tau^2 + a(\tau)^2 \left[ \dd\chi^2
...
\ee
which follows from (\ref{s:lem:OS:tau_r_eta}). We then deduce from Eq.~(\ref{e:lem:OS:g_interior_tau})
that
\be
       \w{g} = \frac{a_0^2}{4} (1 + \cos\eta)^2 \left[ - \dd\eta^2 + \dd\chi^2
        + \sin^2\chi \left( \dd\th^2 + \sin^2\th\, \dd\ph^2 \right) \right] .
\ee
Hence, inside the star, the metric is conformal to the metric of the
...
Finally, the matter density $\rho$ is deduced from Eq.~(\ref{e:lem:LTeqs2}). Again, we get
a function of $\tau$ only:
\be \label{e:lem:OS:rho_tau}
    \rho(\tau) = \frac{3 a_0}{8\pi a(\tau)^3} = \frac{6 m_0}{\pi r_0^3 (1 + \cos\eta)^3} .
\ee
The indepedence of $\rho$ from $\chi$ confirms that the current model is that of
a homogeneous star.
...
\ee
This formula is identical to that given the mass density of a uniform ball of mass
