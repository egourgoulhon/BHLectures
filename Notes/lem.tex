...
is based on the assumption that the star at $\tau=0$ corresponds to an instant of
...
star at $\chi = 0$ and keeping $\tau$ fixed, $r(\tau,\chi)$ increases up to a maximum value
achieved for $\chi=\pi/2$ and which is $r_{\rm s}(\tau)/\sin\chi_{\rm s}$; it then decreases
...
contains the bifurcation sphere\index{bifurcation!sphere} $\Sp$, where $\Hor$ meets $\Hor^-$ (cf. Sec.\ref{s:max:bifur_Kill_hor}).
\end{remark}
