...
are connected by a \emph{Lorentz boost}\index{Lorentz!boost} of velocity $-V$.
\end{remark}
...
The photon's emission direction in the frame of the comoving observer $\Obs_{\rm em}$
is characterized by the angle $\Theta_{\rm em}$ between the photon's linear momentum $\w{P}_{\rm em}$
and the normal to the surface $\w{n}_{\rm s}$; this angle is given by
\be
    \cos\Theta_{\rm em} =
...
\be
    \cos\Theta_{\rm em} = \frac{V +  \eps_r \sqrt{1 - b^2 U(r_{\rm s})}}{1 + \eps_r V
...
is obtained by setting $b=0$ in Eq.~(\ref{e:lem:OS:redshift_pless}):
\be \label{e:lem:OS:redshift_c}
    \encadre{ 1 + z_{\rm c} = \left(1 - \frac{2m}{r_{\rm s}^{\rm em}(0,t_\Obs)} \right)^{-1}
...
For $\tau\to \tau_*$, $r_{\rm s}(\tau) \to 2 m$ and formula~(\ref{e:lem:OS:redshift_c}) yields
