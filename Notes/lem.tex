...
the infalling matter. One defines the
...
where $\w{n}_{\rm s} = n_{\rm s}^t \wpar_t + n_{\rm s}^r \wpar_r$
...
$\w{g}(\w{u}_{\rm s},\w{n}_{\rm s}) = 0$ and $\w{g}(\w{n}_{\rm s},\w{n}_{\rm s}) =1$.
Given the Schwarzschild-Droste components (\ref{e:sch:Schwarz_metric_SD}) for $\w{g}$
and (\ref{e:lem:OS:u_comoving}) for $\w{u}_{\rm s}$,
one gets
\be
    \w{n}_{\rm s} = - \sqrt{\frac{2m}{r_{\rm s}(\tau)} - \frac{2m}{r_0}}
     \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right)^{-1} \, \wpar_t
     + \sqrt{1 - \frac{2m}{r_0} } \, \wpar_r .
\ee
...
Since $\w{p}$ is
a null vector, we have $\w{P}_{\rm em}\cdot\w{P_{\rm em}} = E_{\rm em}^2$.
