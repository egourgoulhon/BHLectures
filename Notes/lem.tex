...
See the legend of Fig.~\ref{f:lem:OS:diag_int_EF} for details.
\textsl{[Figure generated by the notebook \ref{s:sam:Oppenheimer_Snyder}]}
}
\end{figure}
...
Let us suppose that two electromagnetic signals (``flashes'') are emitted in the radial
direction from the surface of the collapsing star at the comoving
observer's proper times $\tau_1$ and $\tau_2$.
Since each signal is carried by a outgoing radial null geodesic (the green curves
in Fig.~\ref{f:lem:OS:diag_delay}), the equation of which with respect to the
IEF coordinates is Eq.~(\ref{e:sch:outgoing_null_geod_EF}), we have
\[
...
We deduce from the above relation the $\ti$ lapse at reception by $\Obs$
between the signal emitted at proper time $\tau_2$ and that emitted at $\tau_1$:
\be
