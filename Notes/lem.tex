...
As an application of the general Lemaître-Tolman framework, let us consider
...
Actually only the solutions with $E(\chi)<0$, i.e. given by Eq.~(\ref{e:lem:sol_E_neg}),
are to be considered because they are the only one that allow a departure
from rest.

In addition to the hypothesis that the collapse starts from rest, let us
assume that the ``star'' is \emph{homogeneous}, i.e. that the density
...
is a priori a function of $(\tau,\chi)$, must be a function of $\tau$ only.
