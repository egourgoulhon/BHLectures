...
are always finite; on the other side, the logarithm term diverges when $\tau_2$ approaches $\tau_*$ [cf. Eq.~(\ref{e:lem:OS:tau_star})], i.e. when $r_{\rm s}(\tau_2)$ approaches $2 m_0$,  so that
\be
...
proceeds. The delay between two successive signals becoming infinite, so that, among the emitted sequence, there exists a very
last signal received by $\Obs$. All subsequent signals do not cross the event horizon,
i.e. are captured by the black hole.
This is illustrated by Fig.~\ref{f:lem:OS:diag_delay}, which depicts the Oppenheimer-Snyder collapse
with $r_0 = 4 m_0 \iff \chi_{\rm s} = \pi/4$. Starting from $\tau=0$, a uniform sequence of 8 signals is emitted from the star's surface, the proper time separation between two successive signals being
...
\begin{greybox}
For a remote static observer, the collapse, as monitored from signals regularily emitted at the star's surface, appears to slow down and even to freeze completely when the areal radius
of the star tends to the gravitational radius $2 m_0$. In particular, the remote observer never sees the
star shrink below its gravitational radius and does not perceives at all the final singularity.
\end{greybox}
...
parameter along them (cf. Secs.~\ref{s:sch:rad_null_geod} and \ref{s:gis:radial}), so that $r = \alpha \lambda$
...
$z = (1 - 2m_0/r_0)^{-1/2} - 1$, which is the standard expression for the redshift at infinity of
...
with a characteristic time scale $4 m_0$. Equivalently, the energies of the received photons
