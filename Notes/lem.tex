...
The stellar surface is marked by the thick red curve and some outgoing radial
null geodesics are drawn as green straight lines.
The black hole event horizon $\Hor$ is indicated by
the thick black line, the shadded region being the black hole interior.
...
It is then natural to consider \emph{maximal} or \emph{inextendible} spacetimes,
i.e. a spacetime in which each geodesic either is complete or terminates
at a singularity. This is clearly not the case for the spacetime depicted in the
diagrams of
Figs.~\ref{f:lem:OS:diag_int_EF} and \ref{f:lem:OS:diag_KS}: when run
in the past direction, causal geodesics
in the star interior stop at the $\tau=0$ hypersurface
...
is based on the assumption that the star at $\tau=0$ corresponds actually to a moment of
...
diagram of Fig.~\ref{f:lem:OS:CP_diag_sym}. Contrary to the ``astrophysical'' extension
discussed above, this one has the advantage to invoke only pressureless matter
and therefore to be described by an exact solution of Einstein's equation:
the Friedmann-Lemaître-Robertson-Walker solution (\ref{e:lem:OS:g_int})
extended to $\eta \in (-\pi, \pi)$
inside the star and the Schwarzschild solution (\ref{e:lem:OS:exterior_metric})
outside the star.
...
Carter-Penrose diagram of a semiclosed world.
}
\end{figure}
