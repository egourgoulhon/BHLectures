...
is based on the assumption that the star at $\tau=0$ corresponds to a moment of
...
The time-symmetric extended solution described above provides us with a way
to construct pressureless solutions with $\chi_{\rm s} \in (\pi/2,\pi)$.
We disregarded these solutions in Sec.~\ref{s:lem:OS_sol}, after having noticing
that they lead to a decreasing areal radius $r$ as a function of $\chi$ for
$\chi\in [\pi/2, \chi_{\rm s}]$. In other words, starting from the center of the
star at $\chi = 0$, the areal radius $r$ increases up a maximum value for
$\chi=\pi/2$, which is $r_{\rm s}(\tau)/\sin\chi_{\rm s}$, and then decreases
to $r_{\rm s}(\tau)$ for $\chi = \chi_{\rm s}$. We realize that such a behavior
...
``to the right''. This happens in the region $\hat{X} < 0$ of the compactified
...
We note that the star exterior comprises the entire $\M_{\rm I}$ region of
...
contains the bifurcation sphere, where $\Hor$ meets $\Hor^-$.
\end{remark}
