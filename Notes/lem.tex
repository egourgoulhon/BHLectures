...
\subsubsection{Apparent collapse freezing}
...
that the maximal value of $b$ is
\be \label{e:lem:OS:bmax_static}
    b_{\rm m}(t_\Obs) = \frac{r_0}{\sqrt{1 - 2m/r_0}} \quad\mbox{for}\quad  t_\Obs \leq t_\Obs^*
...
\be \label{e:lem:OS:bmax_final}
    b_{\rm m}(t_\Obs) = 3\sqrt{3} \, m \quad\mbox{for}\quad  t_\Obs \geq t_\Obs^{**}
        \quad\mbox{or}\quad r_0 \leq 3 m ,
\ee
where $t_\Obs^{**}$ stand's for the proper time of $\Obs$ at which the image's rim
is formed by photons emitted from the star surface when $r_{\rm s}(\tau) = 3m$.
...
the radius of the disk image shrinks from $r_0 / \sqrt{1 - 2m/r_0}$
[Eq.~(\ref{e:lem:OS:bmax_static})]
to $3\sqrt{3} \, m$ [Eq.~(\ref{e:lem:OS:bmax_final})] in a finite proper
time of the remote observer $\Obs$: $\Delta t_\Obs = t_\Obs^{**} - t_\Obs^*$.
After this, it keeps the same value.
\item Collapsing stars that are initially ultracompact
($r_0 < 3 m$) generate images of constant size: a disk of radius
$3\sqrt{3} \, m$.
\end{itemize}
\end{greybox}
...
It is important to realize that the photons that form the star's image
on $\Obs$'s screen at a given instant $t_\Obs$ have been emitted from the
star's surface at various proper times $\tau$, so that the disk image does not
provide any ``instantaneous view'' of the star in terms of $\tau$.
This can be quantified by considering the elapsed
Schwarzschild-Droste time $t$ between the photon emission and its reception
by $\Obs$ along a null geodesic $\Li$ of impact parameter $b$. Thanks to spherical
...
of $\Li$ and $\eps_r = +1$ (resp. $-1$) on portions of $\Li$ where $r$ increases
(resp. decreases) towards the future along $\Li$. Note that $\eps_L$ is always
...
just around the emission point and changing to $+1$ after some periastron
passage.
...
manifests itself first at the center of the disk image.
