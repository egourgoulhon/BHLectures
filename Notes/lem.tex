...
the thick black line. The dashed blue line marks the trapping horizon
...
One may also think about a compactified conformal spacetime diagram, i.e.
...
Since they offer a view of the entire spacetime, these diagrams are
usually \emph{maximal}, i.e. each geodesic is either complete or terminates
on a singularity. This is clearly not the case of the diagrams in
...
(the magenta line). They are thus incomplete (for instance $\tau$
is an affine parameter along the free-falling matter geodesics) and could be extended to
$\tau < 0$. There are actually two ways to extend the spacetime.
The first one corresponds to the astophysical situation of a star in
hydrostatic equilibrium for $\tau < 0$ and in which the pressure drops suddenly to zero at $\tau = 0$.
The corresponding Carter-Penrose diagram is drawn schematically\footnote{This diagram is not
constructed on explicited coordinates.} in Fig.~\ref{f:lem:OS:CP_diag}. It describes a
maximally extended spacetime: all geodesics are inextendible. The star seems to expand from
the past timelike infinity $i^-$, but this is an artifact of the drawning\footnote{Recall that a
Carter-Penrose reflect the conformal structure but does not respect the metric distances.}: the star surface
(red line) follows actually a curve $r = \mathrm{const}$ for $\tau < 0$. For $\tau \geq 0$,
the surface of the star is drawn as a vertical line, while the radius is actually
decreasing, as in the left part of Fig.~\ref{f:lem:OS:diag_int_EF}. Actually, one may consider that this
part of the Carter-Penrose diagram is drawn by means of the conformal coordinates $(\eta,\chi)$ used
in Fig.~\ref{f:lem:OS:diag_int_EF}.
