...
star's surface. By the very definition of $\Theta_{\rm em}$, these photons are
thus emitted in the direction $\Theta_{\rm em} = \pi/2$. One has then $\cos\Theta_{\rm em} = 0$
and Eq.~(\ref{e:lem:cos_Theta_em}) implies $\epsilon_r = -1$ for $V>0$, i.e. as soon as
...
is increasing monoteneously ($\eps_r = +1$) until reaching observer $\Obs$.

The redshift $z_\star$ at the image's rim is thus obtained by
...
$\Gamma$ can be substituted by expression (\ref{e:lem:OS:V_Gamma}), which
...
\ee
Hence the redshift at the image's rim keeps a constant value.
...
in that limit). The Doppler blueshift is of course due to the rim's photon being
...
\be \label{e:lem:elem_flux}
   \D F = I_\nu \, \D\Omega \, \D\nu ,
\ee
where $I_\nu$ is the \defin{specific intensity}\index{specific!intensity} of the electromagnetic
radiation from the surface of the collapsing star\footnote{The notation $I_\nu$ follows
the standard convention in the literature, the subscript $\nu$ being merely an indicator to
...
in arbitrary motion subtend identical solid angles when seen by the other observer, up to some redshift factor $(1+z)^2$. This theorem can be proved via geometrical optics in curved spacetime
...
coordinate time expressed via Eq.~(\ref{e:lem:OS:t_em_b_tO}), $\nu_{\rm em}$ is
the emission frequency and, by the very definition of the redshift, is related
...
The \defin{integrated intensity} $I$ is the specific intensity integrated over all the frequencies:
\be
...
from this formula that the integrated intensity at the center of the image
decay exponentially with a characteristic time equal to $m$:
...
The \defin{specific flux} of the image is obtained by integrating
...
The \defin{integrated flux} of the image is the specific flux integrated over all the frequencies:
\be
...
the emission model, the integrated flux is decaying exponentially according to
...
He obtained the exponential decay formula (\ref{e:lem:integrated_flux}) for the total flux,
...
\cite{AmesT68}. Podurets considered only the total flux, which is relevant for a remote observer who
cannot resolve the collapsing star. The integrated intensity at the center of the image
has been derived by
...
Ames and Thorne in their 1968 study \cite{AmesT68}, who showed that
the image is brightest at its rim, that the redshift is constant there and that the width of this
...
which encompasses extra photons: those that are backward-emitted. In 1979, Kayll Lake\index{Lake, K.} and
