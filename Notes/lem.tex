...
\ee
for the freely specifiable functions appearing in the solutions of the Lemaître-Tolman system presented in
Sec.~\ref{s:lem:sol_lambda_zero},
instead of the choice (\ref{e:lem:OS:free_func}). This leads to
$E(\chi) = 0$ [cf. Eq.~(\ref{e:lem:E_f_chi})]
and hence to the solution~(\ref{e:lem:sol_E_zero}) instead of (\ref{e:lem:sol_E_neg}).
This solution is still homogeneous, $\rho$ being a function of $\tau$ only, but it is less adapted
to describe a star of finite size collapsing from an equilibrium configuration than the
...
the black line, the shadded region being the black hole interior.
...
be extended to
$\tau < 0$. There are actually two ways to extend the Oppenheimer-Snyder spacetime
...
This extension is ``astrophysical'' insofar as it models the gravitational collapse of the core of a massive star. Of course the equilibrium for $\tau\in(-\infty,0)$ requires
some nonzero pressure inside the star. Therefore it cannot be described by the
Lemaître-Tolman equations discussed in Sec.~\ref{s:lem:LT_equat}, but rather
by the Tolman-Oppenheimer-Volkoff\index{Tolman-Oppenheimer-Volkoff equations}
equations, which govern hydrostatic equilibrium in
spherical symmetry (see e.g. Chap.~23 of Ref.~\cite{MisneTW73}).
The Carter-Penrose diagram corresponding to this extension
is drawn schematically\footnote{This diagram is not
constructed on explicit coordinates.} in Fig.~\ref{f:lem:OS:CP_diag}. The star seems to expand from
the past timelike infinity $i^-$, but this is an artifact of the drawning\footnote{Recall that a
Carter-Penrose reflects the conformal structure but does not respect the distances.}: the star surface
...
\begin{figure}
\centerline{\includegraphics[height=0.3\textheight]{lem_OS_CP_diag_sym.pdf}}
...
and the white hole region (bottom shadded area), bounded by the past event
horizon $\Hor^-$.
}
\end{figure}

The second maximal extension of the Oppenheimer-Snyder spacetime
is based on the assumption that the matter time $\tau=0$ is an instant of
...
and hence to be described by an exact solution of Einstein's equation:
the Friedmann-Lemaître-Robertson-Walker solution (\ref{e:lem:OS:g_int})
extended to $\eta \in (-\pi, \pi)$
inside the star and the Schwarzschild solution
outside the star. This maximal solution contains a white hole\index{white hole} region,
...
Note also that the star is not visible from the infinite past of the remote static observer
$\Obs$: it appears suddenly to $\Obs$
at the event denoted by $a$ in Fig.~\ref{f:lem:OS:CP_diag_sym}.
On the contrary, in the ``astrophysical'' model discussed above,
the star is always visible to $\Obs$ (cf. Fig.~\ref{f:lem:OS:CP_diag}),
despite it fades very rapidly after the collapse phase starts to be observed,
as we shall see in Sec.~\ref{s:lem:obs}.

\begin{figure}
\centerline{\includegraphics[height=0.3\textheight]{lem_OS_CP_diag_sclosed.pdf}}
...
The time-symmetric extended solution described above opens the path
...
The corresponding Carter-Penrose diagram is shown in Fig.~\ref{f:lem:OS:CP_diag_sclosed}.
...
or Sec. 2.7.2 of Ref.~\cite{FroloN98}).
...
\begin{remark}
The above results are not specific to presureless collapse: they
...
surface in the static (precollapse) case, which implies that
...
in the frequency bandwith $[\nu, \nu + \D\nu]$ is represented by the
energy flux\index{energy!flux} (energy per unit of time and unit of area)
$\D F$ given by
...
which encompasses extra photons: those that are emitted inward. In 1979, Kayll Lake\index{Lake, K.} and
...
This chapter dealt with spherically symmetric collapse of pressureless matter.
For more realistic models, taking into account pressure, rotation and other physical processes,
see e.g. Chap.~9 of Ref.~\cite{Shiba15} or Chaps.~8, 10 and 14 of Ref.~\cite{BaumgS10}.
In particular, rotation complicates the problem of gravitational collapse significantly, since gravitational waves are emitted in this case.
This prevents the exterior spacetime to be described by an exact simple solution,
such as the Schwarzschild one. Only numerical solutions are known in this case.
