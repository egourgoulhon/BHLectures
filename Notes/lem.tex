...
(cf. Fig.~\ref{f:lem:OS:diag_delay}), with $r_\Obs \gg m$.
...
\ee
If $\Obs$ is asymptotically far, i.e. if $r_\Obs \gg m$, $\ti^{\rm rec}_2 - \ti^{\rm rec}_1$
...
are always finite; on the other side, the logarithm term diverges when $\tau_2$ approaches $\tau_*$ [cf. Eq.~(\ref{e:lem:OS:tau_star})], i.e. when $r_{\rm s}(\tau_2)$ approaches $2 m$,  so that
\be
...
$\delta\tau = \tau_{\rm end}/8 = \pi\sqrt{2} m / 4$. One sees graphically that the signals are received by $\Obs$
...
of the star tends to the gravitational radius $2 m$. In particular, the remote observer never sees the
star shrink below its gravitational radius and does not perceive at all the final singularity.
\end{greybox}
...
\[
    \w{p} = \derd{x^\mu}{\lambda} \wpar_\mu = \alpha \derd{x^\mu}{r} \wpar_\mu
          = \alpha \left[ \left( 1  - \frac{2m}{r} \right) ^{-1} \wpar_t + \wpar_r \right] .
\]
