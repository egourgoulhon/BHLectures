...
\ee
We recognize the \defin{Friedmann-Lemaître-Robertson-Walker (FLRW) metric}\index{Friedmann-Lemaître-Robertson-Walker metric}\index{FLRW metric} corresponding to a closed universe. This is not so surprising since FLRW metrics are solution of Einstein equation based on the hypothesis of homogeneity of constant $\tau$ slices (cf. e.g. Chap.~3 of Ref.~\cite{PeterU09}).
...
\be \label{e:lem:OS:tau_star}
    \tau_* = \frac{\pi - 2 \chi_{\rm s} + \sin(2\chi_{\rm s})}{\sin^3\chi_{\rm s}} m
...
The set formed by all the marginally trapped spheres of fixed $(\eta,\chi)$
is the hypersurface
...
\section{Observational appearance of the black hole formation}
...
\emph{static}\index{static!observer!in Schwarzschid!spacetime}, it is meant
...
\subsection{Apparent collapse freezing}
...
\subsection{Shape and size of the images of the collapse}

%~ As stressed above, the remote static observer $\Obs$ shall never see the star
%~ collapse to a ``point'', but shall instead perceive it to freeze to its gravitational radius.
%~ The image of the star, which is a disk on $\Obs$'s screen, shall even shrink much
%~ less than naively expected, due to strong gravitational lensing effects that
%~ take place in the last stages of the collapse. As analyzed in Secs.~\ref{s:gis:shadow}
%~ and \ref{s:gis:image_disk}, the final images are indeed larger by a factor
%~ $\sim 3\sqrt{3}/2 \sim 2.60$ than those for a collapse
%~ in a flat spacetime. See e.g. Fig.~4 of Ref.~\cite{VinceGN12} for a concrete example
%~ regarding simulated
%~ images of the gravitational collapse of an unstable neutron star into a black hole.
...
Due to spherical
symmetry, the image of the star at a given instant is a disk and is invariant
by rotation around the disk center.
...
travelling radially in the direction $\th=\th_\Obs=\pi/2$ and $\ph=\ph_\Obs=0$.
The image's rim, i.e. the circle bounding the disk, is located at some
maximal value of $b$, $b_{\rm m}(t_\Obs)$ say, where $t_\Obs$
stands for the proper time of observer $\Obs$. Since $\Obs$
is assumed to lie far away in the weak-field region, $t_\Obs$ coincides
with the Schwarzschild-Droste time coordinate at the location of $\Obs$.
A great amount of information on $b_{\rm m}(t_\Obs)$ can be inferred from the effective
...
formed by null geodesics emitted from the star's surface
...
since $U(r)$ is a decreasing function on the interval $(3m,+\infty)$ (cf. Fig.~\ref{f:gis:eff_pot_null}).
...
\be \label{e:lem:OS:bmax_static}
    b_{\rm m}(t_\Obs) = b_0 := \frac{r_0}{\sqrt{1 - 2m/r_0}} \quad\mbox{for}\quad  t_\Obs \leq t_\Obs^{**}
        \quad\mbox{and}\quad r_0 > 3 m .
\ee
For $t_\Obs \leq t_\Obs^{**}$, a
null geodesic at the image's rim, i.e having $b=b_{\rm m}(t_\Obs)$, fullfils
...
$\Li$, even if the latter is extented infinitely far in the past. This implies
...
\be
    b_{\rm m}(t_\Obs) = r_0 \cos\chi_{\rm s} \quad\mbox{for}\quad  t_\Obs \leq t_\Obs^{**}
        \quad\mbox{and}\quad r_0 > 3 m .
\ee
In the non-relativistic limit, $\chi_{\rm s} \to 0$ and we get $b_{\rm m}(t_\Obs) = r_0$,
...
One can also use the effective potential analysis to get the value of $b_{\rm m}(t_\Obs)$
...
decreasing on $(2m, 3m)$, reaches a minimun at $r=3m$ and increases on $(3m, +\infty)$
...
the image boundary $b_{\rm m}(t_\Obs) = b_{\rm c}$ is reached only for
$t_\Obs \to +\infty$:
\be \label{e:lem:OS:bmax_final}
    \lim_{t_\Obs \to +\infty} b_{\rm m}(t_\Obs) = 3\sqrt{3} \, m .
\ee
However, one can show (see e.g. Ref.~\cite{YoshiTN19} for
details) that $b_{\rm m}(t_\Obs)$ converges to $b_{\rm c}$ exponentially fast:
$b_{\rm m}(t_\Obs) - b_{\rm c} \propto \exp(- t_\Obs/b_{\rm c})$.
...
but the convergence is exponentially fast, so that, $b_{\rm m}(t_\Obs) \simeq 3\sqrt{3} \, m$
...
remains fixed at $b_{\rm m} = 3\sqrt{3} \, m$.
\end{itemize}
\end{greybox}

\begin{remark}
The above results are not specific to the Oppenheimer-Snyder collapse: they
...
decrease: only $8\%$. This is of course because such a star has a very compactness.
...
the range $0\leq b \leq b_{\rm m}(t_\Obs)$ and have been emitted from the
...
symmetry, it suffices to consider photons travelling in the plane
...
\ee
if there is a periastron, located at $r=r_{\rm p} < r_{\rm s}^{\rm em}(b,t_\Obs)$.
For a given position of the screen, i.e. a given value of $b$,
Eq.~(\ref{e:lem:OS:t_em_b_tO}) must be solved to get $t_{\rm em}(b,t_\Obs)$. In general, this
is a complicated problem since the lower boundary of the integral, $r_{\rm s}^{\rm em}(b,t_\Obs)$,
depends on $t_{\rm em}(b,t_\Obs)$ in a complicated way, via the parametric
system (\ref{e:lem:OS:free_fall_em}). However, this is not the case for
$t_{\rm em}(b,t_\Obs)< 0$, since then $r_{\rm s}^{\rm em}(b,t_\Obs) = r_0$
...
of $b$, since at fixed $r$, $b \mapsto \left( 1 - b^2 U(r) \right) ^{-1/2}$ is an increasing function of $b$, given that $U(r) > 0$.
...
$b=0$ (which implies $\Delta=0$ since pure radial geodesics do not have any
periastron), $t_{\rm em}(b,t_\Obs) = 0$ and $r_{\rm s}^{\rm em}(b,t_\Obs) = r_0$
in Eq.~(\ref{e:lem:OS:t_em_b_tO}):
\be
    t_\Obs^* = \int_{r_0}^{r_\Obs}
        \left(1 - \frac{2m}{r} \right) ^{-1} \; \D r .
\ee
On the other side, the value of $t_\Obs^{**}$ (start of the decrease of disk image's radius)
...
\ee
By subtracting the above two equations and substituting (\ref{e:ges:eff_pot_null})
for $U(r)$, we get
\be
    t_\Obs^{**} - t_\Obs^* = b_0^2 \int_{r_0}^{r_\Obs}
    \frac{\D r}{r^2 - b_0^2(1 - 2m/r) + r\sqrt{r^2 - b_0^2(1 - 2m/r) }}.
\ee
Since $r^2  - b_0^2(1 - 2m/r) \geq 0$ for $r \geq r_0$, the time delay
$t_\Obs^{**} - t_\Obs^*$ is positive.
...
Since in the Oppenheimer-Snyder collapse, this observer is in radial free fall
from rest,
...
\be \label{e:lem:OS:u_comoving}
    \w{u}_{\rm s} = \sqrt{1 - \frac{2m}{r_0} }
         \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right)^{-1} \, \wpar_t
        - \sqrt{\frac{2m}{r_{\rm s}(\tau)} - \frac{2m}{r_0}} \,
        \wpar_r .
\ee
...
\be \label{e:lem:OS:u_s_static}
    \w{u}_{\rm s} = \Gamma(\tau) \left( \w{e}_{(t)}  - V(\tau) \w{e}_{(r)} \right),
\ee
with
\be \label{e:lem:OS:V_Gamma}
    V(\tau) := \sqrt{\frac{\frac{2m}{r_{\rm s}(\tau)} - \frac{2m}{r_0}}{1 - \frac{2m}{r_0}}}
    \qand
    \Gamma(\tau) := \frac{1}{\sqrt{1 - V(\tau)^2}}  = \sqrt{\frac{1 - \frac{2m}{r_0}}{1 - \frac{2m}{r_{\rm s}(\tau)}}}
\ee
By comparing with the generic formulas~(\ref{e:fra:Gam_V2})-(\ref{e:fra:u_Gamma_V}), we conclude that
$V(\tau)$ is the norm of the velocity $\w{V} = -V(\tau)\w{e}_{(r)}$ of $\Obs_{\rm em}$ with respect to the local
static observer and $\Gamma(\tau)$ is the corresponding Lorentz factor.
...
is a unit spacelike vector normal to the star surface and the vectors
...
\be
    \w{n}_{\rm s} = - \Gamma(\tau) V(\tau) \w{e}_{(t)} + \Gamma(\tau) \w{e}_{(r)}.
\ee
...
the impact parameter $b$. Once reexpressed
...
\be \label{e:lem:OS:p_photon_static}
    ...
    \left( \w{e}_{(t)} + \eps_r \sqrt{1 - b^2 U(r)} \, \w{e}_r \right)
...
\be
    1 + z = \frac{E_{\rm em}}{E_\infty}
    = \Gamma(\tau) \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right) ^{-1/2}
    \left( 1  + \eps_r V(\tau) \sqrt{1 - b^2 U(r_{\rm s}(\tau))} \right) .
\ee
Substituting expressions~(\ref{e:lem:OS:V_Gamma}) for $V(\tau)$
and $\Gamma(\tau)$, as well as (\ref{e:ges:eff_pot_null}) for $U(r)$, yields
\be
    \encadre{
    1 + z = \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right) ^{-1}
    \left[ \sqrt{1 - \frac{2m}{r_0}}
        + \eps_r \sqrt{\frac{2m}{r_{\rm s}(\tau)} - \frac{2m}{r_0}}
        \sqrt{1 - \frac{b^2}{r_{\rm s}(\tau)^2} \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right)}
        \right] } .
\ee
...
$\tau$ has to be replaced by ...


Since $\w{p}$ is
a null vector, we have $\w{P}_{\rm em}\cdot\w{P}_{\rm em} = E_{\rm em}^2$.
...
The energy of a photon measured by an observer of 4-velocity $\w{u}$ is
...
\be \label{e:lem:OS:p_photon}
     \w{p} = E_\infty  \left[ \left( 1  - \frac{2m}{r} \right) ^{-1} \wpar_t + \wpar_r \right] ,
\ee
...
\be
    E_{\rm em} = E_\infty
    \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right)^{-1}
    \left( \sqrt{1 - \frac{2m}{r_0} } + \sqrt{\frac{2m}{r_{\rm s}(\tau)} - \frac{2m}{r_0}}
    \right) .
\ee
The photon's \defin{redshift}\index{redshift!in Oppenheimer-Snyder collapse} when received by $\Obs$ is\footnote{In this expression,
...
\be \label{e:lem:OS:redshift}
    \encadre{ z = \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right)^{-1}
    \left( \sqrt{1 - \frac{2m}{r_0} } + \sqrt{\frac{2m}{r_{\rm s}(\tau)} - \frac{2m}{r_0}}
    \right)  - 1 } .
\ee
...
For $\tau = 0$, $r_{\rm s}(\tau) = r_0$ and formula~(\ref{e:lem:OS:redshift}) reduces to
...
\be \label{e:lem:OS:redshift_sim}
    z \underset{\tau\to\tau_*}{\sim}  2 \sqrt{1 - \frac{2m}{r_0} }
     \left(1 - \frac{2m}{r_{\rm s}(\tau)} \right)^{-1} ,
\ee
...
\be
   \encadre{ z \underset{\tau\to\tau_*}{\sim}  2 \sqrt{1 - \frac{2m}{r_0} }
     \; \mathrm{e}^{\Delta t_\Obs/(4 m)} } .
\ee
%~ Hence we conclude:
%~ \begin{greybox}
%~ The radiation from the surface of the collapsing star is received by the remote static observer $\Obs$
%~ with an increasing redshift. At late times, the redshift grows exponentially
%~ with a characteristic time scale $4 m$. Equivalently, the energies of the received photons
