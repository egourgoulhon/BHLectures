...
The unfamiliar reader is urged to take a look at Appendix~\ref{s:bas}
(Sec.~\ref{s:bas:ext_deriv}) first.
...
\ee
Here $\w{L}$ is the \defin{Lagrangian $n$-form}\index{Lagrangian!form}\footnote{As the integral of a $n$-form over an oriented $n$-dimensional manifold, $\mathcal{S}$ is well-posed and coordinate independent.},
...
extra fields, as for instance in scalar-tensor theories, where $\w{L}$ is a function
...
by setting $\Phi' := \Phi^{-1}$ and
renaming $\Phi'$ to $\Phi$:
\begin{tdef}{diffeomorphism-invariant gravity theory}{}
...
\begin{tprop}{Lagrangian of diffeomorphism-invariant theories
...
\ee
\end{tprop}
The reader is referred to the original article by Iyer \& Wald \cite{IyerW94} for the proof.

\begin{texample}{Lagrangian of general relativity}{tha:EH_Lagrangian}
...
\begin{tprop}{variation of the Lagrangian of a pure metric theory}{tha:var_Lagrangian}
...
depends on both $\w{g}$ and $\delta\w{g}$, and in a linear manner with respect to $\delta\w{g}$.
...
First of all, $\w{\theta}$ in formula~(\ref{e:tha:delta_L_E_Theta})
can be substituted by $\w{\theta'} = \w{\theta} + \dd \w{Y}$,
where $\w{Y}=\w{Y}\lld\w{g},\delta\w{g}\rld$ is an
arbitrary $(n-2)$-form. Indeed, the nilpotence of $\dd$ [Eq.~(\ref{e:ext_der_nilpot})] implies
$\dd\dd \w{Y} = 0$, so that $\dd \w{\theta'} = \dd \w{\theta}$.
Secondly, for a given gravity theory, the Lagrangian $n$-form itself is not unique.
Indeed the $n$-form
...
\begin{tprop}{ambiguities in the presymplectic potential form}{tha:ambiguities_theta}
...
\begin{texample}{presymplectic potential form of general relativity}{tha:theta_GR}
...
obey $e_{01\ldots(n-1)} = 1$ [cf. Eq.~(\ref{e:bas:eps_sqrt_g})].
...
\ee
The last equality involves the Hodge dual $\star\uu{H}$ of the 1-form $\uu{H}$
[cf. Eq.~(\ref{e:sta:Hodge_dual}) with $p=1$].
In index notation, $\theta_{\alpha_1\cdots\alpha_{n-1}} = (16\pi)^{-1}  H^\mu \eps_{\mu\alpha_1\cdots\alpha_{n-1}}$.
Substituting Eq.~(\ref{e:evo:def_H}) for $H^\mu$, we get an explicit expression of
...
\ee
We check that $\w{\theta}\lld\w{g},\delta\w{g}\rld$ is linear in $\delta\w{g}$, as it should (Property~\ref{p:tha:var_Lagrangian}).
\end{texample}

\begin{tprop}{field equations from the least action principle}{}
...
In view of Eqs.~(\ref{e:tha:action_L}) and (\ref{e:tha:delta_L_E_Theta}), there comes
\[
...
\begin{texample}{field equations of general relativity}{tha:field_equations_GR}{}
...
\subsection{Noether current related to diffeomorphism invariance}
...
as the \emph{derivative} at $t=0$ of a 1-parameter family of metric tensors $\hat{\w{g}}(t)$ such that
...
since $\dd \w{L} = 0$, as a $(n+1)$-form on a $n$-dimensional manifold.
...
right-hand side of Eq.~(\ref{e:tha:dd_pre_J_E}) is zero, which means that the
...
\begin{tprop}{Noether theorem for diffeomorphisms generated by a vector field}{tha:Noether_current}
...
\begin{texample}{Noether current form of general relativity}{}
...
\begin{tprop}{Noether potential form}{tha:Noether_pot_form}
...
\begin{texample}{Noether potential form of general relativity}{tha:Noether_potential_form_GR}
...
\begin{tprop}{Noether charge of a spacelike hypersurface}{tha:Noether_charge_hypersurf}
...
\begin{tprop}{Noether charge of a closed spacelike $(n-2)$-surface}{tha:Noether_charge_surf}
Let us consider a diffeomorphism-invariant theory described by a Lagrangian $n$-form $\w{L}$
...
we notice that the $(n-2)$-form $\star (\dd \uu{\xi})$ giving $\w{Q}\lld\w{\xi}\rld$
for general relativity [cf. Eq.~(\ref{e:tha:Q_GR_Komar})]
is exactly the differential form constituting the integrand in
...
\begin{tprop}{Komar mass and angular momentum as Noether charges}{tha:Komar_Noether}
...
\begin{tprop}{Wald entropy as a Noether charge}{}
...
\begin{tprop}{Wald entropy for general relativity}{tha:Wald_S_for_GR}
...
\begin{tprop}{geometric expression of the Wald entropy
\textnormal{(Iyer\index[pers]{Iyer, V.} \& Wald\index[pers]{Wald, R.M.} 1994)}}{tha:S_Wald_explicit}
...
To go further, we must invoke a result of Iyer and Wald
...
\begin{texample}{Wald entropy from formula (\ref{e:tha:S_Wald_explicit}) for general relativity}{}
...
\begin{texample}{phase space of a pendulum}{tha:pendulum}
...
%\begin{texample}{phase space as the cotangent bundle of configuration space}{}
...
\begin{tprop}{Hamiltonian system}{tha:Hamiltonian_system}
...
\begin{texample}{solutions for the gravity pendulum}{}
...
Despite the features stressed in Remark~\ref{r:tha:Ham_syst_no_canonical_coord}, the Hamiltonian formulation sketched in
...
\begin{tprop}{covariant phase space}{tha:cov_phase_space}
...
\cite{LeeW90,HarloW20}, i.e. a topological space that locally resembles\footnote{Cf. Refs.~\cite{ChoquDD82} (Chap.~7) and \cite{Lang95} (Chap.~2) for a
...
\begin{tprop}{space of asymptotically flat solutions}{tha:asymp_flat_sol}
...
\begin{tprop}{presymplectic form in the space of solutions of a gravity theory}{tha:presymplectic_form}
...
\begin{tprop}{presymplectic current $(n-1)$-form}{tha:presymplectic_current}
...
\begin{texample}{presymplectic current form of general relativity}{}
...
\begin{tlemma}{asymptotic vanishing of $\Dd \Upsilon_{\Sigma_r,\Sigma'_r}$}{tha:Upsilon_closed}
...
\begin{tprop}{uniqueness of the presymplectic form}{tha:uniqueness_presympl}
...
\begin{tprop}{closedness of the presymplectic current $(n-1)$-form}{tha:omega_closed}
...
\begin{tprop}{fundamental identity of the covariant phase space formalism}{}
...
\begin{texample}{asymptotic behavior of the presymplectic potential form in general relativity}{}
...
$H_{\w{\xi}}$ is \cite{WaldZ00}

\begin{tprop}{existence of a Hamiltonian conjugate to a vector field
...
\begin{tprop}{canonical energy relative to an asymptotic time translation}{tha:canonical_energy}
...
\begin{texample}{canonical energy for general relativity is ADM energy}{tha:E_GR_ADM}
...
\begin{tprop}{canonical angular momentum relative to an asymptotic spatial rotation}{tha:canonical_angu}
Let $(\M,\w{g})$ be an asymptotically flat spacetime governed by some
...
\begin{texample}{canonical angular momentum of an axisymmetric spacetime in general relativity}{}
...
Finally, we arrive at the first law, as established by Iyer \& Wald \cite{IyerW94,Wald93}:

\begin{tprop}{first law of black hole thermodynamics for diffeomorphism-invariant theories
\textnormal{(Iyer\index[pers]{Iyer, V.} \& Wald\index[pers]{Wald, R.M.} 1994)}}{tha:first_law}
...
$\mathcal{E} = M$ (cf. Example~\ref{x:tha:E_GR_ADM}).
However, one should
...
\begin{tprop}{first law for general relativity}{tha:first_law_GR}
...
\cite{HollaWZ24,Wald25,VisseY24}, to be discussed below.
\end{remark}
...
We refer to \cite{Wald25,VisseY24} for extended discussions about the HWZ entropy, especially
