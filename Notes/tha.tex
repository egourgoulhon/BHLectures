...
(Property~\ref{p:tha:uniqueness_presympl} below). So, for
...
[Eq.~(\ref{e:tha:Theta_gal_freedom})] in the definition of $\w{\theta}$.
\end{prop}
\begin{proof}
First of all, the change $\w{\theta'} = \w{\theta} + \delta\w{\mu}$,
in Eq.~(\ref{e:tha:Theta_gal_freedom}) has no impact
on the value of the presymplectic current $(n-1)$-form $\w{\omega}$.
Indeed,
we may write the variation of the $(n-1)$-form $\w{\mu} = \w{\mu}\lld\w{g}\rld$
as $\delta\w{\mu}=
\w{\mu}^{(1)\mu\nu} \delta g_{\mu\nu}$,
...
Introducing furthermore the type-$(4,n-1)$ tensor
field $\w{\mu}^{(2)} =  \w{\mu}^{(2)}  \lld\w{g}\rld$ such that
$\delta \w{\mu}^{(1)\mu\nu} = \w{\mu}^{(2)\rho\sigma\mu\nu} \delta g_{\rho\sigma}$, we get
\[
  \der{}{\lambda_1}
        \left( \w{\theta'}\Lld\w{g}, \der{\w{g}}{\lambda_2} \Rld\right)
    = \der{}{\lambda_1}
        ...
        +  \w{\mu}^{(2)\rho\sigma\mu\nu} \der{g_{\rho\sigma}}{\lambda_1}
             \der{g_{\mu\nu}}{\lambda_2} .
\]
It is then clear from Eq.~(\ref{e:tha:def_omega_der_theta}) that the terms
involving $\w{\mu}^{(1)}$ and $\w{\mu}^{(2)}$ do not contribute to $\w{\omega}$.
