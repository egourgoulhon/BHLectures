...
and should be such that some integrals considered below converge and that
the asymptotic limit of some surface integrals is zero.
By ``asymptotic limit'' it is meant
the limit $r\to +\infty$ of a family $(\Sp_r)$ of closed $(n-2)$-surfaces
that lie in a hypersurface $\Sigma$ of $\M$ such that
$\Psi(\Sigma) = \hat{\Sigma}$, where $\Psi$ is a diffeomorphism
$\M \to \R\times \hat{\Sigma}$, and
$\Psi(\Sp_r)$ is the $(n-2)$-sphere $\hat{\Sp}_r$  of Euclidean radius
$r$ in $\hat{\Sigma}_{\rm ext}$. For general relativity, a precise definition
...
background future of $\Sigma$. Let $\mathscr{D}$ be the part
of $\M$ bounded by $\Sigma$ and $\Sigma'$. Let us then consider the
...
with respect to $\mathscr{D}$ and $\mathscr{W}_\infty$ is a hypersuface
connecting $\Sigma$ to $\Sigma'$ ``at infinity'' so as to make
$\Sigma\cup\Sigma'\cup\mathscr{W}_\infty$ the ``boundary'' of  $\mathscr{D}$.
More rigourously, one may introduce compact subparts $\Sigma_r$ and $\Sigma'_r$
of $\Sigma$ and $\Sigma'$
extending in the asymptotic flat end and corresponding to balls of radius $r$
in $\hat{\Sigma}_{\rm ext}$, as well as the hypercylinder $\mathscr{W}_r$
connecting $\Sigma_r$ to $\Sigma'_r$ (cf. Fig.~??). Equation~(\ref{e:tha:dS_D_3int}) can then
be viewed as the limit $r\to +\infty$ of Stokes' theorem
applied to the closed surface $\Sigma_r\cup\Sigma'_r\cup\mathscr{W}_r$.
...
\be \label{e:tha:dS_D_Theta_Upsilon}
    \Dd \mathcal{S}_{\mathscr{D}} = \Theta_{\Sigma'} - \Theta_{\Sigma}
    + \Upsilon_{\Sigma,\Sigma'} .
\ee
Here $\Theta_{\Sigma}$ stands the 1-form on $\FF$ defined for any hypersurface
$\Sigma$ of $\M$ diffeomorphic to $\hat{\Sigma}$ by
the following action on a vector $X$ in the
tangent space $T_{\w{g}} \FF$:
\be \label{e:tha:def_Theta_X_theta}
    \langle \Theta_\Sigma, X \rangle := \int_{\Sigma} \w{\theta}\Lld\w{g},\frac{\delta\w{g}}{\delta\lambda}\Rld ,
\ee
where $\delta\w{g}/\delta\lambda$ is the relative variation of $\w{g}$ defining the
vector $X$ by Eq.~(\ref{e:tha:X_dg/dlambda}) and the orientation of $\Sigma$
is that provided by any background-future directed vector transverse to $\Sigma$.
Similarly, $\Upsilon_{\Sigma,\Sigma'}$ stands for the 1-form on $\FF$ defined
by
\be \label{e:tha:def_Upsilon}
    \langle \Upsilon_{\Sigma,\Sigma'}, X \rangle :=
    \lim_{r\to+\infty} \int_{\mathscr{W}_r^{\nearrow}} \w{\theta}\Lld\w{g},\frac{\delta\w{g}}{\delta\lambda}\Rld ,
\ee
with $\mathscr{W}_r$ defined as above.
Note that since $\w{\theta}\lld\cdot,\cdot\rld$ is linear in its second argument,
the maps $\Theta_\Sigma$ and $\Upsilon_{\Sigma,\Sigma'}$ introduced above
are linear and hence define 1-forms on $\FF$

In an attempt to endow $\FF$ with a symplectic form, thereby turning it into a phase space,
let us consider the 2-form $\Omega$ that is the exterior derivative of $\Theta_\Sigma$,
i.e. $\Omega := \Dd \Theta_\Sigma$. Notice that this is analogous to Eq.~(\ref{e:tha:Omega_dTheta}).
While $\Theta_\Sigma$ depends on the choice of $\Sigma$, $\Omega$ does not:
...
\be \label{e:tha:def_Omega_gravity}
   \encadre{ \Omega := \Dd \Theta_\Sigma },
\ee
where $\Theta_\Sigma$ is the 1-form on $\FF$ defined by Eq.~(\ref{e:tha:def_Theta_X_theta}),
is a \defin{presymplectic form} on $\FF$, i.e. a closed 2-form:
$\Dd\Omega = 0$.
Moreover, $\Omega$ does not depend on the choice of the hypersurface $\Sigma$.
\end{prop}

\begin{proof}
That $\Omega$, as defined by Eq.~(\ref{e:tha:def_Omega_gravity}), is a closed
2-form is obvious: $\Dd\Omega = \Dd\Dd \Theta_\Sigma = 0$ (nilpotence of the exterior
...
\[
    0 = \Dd \Theta_{\Sigma'} - \Dd \Theta_{\Sigma}
    + \Dd \Upsilon_{\Sigma,\Sigma'} .
\]
The result follows the asymptotic flatness conditions, which
ensure that $\Dd \Upsilon_{\Sigma,\Sigma'}$ is vanishing (Lemma~\ref{p:tha:Upsilon_closed} below),
leaving us with $\Dd \Theta_{\Sigma'} = \Dd \Theta_{\Sigma}$.
\end{proof}
...
\begin{proof}
Let us compute $\Omega(X_1,X_2) = \Dd \Theta_\Sigma (X_1,X_2)$ via the exterior derivative formula
...
We get
\begin{align}
    \Omega(X_1,X_2) & = X_1\left( \langle \Theta_\Sigma, X_2 \rangle \right)
        -  X_2\left( \langle \Theta_\Sigma, X_1 \rangle \right) \nonumber \\
        & =  \der{}{\lambda_1} \langle \Theta_\Sigma, X_2 \rangle -
            \der{}{\lambda_2} \langle \Theta_\Sigma, X_1 \rangle \nonumber \\
...
$f := \langle \Theta_\Sigma, X_2 \rangle$ [cf. Eq.~(\ref{e:bas:def_vector})], the third equality
...
\begin{lemma}[vanishing of $\Dd \Upsilon_{\Sigma,\Sigma'}$]
\label{p:tha:Upsilon_closed}
The 1-form $\Upsilon_{\Sigma,\Sigma'}$ defined by Eq.~(\ref{e:tha:def_Upsilon})
is closed: $\Dd \Upsilon_{\Sigma,\Sigma'} = 0$.
\end{lemma}
\begin{proof}
By exactly the same computation as that leading to Eq.~(\ref{e:tha:Omega_int_omega}),
we have
\[
     \Dd \Upsilon_{\Sigma,\Sigma'}(X_1,X_2) =
     \lim_{r\to+\infty} \int_{\mathscr{W}_r^{\nearrow}}
     \w{\omega}\Lld\w{g},  \der{\w{g}}{\lambda_1},
                            \der{\w{g}}{\lambda_2} \Rld .
\]
Now the asymptotic flatness conditions must be such that the integral
in the right-hand side tends to zero when $r\to +\infty$. We shall not prove
it in fully generality here. In Ref.~\cite{IyerW94}, it is argued that
