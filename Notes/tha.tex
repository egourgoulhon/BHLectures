...
Let us assume that the diffeomorphisms of $\M$ generated
...
$\w{\xi}$ admits a conjugate Hamiltonian. A sufficient condition, independent
of $\w{\xi}$, is that there exists a $(n-1)$-form $\w{B}\lld\w{g}\rld$ on $\M$ such that
\be \label{e:tha:theta_asymp_B}
   \w{\theta}\lld\w{g},\delta\w{g}\rld
    \underset{r\to +\infty}{\sim} \delta \w{B}\lld\w{g}\rld .
\ee
Then a function $H_{\w{\xi}}$ fulfilling (\ref{e:tha:delta_H_xi}) is
\be
    H_{\w{\xi}}[\w{g}] := \int_{\Sp_\infty}\!\! \left( \w{Q}\lld\w{\xi}\rld
    - \w{\xi}\cdot  \w{B}\lld\w{g}\rld  \right) .
\ee
More generally, a necessary and sufficient criterion is
...
\]
Let $\Li'$ be another curve of $\FF$ connecting $\w{g}_0$ to $\w{g}$.
If $\FF$ is simply connected, there exists a homotopy from $\Li$ to $\Li'$.
This defines a 2-surface $\mathcal{W}$ of $\FF$, the boundary of which is
$\Li\cup (-\Li')$, which is spanned by a 2-parameter family $\w{g}(\lambda_1,\lambda_2)$
of solutions to the field equations. Stokes' theorem (\ref{e:bas:Stokes}) yields then
\[
    \mathcal{H}(\w{g},\Li) - \mathcal{H}(\w{g},\Li') = \int_{\mathcal{W}} \Dd \alpha .
\]
