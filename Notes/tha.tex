...
and have used formula (\ref{e:bas:def_ext_1f_nab}) and the antisymmetry of $\D\w{S}$
to get the last equality. Now, since $\w{\xi}$ generates an asymptotic time translation,
there exists in the vicinity of $\Sp_\infty$ a coordinate system
$(x^\alpha)=(t,x^i)$
($1\leq i \leq n-1$) such that, for $r\to+\infty$, $\w{\xi} \sim \wpar_t$
and $\w{g}\sim \w{f}$, where
$\w{f} = - \dd t^2 + f_{ij} \dd x^i \otimes \dd x^j$ is a flat metric.
$\Sp_{\infty}$ can be considered as the limit $r\to+\infty$ of a $(n-2)$-dimensional
surface lying in the hypersurface
