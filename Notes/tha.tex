...
\ee
where $\w{Y}\lld\w{g},\Lie{\xi}\w{g}\rld$ is any $(n-2)$-form that is linear in $\Lie{\xi}\w{g}$ and $\w{\mu}\lld\w{g}\rld$ is any $(n-1)$-form with compact support.
\end{prop}
...
so that $\dd\w{J}\lld\w{\xi}\rld  = 0$ if $\w{E} = 0$ is fulfilled or $\w{\xi}$ is a Killing vector ($\Lie{\xi}\w{g} = 0$). Finally, if the theory is diffeomorphism-invariant,
...
formula~(\ref{e:bas:delta_L_Lie_xi_L}) for $\delta\w{L}$. We deduce then from
Eq.~(\ref{e:tha:Theta_not_unique}) that
$\w{\theta'}\lld\w{g},\Lie{\xi}\w{g}\rld = \w{\theta}\lld\w{g},\Lie{\xi}\w{g}\rld +
\dd \w{Y}\lld\w{g},\Lie{\xi}\w{g}\rld +  \Lie{\xi}\w{\mu}\lld\w{g}\rld$.
By combining this identity with Eq.~(\ref{e:tha:L_not_unique}),
Eq.~(\ref{e:tha:def_Noether_current}) leads to
\[
    \w{J'}\lld\w{\xi}\rld =  \w{J}\lld\w{\xi}\rld  + \dd \w{Y}\lld\w{g},\Lie{\xi}\w{g}\rld
     + \Lie{\xi}\w{\mu}\lld\w{g}\rld
     - \w{\xi}\cdot\dd\w{\mu}\lld\w{g}\rld .
\]
Thanks to Cartan identity (\ref{e:bas:Cartan}),
...
Finally, the $\w{Y}$ and $\w{\mu}$ ambiguities  in (\ref{e:tha:Q_not_unique}) are inherited from the ambiguity of $\w{J}$ [Eq.~(\ref{e:tha:J_not_unique})], while the $\w{Z}$ ambiguity stems from Eq.~(\ref{e:tha:J_dQ}) defining
...
forming a mesh of it. Now, since $\w{\chi}$ is tangent to $\Delta\Hor$ (for $\w{\chi}$ is a null normal to $\Hor$)
and $\w{L}$ is a
...
The \emph{covariant phase space} approach is a Hamiltonian formalism that
...
which is well defined since $\kappa\neq 0$ (Property~\ref{p:neh:zeroth_law_bifur}) ---
...
Contrary to the entropy, there was thus no need to modify it to get the first law
in any diffeomorphism-invariant gravity theory.
\end{remark}
