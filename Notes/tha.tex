...
\begin{remark}
In the literature \cite{HollaW13,HollaWZ24,Compe19,GrumiS22,HajiaS16}, it is rather
a generalization of
Equation~(\ref{e:tha:fund_identity}), holding
even if $\w{g}$ does not satisfy the field equations, which is called
the \emph{fundamental identity} of the covariant phase space formalism.
\end{remark}
...
to write the integral in the left-hand side. Since $\w{Q}\lld\w{\chi}\rld$
...
Property~\ref{p:tha:first_law} generalizes the first law of
...
sense for a nonstationary black hole as well, provided that $\hat{\Sp}$
is the bifurcation surface in the unperturbed configuration.
For a stationary black hole, both definitions are equivalent
(Property~\ref{p:tha:S_Wald_explicit}). However, for a nonstationary one,
this is not obvious. However, it has been proven by Iyer \& Wald, that
$\delta S_{\rm W} = \delta {S'}_{\rm W}$,
where $S_{\rm W}$ is defined by (\ref{e:tha:def_Wald_entropy_bif})
and ${S'}_{\rm W}$ is defined by (\ref{e:tha:S_Wald_explicit})
(cf. Eq.~(94) in Ref.~\cite{IyerW94}).
\end{remark}
