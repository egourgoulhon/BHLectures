...
where $\hat{\Sigma}$ is an oriented manifold of dimension $(n-1)$ with an
...
that lie in a hyprsurface $\Sigma$ of $\M$ such that
$\Sigma = \Psi(\hat{\Sigma})$, where $\Psi$ is a diffeomorphism
$\R\times \hat{\Sigma} \to \M$, and
$\Sp_r = \Psi(\hat{\Sp}_r)$, where $\hat{\Sp}_r$ is the $(n-2)$-sphere of Euclidean radius
...
both diffeomorphic to $\hat{\Sigma}$, and let $\mathscr{D}$ be the part
...
The variation $\delta\mathcal{S}_{\mathscr{D}}$ of $\mathcal{S}_{\mathscr{D}}$
due to a small displacement $\delta\w{g}$ on $\FF$ can then be viewed
...
$\delta\mathcal{S}_{\mathscr{D}} = \int_{\mathscr{D}} \delta \w{L}\lld\w{g}\rld$,
...
Let $\Sigma$ be a hypersurface of $\M$ diffeomorphic to $\hat{\Sigma}$, i.e. such that $\Psi(\Sigma) = \hat{\Sigma}$,
where $\Psi$ is a diffeomorphism $\M \to \R\times \hat{\Sigma}$.
From the spacetime presymplectic potential current $(n-1)$-form $\w{\theta}\lld\w{g},\delta\w{g}\rld$
(cf. Property~\ref{p:tha:var_Lagrangian}), one defines a 1-form $\Theta_\Sigma$ on $\FF$ as follows.
At any point $\w{g}\in\FF$, the action of $\Theta_\Sigma$ on a vector $X$ in the
tangent space $T_{\w{g}} \FF$ is defined by
...
The map $\Theta_\Sigma: T_{\w{g}} \FF \to \R$ thus defined is clearly linear, given that
$\w{\theta}\lld\cdot,\cdot\rld$ is linear in its second argument.
Hence $\Theta_\Sigma$ is a linear form on the tangent space $T_{\w{g}} \FF$; furthermore we shall assume
that it is continuous (recall that linearity does not imply continuity in infinite dimension),
so that $\Theta_\Sigma$ belongs to the dual space of the tangent space to $\FF$ at $\w{g}$:
$\Theta_\Sigma \in T_{\w{g}}^* \FF$. By letting the point $\w{g}$ vary over all $\FF$, this defines
a 1-form on $\FF$, which we shall denote by the same symbol $\Theta_\Sigma$.
...
While $\Theta_\Sigma$ depends on the choice of $\Sigma$, $\Omega$ does not and is moreover
...
appropriate asymptotic flatness conditions. In particular, it
does not depend on the choice of the
hypersurface $\Sigma$, nor on the freedom $\w{\theta} \to \w{\theta} + \dd \w{Y}$
in the choice of $\w{\theta}$ [Eq.~(\ref{e:tha:Theta_not_unique})].
\end{prop}
...
To establish Eq.~(\ref{e:tha:Omega_int_omega}), let
 us compute $\Omega(X_1,X_2) = \Dd \Theta_\Sigma (X_1,X_2)$ via the exterior derivative formula
...
As for the independence of $\Omega$ from $\Sigma$, it follows
from $\dd \w{\omega} = 0$ (Property~\ref{p:tha:omega_closed} below). Indeed, via
Stokes' theorem (\ref{e:bas:Stokes}),
 $\dd \w{\omega} = 0$ implies that the
value of $\Omega(X_1,X_2)$ given by Eq.~(\ref{e:tha:Omega_int_omega})
...
boundary integral vanishes (cf. Ref.~\cite{IyerW94} for details).
\end{proof}
