...
It turns out that in general, $\Omega$ is a \emph{degenerate} 2-form on $\FF$:
there exists nonzero vectors $X$ on $\FF$, such that $\Omega(X,.) = 0$.
Hence $\Omega$ fails to be a symplectic form.
For instance, one may conceive that some nonzero variation $\delta\w{g}$
is such that $\delta\w{g} = 0$ on the hypersurface $\Sigma$ involved
in Eq.~(\ref{e:tha:Omega_int_omega}). Then, by linearity of $\w{\omega}$,
$\w{\omega}\lld\w{g},\delta\w{g},.) = 0$ on $\Sigma$, so that $\delta\w{g}$ defines
...
well posed only in a fixed gauge, cf. Chap.~VI of Ref.~\cite{Choqu09}). Recall that in the covariant phase space
...
\be \label{e:tha:fund_identity}
    \w{\omega}\lld\w{g},\delta\w{g}, \Lie{\xi}\w{g}\rld =
...
\emph{globally}\footnote{Let us recall that thanks to Poincaré lemma (cf. Sec.~\ref{s:bas:ext_deriv}), one has \emph{locally}:
$\w{\omega}$ closed $\implies$ $\w{\omega}$ exact.} exact as soon as one of these variations results from an infinitesimal diffeomorphism
...
\cite{HollaW13,HollaWZ24,GrumiS22,HajiaS16}.
\end{remark}

Let us assume the vector field $\w{\xi}$ is such that the diffeomorphisms of $\M$ generated
by $\w{\xi}$ preserve the asymptotic flatness conditions stated in Property~\ref{p:tha:asymp_flat_sol}. One may then associate to $\w{\xi}$ a vector field
$X_{\w{\xi}}$ on the space $\FF$ of
asymptotically flat solutions to the field equations,
as the tangent vector to the orbits of the diffeomorphism group generated by
$\w{\xi}$ acting on $\FF$: $X_{\w{\xi}}$ is the
vector field defined by Eq.~(\ref{e:tha:X_dg/dlambda}):
$X_{\w{\xi}} = \delta\w{g} / \delta\lambda$, with $\delta\w{g}$ given
by Eq.~(\ref{e:bas:delta_g_Lie_xi_g}): $\delta\w{g} = \delta\lambda\, \Lie{\xi} \w{g}$.
...
Let us suppose that there exists a $(n-1)$-form $\w{B}\lld\w{g}\rld$ on $\M$ such that
\be \label{e:tha:theta_asymp_B}
   \w{\theta}\lld\w{g},\delta\w{g}\rld
    \underset{r\to +\infty}{\sim} \delta \w{B}\lld\w{g}\rld .
\ee
Then we may rewrite Eq.~(\ref{e:tha:Omega_dg_X_xi}) as
\be \label{e:tha:delta_H_xi}
    \delta H_{\w{\xi}} = \Omega(\delta\w{g}, X_{\w{\xi}}) ,
\ee
with
\be
    H_{\w{\xi}} := \int_{\Sp_\infty}\!\! \left( \w{Q}\lld\w{\xi}\rld
    - \w{\xi}\cdot  \w{B}\lld\w{g}\rld  \right) .
\ee
$H_{\w{\xi}} = H_{\w{\xi}}[\w{g}]$ can be considered as a scalar field on $\FF$
and, since $\delta\w{g}$ is an infinitesimal displacement vector on $\FF$,
we may write $\delta H_{\w{\xi}} = \langle \Dd H_{\w{\xi}}, \delta\w{g} \rangle$
[cf. Eq.~(\ref{e:bas:df})].
Given that $\delta\w{g}$ is a generic variation, we see
that Eq.~(\ref{e:tha:delta_H_xi}) is equivalent to
...
Nevertheless, one says that $H_{\w{\xi}}$ is the
\defin{Hamiltonian conjugate to the vector field $\w{\xi}$}\index{Hamiltonian!conjugate to a vector field}
\cite{WaldZ00}. Hence if the asymptotic property (\ref{e:tha:theta_asymp_B}) is fulfilled,
...
\begin{prop}[existence criterion for a Hamiltonian conjugate to a vector field
...
\end{prop}
\begin{proof}
If  $H_{\w{\xi}}$ exists, then by combining Eqs.~(\ref{e:tha:delta_H_xi}) and (\ref{e:tha:Omega_dg_X_xi}), we get
\[
    \delta_2 H_{\w{\xi}} = \delta_2 \mathscr{Q}_{\Sp_\infty,\w{\xi}}
    \  -  \int_{\Sp_\infty} \!\! \w{\xi}\cdot\w{\theta}\lld\w{g},\delta_2\w{g}\rld ,
\]
where $\mathscr{Q}_{\Sp_\infty,\w{\xi}}$ is the
Noether charge of $\Sp_\infty$ with respect to $\w{\xi}$
[cf. Eq.~(\ref{e:tha:def_Noether_Q_surf})]. Performing the variation $\delta_1$ yields
\[
...
condition for a Hamiltonian conjugate to $\w{\xi}$ to exist.
To prove that it is sufficient when $\FF$ is simply connected, let us select
a point $\w{g}_0\in\FF$ and define for any point ...
\end{proof}
