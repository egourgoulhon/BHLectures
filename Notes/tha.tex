...
$(x^\alpha)=(t,x^1,\ldots,x^{n-1})$, i.e. a coordinate system such that
$\w{g} \sim \w{f}$ for $r:=\sqrt{(x^1)^2 + \cdots (x^{n-1})^2}\to +\infty$,
where $\w{f}$ is the flat metric whose components w.r.t. $(x^\alpha)$ are
$f_{\alpha\beta} = \mathrm{diag}(-1,1,\ldots,1)$.
This proprety is in agreement with the asymptotic flatness conditions.
...
\]
On the other hand, given expression~(\ref{e:tha:E_Theta_GR}) for $\w{\theta}$ in
general relativity, we may write
\be \label{e:tha:theta_GR_asymp_barH}
\w{\theta} \underset{r\to +\infty}{\sim}  (16\pi)^{-1} \, \w{\bar{H}}\cdot\w{e}
\ee
where $\w{e}$ is the Levi-Civita tensor of the flat metric $\w{f}$ and
$\w{\bar{H}}$ is the vector defined from $\delta\w{g}$ in the same
way as $\w{H}$ in Eq.~(\ref{e:evo:def_H}), but substituting $\w{f}$ for $\w{g}$:
\[
    \bar{H}^\alpha = f^{\alpha\rho} f^{\mu\nu} \left( \mathcal{D}_\nu \delta g_{\rho\mu}
        - \mathcal{D}_\rho \delta g_{\mu\nu} \right)
        = \delta K^\alpha, \quad\mbox{with}\quad
        K^\alpha := f^{\alpha\rho} f^{\mu\nu} \left( \mathcal{D}_\nu g_{\rho\mu}
        - \mathcal{D}_\rho g_{\mu\nu} \right) ,
\]
where $\w{\mathcal{D}}$ stands for the covariant derivative associated to $\w{f}$.
The identity $\bar{H}^\alpha = \delta K^\alpha$ holds because
$\delta \w{f} = 0$ and $\w{\mathcal{D}}\delta = \delta\w{\mathcal{D}}$,
given that $\w{f}$ is independent of $\w{g}$. For the same reason, $\delta\w{e} = 0$
and we deduce from Eq.~(\ref{e:tha:theta_GR_asymp_barH}) that
\[
    \w{\theta} \underset{r\to +\infty}{\sim} \delta \w{B},
    \quad\mbox{with}\quad
    \w{B} := (16\pi)^{-1} \, \w{K}\cdot\w{e} .
\]
The $(n-1)$-form $\w{B}$
defined above is thus fulfilling Eq.~(\ref{e:tha:int_xi_theta_B}),
thereby ensuring that a Hamiltonian conjugate to $\w{\xi}$ exists.
We have then
$\w{\xi}\cdot\w{B} = (16\pi)^{-1} \, \w{\xi}\cdot(\w{K}\cdot\w{e})
= (16\pi)^{-1} \, \w{e}(\w{K},\w{\xi},\ldots) = (16\pi)^{-1} \star_f (\uu{K}_{f}\wedge\uu{\xi}_f)$,
...
of $\uu{H}\wedge\uu{\xi}$. Using again Lemma~\ref{p:sta:flux_integ_2form}
and the antisymmetry property
$(H_\mu \xi_\nu - \xi_\mu H_\nu) \, \D S^{\mu\nu} = 2 H_\mu \xi_\nu \, \D S^{\mu\nu}$,
we arrive at
\bea
\int_{\Sp_\infty}\!\! \w{\xi}\cdot\w{\theta}
        & = & \frac{1}{16\pi}
        \int_{\Sp_\infty}\!\! H_\mu \xi_\nu  \, \D S^{\mu\nu}
        = \frac{1}{16\pi}
        \int_{\Sp_\infty}\!\! H_\mu g_{t\nu}
        (s^\mu \delta^\nu_{\ \, t} - \delta^\mu_{\ \, t} s^\nu) \,  \D S \nonumber \\
    & = & \frac{1}{16\pi}
        \int_{\Sp_\infty}\!\!  (H_i  \underbrace{g_{tt}}_{-1} - H_t
