...
some conformal infinity (stationarity property) and hypersurface-orthogonal (staticity property). Let us denote by $\w{\xi}$ the associated Killing vector
...
that $\Sp\subset\Sigma_t$. Because of the spherical symmetry, the Riemannian
metric $\w{q}$ induced by $\w{g}$ on $\Sp$ is necessarily of constant curvature
(i.e. its Ricci scalar is constant over $\Sp$). This implies that there exists
a coordinate system $(\th,\ph)$ on
$\Sp$ such that $\theta\in[0,\pi]$, $\ph\in[0,2\pi)$ %]$
and
\be
    \w{q} = r^2 \left( \dd\th^2 + \sin^2\th\, \dd\ph^2 \right) ,
\ee
where $r$ is a positive constant.
The area of $\Sp$ with respect to $\w{q}$ is then $A=4\pi r^2$. Accordingly, $r$ is called the \defin{areal radius}\index{areal!radius}
...
\end{array}
\ee
the remaining Christoffel symbols being zero.
...
\be \label{e:sch:A_sol}
    A(r) = 1 - \frac{2 m}{r} - \frac{\Lambda}{3} \,  r^2 ,
\ee
where $m$ is a constant.
Furthermore, we check easily that the component
(\ref{e:sch:EE_pp}) of the Einstein equation, which is reduced to
$\D^2 A / \D r^2 + (2/r) \D A / \D r + 2\Lambda = 0$ thanks to (\ref{e:sch:B_1_over_A}),
is fulfilled by (\ref{e:sch:A_sol}),
We therefore conclude:
\begin{tprop}{static and spherically symmetric solutions to the vacuum Einstein equation}{}
The general static and spherically symmetric solution to the vacuum
Einstein equation (\ref{e:sch:vac_Einstein_eq_Lamb}) is, in terms of areal coordinates,
...
often abridged as \defin{Schwarz\-schild-dS metric}, while if $\Lambda<0$, it
...
In the rest of this chapter, we shall focus on the Schwarzschild metric (\ref{e:sch:Schwarz_metric_SD}).
...
takes the standard weak-field form (see e.g. \cite{Carro04,MisneTW73} or Eq.~(\ref{e:sta:asymptotic_metric}) with $n=4$ and $J_{ij}=0$):
...
Example~\ref{x:sta:Komar_mass_Schw}, where
...
\begin{tprop}{the Schwarzschild exterior $\M_{\rm I}$}{}
...
\begin{tprop}{the Schwarzschild interior $\M_{\rm II}$}{sch:prop_M_II}
...
we conclude
\begin{tprop}{radial null geodesics}{sch:radial_null_geod}
...
\begin{tprop}{timelike character of the IEF coordinate $\ti$}{sch:ti_timelike}
...
the metric (\ref{e:def:Schw_metric}) introduced in Example~\ref{x:def:Schw_hor}, up to the change of notation $\ti \leftrightarrow t$ (compare (\ref{e:def:Schw_metric_inv}) and
(\ref{e:sch:inv_metric_EF}) as well).
We have thus the fundamental result,
the proof of which is given in Example~\ref{x:neh:Schwarz_KH}:
\begin{tprop}{Killing horizon in Schwarzschild spacetime}{}
...
whose area, as defined in Sec.~\ref{s:neh:invar_area}, is (cf. Example~\ref{x:neh:Schwarz_hor_area})
\be
    A=16\pi m^2 .
\ee
...
\begin{tprop}{outgoing radial null vector field}{}
The vector field
...
\begin{tprop}{time orientation of Schwarzschild spacetime}{}
...
According to Lemma~\ref{p:fra:lem2} with $\w{u}=\w{k}$,
...
\begin{tprop}{$\wpar_r$ past-directed timelike in the black hole region}{sch:wpar_r_past_MII}
...
A corollary is:
\begin{tprop}{decreasing of $r$ in the black hole region}{sch:r_decreasing}
...
in $\M_{\rm II}$, so that we can apply Lemma~\ref{p:fra:lem1}
...
formal definition of a black hole region (Definition~\ref{d:glo:BH}).
...
listed in Definition~\ref{d:glo:conformal_completion} of a \emph{conformal completion at infinity}
of $(\M,\w{g})$.
However, it is not adapted to Definition~\ref{d:glo:BH} of a black hole. Indeed
...
\begin{tprop}{black hole region in Schwarzschild spacetime}{}
...
of the formal Definition~\ref{d:glo:BH} of a black hole:
