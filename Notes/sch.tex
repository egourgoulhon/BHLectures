...
\be
    \ti = 4 m \ln (X+T) - r.
\ee
This expression is valid as long as $X+T>0$. Besides, we already noticed
that the function $F$ defined by (\ref{e:sch:def_F}) from the range of $r$
...
\begin{align}
 & \ti = 4 m \ln (X+T) - r\\
 & \underbrace{\mathrm{e}^{r/2m} \left( \frac{r}{2m} - 1 \right)}_{F(r)} = X^2-T^2 .
 \end{align}
\end{subequations}
The range of the coordinates $(T,X)$ on $\M_{\rm IEF}$ is then ruled by
\[
    \M_{\rm IEF}:\qquad X+T > 0 \quad\mbox{and}\quad X^2 - T^2 > -1 ,
\]
which can be rewritten as
\be
    \M_{\rm IEF}: \qquad -X < T < \sqrt{X^2+1}.
\ee
