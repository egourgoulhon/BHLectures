...
\subsection{Time orientation of the spacetime manifold}

In view of (\ref{e:sch:ingoing_null_geod_EF}), we may consider $\lambda=-r$
as a parameter along the ingoing radial null geodesics. The corresponding
tangent vector $\w{k}$ has components
\[
    {\tilde k}^\alpha = - \frac{\D {\tilde x}^\alpha}{\D r}
        = (1, -1, 0, 0)
\]
...
$\M_{\rm IEF}$. It may therefore be used to set the time orientation\index{time-orientable}\index{orientable!time --}
of the spacetime $(\M_{\rm IEF}, \w{g})$ (cf. Sec.~\ref{s:fra:spacetime})
by demanding that $\w{k}$ is future
directed everywhere. This induces a time orientation of the subdomains
$\M_{\rm I}$ and $\M_{\rm II}$.

In particular, since
we have noticed in Sec.~\ref{s:sch:SD_domain} that the coordinate vector
$\wpar_r$ of Schwarzschild-Droste coordinates is timelike in $\M_{\rm II}$,
we may form the scalar product $\w{k}\cdot\wpar_r$ to get its time orientation
[cf. Eq.~(\ref{e:fra:time_orientation})]. In view of
Eqs.~(\ref{e:sch:wpar_tilde_r}) and (\ref{e:sch:wpar_tilde_t}), we have
\[
    \wpar_r = - \left( 1 - \frac{2m}{r} \right) ^{-1} \wpar_{\ti}
        + \wpar_{\tilde r} .
\]
Via (\ref{e:sch:null_vector_k}) and (\ref{e:sch:Schwarz_metric_EF}), we deduce immediately that
\[
    \w{k}\cdot\wpar_r = \frac{r}{2m} \left( 1 - \frac{2m}{r} \right) ^{-1}
    > 0 \quad\mbox{in} \ \M_{\rm II} .
\]
We conclude that $\wpar_r$ is a past-directed timelike vector in $\M_{\rm II}$.
