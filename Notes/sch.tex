...
\defin{null ingoing Eddington-Finkelstein coordinates}\index{Eddington-Finkelstein!coordinates}\index{null!ingoing Eddington-Finkelstein coordinates}. The qualifier \emph{null} stems from the fact that
...
To deal with a timelike coordinate in $\M_{\rm I}$ instead of a null one, let us set
\be  \label{e:sch:ti_v_r}
    \encadre{\ti := v - r} \iff \encadre{v = \ti + r}
\ee
and define the \defin{ingoing Eddington-Finkelstein (IEF) coordinates}\index{Eddington-Finkelstein!coordinates}\index{ingoing!Eddington-Finkelstein!coordinates}
to be
\be
...
\be
    ...
    F: & (0,+\infty) & \longrightarrow & (-1,+\infty) \\
        & r & \longmapsto & \mathrm{e}^{r/2m} \left( \frac{r}{2m} - 1 \right)
    \end{array}
\ee
...
\\[2ex]
    ...
    r = F^{-1}(X^2 - T^2)
        \end{array}\right. }
\ee
Note that we have used the identity $\mathrm{artanh}\, x = 1/2 \ln((1+x)/(1-x))$.
\begin{remark}
...
this in details. Back on $\M_{\rm I}$, we can express the IEF coordinate
$\ti$ in terms of $(T,X)$ by combining (\ref{e:sch:ti_t_r}),
(\ref{e:sch:KS_SD_I}) and (\ref{e:sch:X2mT2}):
\bea
    \ti & = & t + 2 m \ln \left( \frac{r}{2m} - 1 \right) \nonumber \\
        & = &  2 m \,  \ln \left( \frac{X+T}{X-T} \right)
            + 2 m \ln \left[ (X^2-T^2) \, \mathrm{e}^{-r/2m} \right] \nonumber \\
        & = & 2 m \ln \left[ \frac{X+T}{X-T}\, (X^2-T^2) \right]
            + 2 m \ln \mathrm{e}^{-r/2m} , \nonumber
\eea
i.e.
\be
    \ti = 4 m \ln (X+T) - r.
