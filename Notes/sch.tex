...
The metric $\w{g}$ on the whole $\M$ is then defined by (\ref{e:sch:metric_KS_TX}).
...
Kruskal-Szekeres coordinates are easily found from the
...
\ee
Now, this formula defines a smooth vector field on all $\M$.
Moreover, on $\M_{\rm III}\cup\M_{\rm IV}$, this vector coincides with
...
It is clear that the bifurcate Killing horizon with respect to $\w{\xi}$
which extends $\Hor$
...
\ee
which is nothing but the general property (\ref{e:sta:xi_S_zero}).
