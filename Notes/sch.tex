...
We also note that, in $\M_{\rm II}$, $t$ becomes a spacelike coordinate
(i.e. the hypersurfaces $t=\mathrm{const}$ are timelike, cf. Sec.~\ref{s:bas:signature})
and $r$ a timelike one (i.e. the hypersurfaces $r=\mathrm{const}$ are spacelike).
This follows from $g^{tt} > 0$ and $g^{rr} < 0$ in
$\M_{\rm II}$ [cf. Eq.~(\ref{e:bas:char_causal_type_coord})].
