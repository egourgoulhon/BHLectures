...
\be \label{e:sch:ingoing_null_geod}
  \encadre{ t = - r - 2 m \ln \left| \frac{r}{2m} - 1 \right| + v },
\ee
...
\ee
For the $-$ sign in $\pm$, we get $\D r / \D \ti = -1$, so that the integration is
immediate:
\be \label{e:sch:ingoing_null_geod_EF}
    \encadre{ \ti = - r + v },\quad (\th,\ph) = \mathrm{const},
\ee
...
For $\pm$ equal to $+$ in Eq.~(\ref{e:sch:drdt_rad_null_geod}), we get
...
\be \label{e:sch:outgoing_null_geod_EF}
    \encadre{ \ti = r + 4 m \ln \left| \frac{r}{2m} - 1 \right| + u },
...
given relation (\ref{e:sch:ti_t_r}) between $\ti$ and $t$. If $r=2m$,
Eq.~(\ref{e:sch:drdt_rad_null_geod}) with $\pm$ equal to $+$ leads to
...
\be \label{e:sch:outgoing_null_geod_H}
    \encadre{r = 2 m} ,\quad (\th,\ph) = \mathrm{const}.
\ee
These null curves, which form a family labelled by $(\th,\ph)$, are actually
the null geodesics generating the Killing horizon $\Hor$ (cf. Sec.~\ref{s:sch:Schwarz_hor}).
Indeed, it follows from Eq.~(\ref{e:sch:outgoing_null_geod_H}) that their tangent
vector is $\wpar_{\ti} = \w{\xi}$, which, for $r=2m$, is the null normal of $\Hor$.

The null geodesics (\ref{e:sch:outgoing_null_geod_H}) had not been found in
Sec.~\ref{s:sch:rad_null_geod}, since they don't belong to $\M_{\rm SD}$. We may consider
that they extend the outgoing family, since (i) they have been obtained for the same
choice $+$ of the $\pm$ sign in Eq.~(\ref{e:sch:drdt_rad_null_geod}) as the
outgoing geodesics (\ref{e:sch:outgoing_null_geod_EF}) and (ii) they correspond
to the limiting case $r = \mathrm{const}$ separating the two subfamilies of the outgoing
geodesics (\ref{e:sch:outgoing_null_geod_EF}): those with $r>2m$, which have $r$ increasing
towards the future (and therefore are truely ``outgoing'') and those with $r<2m$,
which have $r$ decreasing towards the future.

The ingoing and outgoing radial null geodesics are depicted in Fig.~\ref{f:sch:rad_null_geod_EF}
in terms of the IEF coordinates.
