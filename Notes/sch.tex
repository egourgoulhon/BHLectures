...
that a curve defined by (\ref{e:sch:outgoing_null_geod_EF}) nether crosses $\Hor$, i.e.
...
Eq.~(\ref{e:sch:drdt_rad_null_geod}) with $\pm$ equal to $+$; the equation
reduces then to
...
the null geodesic generators of the Killing horizon $\Hor$ (cf. Sec.~\ref{s:sch:Schwarz_hor}).
...
along the radial null geodesics $\Li^{\rm in}_{(v,\th,\ph)}$ and
$\Li^{\rm out}_{(u,\th,\ph)}$.
We may then choose $\lambda=-r$ as an
affine parameter along $\Li^{\rm in}_{(v,\th,\ph)}$; according to
Eq.~(\ref{e:sch:ingoing_null_geod_EF}), the equation of $\Li^{\rm in}_{(v,\th,\ph)}$
becomes then
\[
