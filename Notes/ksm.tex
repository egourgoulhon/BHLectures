...
A spacetime $(\M,\w{g})$ of dimension $n\geq 2$ is said to be
\defin{Kerr-Schild}\index{Kerr-Schild!spacetime}, or
to have a
...
vector associated to it via $\w{f}$-duality (cf. Sec.~\ref{s:bas:metric_dual})
is a null vector of $\w{f}$. The last requirement is equivalent to
\be
     f^{\mu\nu} k_\mu k_\nu = 0 ,
\ee
...
are coordinates $(x^\alpha) = (t,x^1,\ldots,x^{n-1})$ that are \emph{Minkowskian}\index{Minkowskian coordinates}
for $\w{f}$, i.e. coordinates in which the flat metric
$\w{f}$ takes the form
\be
    \w{f} = - \dd t^2 + (\dd x^1)^2 + \cdots + (\dd x^{n-1})^2 .
\ee

\subsection{Geodesic character of \texorpdfstring{$\w{k}$}{k} for Einstein metrics} \index{Einstein!metric}

\begin{tprop}{null vector of Kerr-Schild metric as a geodesic vector}{}
Let $\w{g}$ be a Kerr-Schild metric
such that the Ricci tensor $\w{R}$ of $\w{g}$ obeys $\w{R}(\w{k},\w{k}) = 0$.
In particular, this holds if $\w{g}$ obeys the vacuum Einstein
equation\index{Einstein!equation!vacuum --}\index{vacuum!Einstein equation},
with or without any cosmological constant,
i.e. if $\w{R} = 0$ [Eq.~(\ref{e:fra:vac_Einstein})]
or $\w{R} \propto \w{g}$ [Eq.~(\ref{e:fra:vac_Einstein_Lambda})].
...
The proof can be found in Ref.~\cite{OrtagPP09} (or Ref.~\cite{KerrS65} for the case
special case $\w{R} = 0$).
...
Let us consider the Kerr spacetime $(\M,\w{g})$: $n=4$,  $\M$ is the manifold
...
in terms of the Kerr coordinates $(\ti, r, \th,\tph)$.
...
Its expression in terms of Kerr coordinates is given by Eq.~(\ref{e:ker:k_ti_tr}):
...
we conclude:
\begin{tprop}{Kerr metric as a Kerr-Schild metric}{}
...
\subsection{The double-disk \texorpdfstring{$r=0$}{r=0}} \label{s:ksm:r_zero}
...
\subsection{Kerr-Schild coordinates on the \texorpdfstring{$r\leq 0$}{r<=0} part}
