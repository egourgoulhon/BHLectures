...
mark the two horizons: $\Hor$ (black) and $\Hor_{\rm in}$ (brown).
...
above the grey zone correspond to spherical photon orbits.
}
\end{figure}
...
and $r_{\rm ph}^- \geq 3m$ ($\pm = -$). They are given by Eqs.~(\ref{e:gek:def_r_min_pm})
and (\ref{e:gek:def_r_star}):
\be
...
\ee
As for $r_{\rm ph}^{**}$, since $q(r_0) = - (|\ell(r_0)| - a)^2$ with
...
$q(r_0) = - (\ell(r_0) + a)^2$. Given expressions (\ref{e:gik:spher_ell_r0}) and (\ref{e:gik:spher_q_r0})
