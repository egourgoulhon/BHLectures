...
\emph{angle} coordinates, measuring the departure from the direction
...
only ones in the equatorial plane, because the only other root of $q_{\rm c}(r_0)=0$ is $r_0=0$
...
critical null geodesics. The black area itself is called the
...
Shadow of a Kerr black hole with the spin parameter $a=0.95\, m$ on the screen of an asymptotic
inertial observer located in the equatorial plane ($\th_{\Obs} = \pi/2$, upper
panel), at the inclination $\th_{\Obs} = \pi/6$ (lower left) and on the rotation
axis ($\th_{\Obs} = 0$ (lower right).
The color code indicates the range of the parameter $r_0$
...
Some results are shown in Fig.~\ref{f:gik:shadow_a95}, where the colors indicate
various ranges of $r_0$. As observed above, only for $\th_{\Obs} = \pi/2$
does $r_0$ span the full interval $[r_{\rm ph}^+, r_{\rm ph}^-]$.
...
\be \label{e:gik:R_shadow_axis}
   \encadre{ R_{\rm shadow} = \sqrt{\alpha^2 + \beta^2} = \frac{2\sqrt{m r_{\rm ph}^{\rm pol}}}{r_{\Obs}}
    \frac{\sqrt{(r_{\rm ph}^{\rm pol})^2 - a^2}}{r_{\rm ph}^{\rm pol} -  m} }
    \qquad (\th_{\Obs} = 0\ \mbox{or}\  \th_{\Obs} = \pi)
\ee
where $r_{\rm ph}^{\rm pol}$ is the function (\ref{e:gik:rph_pol}) of $(m, a)$.
\begin{remark}
At the limit $a\to 0$, we have $r_{\rm ph}^{\rm pol} \to 3 m$, and formula~(\ref{e:gik:R_shadow_axis})
yields $R_{\rm shadow} = 3\sqrt{3}\, m$, in agreement with the result
for Schwarzschild black hole obtained in Sec.~\ref{s:gis:shadow}.
\end{remark}
