...
being exactly zero at the limit $r_{\Obs}\to +\infty$. They can thus be identified
...
In Sec.~\ref{s:gik:remote_screen}, we have shown that the position $(\alpha,\beta)$
...
But not all these values of $(\ell,q)$ can be those of a null geodesic
that reaches observer $\Obs$: the constraints $\mathcal{R}(r_{\Obs}) \geq 0$
[Eq.~(\ref{e:mcR_non_neg})]
and $\tilde\Theta(\th_{\Obs}) \geq 0$ [Eq.~(\ref{e:gek:Theta_non_neg})]
have to be fulfilled. The first one is always satisfied due to the
...
limits the range of $\ell_{\rm c}(r_0)$ around $\ell_{\rm c}(r_0)=0$, this last value being always allowed.
Actually if $\th_{\Obs} = 0$ (observer along the rotation axis), $\ell_{\rm c}(r_0)=0$
is the only value compatible with (\ref{e:gik:constraint_theta_obs}).
Then $r_0$ can then take a unique value: $r_0 = r_{\rm ph}^{\rm pol}$, which is
given by Eq.~(\ref{e:gik:rph_pol}).
