...
\begin{align}
& \alpha = \alpha_{\rm c}(r_0) := - \frac{\ell_{\rm c}(r_0)}{r_{\Obs}\sin\th_{\Obs}}  \\
& \beta = \beta_{\rm c}(r_0) := \frac{\eps_\th}{r_{\Obs}}
...
for $\ell_{\rm c}(r_0)$ and $q_{\rm c}(r_0)$ simplify consequently:
\be
...
\ee
We notice that $q_{\rm c}(r_{\rm ph}^-) = 0$, while $q_{\rm c}(r_{\rm ph}^+) = 3 m^2 \neq 0$.
This last result seems to contradict the fact that
$r_{\rm ph}^+$ and $r_{\rm ph}^-$ have been
...
on that expression yields the undeterminate form ``0/0''.
