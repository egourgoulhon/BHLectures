...
Part of the boundary of an extremal Kerr black hole ($a=m$)
...
For $\th_{\rm crit} < \th_{\Obs} < \pi - \th_{\rm crit}$, the parametric curve defined by
...
for $\eps_\th=-1$). Given that $\beta \neq 0$ for these two points, the curve is not closed
(cf. Fig.~\ref{f:gik:shadow_a1_th90_part} for the case $\th_{\Obs} = \pi/2$).
