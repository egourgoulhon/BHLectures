...
which have obtained for an optically thick disk model.
...
is the easiest to understand since for $\th_{\Obs}=0$, $\mathscr{C}$ involves only the $\ell=0$ critical null geodesics, i.e. those that are rolling indefinitely around the sphere $r_0 = r_{\rm ph}^{\rm pol}$
...
that the secondary image is brighter than the primary one
...
in the vicinity of some spherical photon orbit can give rise to bright higher-order images.
...
\begin{figure}
\centerline{\includegraphics[height=0.3\textheight]{gik_M87_EHT_crit.pdf}}
...
Schwarzschild black hole (cyan dotted circle)
and that of an extremal Kerr black hole seen under the inclination $\th_{\Obs} = 160^\circ$
(green dotted curve), with the projection of the spin axis onto the screen indicated by the
green arrow (angle of $-160^\circ$ with respect to the $\alpha$-axis).
...
(approx. $20\; \mu{\rm as}$).
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_image_M87}]}
}
\end{figure}


\subsection{EHT image of M87*}
