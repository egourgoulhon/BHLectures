...
\begin{greybox}
A given null geodesic has
\begin{itemize}
\item at most one $r$-turning point in $\M_{\rm I}$
\item no $r$-turning point in $\M_{\rm II}$
\item at most one $r$-turning point in $\M_{\rm III}$
\end{itemize}
\end{greybox}
\begin{proof}
The proof is based on expressions (\ref{e:gik:mcR}) of $\mathcal{R}(r)$.
The easy case is the region $\M_{\rm II}$: in that region $\Delta < 0$
and Eq.~(\ref{e:gik:mcR_Delta}) yields immediately $\mathcal{R}(r) > 0$
for $q + (\ell - a)^2 > 0$.
\end{proof}
