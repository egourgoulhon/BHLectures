...
\begin{figure}
\centerline{\includegraphics[width=0.7\textwidth]{gik_spher_orb_range.pdf}}
...
\be
    - \frac{m}{2} \leq r_{\rm ph}^{**} \leq 0 \leq r_{\rm ph}^* \leq r_- \leq m \leq r_+ \leq r_{\rm ph}^+
    \leq 3 m \leq r_{\rm ph}^- \leq 4 m ,
\ee
with the limits (\ref{e:gek:lim_rph_pm}) and (\ref{e:gek:lim_rph_s}).
...
only ones, because the only other root of $q(r_0)=0$ is $r_0=0$
(see also Fig.~\ref{f:gik:spher_orb_exist}),
which would correspond to the ring singularity.
The spherical orbit with $r_0 = r_{\rm ph}^{**}$ has $q < 0$. It is thus
a vortical geodesic. Moreover, it obeys $q=q_{\rm min}=-(a - |\ell|)^2$. Thus
according to Sec.~\ref{s:gik:th_motion}, it stays at a fixed value of $\th$.
