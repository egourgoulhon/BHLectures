...
\subsection{Zero-energy null geodesics}
...
The geodesic starts at $\lambda=0$ in the equatorial plane, in the direction
of the Southern hemisphere ($\D\th/\D\lambda>0$).
The left panel corresponds to $0 \leq \lambda \leq 7.7$,
while the right one extends the range to $0 \leq \lambda \leq 70$.
...
The geodesic starts at $\lambda=0$ in the equatorial plane, in the direction
of the Southern hemisphere. The plotted range is $0 \leq \lambda \leq 38$.
The grey sphere is the black hole event horizon.
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_null_geod_plots}]}
}
\end{figure}
...
Spherical photon orbit at $r_0=3 m$ around a Kerr black hole with
$a=0.95\, m$, depicted in terms of the Cartesian Boyer-Lindquist coordinates
$(x,y,z)$ defined by Eq.~(\ref{e:gek:Cartesian_BL}) and scaled in units of $m$.
The right panel depicts the projection of the orbit onto the $xy$-plane.
The geodesic starts at $\lambda=0$ in the equatorial plane, in the direction
of the Southern hemisphere. The plotted range is $0 \leq \lambda \leq 32$.
The grey sphere is the black hole event horizon.
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_null_geod_plots}]}
}
\end{figure}
...
\begin{example}
Figure~\ref{f:gik:spher_3d_r_16} shows an outer spherical photon orbit
around a Kerr black hole with $a=0.95\, m$. It is located at $r_0 = 1.6\, m$
and has $\ell = 2.171\, m$ and $q=5.976\, m^2$. Note that this orbit
has $\D\ph/\D\lambda > 0$ everywhere.
\end{example}

\begin{example}
Figure~\ref{f:gik:spher_3d_r_28} shows the spherical photon orbit
located at $r_0 = 2.8\, m$ around a Kerr black hole with $a=0.95\, m$. It
has $\ell = -1.089\, m$ and $q=26.260\, m^2$. Note that despite $\ell < 0$,
this orbit has $\D\ph/\D\lambda > 0$ near the equator.
\end{example}
...
the meridional plane $(t,\ph)=\mathrm{const}$ of
...
colored in dark green, with its boundary at $r=r_{\rm ph}^{\rm ms}$ shown in blue.
...
Contrary to the meridional plane plot of Fig.~\ref{f:gik:zero_ener_merid}, which is based
on O'Neill coordinates, Fig.~\ref{f:gik:spo_meridional} is drawn in terms
on coordinates $(\hat{r},\th)$, where $\th$ is the Boyer-Lindquist/Kerr coordinate
and $\hat{r}$ is related to the Boyer-Lindquist/Kerr $r$ by
\be
    \hat{r} := \frac{1}{2} \left( r + \sqrt{r^2 + 4m^2} \right) .
\ee
As O'Neill coordinates\index{O'Neill!coordinates}, this brings the whole range $(-\infty,+\infty)$ of $r$
to $(0,+\infty)$, with $0$ corresponding to $r\to -\infty$ (the black small circle
in Fig.~\ref{f:gik:spo_meridional}).
Contrary to O'Neill exponential coordinate $\mathrm{e}^{r/m}$, the
coordinate $\hat{r}$ does not enlarge too much the region exterior
to the black hole, which makes it more suited to the plot of the photon region.
Note that asymptotically in $\M_{\rm I}$, $\hat{r}$ is similar to $r$:
...
\end{align}
\end{subequations}
As a function of $a$, $r_{\rm ph}^{\rm pol}$ decays monotically from
$\lim_{a\to 0} r_{\rm ph}^{\rm pol} = 3 m$ to
$\lim_{a\to m} r_{\rm ph}^{\rm pol} = (\sqrt{2} + 1) m \simeq 2.414214\, m$,
while $r_{\rm ph}^{\rm pol,in}$ decays monotically from
