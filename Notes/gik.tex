...
with the advent of the Event Horizon Telescope\index{Event Horizon Telescope} \cite{EHT19a},
...
apparent superluminal motions\index{superluminal motion}, up to $v_{\rm app} \sim 6 c$, requires
...
Figure~\ref{f:gik:shadow_comp_th} compares the critical curves at a fixed
...
black hole as displayed in Fig.~\ref{f:gis:disk_images}. In particular, one notices
...
The image of order $n$ is generated by null geodesics that have performed
...
convergence of higher order images to $\mathscr{C}$. This exponential convergence
has been established in the Schwarzschild case by Eq.~(\ref{e:gis:bn_sequence}),
but it holds as well for the Kerr case \cite{GrallL20a}.
...
The major difference between the images shown in Fig.~\ref{f:gik:img_disk_a50}
and the Schwarzschild ones shown in Fig.~\ref{f:gis:disk_images}
is that in the latter case, the
...
in Sec.~\ref{s:gis:image_disk} an optically thick disk was considered, while we are
dealing here with an optically thin one. Hence in Fig.~\ref{f:gis:disk_images},
the bottom of images of order $\geq 2$ is blocked by the part of the accretion
disk standing in the foreground. Another difference between the two sets of
images is that in
...
(for $\th_{\Obs} = 0$, the two images seem even to touch each other). This is
a consequence of the inner radius of the accretion disk being closer to the
...
This is actually due to the inner part of the accretion disk being
...
is the easiest to understand since $\mathscr{C}$ involves only the $\ell=0$ critical null geodesics, i.e. those that are rolling indefinitely around the sphere $r_0 = r_{\rm ph}^{\rm pol}$
...
The emitted photons impact then the screen scritly inside $\mathscr{C}$. We shall
not provide a full demonstration here, but simply recall that this was established rigorously
for $a=0$ in Chap.~\ref{s:gis}: formula (\ref{e:gis:bn_sequence}) in conjunction with
$A(r_{\rm em}) < 0$ for $r_{\rm em} < 3 m = r_{\rm ph}^{\rm pol}(a=0)$ (cf. Fig.~\ref{f:gis:A_rem})
yields impact parameters $b_n^\pm$ that are lower than $b_{\rm c}$, which
is nothing but the radius of $\mathscr{C}$ for $a=0$.
...
as shown in Ref.~\cite{NarayJG19}.
...
major effect of a high inclination angle is to move the high order images to
to the right.

Moreover, we note that the secondary image is brighter than the primary one
and the tertiary one, which is very thin and superposed to the secondary image,
is even brighter. This is because the higher the image order, the larger the
number of turns around the black hole of the involved geodesics, so that
...
impacting the screen, even if very close to a critical geodesic, would carry only one photon: the photon emitted at the
...
in a more or less straight line. Hence only optically thin material located
around some spherical photon orbit can give rise to bright higher-order images.



% shadow: \cite{FalckMA00}


\subsubsection{Note on terminology}
The set formed by the secondary and higher order images has been called the \emph{photon ring}
\cite{BeckwD05,JohanP10,Johns_al20,GrallL20a,GrallL20c};
more recently the term \emph{secondary ring} has been introduced
\cite{VinceWAGLPG21}. Some authors have distinguished the secondary image
by calling it the \emph{lensing ring} \cite{GrallHW19}.

%\begin{hist}
% Rings:
%  - Darwin 1959;: point source
%  - Luminet 1979: series of rings of illuminated BH,
%  - Jaroszynski & Kurpiewski 1997: bright ring surrounding the shadow for optically thin accretion
%    flow
%\end{hist}
...
For a detailed study of images of the surroundings of a Kerr black hole, based on exact
