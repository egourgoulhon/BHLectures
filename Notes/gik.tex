...
Each image in a given sequence can be labeled by the number $n$ of round trip around the black hole
...
the release of the very first observed image of a black hole by the
...
the photon ring, as discussed above. In order to do so, one must
...
to the large relativistic jet\index{jet} emanating from the close vicinity of the black hole\footnote{This jet has been discovered as early as 1918 \cite{Curtis1918},
...
Assuming that the jet axis is aligned with the rotation axis, we have either
$\th_{\Obs} = \iota$ or $\th_{\Obs} = \pi - \iota$.

The main feature of the EHT image is a broad annular region, whose bottom part is much brighter
...
It follows that it is not possible to infer the spin parameter $a$ from that image,
...
will undoubtedly be sharper and will revolutionize black hole astronomy.
...
We have only discussed images as seen by a remote observer (``astronomical'' setting).
For the images seen by an observer travelling close to, and even inside, a Kerr black hole,
see the recent study~\cite{Riazu20}.
