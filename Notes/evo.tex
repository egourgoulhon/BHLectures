...
Then $\mathscr{Q}_{\Sigma}$ is the electric charge in the volume $\Sigma$
(cf. e.g. Eq.~(18.2) in Ref.~\cite{Gourg13}) and $\mathscr{Q}_{\Sp}$ is $\mu_0^{-1}$ times the flux of the electromagnetic
...
Noether charge with respect to a Killing vector. For general relativity,
this Noether charge is the Komar mass ($\w{\xi}$ stationary) or
the Komar angular momentum ($\w{\xi}$ axisymmetric).
\end{remark}
