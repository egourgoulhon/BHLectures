...
In order to endow $\FF$ with a symplectic form for defining a covariant phase space,
let us consider the exterior derivative of $\Theta$, i.e.
the 2-form $\Omega := \Dd \Theta$. This is analogous to Eq.~(\ref{e:evo:Omega_dTheta})
...
on $\FF$ that are related to two independent variations of $\w{g}$ in the following sense.
Instead of considering a curve on $\FF$, which provides a variation  $\delta\w{g}$
in a single direction, let us consider a parametrized 2-surface on $\FF$,
i.e. a map $\Phi: I_1\times I_2 \to \FF$, $(\lambda_1,\lambda_2) \mapsto \w{g}(\lambda_1,\lambda_2)$,
where $I_1$ and $I_2$ are two real intervals.
