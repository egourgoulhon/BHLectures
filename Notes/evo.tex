...
\subsection{Irreversibily in the evolution of a Kerr black hole}

Let us consider a particle $\mathscr{P}$ in free fall from infinity into a
(non-extremal) Kerr
black hole of mass $M$ and spin $J= a M$, with
$a < M$. Let us denote by $E$ and $L$ the
conserved energy and conserved angular momentum of $\mathscr{P}$ (cf. Property~\ref{p:gek:conserved_E_L}), with $E\ll M$ and $|L| \ll J$.
At some event $A$ in the outer ergoregion, we allow $\mathscr{P}$ to split
in two particles, $\mathscr{P}'$ and $\mathscr{P}''$ of conserved
energy and conserved angular momentum $(E',L')$ and $(E'',L'')$ respectively,
such that $\mathscr{P}''$ falls into the black hole and $\mathscr{P}'$
escapes to infinity, as in the Penrose process\index{Penrose!process} described
in Sec.~\ref{s:gek:Penrose_proc}. As shown in Sec.~\ref{s:gek:Penrose_proc},
one has $E = E' + E''$ and $L = L' + L''$. If the particle split does not occur
(direct fall into a black hole), then we shall set $E'=0$, $E''=E$, $L'=0$
and $L''=L$.
By virtue of the no-hair theorem~\ref{p:sta:no-hair_thm},
after $\mathscr{P}'$ has gone sufficiently far,
the black hole
...
\be \label{e:evo:dM_E_dJ_L}
    \delta M = E'' \qand \delta J = L'' .
\ee
From Eqs.~(\ref{e:evo:mass_variation_Kerr}) and (\ref{e:evo:dM_E_dJ_L}), the evolution of the black hole
area $A$ in the process is
\be
    \delta A = \frac{8\pi}{\kappa} \left( \delta M - \Omega_{\Hor} \delta J \right)
        =  \frac{8\pi}{\kappa} \left( E'' - \Omega_{\Hor}  L'' \right) .
\ee
Note that $\kappa > 0$ since the black is not extremal.
Now, we have established in Sec.~\ref{s:gek:Penrose_proc} that
$E'' - \Omega_{\Hor}  L'' > 0$ [Eq.~(\ref{e:gek:OmegaH_Lpp_Epp})] as
necessary condition for $\mathscr{P}''$ to cross the event horizon.
...
for a Penrose process ($E'>E$), since then $E''<0$ and Eq.~(\ref{e:evo:dM_E_dJ_L})
implies $\delta M < 0$. The limit $ E'' - \Omega_{\Hor}  L'' = 0$,
which would yield $\delta A = 0$, would correspond to the particle $\mathscr{P}$
having a vanishing radial affine velocity $\D r / \D\lambda$ on the event horizon
$\Hor$. Indeed, the equation of geodesic motion (\ref{e:gek:drdl}) at
$r=r_+$ writes
\be
    (r_+^2 + a^2 \cos^\th) \derd{r}{\lambda} = - (r_+^2 + a^2) \left| E'' - \Omega_{\Hor}  L'' \right| ,
\ee
where we have specialized expression (\ref{e:gek:def_R_Q}) for $R(r)$ to $\Delta = 0$
and used the identity $a^2 / (r_+^2 + a^2) =\Omega_{\Hor}$.
