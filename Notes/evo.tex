...
\begin{remark}
The above formulation of Hamiltonian systems is quite covariant: it does not
refer to any coordinate system on $\mathcal{F}$, not even speaking
about canonical coordinates $(q^i, p_i)$.
It does not even assumes that $\mathcal{F}$ is the cotangent bundle $T^*\mathcal{C}$
of some ``configuration space'' $\mathcal{C}$, which is the standard
arena for Hamiltonian mechanics \cite{AbrahM78}.
\end{remark}
...
Consequently, one may view $\mathcal{F}$ as the space of solutions
