...
Let us apply the general mass variation formula (\ref{e:evo:mass_variation_gal}) to a black hole surruounded
by some perfect-fluid matter in 4-dimensional general relativity.

\begin{prop}[mass variation formula for black holes surrounded by a fluid in general relativity]
...
of a perfect fluid\index{perfect fluid}\index{fluid!perfect --}
of 4-velocity $\w{u}$, proper energy density $\veps$ and pressure $p$:
\be
    \w{T} = (\veps + p) \uu{u} \otimes \uu{u} + p\w{g}
\ee
We shall assume the equation of state\index{equation!of state}
$\veps = \veps(n_{\rm b},s)$, where $n_{\rm b}$ is the proper baryon density\index{baryon!density}
and $s$ the proper entropy density\index{entropy!density}.
