...
(\ref{e:evo:delta_L_E_Theta}): any change $\w{\Theta} \to \w{\Theta} + \dd \w{Y}$, where $\w{Y}=\w{Y}[\w{g},\delta\w{g}]$ is an
...
the linearity of $\w{\Theta}[.,.]$ with respect to its second argument. Also the
