...
$\w{\Theta}$ is a $(n-1)$-form, which is a function of $\w{g}$ and $\delta\w{g}$, with a
linear dependence in the latter.
The $n$-form  $\dd \w{\Theta}$ is the exterior derivative of $\w{\Theta}$
(cf. Sec.~\ref{s:bas:ext_deriv}).

\begin{example}[variation of the Einstein-Hilbert Lagrangian]
...
\ee
Notice that $\w{\Theta}$ linear in  $\delta\w{g}$.
\end{example}
...
\begin{example}[Einstein equation]
For general relativity, in view of Eq.~(\ref{e:evo:E_Theta_GR}),
the equations of motion (\ref{e:evo:eom_E}) become $\w{G} = 0$, which constitutes
the vacuum Einstein equation\index{Einstein!equation!vacuum --}\index{vacuum!Einstein equation}.
\end{example}
