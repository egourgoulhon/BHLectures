...
and $\{.,.\}$
stands for the Poisson bracket\index{Poisson!bracket}:
\be
...
$(y^a)$ are $\Omega_{ab} = \left( \sum_{i=1}^N \delta^{i+N}_a \delta^i_b - \delta^i_a \delta^{i+N}_b\right) = \delta_{a,b+N} - \delta_{b,a+N}$. Hence, we may write,
