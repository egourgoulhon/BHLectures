...
and the
baryon \defin{relativistic chemical potential}\footnote{By \emph{relativisitc} it is meant that the rest mass is included in the chemical potential.}\index{chemical potential}\index{potential!chemical --} $\mu_{\rm b}$ are then
...
Let us assume that $\w{g}$ obeys the Einstein equation (\ref{e:fra:Einstein_eq_G});
then the equation of
energy-momentum conservation\index{energy-momentum!conservation}\index{conservation!of energy-momentum}
(\ref{e:fra:divT}) holds:
$\wnab\cdot\vw{T} = 0$. Its projection along $\w{u}$
lead to the conservation of the entropy current $s \w{u}$, i.e.
$\wnab \cdot (s \w{u}) = 0$ (there is no heat exchange between neighboring elements
of a perfect fluid).
Given a spacelike hypersurface $\Sigma$, the conserved currents
$n_{\rm b} \w{u}$ and $s \w{u}$ give rise
to the concepts of \emph{total number of baryons} $\mathscr{N}_{\rm b}$ in $\Sigma$
and \emph{total fluid entropy} $S_{\rm f}$ in $\Sigma$:
\be
...
giving rise to the concept of the \emph{total fluid angular momentum} $J_{\rm f}$ in $\Sigma$:
\be
...
projected along $\w{u}$, the identities (\ref{e:evo:first_law_simple_fluid}), (\ref{e:evo:pressure_simple_fluid})
and $\wnab \cdot (n_{\rm b} \w{u}) = 0$ (see e.g. Ref.~\cite{Gourg06} for details).
\end{proof}

\begin{prop}[mass variation formula for a black hole surrounded by a fluid in general relativity]
Let $(\M,\w{g})$ be a stationary and axisymmetric 4-dimensional spacetime
such that $\w{g}$ obeys Einstein equation (\ref{e:fra:Einstein_eq_G}) with $\Lambda=0$:
$\w{G} = 8\pi \w{T}$, where $\w{T}$ is the energy-momentum tensor
of a simple perfect fluid as described in Propery~\ref{p:evo:simple_fluid}.
