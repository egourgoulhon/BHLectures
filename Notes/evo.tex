...
\delta\kappa = - \frac{1}{2} \chi_\mu \nabla^\nu h^\mu_{\ \, \nu}
    + \sum_{i=1}^L \delta \Omega^{(i)} k^\mu \chi^\nu \nabla_\mu \eta_{(i)\nu} .
\ee
Let us integrate this relation over the cross-section $\Sp_k$. Since $\delta\kappa$ and $\delta \Omega^{(i)}$
are constant, we get
\[
    \delta\kappa  \underbrace{\int_{\Sp_k} \, \sqrt{q}\,  \D^{n-2} x}_{A_k} =
    - \underbrace{\frac{1}{2}  \int_{\Sp_k} \chi_\mu \nabla^\nu h^\mu_{\ \, \nu} \, \sqrt{q} \, \D^{n-2} x}_{I_{\Sp_k}}
    + \sum_{i=1}^L \delta \Omega^{(i)}
       \underbrace{\int_{\Sp_k} \nabla_\mu \eta_{(i)\nu} \, k^\mu \chi^\nu \, \sqrt{q}\,  \D^{n-2} x}_{-8\pi J_{\Hor_{k}(i)}} .
\]
The identification of the last integral with $-8\pi J_{\Hor_{k}(i)}$ readily follows from expression
(\ref{e:sta:J_Komar_cov_der}) for the Komar angular momentum, once the area element bivector normal to $\Sp_k$
is expressed as $\D S^{\mu\nu}=(\chi^\mu k^\nu - k^\mu \chi^\nu) \sqrt{q}\, \D^{n-2} x $ [Eq.~(\ref{e:sta:dS_chi_k})].
