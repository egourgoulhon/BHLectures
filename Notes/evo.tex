...
\ee
\begin{remark}
The mass variation formula (\ref{e:evo:mass_variation_Kerr}) is \emph{not}
...
Eq.~(\ref{e:evo:mass_variation_Kerr}).
\end{remark}
...
by the zeroth law of black hole dynamics (Property~\ref{p:neh:zeroth_law} or Property~\ref{p:neh:zeroth_law_bifur}), this
...
is related to the changes $\delta A_k$ of the areas and to the changes
$\delta J_{\Hor_k}^{(i)}$ of the Komar angular momenta by
\bea
...
(connected event horizon) and
$n=4$ (which implies $0 \leq L_1 \leq 1$), this formula reduces to
...
\be \label{e:evo:I_prov}
    I = \frac{1}{2\mu_0} \int_{\Sp_{\rm int}} A_\sigma \xi^\sigma F_{\mu\nu} \D S^{\mu\nu}
        - \frac{1}{4\mu_0} \int_\Sigma F_{\mu\nu} F^{\mu\nu} \xi^\rho \D V_\rho ,
\ee
where $\Sp_{\rm int} := \cup_{k=1}^K \Sp_k$ is the inner boundary of $\Sigma$ ($\Sp_k := \Hor_k \cup \Sigma$)
...
as
\[
    \delta I = \frac{1}{2\mu_0} \, \delta \int_{\Sp_{\rm int}} A_\sigma \xi^\sigma F_{\mu\nu} \D S^{\mu\nu}
        - \frac{1}{\mu_0} \int_\Sigma \nabla^\nu \Omega_{\mu\nu} \, \D V^\mu
        + \frac{1}{2} \int_\Sigma T^{\mu\nu} h_{\mu\nu} \,  \xi^\rho \D V_\rho .
\]
Since $\w{\Omega}$ is antisymmetric, $\Omega_{\mu\nu}$ are the components of a 2-form and we may invoke
Lemma~\ref{p:sta:flux_div_2_form} to express the second integral as a flux integral through $\Sp_{\rm int}$
(setting to zero the flux through $\Sp_{\infty}$ due to the fast decay of $\w{A}$ and $\w{F}$ and hence of
$\w{\Omega}$). We thus get
\[
  \delta I = \frac{1}{2\mu_0} \, \delta \int_{\Sp_{\rm int}} A_\sigma \xi^\sigma F_{\mu\nu} \D S^{\mu\nu}
  + \frac{1}{\mu_0}  \int_{\Sp_{\rm int}} \delta A_\rho F^\rho_{\ \, \mu} \xi_\nu \, \D S^{\mu\nu}
  + \frac{1}{2} \int_\Sigma T^{\mu\nu} h_{\mu\nu} \,  \xi^\rho \D V_\rho ,
\]
where we have used Eq.~(\ref{e:evo:def_Omega_AFxi}) and the antisymmetry of $\D S^{\mu\nu}$ to write
$\Omega_{\mu\nu} \D S^{\mu\nu} = \delta A_\rho ( F^\rho_{\ \, \mu} \xi_\nu - F^\rho_{\ \, \nu} \xi_\mu)  \D S^{\mu\nu}
= 2 \delta A_\rho F^\rho_{\ \, \mu} \xi_\nu\, \D S^{\mu\nu}$.
...
\[
    ...
    \frac{\kappa_{k}}{8\pi}\, \delta A_k
    +  \sum_{i=1}^{L_{k}} \Omega^{(i)} \, \delta J_{\Hor_k}^{(i)} \right)
    - \frac{1}{2\mu_0} \, \delta \int_{\Sp_{\rm int}} A_\sigma \xi^\sigma F_{\mu\nu} \D S^{\mu\nu}
    - \frac{1}{\mu_0}  \int_{\Sp_{\rm int}} \delta A_\rho F^\rho_{\ \, \mu} \xi_\nu \, \D S^{\mu\nu} .
\]
to be continued...
\end{proof}
...
Let us apply the general mass variation formula (\ref{e:evo:mass_variation_gal}) to a black hole surrounded
...
matter surrounding the black hole forms an accretion disk\index{accretion!disk} or an accretion torus, so that it can be modeled by a perfect fluid in
...
at the same summer school \cite{Carte73b}.
...
At this stage, it would be premature to call formula~(\ref{e:evo:mass_variation_vacuum}) or
(\ref{e:evo:mass_variation_vacuum_n4})
