...
a time function $t$, with respect to which the evolution of the system is governed by
Hamilton's equations\index{Hamilton's equations}:
\be
    \derd{q}{t} = \{q, H \} \qand \derd{p}{t} = \{p, H\} ,
\ee
where $(q,p)$ are canonical coordinates on the system's phase space and $\{.,.\}$
