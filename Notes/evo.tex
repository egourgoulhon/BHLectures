...
\be
    \kappa = \mathrm{const}.
\ee
\item[First law:]\index{first law!of BH dynamics} Under the hypotheses of Property~\ref{p:evo:mass_var_electrovac},
...
if one defines the temperature of such systems from their kinetic energy.
\end{remark}
...
naively conceived to violate it \cite{Carli14,Wald01,Wall09}.

\begin{hist}
In 1970-71, the irreversible increase of the area of a Kerr black hole interacting with particles
obtained by Roger Penrose\index[pers]{Penrose, R.}, Roger Floyd\index[pers]{Floyd, R.M.}
and Demetrios Christodoulou\index[pers]{Christodoulou, D.} \cite{PenroF71,Chris70},
as well as the general area theorem established by  Stephen Hawking\index[pers]{Hawking, S.W.} \cite{Hawki71}
(cf. the historical notes on p.~\pageref{h:evo:irreducible_mass} and \pageref{h:evo:area_increase})
suggested an analogy between the black hole area and the entropy of ordinary matter.
...
a black hole is endowed with a genuine thermodynamic entropy $S$ proportional to its area $A$.
Moreover, by means of some more or less heuristic arguments, he stated that the relation
must be $S = \eta k_{\rm B} A/(4\pi\hbar)$, where $\eta$ is a dimensionless coefficient close to unity
\cite{Beken72,Beken73b}. This is exactly formula~(\ref{e:evo:S_BH}) provided that $\eta = \pi$.
\end{hist}

\subsection{Black hole entropy as a statistical entropy}
...
as counting some (quantum) microscopic states corresponding to a given macroscopic
state of the black hole. Defining the microscopic states clearly requires a quantum theory of gravity, which we do not have yet. However some partial results have been
obtained, based on two current approaches to quantum gravity: string theory\index{string theory} \cite{Peet01}
and loop quantum gravity\index{loop quantum gravity} \cite{Perez17,Rovel04}. Regarding the former, Strominger\index[pers]{Strominger, A.} and Vafa\index[pers]{Vafa, C.} \cite{StromV96} have established in 1996
the Bekenstein-Hawking formula (\ref{e:evo:S_BH}) by computing the entropy
...
which corresponds to a low energy limit of a string theory. Further developments are
discussed in the review articles \cite{Carli14,Damou04,Horow98,Wald01,Walla19}; they are
still limited to extremal black holes or near-extremal ones. In particular,
the Bekenstein-Hawking entropy of a Schwarzschild black hole has not been
recovered by string theory yet. The reason for which computations are easier for
extremal black holes is that their near-horizon geometries have additional
symmetries, as we have seen in the example of the extremal Kerr black hole
(cf. Sec.~\ref{s:exk:NHEK}).
...
formula for the entropy of a spherically symmetric macroscopic black hole of area $A$ evaluated
by counting the number of geometric states of the horizon (see e.g. Ref.~\cite{EngleNP10} and
Refs.~\cite{BarbeP23,Carli14,Perez17} for reviews):
