...
\subsection{Noether charges associated to diffeomorphism invariance}
...
definition of $\w{J}[\w{\xi}]$, it is equivalent to
$\dd\w{J}[\w{\xi}]  = - E^{\mu\nu} \Liec{\xi} g_{\mu\nu} \,\weps$,
so that $\dd\w{J}[\w{\xi}]  = 0$ if $\w{E} = 0$ is fulfilled (or if $\w{\xi}$ is a Killing vector).
\end{proof}
...
formula (\ref{e:sta:def_Komar_mass}) defining the Komar mass or in formulahttps://www.decitre.fr/livres/pauline-kergomard-9791093661469.html
...
\be \label{e:evo:Noether_Q_Sigma_1_2}
    \mathscr{Q}_{\Sigma_2,\w{\xi}}  = \mathscr{Q}_{\Sigma_1,\w{\xi}}
...
\end{prop}
\begin{proof}
This is a direct consequence of Stokes' theorem and
the closedness of $\w{J}[\w{\xi}]$ (Property~\ref{p:evo:Noether_current}). Indeed, given
that $\partial\mathscr{D} = \Sigma_1\cup\Sigma_2\cup\mathscr{W}$, Stokes' theorem (\ref{e:bas:Stokes})
...
yields the identity (\ref{e:evo:Noether_Q_Sigma_1_2}).
\end{proof}
...
\defin{Noether charge of $\Sp$ with respect to $\w{\xi}$}\index{Noether!surface charge} is
\be
    \mathscr{Q}_{\Sp,\w{\xi}} := \int_{\Sp} \w{Q}[\w{\xi}] ,
\ee
where $\w{Q}[\w{\xi}]$ is the Noether charge $(n-2)$-form of $\w{\xi}$
and the orientation of $\Sp$ for the above integral is the \emph{future-outward} orientation
...
Then
\be
     \mathscr{Q}_{\Sp_{\rm ext},\w{\xi}} = \mathscr{Q}_{\Sp_{\rm int},\w{\xi}}
        + \mathscr{Q}_{\Sigma,\w{\xi}}  .
\ee
\end{prop}
\begin{proof}
Let us apply Stokes' theorem (\ref{e:bas:Stokes}) on $\Sigma$ oriented endowed with the future
orientation (cf. Property~\ref{p:evo:Noether_charge_hypersurf}); given
that $\partial\Sigma = $ $\dd \w{Q}[\w{\xi}] =  \w{J}[\w{\xi}]$ [Eq.~(\ref{e:evo:J_dQ})],  we get
\[
    \mathscr{Q}_{\Sp_{\rm int},\w{\xi}}
\]
