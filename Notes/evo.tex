...
and $\Sigma_2$ in a smooth part of the event horizon $\Hor$. The green curves
are the null geodesic generators of $\Hor$.}
\end{figure}



\begin{prop}[area theorem \textnormal{(Hawking\index[pers]{Hawking, S.W.} 1971 \cite{Hawki71},
Chru\'sciel\index[pers]{Chrusciel, P.T.@Chru\'sciel, P.T.} et al. 2001 \cite{ChrusDGH01})}]
Let $(\M,\w{g})$ be a $n$-dimensional spacetime containing a black hole
...
this is equivalent to demanding the existence of a neighbourhood $\mathscr{U}$
of $\Hor$ such that for every compact set $\mathscr{C}\subset\mathscr{U}$ whose
chronological future $I^+(\mathscr{C})$ intersects $\scri^+$, there is a least one
generator of $\scri^+$ that intersects $\overline{I^+(\mathscr{C})}$ and leaves it
...
a dashed curve. The dark green dashed curves depict other null geodesics that
...
$\Sp_1$ and $\Sp_2$, cf. Fig.~\ref{f:evo:area_thm_smooth}).
$\Sp_1$ and $\Sp_2$ are then cross-sections of $\Hor$, which implies that
every null geodesic generator of $\Hor$ intersect each of them once, and only
once. One may introduce a 1-parameter foliation $(S_t)_{t\in [0,1]}$ of $\Hor$ by
...
Let $\wl = \D\w{x}/\D t$ be the associated tangent vector. By the very definition
