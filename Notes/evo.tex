...
Finally let us assume that $(\M,\w{g})$ is asymptotically flat with the
...
\]
where $\bar{q}$ stands for the determinant of the round metric of the unit sphere $\SS^{n-2}$ with respect to the coordinates $(y^a)$. Substituting $\alpha_n$ by its
expression in terms of $\Omega_{n-2}$ yields
\[
...
$a < M$ (non-extremal black hole). Let us denote by $E$ and $L$ the
conserved energy and conserved angular momentum of $\mathscr{P}$ (cf. Property~\ref{p:gek:conserved_E_L}), with $E\ll M$ and $|L| \ll J$.
At some event $A$ in the ergoregion, we allow $\mathscr{P}$ to split
in two particles, $\mathscr{P}'$ and $\mathscr{P}_*$,
...
$E = E' + E_*$ and $L = L' + L_*$. If the particle split does not occur
($\mathscr{P}$ falling directly into the black hole), we shall set $E'=0$, $E_*=E$, $L'=0$
and $L_*=L$.
...
When $\mathscr{P}$ was far apart from the black hole, the total energy (ADM mass)
...
of the gravitational radiation generated
by the capture of $\mathscr{P}$, so that the final spacetime energy is
...
\ee
From Eq.~(\ref{e:evo:mass_variation_Kerr}), the evolution of the black hole
area $A$ in the process is $\delta A = 8\pi/\kappa \left( \delta M - \Omega_{\Hor} \delta J \right)$.
In view of Eq.~(\ref{e:evo:dM_E_dJ_L}), this yields
\be
    \delta A = \frac{8\pi}{\kappa} \left( E_* - \Omega_{\Hor}  L_* \right) .
\ee
Note that the above equation is well-posed, given that $\kappa > 0$ since the black hole is not extremal.
...
\be
    \encadre{\delta A > 0} .
\ee
Hence in the process of particle accretion, the area of a Kerr black hole
necessarily increases. This holds even if $\delta M < 0$, which occurs
for a Penrose process ($E'>E$), since then $E_*<0$ and Eq.~(\ref{e:evo:dM_E_dJ_L})
implies $\delta M < 0$.
The particle accretion is thus an \emph{irreversible transformation}
\index{irreversible transformation} of the black hole. The irreversibility traces back
to the fundamental inequality $E_* > \Omega_{\Hor}  L_*$ [Eq.~(\ref{e:gek:OmegaH_Lpp_Epp})],
which must hold if $\mathscr{P}_*$ crosses the event horizon: in view of Eq.~(\ref{e:evo:dM_E_dJ_L}),
one may think naively that one could revert the transformation $(M,J)\to (M+\delta M, J+\delta J)$
by throwing into the black hole a particle of conserved energy $E'_* = - E_*$
and conserved angular momentum $L'_* = - L_*$. However this is not permitted
because the constraint $E'_* > \Omega_{\Hor}  L'_*$ is violated if
$E_* > \Omega_{\Hor} L_*$ holds.

The limit case $ E_* = \Omega_{\Hor}  L_*$,
which would yield $\delta A = 0$, would correspond to the particle $\mathscr{P}_*$
having a vanishing radial affine velocity $\D r / \D\lambda$ on the event horizon
$\Hor$ ($\lambda$ being the affine parameter along $\mathscr{P}_*$'s worldline associated
with $\mathscr{P}_*$'s 4-momentum).
Indeed, the equation of geodesic motion (\ref{e:gek:drdl}) on $\Hor$ ($r=r_+$) yields
\be
    (r_+^2 + a^2 \cos^2 \th) \derd{r}{\lambda} = - (r_+^2 + a^2) ( E_* - \Omega_{\Hor}  L_* ) .
\ee
...
($r = r_+$), which yields $\sqrt{R(r)} = |E_* - \Omega_{\Hor}  L_*| = E_* - \Omega_{\Hor}  L_*$
