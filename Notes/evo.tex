...
\begin{remark} \label{r:evo:Theta_not_unique}
The symplectic potential form $\w{\Theta}$ is not uniquely determined by
(\ref{e:evo:delta_L_E_Theta}):
since $\dd$ is nilpotent
[Eq.~(\ref{e:ext_der_nilpot})],
any change $\w{\Theta} \to \w{\Theta} + \dd \w{Y}$, where $\w{Y}=\w{Y}[\w{g},\delta\w{g}]$ is an
arbitrary $(n-2)$-form that is a local function of $\w{g}$ and $\delta\w{g}$
and depends linearly on $\delta\w{g}$,
would result in the same formula (\ref{e:evo:delta_L_E_Theta}).
\end{remark}
...
\subsection{Noether currents and charges  associated to diffeomorphism invariance}

%Let $\w{\xi}$ be a smooth vector field on $\M$.
...
(cf. Sec.~\ref{s:neh:symmetries}). Actually, to simplify expressions, we shall consider the
transformation of $\w{g}$ under $\Phi_t^{-1}$, so that the pullback
$\Phi_t^*\w{g}$ appears instead of $(\Phi_t^{-1})^*\w{g}$
(cf. the above discussion). The variation of $\w{g}$ is then
\be \label{e:evo:delta_g_Phistar}
    \delta\w{g} = \Phi_t^* \w{g} - \w{g} .
\ee
Let us restrict to an infinitesimal diffeomorphism, i.e. $|t| \ll 1$. From the very definition of the
Lie derivative, as given by Eq.~(\ref{e:bas:def_Lie_der_covar}), we have then
$\Phi_t^*\w{g} - \w{g} \simeq t \Lie{\xi} \w{g}$, where the vector field $\w{\xi}$ is the
...
\ee
The corresponding variation of the Lagrangian $n$-form is $\delta\w{L} = \w{L}[\Phi_t^*\w{g}] - \w{L}[\w{g}]$.
...
as the \emph{derivative} of a 1-parameter family of metric tensors $\hat{\w{g}}(\lambda)$ such that
$\hat{\w{g}}(0) = \w{g}$. Using the symbol $\hat\delta$ to distinguish from the variation considered here,
the definition is
$\hat{\delta}\w{g} := \left. \D{\hat{\w{g}}}/\D{\lambda} \right|_{\lambda=0}$.
...
$\w{J}[\w{\xi}]$ is a linear and local function of $\w{\xi}$.

If the gravity theory described by the Lagrangian $n$-form $\w{L}$ is diffeomorphism-invariant, then
the differential form $\w{J}[\w{\xi}]$ is closed as soon as the equations of motion (\ref{e:evo:eom_E}) are satisfied:
...
that of $\w{\Theta}[.,.]$.
We have $\dd\w{J}[\w{\xi}]  = \dd( \w{\Theta}[\w{g},\Lie{\xi}\w{g}]) - \dd(\w{\xi}\cdot\w{L})$.
...
Hence
\[
    \dd\w{J}[\w{\xi}]  = \dd( \w{\Theta}[\w{g},\Lie{\xi}\w{g}])  - \Lie{\xi} \w{L}  .
\]
Let $\Phi_t$ be the flow map along
$\w{\xi}$ corresponding to an infinitesimal parameter $t$,
as defined in Sec.~\ref{s:bas:Lie_der_vector}. In other words,
$\Phi_t$ is an infinitesimal diffeomorphism of $\M$
belonging to a 1-dimensional Lie group whose generator is $\w{\xi}$.
It induces the variation (\ref{e:bas:delta_g_Lie_xi_g}) of $\w{g}$, namely
$\delta\w{g} = t \Lie{\xi} \w{g}$. Since the gravity theory is diffeomorphism-invariant,
the corresponding variation of $\w{L}$ obeys Eq.~(\ref{e:bas:delta_L_Lie_xi_L}):
$\delta\w{L} = t \Lie{\xi} \w{L}$.
Equation~(\ref{e:evo:delta_L_E_Theta}) then yields
\[
    \Lie{\xi} \w{L}  = \frac{1}{t} \left( E^{\mu\nu} \delta g_{\mu\nu} \, \weps
        + \dd (\w{\Theta}[\w{g},\delta\w{g}]) \right) .
\]
Substituting (\ref{e:bas:delta_g_Lie_xi_g}) for $\delta\w{g}$ and using the linearity
of $\w{\Theta}[\w{g},\delta\w{g}]$ with respect to its second argument to write
$\w{\Theta}[\w{g},t \Lie{\xi} \w{g}] = t \w{\Theta}[\w{g},\Lie{\xi} \w{g}]$,
we get
$\Lie{\xi} \w{L}  = E^{\mu\nu} \Liec{\xi} g_{\mu\nu} \,\weps + \dd( \w{\Theta}[\w{g},\Lie{\xi}\w{g}])$.
The above expression of $\dd\w{J}[\w{\xi}]$ reduces then to
\be
    \dd\w{J}[\w{\xi}]  = - E^{\mu\nu} \Liec{\xi} g_{\mu\nu} \,\weps .
\ee
Accordingly, $\dd\w{J}[\w{\xi}]  = 0$ if $\w{E} = 0$ holds (or if $\w{\xi}$ is a Killing vector).
\end{proof}

\begin{remark} \label{r:evo:J_not_unique}
As the symplectic potential form $\w{\Theta}$ (cf. Remark~\ref{r:evo:Theta_not_unique}), the Noether current form $\w{J}$ is not unique: any change
$\w{\Theta} \to \w{\Theta} + \dd \w{Y}$ yields a change $\w{J} \to \w{J} + \dd \w{Y}$,
for any $(n-2)$-form $\w{Y} = \w{Y}[\w{g},\Lie{\xi}\w{g}]$.
\end{remark}
...
Given a diffeomorphism-invariant theory described by the Lagrangian $n$-form $\w{L}$ and
a vector field $\w{\xi}$ on $\M$, there exists a $(n-2)$-form $\w{Q}[\w{\xi}]$ on $\M$,
...
$\w{J}[\w{\xi}]$ is globally exact, i.e. takes the form (\ref{e:evo:J_dQ})
with $\w{Q}[\w{\xi}]$ being a local function of $\w{\xi}$ \cite{Wald90}.
\end{proof}

\begin{remark} \label{r:evo:Q_not_unique}
The Noether charge form $\w{Q}[\w{\xi}]$ is not unique: for one hand, Eq.~(\ref{e:evo:J_dQ}) defines
$\w{Q}$ up to the addition of the exterior derivative of a $(n-3)$-form, $\w{Z}$ say:
$\w{Q} \to \w{Q} + \dd \w{Z}$ and on the other hand, $\w{Q}$ inherits from the ambiguity
$\w{J} \to \w{J} + \dd \w{Y}$ in the definition of $\w{J}$ (cf. Remark~\ref{r:evo:J_not_unique}), which yields
$\w{Q} \to \w{Q} + \w{Y}$. In total, one has the freedom
$\w{Q} \to \w{Q} + \w{Y} + \dd\w{Z}$, with $\w{Y} = \w{Y}[\w{g},\Lie{\xi}\w{g}]$
and $\w{Z} = \w{Z}[\w{g},\w{\xi}]$. Moreover, $\w{Z}$ can be chosen so that it does not contain
any derivative of $\w{\xi}$ (cf. Lemma~1 in Ref.~\cite{Wald90}).
\end{remark}
...
[Eq.~(\ref{e:bas:def_ext_1f_nab})] and the antisymmetry of $\weps$, we get
\be
...
From the above example, we note that the $(n-2)$-form $\star (\dd \uu{\xi})$ defining $\w{Q}[\w{\xi}]$
in Eq.~(\ref{e:evo:Q_GR_Komar}) is exactly the differential form that constitutes the integrand in
...
is, up to a constant factor, the integral over $\Sp$ of the
Noether charge $(n-2)$-form of the stationary Killing vector $\w{\xi}$.
Similarly, in an axisymmetric spacetime governed by general relativity,
the Komar angular momentum\index{Komar!angular momentum}\index{angular!momentum!Komar --} over $\Sp$ [Eq.~(\ref{e:sta:def_Komar_J})] is, up to a constant factor, the integral over $\Sp$ of the
Noether charge $(n-2)$-form of the axisymmetric Killing vector $\w{\eta}$.
\end{prop}

\begin{remark}
We have stressed in Remark~\ref{r:evo:Q_not_unique} that the Noether charge form of a given vector fied
$\w{\xi}$ is nonunique, being subject to the ambiguity
$\w{Q}[\w{\xi}] \to \w{Q}[\w{\xi}] +  \w{Y}[\w{g},\Lie{\xi}\w{g}] + \dd \w{Z}[\w{g},\w{\xi}]$.
However, when $\w{\xi}$ is a Killing vector, $\Lie{\xi}\w{g}= 0$, which
implies $\w{Y}[\w{g},\Lie{\xi}\w{g}] = 0$ since $\w{Y}$ is linear in its second argument
(cf. Remark~\ref{r:evo:Theta_not_unique} on p.~\pageref{r:evo:Theta_not_unique}).
Moreover the second ambiguity term, $\dd\w{Z}[\w{g},\w{\xi}]$, is an exact form and therefore does not contribute to the integral of
$\w{Q}[\w{\xi}]$ over any closed surface. Hence the Komar mass and Komar angular momentum over a given
closed surface $\Sp$ take a unique value, even if computed from distinct Noether charge forms of the involved
Killing vector.
\end{remark}
