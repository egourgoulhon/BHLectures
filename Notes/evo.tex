...
since $h_{\alpha\beta}:= \delta g_{\alpha\beta}$. Now, the identity (\ref{e:bas:dveps_divv}) yields
$(\nabla\cdot\w{H}) \weps = \dd (\w{H}\cdot\weps)$, so that
$\delta\w{L} = - G^{\mu\nu} \delta g_{\mu\nu} \, \weps + 2 \dd (\w{H}\cdot\weps)$.
...
\be \label{e:evo:E_Theta_GR}
    ...
    \qand
    \w{\Theta} = 2 \w{H}\cdot\weps .
\ee
For $n=4$, $\w{\Theta}$ is a 3-form and its expression in index notation
is $\Theta_{\alpha\beta\gamma} = 2  H^\mu \eps_{\mu\alpha\beta\gamma}$.
Substituting Eq.~(\ref{e:evo:def_H}) for $H^\mu$, we get the explicit expression of $\w{\Theta}$
in terms of $\w{g}$ and $\delta\w{g}$:
\be
    \Theta_{\alpha\beta\gamma} = g^{\mu\nu} g^{\rho\sigma}
...
\be \label{e:evo:eom_E}
    \w{E} = 0 .
\ee
This is the so-called \emph{Euler-Lagrange equations}\index{Euler-Lagrange equations} or \emph{equations of motion} for the considered gravity theory.

\begin{example}
For general relativity, in view of Eq.~(\ref{e:evo:E_Theta_GR}),
the equations of motion (\ref{e:evo:eom_E}) become $\w{G} = 0$, which
...
One defines the \defin{Noether current $(n-1)$-form associated to $\w{\xi}$} by \cite{Wald93,IyerW94}
\be
...
\]
By combining Eqs.~(\ref{e:evo:delta_L_E_Theta}) and (\ref{e:bas:delta_L_Lie_xi_L}), we get
\[
