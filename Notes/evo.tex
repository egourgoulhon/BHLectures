...
by symplectic duality. It thus obeys Eq.~(\ref{e:evo:Df_Omega_X_f}):
...
In particular, this encloses Eq.~(\ref{e:evo:dfdt_X_H_f}), since
$\Liesymbol_{X_H} f = X_H(f)$ for any scalar field $f$.
An important feature of the evolution hence defined is that
...
the \defin{Hamiltonian vector field}, i.e. the unique vector field $X_H$
on $\mathcal{F}$ such that
\be
\Dd H = \Omega(., X_H) .
\ee
The \defin{Poisson bracket} of two smooth
functions $f:\ \mathcal{F} \to \R$ and $g:\ \mathcal{F} \to \R$
is defined by any of the following equivalent equalities:
\be
     \{f,g\} := \Omega^{-1}(\Dd f, \Dd g)  = \Omega(X_g, X_f)
     = \langle \Dd f, X_g \rangle = X_g(f) ,
\ee
where $X_f$ and $X_g$ are the vector fields associated to $\Dd f$ and
$\Dd g$ by symplectic duality: $X_f := \Omega^{-1}(., \Dd f)$ $\iff$
$\Dd f =: \Omega(., X_f)$.
\end{prop}
