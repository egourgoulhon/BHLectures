...
(\ref{e:neh:null_dominant_cond_T}), but may be obeyed in some alternative theories as well.
...
based a least-action principle\index{least action} involving the action
\be
    S = \int_{\M} \w{L} = \int_{\M} L \, \weps = \int_{\M} L \sqrt{-g} \, \D^n x,
\ee
where $\w{L}$ is the \emph{Lagrangian $n$-form}\footnote{As the integral of a $n$-form over a $n$-dimensional manifold, $S$
is well-posed and coordinate independent; moreover, the space of $n$-forms at any point $p\in\M$ being 1-dimensional, $\w{L}$ and $\weps$ are
necessarily proportional, so that there is no loss of generality in the writing $\w{L} = L \weps$.},
$\weps$ is the Levi-Civita tensor (cf. Sec.~\ref{s:bas:Levi-Civita_tensor}), the
second equality stems from the identity $\w{L} = L \, \weps$, where $L$ is the
\emph{Lagrangian scalar}, and the third identity provides an expression of the integral
...
involving only the metric tensor and its derivatives (with respect to an arbitrary connection on $\M$) can
...
(Einstein-Hilbert action\index{Einstein-Hilbert action}). By \emph{diffeomorphism-invariant}, it is meant
that the Lagrangian $n$-form $\w{L}$ is covariant under
the action of a diffeomorphism $\Phi:\M \to \M$. More precisely, under such an action, the metric tensor
and the Levi-Civita tensors are transformed to their pullbacks $(\Phi^{-1})^*\w{g}$ and $(\Phi^{-1})^*\weps$ by
$\Phi^{-1}$
(cf. Secs.~\ref{s:neh:symmetries} and \ref{s:bas:Lie}).
Let $\w{L'}$ be the Lagrangian $n$-form constructed by keeping the same functional
dependence in $\Phi^*\w{g}$ as (\ref{e:evo:Lagrangian_diffeo_inv}), i.e.
$\w{L'} = L((\Phi^{-1})^* g_{\alpha\beta}, (\Phi^{-1})^* R_{\alpha\beta\gamma\delta}, \ldots) \, \Phi^*\weps$
with the same function $L$ as in (\ref{e:evo:Lagrangian_diffeo_inv}). The \emph{diffeomorphism-invariant}
condition amounts to demand that $\w{L'}$ coincides with the pullback of the $n$-form $\w{L}$ by $\Phi^{-1}$:
