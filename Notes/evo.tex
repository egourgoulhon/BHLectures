...
\begin{remark}
The above formulation of Hamiltonian mechanics is quite covariant: it does not
refer to some specific canonical coordinates $(q^i, p_i)$. It even does not rely
on a split between the ``configuration space'',  spanned by some coordinates $(q^i)$
and the ``momentum space''
momenta coordinates $(p_i)$.
\end{remark}

Except at equilibrium points, there is one, and exactly one,
integral curve of $X_H$ through each point of $\mathcal{F}$.
It follows that a solution is entirely determined by a point $a\in\mathcal{F}$,
