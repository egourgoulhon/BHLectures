...
to the case $n=4$, $K=1$, $L_1 = 1$ and $\w{g}$ fulfilling the Einstein equation
(so that Einstein tensor $\w{G}$ can be replaced by $8\pi\w{T}$). We obtain
\be
...
\ee
Using the perfect fluid expression (\ref{e:evo:perfect_fluid_T}) for $\w{T}$,
we rewrite the integrand of the last integral as
\[
     T^\mu_{\ \, \nu} \, \xi^\nu \D V_\mu =
\]
\end{proof}
