...
and the Levi-Civita tensors are transformed to their pullbacks $\Phi^*\w{g}$ and $\Phi^*\weps$ by
$\Phi^{-1}$
...
$\w{L'} = L(\Phi^* g_{\alpha\beta}, \Phi^* R_{\alpha\beta\gamma\delta}, \ldots) \, \Phi^*\weps$
with the same function $L$ as in (\ref{e:evo:Lagrangian_diffeo_inv}). The \emph{diffeomorphism-invariant}
condition amounts to demand that $\w{L'}$ coincides with the pullback of the $n$-form $\w{L}$ by $\Phi$:
$\w{L'} = \Phi^*\w{L}$, i.e. $\w{L'}$ and $\w{L}$ are basically the same objects.
