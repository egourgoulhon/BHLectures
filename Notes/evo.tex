...
that appears in Eq.~(\ref{e:evo:identif_S_T}): comparing Eq.~(\ref{e:evo:identif_T_kappa})
with formula~(\ref{e:evo:Hawking_temp}) for the Hawking temperature, we get $\alpha = k_{\rm B}/(4\hbar)$. Accordingly, Eq.~(\ref{e:evo:identif_S_A}) becomes
...
so that formula~(\ref{e:evo:S_BH}) can be rewritten as
\be
...
so that the Bekenstein-Hawking entropy (\ref{e:evo:S_BH}) can be written as
\be
    \encadre{S_{\rm BH} = 2\pi k_{\rm B} \frac{G M^2}{\hbar c}
        \left( 1 + \sqrt{1 - \bar{a}^2}  \right) } _{\rm\; Kerr} .
\ee
Note that, as in formula~(\ref{e:evo:T_H_Kerr}) for the Hawking temperature,
we have restored $G$ and $c$ in the above formula.
