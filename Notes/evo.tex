...
$\M\to \M$, we have of course $\Phi(\M)=\M$; however we keep the notation $\Phi(\M)$ to stress the image character of this set.} $\Phi(\M)$ to $\M$, while
...
\be \label{e:evo:eom_E}
    \w{E} = 0 .
\ee
This is the so-called \emph{Euler-Lagrange equations}\index{Euler-Lagrange equations}, for the considered gravity theory, also called the \emph{field equations} or \emph{equations of motion}.
...
$\w{L'}$ is the $n$-form introduced above. The diffeomorphism invariance of the theory implies
that $\w{L}' = (\Phi_t^{-1})^* \w{L}$ and hence that $\delta\w{L} = (\Phi_t^{-1})^* \w{L} - \w{L}$. This
is the same form as the variation of $\w{g}$ expressed in Eq.~(\ref{e:evo:delta_g_Phistar}) and, again, the
...
\begin{remark}
The variation $\delta\w{g}$ is often defined as a \emph{variation rate} instead of a genuine variation, namely
...
\ee
When comparing with Eq.~(\ref{e:bas:delta_g_Lie_xi_g}), we see that the derivative-based definition has the advantage of getting rid of the infinitesimal factor $t$, which is not particularly significant.
\end{remark}
