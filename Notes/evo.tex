...
In the two preceding chapters, we have discussed the formation and evolution
of black holes through specific scenarios of spherically symmetric gravitational
collapses. Here, we derive laws governing the evolution of generic (not spherically
symmetric) black holes, possibly surrounded by electromagnetic or matter fields.
We start by a formula that rules the change in mass between two nearby stationary
black hole configurations (Sec.~\ref{s:evo:towards_1st_law}), before
establishing a law, known as \emph{Hawking's area theorem}, stipulating that the area of
the event horizon
cross-sections can only increase during the black hole evolution,
be the latter quasi-stationary or highly dynamical (Sec.~\ref{s:evo:evol_area}).
These two results,
along with the zeroth law of black hole dynamics established in  Chap.~\ref{s:neh},
exhibit a striking resemblance with the laws of thermodynamics, which is
discussed in Sec.~\ref{s:evo:BHdynamics}. In particular, the surface gravity $\kappa$
and the area $A$ of a stationary black hole seem analogous to respectively the temperature and
the entropy of a thermodynamical system.
The phenomenon of Hawking radiation, discussed in Sec.~\ref{s:evo:Hawing_rad_main},
turns this mathematical similarity into a physical identity, by
making the radiation temperature proportional to $\kappa$. This opens the
realm of black hole thermodynamics (Sec.~\ref{s:evo:BH_thermo}).
...
In this subsection, we assume 4-dimensional general relativity and we
consider an initially isolated Kerr black hole of mass and spin parameters $(m,a)$
...
a much more general framework, without assuming that it regards changes between
...
It even holds for gravity theories other than general relativity, more specifically
...
\begin{tprop}{mass variation formula for generic black holes}{evo:first_law_gen}
...
\begin{tprop}{mass variation formula for black holes in vacuum general relativity}{evo:first_vac_GR}
...
\begin{tprop}{mass variation formula for charged black holes in general relativity}{evo:mass_var_electrovac}
...
\begin{tprop}{simple perfect fluid in axisymmetric spacetimes}{evo:simple_fluid}
...
The \emph{fluid angular momentum} $J_{\rm f}$ in $\Sigma$ is the integral of $\w{j} \cdot \D\w{V}$;
...
\begin{tprop}{mass variation formula for a black hole surrounded by a fluid}{}
...
that changed by the amount $\delta J$ the angular momentum of a body rotating
at the angular velocity $\Omega_{\Hor}$ and the term $\Phi_{\Hor} \delta Q_{\Hor}$
as the work performed to change by the amount $\delta Q_{\Hor}$ the electric charge
...
\subsection{Irreversibily in the evolution of an accreting black hole} \label{s:evo:irreversible_mass}
...
\begin{tprop}{area increase for accreting Kerr black holes}{evo:area_law_Kerr}
...
%\begin{tprop}{extremal state of a Kerr black hole unreachable by accretion}{}
...
\ee
We conclude:
\begin{tprop}{maximal energy extraction by a Penrose process}{}
...
by Hawking in 1971 \cite{Hawki71}.

The first step towards the area theorem is:

\begin{tprop}{non-negative expansion of a black hole horizon\index{expansion!positive -- theorem}}{evo:positive_expansion}
...
\begin{tprop}{area theorem\index{area!theorem} \textnormal{(Hawking\index[pers]{Hawking, S.W.} 1971,
Chru\'sciel\index[pers]{Chrusciel, P.T.@Chru\'sciel, P.T.},
Delay\index[pers]{Delay, E.},
Galloway\index[pers]{Galloway, G.J.} and Howard\index[pers]{Howard, R.} 2001)}}{evo:area_thm}
...
it is necessarily a null hypersurface there (Property~\ref{p:glo:prop4}),
so that the concept of cross-section (Definition~\ref{d:nul:cross_section})
...
We refer to the original article by Chru\'sciel et al. \cite{ChrusDGH01} for the proof in the case where
$\Hor$ is not assumed smooth between $\Sp_1$ and $\Sp_2^*$ (70 pages!).
\end{proof}

\begin{texample}{Oppenheimer-Snyder collapse}{}
...
\begin{texample}{Vaidya collapse}{}
...
\section{The laws of black hole dynamics} \label{s:evo:BHdynamics}
...
\begin{tprop}{zeroth law of black hole dynamics}{evo:zeroth_law}
...
\begin{tprop}{laws of black hole dynamics}{evo:laws_BH_dyn}
\begin{description}
...
the third law as a conjecture, stating that \emph{``there are strong reasons for believing in it''}
(cf. Sec.~\ref{s:evo:third_law}).
\end{hist}

The three laws in Property~\ref{p:evo:laws_BH_dyn} are listed as laws of black hole \emph{dynamics} and not \emph{thermodynamics}, because they still appear as a mathematical analogy without any definite physical content. In particular, if we stick to a
...
\section{Hawking radiation} \label{s:evo:Hawing_rad_main}


\subsection{The phenomenon of Hawking radiation} \label{s:evo:Hawking_rad}

\begin{tprop}{Hawking radiation}{}
...
e.g. \cite{Wald84,Wald94,FroloN98,Carro04,FabbrN05}, as well in review
articles, e.g. \cite{BroutMPS95,Damou04,Jacob96,Jacob05}. Rather, we shall limit
...
(see e.g. \cite{Wald94}). It has not been derived from any quantum theory of gravity.
In other words, the background spacetime $(\M,\w{g})$ is not quantized. One says that Hawking radiation is
...
(energy radiated per unit time, also called \emph{luminosity}) of Hawking radiation received at infinity
...
value of $\mathcal{A}$ suitable for an estimate is
...
\begin{tprop}{Hawking evaporation of a Schwarzschild black hole}{}
...
a non-rotating black hole. The latter has no outer ergoregion and thus cannot trigger
...
\section{Black hole thermodynamics} \label{s:evo:BH_thermo}
...
\begin{tprop}{Bekenstein-Hawking entropy}{}
...
the first law of black hole dynamics (\ref{e:evo:first_law}) then writes
...
\begin{remark}
The subscript ``BH'' in $S_{\rm BH}$ stands for \emph{Bekenstein-Hawking},
...
Universe is about $1.1\; 10^{90} \, k_{\rm B}$. The latter mostly originates from the
...
into the black hole. One then has to replace the standard second law by the
GSL \cite{Beken73b,Beken74}:

\begin{tprop}{generalized second law of thermodynamics (GSL)
 \textnormal{(Bekenstein\index[pers]{Bekenstein, J.D.} 1973)}}{evo:GSL}
...
puts $S_{\rm BH}$ on the same footing as $S_{\rm mat}$.
...
\subsection{Black hole entropy from quantum gravity} \label{s:evo:quantum_entropy}

As argued above, the GSL promotes the Bekenstein-Hawking entropy
$S_{\rm BH} = k_{\rm B} {A}/(4\hbar)$ to a genuine thermodynamic entropy.
It would be then fully satisfactory to
establish some ``statistical mechanics'' origin for $S_{\rm BH}$, i.e. to interpret $S_{\rm BH}$
...
\ee
Defining the microscopic states a priori requires a quantum theory of gravity\index{quantum gravity}, which we do not have yet. However some partial results have been
...
(cf. Sec.~\ref{s:exk:NHEK}). For the latter, the Bekenstein-Hawking entropy
...
The fact that the entropy of black holes is proportional to their area, as exhibited in the
Bekenstein-Hawking formula (\ref{e:evo:S_BH}), is a striking peculiarity
of black holes: ordinary physical systems have an entropy that scales with
their \emph{volume}, not with their \emph{area}. This remarkable feature turned out to have profound
consequences, being at the root of the \defin{holographic principle}\index{holographic principle}, which is a major concept in contemporary theoretical physics.
This principle, initially formulated by Gerard t'Hooft (1993) \cite{tHoof94}
and Leonard Susskind (1995) \cite{Sussk95}, states that all physical processes in a given region
can be entirely described in terms of fundamental degrees of freedom at the boundary
of this region (see e.g. Ref.~\cite{Bouss02} for a review).
A concrete realization of the holographic principle is the famous \defin{AdS/CFT correspondence}\index{AdS/CFT correspondence} established in 1997 by Juan Maldacena \cite{Malda98},
which asserts that a conformal field theory (CFT) in a $n$-dimensional spacetime $\M$
is equivalent to a certain string theory in a $(n+1)$-dimensional anti-de Sitter\index{anti-de Sitter spacetime} (AdS)\index{AdS} spacetime $\mathscr{B}$, the conformal boundary of which is\footnote{From Example~\ref{x:glo:AdS},
recall that the boundary $\scri$ of the conformal completion of AdS spacetime is a \emph{timelike} hypersurface, i.e. a Lorentzian submanifold, and hence can be considered as a spacetime by itself.} $\M$.
This correspondence is also called a \defin{gauge/gravity duality}\index{gauge/gravity duality}, given
that it makes a non-gravitational gauge theory (CFT) equivalent to a gravity theory (strings in AdS),
and is pivotal in considerations about quantum gravity\index{quantum gravity}
(see e.g. Refs.~\cite{Natsu15,CasalLMRW14} for a review).

By itself, the Bekenstein-Hawking formula $S_{\rm BH} = k_{\rm B}{A}/(4\hbar)$
is ``holographic'', since it
suggests that the (quantum) microscopic degrees of
freedom of a black hole (cf. Sec.~\ref{s:evo:quantum_entropy}) somehow reside on the black hole's boundary (the event horizon, of area $A$) and not in
its interior. But there is much more: if one assumes the generalized second law of thermodynamics
(GSL), one can reach a conclusion valid
for any physical system, not necessarily a black hole \cite{Sussk95}:

\begin{tprop}{spherical entropy bound
\textnormal{(Susskind\index[pers]{Susskind, L.} 1995)}}{evo:spher_ent_bound}
Assuming that the GSL (Propery~\ref{p:evo:GSL}) holds, the entropy $S$ of any
approximately spherically symmetric physical system enclosed in a sphere of area $A$
must obey:
\be \label{e:evo:spher_ent_bound}
    S \leq k_{\rm B}\frac{A}{4\hbar} .
\ee
\end{tprop}

\begin{proof}
If the system is a black hole, then $S = S_{\rm BH}$ and (\ref{e:evo:spher_ent_bound}) is fulfilled
by virtue of Eq.~(\ref{e:evo:S_BH}) (the inequality is saturated).
In the opposite case, the system has necessarily a gravitational
mass $M$ that obeys $2M < R$, where $R := \sqrt{A/(4\pi)}$. Let us then throw to the system some
matter arranged in a spherical shell, of gravitational mass $R/2 - M$. The initial system and the shell
will necessarily collapse to a spherical black hole of mass $R/2$ and hence of area $A$.
In this process, the initial total entropy is $S + S_{\rm shell}$ and the final one
is $S_{\rm BH} = k_{\rm B} A / (4\hbar)$ [Eq.~(\ref{e:evo:S_BH})]. The GSL [Eq.~(\ref{e:evo:GSL})]
then implies $k_{\rm B} A / (4\hbar) \geq S + S_{\rm shell}$. Given that $S_{\rm shell} \geq 0$,
the inequality (\ref{e:evo:spher_ent_bound}) follows.
\end{proof}

\begin{remark}
As its formulation may suggest (``approximately spherically symmetric'', ``enclosed in a sphere''),
Property~\ref{p:evo:spher_ent_bound} is not a rigorous theorem; see Wald's article~\cite{Wald01}
for a critical assessment and counter-examples.
\end{remark}

\begin{remark}
For weakly gravitating systems, Property~\ref{p:evo:spher_ent_bound} can be shown to follow from
a more general entropy bound, also derived from the GSL and called the \defin{Bekenstein bound}\index{Bekenstein}\index[pers]{Bekenstein, J.D.}  \cite{Beken81}. The latter states that
$S \leq 2\pi k_{\rm B} R E/\hbar$, where $E$ is the total energy of the system and
$R$ the radius of a sphere enclosing it.
\end{remark}

Because it regards any physical system, not only black holes, the
spherical entropy bound (\ref{p:evo:spher_ent_bound})
was the starting point of t'Hooft \cite{tHoof94} and Susskind \cite{Sussk95}
reasoning leading to the formulation of the holographic principle.
