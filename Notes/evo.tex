...
A standard computation (see e.g \cite{Compe19,LeeW90}) enables one to write the
...
\ee
Notice that $\w{\Theta}[\w{g},\delta\w{g}]$ is linear in $\delta\w{g}$, as stated above.
\end{example}

The
extremalization $\delta S = 0$ of the action (\ref{e:evo:action_L}) is equivalent to
$\int_\M \delta\w{L} = 0$.
Assuming that $\delta\w{g}$ has compact support,
the part $\dd\w{\Theta}$ of $\delta\w{L}$ in
expression~(\ref{e:evo:delta_L_E_Theta}) does not contribute:
the integral of $\dd\w{\Theta}$  over $\M$
can be reduced to a surface integral of $\w{\Theta}$ thanks to Stokes' theorem (\ref{e:bas:Stokes}),
which vanishes outside the support of $\delta\w{g}$, since $\w{\Theta}$ is linear in $\delta\w{g}$.
...
\be \label{e:evo:def_Noether_current}
    \w{J} := \w{\Theta}[\w{g},\Lie{\xi}\w{g}] - \w{\xi}\cdot\w{L} .
\ee
...
\ee
\begin{proof}
We have $\dd\w{J} = \dd( \w{\Theta}[\w{g},\Lie{\xi}\w{g}]) - \dd(\w{\xi}\cdot\w{L})$.
...
Hence
\[
    \dd\w{J} = \dd( \w{\Theta}[\w{g},\Lie{\xi}\w{g}])  - \Lie{\xi} \w{L}  .
\]
