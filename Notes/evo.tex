...
We shall suppose that either $\mathscr{P}$ falls directly into the black hole or
...
necessarily increases. This holds even if the mass $M$ decreases, which occurs
for a Penrose process ($E'>E$), since then $E_*<0$ and thus $\delta M < 0$
by virtue of
Eq.~(\ref{e:evo:dM_E_dJ_L}).
Particle accretion constitutes thus an \emph{irreversible transformation}
...
which must hold if $\mathscr{P}_*$ is to cross the event horizon (i.e. some accretion
takes place): in view of Eq.~(\ref{e:evo:dM_E_dJ_L}),
one might think naively that the transformation $(M,J)\to (M+\delta M, J+\delta J)$
could be reverted by throwing into the black hole a particle of conserved energy $E'_* = - E_*$
...
A \emph{reversible transformation}\index{reversible transformation}
corresponds to the limit case $E_* = \Omega_{\Hor}  L_*$,
and yields $\delta A = 0$ by virtue of Eq.~(\ref{e:evo:dA_Estar_Jstar}).
The particle $\mathscr{P}_*$ reaches then the event horizon $\Hor$ with
a vanishing radial affine velocity $\D r / \D\lambda$
($\lambda$ is the affine parameter along $\mathscr{P}_*$'s worldline associated
...
We conclude that a reversible transformation corresponds to the idealized case where
the particle $\mathscr{P}_*$ is actually not accreted by the black hole.

The square root of the horizon area $A$ (rescaled by $16\pi$) is called
the black hole's \defin{irreducible mass}\index{irreducible mass}:
\be
    M_{\rm irr}^2 := \frac{A}{16\pi} .
\ee
$M_{\rm irr}$ has the dimension of a mass and
Eq.~(\ref{e:evo:dA_positive_Kerr}) is equivalent to $\delta M_{\rm irr} > 0$,
i.e. $M_{\rm irr}$ cannot decrease in any accretion process (contrary to $M$
for instance), hence its name.
Formula~(\ref{e:ker:A_a_m}) for $A$ can be turned into an expresson
of $M_{\rm irr}$ in terms of $M$ and $J=aM$:
\be
