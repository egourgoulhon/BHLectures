...
from the black hole to infinity, called the
...
\ee
where $\hbar \simeq 1.05457\; 10^{-34} {\rm\; J\, s}$ is the reduced Planck constant and
$k_{\rm B} \simeq 1.38065\; 10^{-23} {\rm\;  J\, K}^{-1}$ is the Botlzmann constant. $T_{\rm H}$ is called the
\defin{Hawking temperature}\index{Hawking!temperature}.
\end{prop}

We shall not establish this property here; this is done in many textbooks,
e.g. \cite{Wald84,Wald94,FroloN98,Carro04}. Rather, we shall limit
ourselves to a few remarks:

\begin{remark}\label{r:evo:Hawking_temp_units}
Formula~(\ref{e:evo:Hawking_temp}) is given within the
units $c=1$, which we are using throughout. If one restores $c$ and assumes
...
Moreover, the Hawking temperature, as given by formula~(\ref{e:evo:Hawking_temp}), is independent from the spacetime dimension
(see e.g. Ref.~\cite{KantiW15} for a detailed discussion).
\end{remark}
...
the stationary Killing vector $\w{\xi}$ is assumed to be null on the event horizon and
timelike in the black hole exterior. Let us then consider static observers at a finite distance
from the black hole, i.e. observers
...
be $T = T_{\rm H} / V$. In particular, $T\to +\infty$ when the static observers
are located infinitely close to the event horizon ($V\to 0$). Note that those observers are
...
all species of massless and massive particles. For the latter ones,
the contribution is significant only if
$k_{\rm B} T > m$, where $m$ is the particle mass.
\end{remark}
...
with the spacetime curvature (backscatterng).
\end{remark}
...
of the black hole mass $M=m$ and reduced angular momentum parameter
$\bar{a} := a/m = J/M^2$ ($0\leq \bar{a}\leq 1$):
\be
    T_{\rm H} = \frac{\hbar}{k_{\rm B}} \frac{c^3}{8\pi G M}
    \frac{2 \sqrt{1 - \bar{a}^2}}{1 + \sqrt{1 - \bar{a}^2}} .
\ee
...
\be \label{e:evo:T_H_Kerr_num}
    T_{\rm H} = 6.17\; 10^{-8} \left( \frac{M_\odot}{M} \right) \frac{2 \sqrt{1 - \bar{a}^2}}{1 + \sqrt{1 - \bar{a}^2}} {\rm\; K} .
\ee
Hence, for a Schwarzschild black hole ($\bar{a}=0$),
$T_{\rm H} = 6.17\; 10^{-8} ({M_\odot}/{M}) {\rm\; K}$. For astrophysical black holes,
this leads to tiny values: $T_{\rm H} \simeq 4\; 10^{-9} {\rm\; K}$
...
Another conclusion that one can draw from formula (\ref{e:evo:T_H_Kerr_num}) is
that $T_{\rm H}=0$ for an extremal Kerr black hole ($\bar{a}=1$): such a black
hole does not emit any Hawking radiation.
