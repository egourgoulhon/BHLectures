...
[cf. Eq.~(\ref{e:bas:v_f_partial_f})]. Accordingly, we may rewrite the evolution
equation (\ref{e:evo:dfdt_Poisson_fH}) as
\be \label{e:evo:dfdt_X_H_f}
    \derd{f}{t} = X_H(f).
\ee
In particular, Hamilton's equations (\ref{e:evo_Hamilton_eqs_finite}) can be rewritten as
...
manifold, called the \defin{phase space} of the system, $\Omega$ is
a \defin{symplectic form} on $\mathcal{F}$, i.e. a closed non-degenerate
...
the \defin{Hamiltonian vector field}, namely the unique vector field $X_H$
...
by the Hamiltonian vector field: any solution of the equation of motion
is an integral curve of $X_H$.
A smooth function $f: \ \mathcal{F} \to \R$ is called a
\defin{conserved quantity} iff $f$ is constant on every integral curve
...
which can be thought of as representing the state of the system at the initial
