...
By \emph{diffeomorphism-invariant}, it is meant
...
The inverse of $\Phi$ is invoked here because, as $\w{g}$ and $\weps$ are
tensors of type $(0,\ell)$, $\Phi$ itself would carry them from\footnote{Since $\Phi$ is a diffeomorphism
...
dependence in $(\Phi^{-1})^*\w{g}$ as in (\ref{e:evo:Lagrangian_diffeo_inv}), i.e.
$\w{L'} = L((\Phi^{-1})^* g_{\alpha\beta}, (\Phi^{-1})^* R_{\alpha\beta\gamma\delta}, \ldots) \,
(\Phi^{-1})^*\weps$
with the same function $L$ as in (\ref{e:evo:Lagrangian_diffeo_inv}). The
\defin{diffeomorphism-invariance}\index{diffeomorphism!invariance}
condition amounts to demanding that, as a $n$-form on $\M$, $\w{L'}$ is nothing but
the $n$-form $\w{L}$ transformed under the action of $\Phi$, i.e. the pullback of $\w{L}$ by
$\Phi^{-1}$:  $\w{L'} = (\Phi^{-1})^*\w{L}$.
Given that the spacetimes
$(\M, \w{g})$ and $(\Phi(\M),(\Phi^{-1})^* \w{g})$ are physically equivalent,
this means that $\w{L}$ and $\w{L'}$ are basically the same object.
In particular, the action (\ref{e:evo:action_L}) takes the same value if
$\w{L'}$ is substituted for $\w{L}$. If one interprets the diffeomorphism
...
The variation of the Lagrangian $n$-form resulting from a generic variation $\delta\w{g}$ of
the metric tensor can always be written as \cite{LeeW90,IyerW94,Compe19}
\be \label{e:evo:delta_L_E_Theta}
    \delta \w{L} = E^{\mu\nu} \delta g_{\mu\nu} \, \weps + \dd \w{\Theta} ,
\ee
where $E^{\mu\nu}$ are the components of a symmetric type-$(2,0)$ tensor field $\w{E}$, which does
not depend on $\delta\w{g}$ and
$\w{\Theta}$ is a $(n-1)$-form, called the
\defin{symplectic potential form}\index{symplectic!potential form} \cite{IyerW94}, which depends on $\w{g}$ and $\delta\w{g}$, in a linear way for $\delta\w{g}$. We shall write $\w{\Theta} = \w{\bar\Theta}(\w{g},\delta\w{g})$
to stress this dependence.
The $n$-form  $\dd \w{\Theta}$ is the exterior derivative of $\w{\Theta}$
(cf. Sec.~\ref{s:bas:ext_deriv}). Note that $\w{\Theta}$ is not uniquely determined by
(\ref{e:evo:delta_L_E_Theta}): any change $\w{\Theta} \to \w{\Theta} + \dd \w{Y}$, where $\w{Y}$ is an
arbitrary $(n-2)$-form would result in the same formula (\ref{e:evo:delta_L_E_Theta}), since $\dd$ is nilpotent
...
Substituting Eq.~(\ref{e:evo:def_H}) for $H^\mu$, we get the explicit expression of $\w{\Theta}$
in terms of $\w{g}$ and $\delta\w{g}$:
...
\ee
Notice that $\w{\Theta}$ is linear in $\delta\w{g}$, as stated above.
\end{example}

The extremalization $\delta S = 0$ of the action (\ref{e:evo:action_L}) is equivalent to
$\int_\M \delta\w{L} = 0$. The part $\dd\w{\Theta}$ of $\delta\w{L}$ in
expression~(\ref{e:evo:delta_L_E_Theta}) does not contribute:
the integral of $\dd\w{\Theta}$  over $\M$
can be reduced to a surface integral thanks to the Stokes theorem (\ref{e:bas:Stokes}) and provided
$\delta\w{g}$ is
compactly supported or decay sufficiently fast, this surface integral vanishes.
Consequently, having $\delta S = 0$ for any variation $\delta\w{g}$ is equivalent to
\be \label{e:evo:eom_E}
    \w{E} = 0 .
\ee
This constitutes the so-called \emph{Euler-Lagrange equations}\index{Euler-Lagrange equations} for the considered gravity theory, also called the \emph{field equations} or \emph{equations of motion}.

\begin{example} \label{x:evo:field_equations_GR}
For general relativity, in view of Eq.~(\ref{e:evo:E_Theta_GR}),
the field equations (\ref{e:evo:eom_E}) become $\w{G} = 0$, which
...
Let us consider a variation $\delta\w{g}$ induced by a diffeomorphism $\Phi_t: \M \to \M$ that
belongs to a 1-dimensional Lie group of diffeomorphisms, parametrized by $t\in\R$
(cf. Sec.~\ref{s:neh:symmetries}).
According to the above discussion regarding the action
of a diffeomorphism on tensors of type $(0,\ell)$, the variation of $\w{g}$ induced by $\Phi_t$ is
\be \label{e:evo:delta_g_Phistar}
    \delta\w{g} = (\Phi_t^{-1})^* \w{g} - \w{g} = \Phi_{-t}^* \, \w{g} - \w{g} ,
\ee
where the second equality stems from $\Phi_t^{-1} = \Phi_{-t}$.
Let us consider an infinitesimal diffeomorphism, i.e. $|t| \ll 1$. From the very definition of the
Lie derivative, as given by Eq.~(\ref{e:bas:def_Lie_der_covar}), we have then
$\Phi_{-t}^* \, \w{g} - \w{g} \simeq -t \Lie{\xi} \w{g}$, where the vector field $\w{\xi}$ is the
generator of the diffeomorphism group associated to the $t$-parametrization [cf. Eq.~(\ref{e:neh:xi_dxdt})].
It follows that, in the limit $t\to 0$,
\be  \label{e:bas:delta_g_Lie_xi_g}
    \delta\w{g} = - t \Lie{\xi} \w{g} .
\ee
The corresponding variation of the Lagrangian $n$-form is $\delta\w{L} = \w{L'} - \w{L}$, where
$\w{L'}$ is the $n$-form obtained by substuting $(\Phi_t^{-1})^* \w{g}$ and $(\Phi_t^{-1})^* \weps$
for $\w{g}$ and $\weps$ in the expression of $\w{L}$, as described above.
Since the theory is assumed to be diffeomorphism-invariant, one has
$\w{L}' = (\Phi_t^{-1})^* \w{L}$, which yields $\delta\w{L} = (\Phi_t^{-1})^* \w{L} - \w{L}$. This
...
$\w{L}$ is a tensor of type $(0,n)$) leads to
...
as the \emph{derivative} of a 1-parameter family of metric tensors $\w{g}(\lambda)$ such that
$\w{g}(0) = \w{g}$. Using the symbol $\hat\delta$ to distinguish from the variation considered here,
the definition is
$\hat{\delta}\w{g} := \left. \D{\w{g}}/\D{\lambda} \right|_{\lambda=0}$.
Since the metric family induced by the diffemorphism group is
$\w{g}(t) = (\Phi^{-1}_t)^* \w{g} = \Phi_{-t}^*\, \w{g}$, one gets
$\hat{\delta}\w{g} = \lim_{t\to 0} t^{-1} \left( \Phi_{-t}^*\, \w{g} - \w{g} \right)$.
...
\ee
When comparing with Eq.~(\ref{e:bas:delta_g_Lie_xi_g}), we see that the derivative-based definition has the advantage of getting rid of the (not particularly significant) infinitesimal factor $t$.
Besides, since the variation formula (\ref{e:evo:delta_L_E_Theta}) is linear in $\delta\w{g}$, the minus
sign in Eq.~(\ref{e:evo:hat_delta_g_Lie}) is not significant as well and many authors get rid of it
by defining $\hat{\delta}\w{g} := \Lie{\xi} \w{g}$.
\end{remark}

One defines the \defin{Noether current $(n-1)$-form of $\w{\xi}$} by \cite{Wald93,IyerW94}
\be \label{e:evo:def_Noether_current}
    \w{J} := \w{\bar\Theta}(\w{g},\Lie{\xi}\w{g}) - \w{\xi}\cdot\w{L} .
\ee
This differential form is closed as soon as the equations of motion (\ref{e:evo:eom_E}) are satisfied:
\be
    \dd \w{J} = 0 .
\ee
\begin{proof}
We have $\dd\w{J} = \dd[ \w{\bar\Theta}(\w{g},\Lie{\xi}\w{g}) ] - \dd(\w{\xi}\cdot\w{L})$.
Now, the Cartan identity (\ref{e:bas:Cartan}) implies
$\dd(\w{\xi}\cdot\w{L}) = \Lie{\xi} \w{L} - \w{\xi} \cdot\dd \w{L} = \Lie{\xi} \w{L}$,
since $\dd \w{L} = 0$, as any $(n+1)$-form on a manifold of dimension $n$.
Hence
\[
    \dd\w{J} = \dd[ \w{\bar\Theta}(\w{g},\Lie{\xi}\w{g}) ]  - \Lie{\xi} \w{L}  .
\]
On the other hand, by combining Eqs.~(\ref{e:evo:delta_L_E_Theta}) and (\ref{e:bas:delta_L_Lie_xi_L}), we get
\[
    \Lie{\xi} \w{L}  = - \frac{1}{t} \left( E^{\mu\nu} \delta g_{\mu\nu} \, \weps
        + \dd [\w{\bar\Theta}(\w{g},\delta\w{g})] \right) .
\]
Substituting expression (\ref{e:bas:delta_g_Lie_xi_g}) for $\delta\w{g}$ and using the linearity
of $\w{\bar\Theta}$ with respect to its second argument to write
$\w{\bar\Theta}(\w{g},-t \Lie{\xi} \w{g}) = - t \w{\bar\Theta}(\w{g},\Lie{\xi} \w{g})$,
we get
$\Lie{\xi} \w{L}  = E^{\mu\nu} \Liec{\xi} g_{\mu\nu} \,\weps + \dd[ \w{\bar\Theta}(\w{g},\Lie{\xi}\w{g}) ]$.
It immediately follows that
\be
    \dd\w{J} = - E^{\mu\nu} \Liec{\xi} g_{\mu\nu} \,\weps .
\ee
Accordingly, $\dd\w{J} = 0$ if $\w{E} = 0$ holds.
\end{proof}
...
\bea
  \w{\bar\Theta}(\w{g},\Lie{\xi}\w{g})_{\alpha_1\cdots\alpha_{n-1}} & = &
  g^{\mu\nu} \left( \nabla_\sigma \nabla_\nu \xi^\sigma + \nabla_\sigma \nabla^\sigma \xi_\nu
...
the definition (\ref{e:evo:def_Noether_current}) of $\w{J}$ yields
\[
    J_{\alpha_1\cdots\alpha_{n-1}} = \left(2 G^\mu_{\ \, \nu} \xi^\nu +
        \nabla_\nu \nabla^\nu \xi^\mu - \nabla_\nu \nabla^\mu \xi^\nu \right) \epsilon_{\mu\alpha_1\cdots\alpha_{n-1}} .
\]
If the field equations are obeyed, then $\w{G}=0$ (Example~\ref{x:evo:field_equations_GR}) and the above
expression reduces to
\be
    J_{\alpha_1\cdots\alpha_{n-1}} = \left(
        \nabla_\nu \nabla^\nu \xi^\mu - \nabla_\nu \nabla^\mu \xi^\nu \right) \epsilon_{\mu\alpha_1\cdots\alpha_{n-1}} .
\ee
...
\be
    \w{J} = \w{\mathcal{J}} \cdot \weps = \star\uu{\mathcal{J}} ,
\ee
...
That $\w{J}$ is closed in that case follows readily from the identity (\ref{e:bas:dveps_divv}):
$\dd\w{J} = \dd( \w{\mathcal{J}} \cdot \weps ) = (\wnab\cdot\w{\mathcal{J}}) \weps$, along
with the conservation law (\ref{e:sta:divJ_zero}): $\wnab\cdot\w{\mathcal{J}} = 0$.
\end{example}
