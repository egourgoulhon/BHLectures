...
\defin{canonical 1-form}\index{canonical!1-form} on $\mathcal{F}$, because its expression
...
the vector field on $\mathcal{F}$ associated to the Hamiltonian differential $\Dd H$
...
$X_f$ associated to any scalar function $f$ on $\mathcal{F}$
...
Phase space $\mathcal{F}$ of a simple gravity pendulum, depicted in terms of the
...
that $\mathcal{F} = \SS^1 \times \mathbb{R}$.
...
$\{f,H\} = X_H(f)$ for any scalar field $f$ on $\mathcal{F}$.
...
For tensor fields on $\mathcal{F}$ of valence higher than a scalar field,
...
is a triplet $(\mathcal{F},\Omega,H)$ where $\mathcal{F}$ is a smooth
manifold, called the \defin{phase space}\index{phase space} of the system, $\Omega$ is
a \defin{symplectic form}\index{symplectic!form} on $\mathcal{F}$, i.e. a closed non-degenerate
2-form, and $H$ is a smooth function $\mathcal{F} \to \R$ called
the \defin{Hamiltonian}.
The differential of $H$ is a 1-form $\Dd H$ on
$\mathcal{F}$, whose symplectic dual defines
...
A smooth function $f: \ \mathcal{F} \to \R$ is called a
...
An \defin{equilibrium point} is a point of $\mathcal{F}$ where $X_H = 0$,
...
refer to any system of canonical coordinates $(q^i, p_i)$ on $\mathcal{F}$
and, more generally, to any kind of coordinates.
It does not even assume that $\mathcal{F}$ is the cotangent bundle $T^*\mathcal{C}$
...
integral curve is reduced to the point itself.} of $\mathcal{F}$.
It follows that a solution is entirely determined by a single point $a\in\mathcal{F}$,
...
correspondance between a point of $\mathcal{F}$ and a solution.
Consequently, one may view $\mathcal{F}$ as the \emph{space of solutions}
...
the space $\mathcal{F}$ of solutions $\w{g}$ to the field equations is
an \emph{infinite-dimensional manifold}, more precisely a
\emph{infinite-dimensional Banach manifold}, i.e. a topological space
that locally resembles a Banach space \cite{Lang95}, in the same
way that a $n$-dimensional manifold locally resembles the vector space $\mathbb{R}^n$.
