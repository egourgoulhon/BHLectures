...
\be \label{e:evo:S_Wald_explicit}
    S_{\rm W} = - 2\pi \int_{\hat{\Sp}} \frac{\delta L}{\delta R_{\rho\sigma\mu\nu}}
        \; {}^{\hat{\Sp}}\!\!\eps^\perp_{\mu\nu}\,  \eps_{\rho\sigma\alpha_1\dots\alpha_{n-2}} ,
\ee
where
\begin{itemize}
\item ${\delta L}/{\delta R_{\alpha\beta\gamma\delta}}$ is the Euler-Lagrange derivative
...
to $\hat{\Sp}$, i.e. the 2-form defined by
\be
    {}^{\hat{\Sp}}\!\!\weps^\perp := \uu{s} \wedge \uu{n} \quad \iff \quad
    {}^{\hat{\Sp}}\!\!\eps^\perp_{\alpha\beta} = s_\alpha n_\beta
        - n_\alpha s_\beta ,
\ee
where the vector fields $(\w{n},\w{s})$ form at each point $p\in\hat{\Sp}$
an orthonormal basis of the vector space $T_p^\perp  \hat{\Sp}$ normal to $\hat{\Sp}$,
with $\w{n}$ future-directed unit timelike and $\w{s}$ outward-directed\footnote{See item (iv)
on p.~\pageref{e:sta:def_weps_S} for details.} unit spacelike; the binormal 2-form
is independent of the choice of the orthonormal basis $(\w{n},\w{s})$
and is related to the area element normal bivector
to $\hat{\Sp}$, $\D\w{S}$ (cf. Property~\ref{p:sta:area_elem_bivector}), by
$\D S_{\alpha\beta} = {}^{\hat{\Sp}}\!\!\eps^\perp_{\alpha\beta} \,  \sqrt{q}\, \D^{n-2} x$;
\item $\eps_{\rho\sigma\alpha_1\dots\alpha_{n-2}}$ is the spacetime Levi-Civita tensor,
so that the integral over the $(n-2)$-dimensional manifold $\hat{\Sp}$ in Eq.~(\ref{e:evo:S_Wald_explicit})
is that of the $(n-2)$-form corresponding to the free indices $\alpha_1\dots\alpha_{n-2}$.
\end{itemize}
\end{prop}
