...
Given the identity $\sqrt{-g} = N\sqrt{\gamma}$ (see e.g. Eq.~(5.55) of Ref.~\cite{Gourg12}), we get
...
$\w{v} = 2\w{H}$ (compare Wald's Eq.~(E.1.16) with Eq.~(\ref{e:evo:def_H})).}, which is at the root of the derivation of the Einstein equation by extremalization of
...
\[
    g^{\mu\nu} \, \delta R_{\mu\nu}
     ...
     = 2 \nabla_\mu H^\mu .
\]
On the other side, using formula (\ref{e:bas:variation_det}) for the variation
of the determinant $g$ of the matrix $(g_{\mu\nu})$, we have
\[
...
Let $(\M,\w{g})$ be a $n$-dimensional stationary spacetime such that $\w{g}$ obeys
...
\be \label{e:evo:mass_variation_vacuum}
   \encadre{ \delta  M = \sum_{k = 1}^K
    ...
    +  \sum_{i=1}^{L_{k}} \Omega^{(i)}_{\Hor_k} \, \delta J_{\Hor_{k}(i)} \right) } .
\ee
In the particular case $n=4$ (which implies $0 \leq L_k \leq 1$) and $K=1$
(connected event horizon), this formula reduces to
