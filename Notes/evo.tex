...
$L = (16\pi)^{-1} R = (16\pi)^{-1} g^{\mu\nu} R_{\mu\nu} =  (16\pi)^{-1} g^{\mu\nu} R^\sigma_{\ \,\mu\sigma\nu}$, hence $L = (16\pi)^{-1} g^{\mu\nu} g^{\rho\sigma} R_{\rho\mu\sigma\nu}$, which is of the form (\ref{e:evo:Lagrangian_diffeo_inv}).
\end{example}
...
$\dd$ is nilpotent
[Eq.~(\ref{e:ext_der_nilpot})], $\w{\theta}$ in formula~(\ref{e:evo:delta_L_E_Theta})
...
$\w{\theta'} = \w{\theta} + \dd \w{Y} + \delta\w{\mu}$, instead of (\ref{e:evo:Theta_not_unique}).
...
\ee
We check that $\w{\theta}[\w{g},\delta\w{g}]$ is linear in $\delta\w{g}$, as stated in Property~\ref{p:evo:var_Lagrangian}.
\end{example}
...
Since $\w{\theta}$ is linear with respect to its second argument (cf. Property~\ref{p:evo:var_Lagrangian}),
one has
...
expression (\ref{e:evo:Theta_GR}) for $\w{\theta}$ with
...
for general relativity (cf. Property~\ref{p:evo:Wald_S_for_GR})
...
\be \label{e:evo:dfdt_Poisson_fH}
     \derd{f}{t} = \{f, H \} .
\ee
...
\be \label{e:evo:Omega_dTheta}
    \Omega = \Dd \Theta,
\ee
where $\Theta$ is the 1-form defined by
\be \label{e:evo:Theta_pdq}
    \Theta := \sum_{i=1}^N  p_i\,  \Dd q^i .
\ee
Because of (\ref{e:evo:Omega_dTheta}), the 1-form $\Theta$ is called the
...
(\ref{e:evo:Theta_pdq}) is independent of the choice of the
...
express in terms of the Poisson bracket by combining Eqs.~(\ref{e:evo:def_X_f}) and (\ref{e:evo:Poisson_bracket}):
\be \label{e:evo:Xf_Poisson_f}
    \encadre{ X_f = \{., f\} },
\ee
...
By combining Eqs.~(\ref{e:evo:Df_Omega_X_f}) and (\ref{e:evo:Poisson_bracket_X})
there comes $\langle \Dd f, X_g \rangle = \Omega(X_g, X_f) = \{f,g\}$.
Hence, we get various equivalent expressions for the
Poisson bracket:
\be \label{e:evo:crochet_Poisson_equiv}
...
It obeys Eq.~(\ref{e:evo:Df_Omega_X_f}):
...
%In Example~\ref{x:evo:pendulum}, the space in which the degree of
...
Equation~(\ref{e:evo:Xf_Poisson_f}) with $f=H$ leads to
$\{f,H\} = X_H(f)$ for any scalar field $f$ on $\FF$.
Accordingly, the evolution
equation (\ref{e:evo:dfdt_Poisson_fH}) can be rewritten as
...
In view of Eq.~(\ref{e:evo:dfdt_X_H_f}), one may consider the vector field $X_H$ as
...
In particular, this encompasses Eq.~(\ref{e:evo:dfdt_X_H_f}), since
...
identity (\ref{e:bas:Cartan}) and Eq.~(\ref{e:evo:DH_Omega_X_H}), we get
\[
...
on $\FF$ such that
\be \label{e:evo:DH_Omega_X_H_2}
\Dd H = \Omega(., X_H) .
\ee
By construction, $X_H$ preserves the symplectic
form: $\Liesymbol_{X_H} \Omega = 0$ [Eq.~(\ref{e:evo:Lie_X_H_Omega})].
...
of $X_H$, i.e. iff $X_H(f) = 0$. In view of Eq.~(\ref{e:evo:crochet_Poisson_equiv}),
...
or equivalently, given Eq.~(\ref{e:evo:DH_Omega_X_H_2}), a point where
$\Dd H = 0$.
\end{prop}

\begin{example}[solutions for the pendulum motion]
The integral curves of $X_H$ are depicted in Fig.~\ref{f:evo:pendulum_phase}
for the gravity pendulum (Example~\ref{x:evo:pendulum}). Two of them are
...
(cf. Fig.~\ref{f:evo:pendulum_phase}).
...
theory governed by a Lagrangian of the type (\ref{e:evo:Lagrangian_diffeo_inv}).
...
(cf. Property~\ref{p:evo:var_Lagrangian}), we may define a 1-form $\Theta$ on $\FF$ as follows.
...
\be \label{e:evo:def_Theta_X_theta}
...
so that
Eq.~(\ref{e:evo:X_dg/dlambda}) holds, i.e. $\delta\w{g} = \delta\lambda \, X$.
The integral in the right-hand side of Eq.~(\ref{e:evo:def_Theta_X_theta}) is a priori well-defined
...
(cf. Property~\ref{p:evo:var_Lagrangian}), we may rewrite (\ref{e:evo:def_Theta_X_theta}) as
\be \label{e:evo:Theta_X_theta_partial}
...
\be
    \Omega := \Dd \Theta .
\ee
This is analogous to Eq.~(\ref{e:evo:Omega_dTheta})
...
on $\FF$. According to Eq.~(\ref{e:evo:X_dg/dlambda}),
...
makes use of Eqs.~(\ref{e:evo:Theta_X_theta_partial}) and (\ref{e:evo:def_X1_X2})
...
\ee
with
\be \label{e:evo:def_omega_der_theta}
...
Eq.~(\ref{e:evo:def_omega_der_theta}), cancel each other thanks to the
identity
...
given by Eq.~(\ref{e:evo:delta_L_E_Theta}) with $E^{\mu\nu} = 0$ since
...
yields, in view of $\w{\omega}$'s definition (\ref{e:evo:def_omega_der_theta}),
...
is trivially zero, hence the result (\ref{e:evo:omega_closed}).
\end{proof}
