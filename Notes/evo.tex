...
as the \emph{derivative} of a 1-parameter family of metric tensors $\w{g}(\lambda)$ obeying
$\w{g}(0) = \w{g}$. The defining formula is then
$\hat{\delta}\w{g} := \left. \D{\w{g}}/\D{\lambda} \right|_{\lambda=0}$.
Since the family corresponding to the diffemorphism group is
...
\ee
When comparing with Eq.~(\ref{e:bas:delta_g_Lie_xi_g}), we see that the derivative-based definition has the advantage of getting rid of the infinitesimal factor $t$.
\end{remark}
