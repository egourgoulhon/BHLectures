...
is a hypersurface $x^0 = \mathrm{const}$, $\w{\xi} = \partial_0$ and
...
\]
where on $\Sp_\infty$, $\w{n} = \w{\xi}$ and $\w{s} = (x^i/r)\wpar_i$, with
...
with partial ones. Hence we get
\[
    \int_{\Sp_\infty} \omega_{\mu\nu} \,\D S^{\mu\nu} =
    \int_{\Sp_\infty} \frac{x^i}{r} ( - \partial_0 h_{i0} + \partial_j h_{ij} -
    \partial_i h^\sigma_{\ \, \sigma}) \sqrt{q} \, \D^{n-2} x .
\]
Given the asymptotic behavior (\ref{e:sta:asymptotic_metric}) of the metric tensor,
we get
\[
    h_{00} = \alpha_n \frac{\delta M}{r^{n-3}} + \bigO\left( \frac{1}{r^{n-2}} \right),\quad
    h_{0i} = \bigO\left( \frac{1}{r^{n-2}} \right), \quad
    h_{ij} = \frac{\alpha_n}{n-3} \frac{\delta M}{r^{n-3}} + \bigO\left( \frac{1}{r^{n-2}} \right) ,
\]
