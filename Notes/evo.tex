...
framework based on the co-called \emph{covariant phase space}\index{covariant!phase space} approach. The Hamiltonian formulation naturally provides the concept
...
thereby generalizating the concept of ADM mass\index{ADM!mass} beyond general relativity.
...
\defin{Hamilton's equations}\index{Hamilton's equations} are
...
the $p_i$'s are the conjuguate momenta (i.e. $p_i := \dert{L}{{\dot q}^i}$, where
...
$2N$ called the \defin{phase space}\index{phase space} of the system.
...
are called  \defin{canonical coordinates}\index{canonical!coordinates}\index{coordinates!canonical --}.
...
To any scalar field $f$ on $\mathcal{F}$, one associate a unique vector
...
Phase space of a simple gravity pendulum: this is a system with $N=1$, ruled by the Hamiltonian $H = H(\theta,p_\theta) = p_\theta^2 / (2m\ell^2) - mg\ell\cos\theta$,
where $\ell$ is the cord's length, $m$ the bob's mass and $g$
the magnitude of the vertical gravity field.}
\end{figure}
...
[Eq.~(\ref{e:bas:v_f_partial_f})]. Accordingly, Hamilton's equations (\ref{e:evo_Hamilton_eqs_finite}) can be rewritten as
...
integral curve of the Hamiltonian vector field in the phase space. Such a
curve is entirely determined by a point of $\mathcal{F}$, since there
is one, and exactly one,
integral curve of $X_H$ through each point of $\mathcal{F}$ (except at possibe singular points of $H$).
