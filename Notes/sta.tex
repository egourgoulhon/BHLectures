...
$\Omega_{n-2} = 2\pi^{(n-1)/2}/\Gamma((n-1)/2)$, with $\Gamma(u) := \int_0^{+\infty} t^{u-1} e^{-t} \D t$,
so that
\be
...
\be
    {\bar{h}}_{\alpha\beta}(x)  = \frac{16\pi}{(n-3)\Omega_{n-2}}
        \left( \frac{1}{r^{n-3}} \int_\Sigma T_{\alpha\beta}(\bar{x}) \, \D^{n-1} \bar{x}
        + (n-3) \frac{x^i}{r^{n-1}}  \int_\Sigma \bar{x}^i T_{\alpha\beta}(\bar{x}) \, \D^{n-1} \bar{x}
        + \cdots \right) .
\ee
