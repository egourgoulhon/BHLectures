...
\begin{prop}[Volume integral formula of the Komar mass]
Let $(\M, \w{g})$ be an asymptotically flat stationary spacetime,
...
to check that at the Newtonian limit, the Komar mass taken
on a surface $\Sp_{\rm ext}$ surrounding a star
...
Indeed, let us assume that the spacetime is strictly static, i.e. that $\w{\xi}$
...
$p$ is the fluid pressure and the fluid 4-velocity is collinear to the
...
so that $M_{\Sp_{\rm int}}$
can be set to zero in Eq.~(\ref{e:sta:Komar_mass_vol_integ}).
...
$\Delta \Phi = 4\pi \rho$ vanishing at infinity. We may then write
\be
