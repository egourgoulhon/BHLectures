...
\be \label{e:sta:xi_zero_t_inf}
    \w{\xi} \rightarrow 0 \quad\mbox{when}\quad t\rightarrow -\infty .
\ee
...
null geodesic $\tilde L$.
We have the freedom to choose the same value of $\lambda_0$
in Eq.~(\ref{e:sta:lambda_t}) for all the null geodesic generators
$\Li$ of $\Hor$. By extending all the null geodesics generators of $\Hor$ to
$\lambda\in (-\infty,\lambda_0]$, one gives birth to a null hypersurface, $\tilde\Hor$ say, which contains $\Hor$. Let $\Sp\subset\tilde\Hor$ be the set of points of
...
Let us denote by $\Hor^-$ the subset of $\tilde\Hor$ corresponding to
$\lambda<\lambda_0$:
\[
...
as the generator of Lorentz boosts and $\Sp$ is the bifurcation surface
...
If $\kappa<0$, we see from Eq.~(\ref{e:sta:lambda_t})
