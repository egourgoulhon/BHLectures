...
Now, the contracted Ricci identity (\ref{e:bas:contract_Ricci_ident})
applied to the vector field $\w{\xi}$ gives
$\nabla_\mu \nabla_\alpha \xi^\mu - \nabla_\alpha \nabla_\mu \xi^\mu = R_{\mu\alpha} \xi^\mu = R_{\alpha\mu} \xi^\mu$.
Thanks to the Killing equation (\ref{e:neh:Killing_equation}), the divergence
of $\w{\xi}$ is zero: $\nabla_\mu \xi^\mu = 0$, so that we get the following identity,
valid for any Killing vector\footnote{This identity
...
\be \label{e:sta:J_Killing_Ricci}
    \encadre{ \mathcal{J}^\alpha = - 4 R^\alpha_{\ \, \mu} \xi^\mu } .
\ee
...
\be \label{e:sta:Komar_angul_vol_integ_R}
    ...
    \int_{\Sigma} \w{\mathcal{J}} \cdot \w{n} \sqrt{\gamma} \, \D^{n-1} x  , }
    \quad \mbox{with}\quad
    \mathcal{J}^\alpha := \frac{1}{8\pi} R^\alpha_{\ \, \mu} \eta^\mu ,
\ee
where
$\w{n}$ is the future-directed unit normal to $\Sigma$
and $\sqrt{\gamma}\,\D^{n-1} x$ is the volume element on $\Sigma$
induced by $\w{g}$.
The vector $\w{\mathcal{J}}$ is actually the Komar current\index{Komar!current}\index{current!Komar --}
