...
dominant null energy condition (\ref{e:neh:null_dominant_cond})
is fulfilled for some scalar field $f \geq 0$.
In particular, for general relativity with $f=\Lambda$,
this is equivalent to assuming $\Lambda \geq 0$ and
the matter dominant null energy condition (\ref{e:neh:null_dominant_cond_T}).
Then, any connected component of a cross section of the black hole
...
as a vector field normal to each $\Hor_u$.
By Property~\ref{p:sta:hor_non_exanding}, we have $\theta_{(\wl)} = 0$
on $\Sp$. A standard identity (cf. e.g. Eq.~(3b) in \cite{Haywa94}
or Eq.~(36) in \cite{Jaram13}) gives the variation of $\theta_{(\wl)}$
along $\w{k}$ as
\be
...
\begin{prop}[topology theorem 2 \textnormal{(Chru\'sciel \& Wald 1994 \cite{ChrusW94b})}]
Let $(\M, \w{g})$ be a 4-dimensional asymptotically flat stationary spacetime
containing a black hole region.
