...
\section{Stationary and static spacetimes} \label{s:sta:sta_st}
...
\begin{prop}[Boyer's theorem \textnormal{(Boyer\index[pers]{Boyer, R.H.} 1969 \cite{Boyer69})}]
...
induces a $\mathrm{U}(1)$ action $\hat{\Phi}$ on the set of
null geodesic generators of $\Hor$.
Each $\Phi_t$ being an isometry leaving $\Hor$ globally invariant (Property~\ref{p:sta:stationary_hor}), it maps
...
and $p\in\Li$, we have
$\Phi_{t_2 + t_1}(p)\in\hat{\Phi}_{t_2+t_1}(\Li)$ and
...
each pair $(p,p')$ lie on the same generator). Hence $\hat{\Phi}$ is
a $\mathrm{U}(1)$ group action on the set of null geodesic generators of $\Hor$.
...
induces a 1-dimensional group action $\Psi: \R\times \Sp \to \Sp$, $(t,p) \mapsto \Psi_t(p)$
...
null geodesic generator of $\Hor$ through $\Phi_{-t}(p)$. Since $\Sp$
is a complete cross-section of $\Hor$, $\Li$ intersects $\Sp$ at a unique
point, which we define as $\Psi_t(p)$ (cf. Fig.~??).
In other words, $\Psi_t(p)$ is obtained by displacing $p$ by a parameter $-t$ along the field lines of $\w{\xi}$ and then going back to $\Sp$ along a null geodesic generator of $\Hor$.
Equivalently, we may write
\be \label{e:sta:def_Psi_action}
    \Psi_t(p) = \hat{\Phi}_{-t}(\Li_p)\cap\Sp,
\ee
where $\Li_p$ is the null geodesic generator through $p$.
...
from $\hat{\Phi}$ and thus is a $\mathrm{U}(1)$ action.
...
\ee
where $p_0 := \Phi_{-t}(p)$ and $q_0 := \Phi_{-t}(q)$.
...
energy condition is fulfilled, the metric $\w{q}$ is preserved along the null
generators of $\Hor$: $\vw{q}^* \Lie{\el} \w{q} = 0$ [Property~\ref{p:neh:invariance_of_2metric}]. Since $p_1:= \Psi_t(p)$ and $q_1:=\Psi_t(q)$
are obtained from $p_0$ and $q_0$ by transport along the null generators
(displacement by a parameter $\delta\lambda=1$ along the field lines of $\wl$),
we have $\w{q}(\vp{p_0q_0},\vp{p_0q_0}) =  \w{q}(\vp{p_1 q_1},\vp{p_1 q_1})$.
...
isometry group isomorphic to $\mathrm{U}(1)$, i.e. is axisymmetric.
One may say that, in addition to be stationary,
...
is ill-defined in the case of $\Hor$, given that, as null manifold, $\Hor$
is not endowed with a proper metric tensor.

The next step is to show that there exists a Killing vector
field $\w{\chi}$ of $(\M,\w{g})$ that, on $\Hor$, is tangent to the null generators.
Then the spacetime $(\M,\w{g})$ will be axisymmetric, the corresponding
Killing vector being a linear combination of $\w{\xi}$ and $\w{\chi}$.
We first define $\w{\chi}$ on $\Hor$ only by demanding that it is the vector tangent to the null generators of $\Hor$
corresponding to a parameter $v$ of the generators that is connected to the parameter
$t$ of the stationary action $\Phi$, in the sense that along a given null generator\footnote{Recall that on $\Hor$, $\Phi_T(p)$ and $p$ always lie on the same null generator.},
\be
\label{e:sta:vPhiT_v_p_T}
    v(\Phi_T(p)) = v(p) + T.
\ee
In addition, we require that the non-affinity coefficient $\kappa$ of
$\w{\chi}$ is constant along each null generator.
