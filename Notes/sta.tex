...
\begin{prop}[staticity theorem\index{staticity theorem} \textnormal{(Sudarsky \& Wald 1993 \cite{SudarW93})}]
\label{p:sta:staticity_thm}
Let $(\M,\w{g})$ be a stationary spacetime of dimension $n\geq 4$
that contains a black hole of event horizon $\Hor$.
If
(i) the stationary Killing vector $\w{\xi}$ is null on $\Hor$,
(ii) $\Hor$ is part of a bifurcate Killing horizon
and (iii) $\w{g}$ obeys the vacuum Einstein equation, then the spacetime
...
\begin{proof}
Let $\Sp$ be the bifurcation surface of the bifurcate Killing horizon
containing $\Hor$.
Thanks to a theorem by Chrusciel \& Wald (Theorem~4.2 in Ref.~\cite{ChrusW94}),
the domain domain of outer communications can be foliated by a family of
spacelike hypersurfaces $(\Sigma_t)_{t\in\R}$
that are asymptotically flat, have $\Sp$ as inner boundary, are Lie-dragged\footnote{$\Sigma_{t+\D t}$ is the image of $\Sigma_t$ by the infinitesimal displacement $\D t\, \w{\xi}$.}
to each other by $\w{\xi}$ and are \emph{maximal}. The hypersurface $\Sigma_t$ is said
\defin{maximal}\index{maximal!hypersurface} iff its
extrinsic curvature\index{extrinsic curvature}\index{curvature!extrinsic --}
tensor $\w{K}$ has a vanishing trace: $K^i_{\ \, i} = 0$, or equivalently, iff
...
introduced in Sec.~\ref{s:sta:np1_decomp}, and in particular the Komar mass
formula (\ref{e:sta:Komar_mass_3p1}) with $\Sigma = \Sigma_t$,
$\Sp_{\rm int} = \Sp$ and $\Sp_{\rm ext}$ being a sphere
with $r\to +\infty$ (``outer boundary'' of $\Sigma_t$).
...
We may then rewrite the term $\langle \w{p}, \w{\beta} \rangle$ in the right-hand side of
Eq.~(\ref{e:sta:Komar_mass_3p1}) as
\[
    \langle \w{p}, \w{\beta} \rangle = p_i \beta^i = \frac{1}{8\pi} D^j K_{ij} \beta^i
    = \frac{1}{8\pi} \left[ D^j \left( K_{ij} \beta^i \right)
        - K_{ij} D^j \beta^i  \right]
     = \frac{1}{8\pi} \left[ D^j \left( K_{ij} \beta^i \right)
        - N K_{ij} K^{ij}  \right] ,
\]
...
We have then, using the Gauss-Ostrogradsky theorem,
\bea
    8 \pi \int_{\Sigma_t} \langle \w{p}, \w{\beta} \rangle  \sqrt{\gamma} \, \D^{n-1} x & =&
    \int_{\Sp_{\rm ext}} s^j K_{ij} \beta^i \sqrt{q} \,  \D^{n-2} x
    - \int_{\Sp} s^j K_{ij} \beta^i \sqrt{q} \, \D^{n-2} x \nonumber \\
    & &
    -  \int_{\Sigma_t} N K_{ij} K^{ij}  \sqrt{\gamma} \, \D^{n-1} x ,  \nonumber
\eea
where $\w{s}$ is the unit normal to $\Sp_{\rm ext}$ and $\Sp$ in $\Sigma_t$,
oriented towards the asymptotic flat end of $\Sigma_t$. Now, each of the two surface
integrals in the first line of the above equation vanishes identically:
...
in order for $\w{\xi}$ to coincide with the timelike unit normal $\w{n}$
[cf. Eq.~(\ref{e:sta:xi_3p1})].
...
$\left. \w{\beta} \right| _{\Sp} = 0$. We may therefore rewrite the Komar
mass formula (\ref{e:sta:Komar_mass_3p1}) as
\[
 M_{\Sp_{\rm ext}} = M_{\Sp} +
    2 \int_{\Sigma_t} N \left[ \frac{1}{n-2} \left[ (n-3) E + S \right]
    + \frac{1}{8\pi} K_{ij} K^{ij} \right]
    \sqrt{\gamma} \, \D^{n-1} x .
\]
In the vaccum case considered here, $E=0$ and $S=0$, and
$M_{\Sp_{\rm ext}} = M_{\Sp}$ (Property~\ref{p:sta:Komar_mass_invariant}),
...
the ADM Hamiltonian formulation\index{Hamiltonian!formulation of general relativity}\index{ADM!Hamiltonian formulation} of general relativity \cite{ArnowDM62}.
...
It is equivalent to say that $\w{m}$ is a Killing vector of the spacetime metric
...
i.e. the domain of outer communication, is strictly static. The connection
between $\w{m}$ and $\w{\xi}$ is given by
Eq.~(\ref{e:sta:xi_3p1}): $\w{\xi} = \w{m} + \w{\beta}$. Hence the Killing vectors
$\w{\xi}$ and $\w{m}$ coincide iff the shift vector $\w{\beta}$ vanishes. As seen above,
$\w{\beta}$ vanishes at the inner boundary $\Sp$ of $\Sigma_t$
and at the ``outer boundary'', i.e. for $r\to +\infty$. In most cases, these constraints will result in $\w{m} = \w{\xi}$.
\end{proof}

\begin{remark}
The theorem actually proved by Sudarsky \& Wald (Theorem~2 in Ref.~\cite{SudarW93})
is actually more general: it regards solutions of the \emph{electrovacuum}\index{electrovacuum} Einstein
