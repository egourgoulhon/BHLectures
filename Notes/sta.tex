...
Let us assume that
(i) the null dominance condition\index{null!dominance condition}\index{dominance!null -- condition} (\ref{e:neh:null_dominant_cond})
...
In particular, condition (ii) holds if $\theta_{(\w{k})} < 0$ and there is no trapped
...
is that of $\Sp$ having the topology of the 2-torus $\mathbb{T}^2 = \SS^1\times\SS^1$; this could occur only under special circumstances,
among which the metric induced by $\w{g}$ on $\Sp$ should be flat.
\end{prop}
...
version given by Hawking \cite{Hawki72,Hawki73,HawkiE73}, for it adds the ``outermost'' hypothesis (ii). Actually in Hawking's, (ii) is deduced from the non-existence of
...
condition, which is weaker than the null dominance condition (hypothesis (i) above),
and replaces hypothesis (ii) by other ones, which are more global. It also
