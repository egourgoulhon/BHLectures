...
For $n=4$,  $[(n-1)/2] = 1$, so that one has necessarily $L=1$. Denothing then
$\w{\eta}_{1}$ by simply $\w{\eta}$ and $\Omega_{1}$ by $\Omega_H$,
Eq.~(\ref{e:sta:chi_xi_Omega_i_eta_i}) reduces to
\be \label{e:sta:chi_xi_OmegaH_eta}
    \w{\chi} = \w{\xi} + \Omega_H \w{\eta} .
\ee
The constant $\Omega_H$ is then called the \defin{black hole rotation velocity}\index{black hole!rotation velocity}\index{rotation!velocity} if $\Hor$ coincides with
...
The constancy of $\Omega_{\rm H}$ in Eq.~(\ref{e:sta:chi_xi_OmegaH_eta})
...
$\w{\eta}_{i}$ and the constants $\Omega_{i}$ may vary from one connected
...
$\Omega_{1}^{\Hor_{\rm p}} \neq \Omega_{1}^{\Hor_{\rm r}}$, and hence
different normal Killing vectors: $\w{\chi}^{\Hor_{\rm p}} \neq \w{\chi}^{\Hor_{\rm r}}$.
\end{remark}
...
$\w{\chi}_{(k)} = \w{\xi} + \sum_{i=1}^{L_{(k)}} \Omega_i^{(k)} \w{\eta}_i^{(k)}$
(Properties~\ref{p:sta:strong_rigidity_thm} and \ref{p:sta:axisymmetry_BH}),
where the $\Omega_i^{(k)}$ are constants and
$\w{\eta}_i^{(k)}$ are axisymmetric Killing vectors, which are labelled with $(k)$
for they depend on the connected component $\Hor_{(k)}$, cf. Remark~\ref{r:sta:dep_eta_Omega}
...
\be \label{e:sta:Smarr_H_k}
    ...
    + 2  \sum_{i=1}^{L_{(k)}} \Omega_i^{(k)} J_{\Hor_{(k)}}^i \right) ,
\ee
where $J_{\Hor_{(k)}}^i$ is the Komar angular momentum with respect to
the axisymmetric Killing vector $\w{\eta}_i^{(k)}$ as given by Eq.~(\ref{e:sta:def_Komar_J}).
...
of which is a cross-section of the event horizon, i.e. a union of cross-sections
...
\begin{proof}
Let us consider formula (\ref{e:sta:Komar_mass_flux_nabla_xi}) for the
Komar mass $M_{\Hor_{(k)}}$. If we denote the cross-section of $\Hor_{(k)}$
by $\Sp_{(k)}$ and we substitute $\w{\xi}$ by
its expression arising from Eq.~(\ref{e:sta:chi_xi_Omega_i_eta_i}), we get
(here we drop the index ${(k)}$ on $\w{\chi}$ and $\w{\eta}_i$ for clarity, given that the integral regards
a single connected component $\Hor_{(k)}$)
\bea
    M_{\Hor_{(k)}} &= & - \frac{n-2}{16\pi(n-3)} \int_{\Sp_{(k)}} \nabla_\mu
    \left( \chi_{\nu} - \sum_{i=1}^{L_{(k)}} \Omega_i^{(k)}  (\eta_i)_\nu
    \right) \D S^{\mu\nu} \nonumber \\
    & = & - \frac{n-2}{16\pi(n-3)} \Bigg[
        \underbrace{\int_{\Sp_{(k)}} \nabla_\mu \chi_\nu \, \D S^{\mu\nu}}_{-2 \kappa_{(k)} A_{(k)}}
        - \sum_{i=1}^{L_{(k)}} \Omega_i^{(k)}
        \underbrace{\int_{\Sp_{(k)}} \nabla_\mu (\eta_i)_\nu
...
such that $\w{\eta}\cdot\w{k} = -1$.
At each point $p\in\Sp_{(k)}$,
the pair $(\w{\eta},\w{k})$ is then a null basis of the 2-plane $T_p^\perp\Sp_{(k)}$
(cf. Fig.~\ref{f:def:TS_ortho} where $\wl$ stands for $\w{\chi}$) and
we may rewrite the normal area element bivector $\D S^{\alpha\beta}$
as $\D S^{\alpha\beta} = (\chi^\alpha k^\beta - k^\alpha \chi^\beta) \sqrt{q}\, \D^{n-2} x$
(this follows readily by setting $\w{n} = (\w{\chi} + \w{k})/\sqrt{2}$
...
or the Killing vector $\w{\chi} = \w{\xi} + \Omega_H \w{\eta}$
(Property~\ref{p:sta:strong_rigidity_thm} and Eq.~(\ref{e:sta:chi_xi_OmegaH_eta})),
where $\Omega_H$ is the angular velocity of $\Hor$ and $\w{\eta}$ is
an axisymmetric Killing vector. We shall combine these two cases by stating
that the former corresponds to $\Omega_H = 0$, so that $\w{\chi} = \w{\xi}$.
...
\be
    M_{\Hor} = \frac{\kappa}{4\pi} A + 2 \Omega_H J_{\Hor} ,
\ee
...
\be \label{e:sta:Smarr_M_infty_n4}
    M_\infty = \frac{\kappa}{4\pi} A + 2 \Omega_H J_{\Hor}
...
term disappears. They can also be simplified when $\w{T}$ is the energy-momentum
tensor of a source-free electromagnetic field, which occurs in
electrovacuum spacetimes [cf. Eq.~(\ref{e:fra:T_em})]. To do so,
we shall need first to characterize a stationary electromagnetic field on
a Killing horizon:
...
were a unit timelike vector, $\w{E}$ would be a geguine electric field: the one measured
by the observer of 4-velocity $\w{\chi}$.}
$\w{E}$ defined by
\be
...
\defin{electromagnetic Killing potential}\index{electromagnetic!Killing potential}\index{Killing!electromagnetic -- potential}. Furthermore, on $\Hor$, the vector field
...
the \emph{electric charge density} of any cross-section $\Sp$ of $\Hor$.
Indeed the \defin{electric charge}\index{electric!charge} within the cross-section $\Sp$ is
\be
    Q = \frac{1}{\mu_0}\int_\Sp \star \w{F} = \frac{1}{2\mu_0} \int_\Sp F_{\mu\nu} \D S^{\mu\nu}
    = \int_\Sp \sigma  \sqrt{q} \, \D^{n-2} x .
\ee
Finally, the electromagnetic Killing potential $\Phi$ is constant on $\Hor$:
\be
...
If $\w{A}$ obeys the $\w{\chi}$-symmetry, we have $\Lie{\chi} \w{A} = 0$.
Now, thanks to the Cartan identity (\ref{e:bas:Cartan}),
...
Let $\w{\tilde{A}}$ be another electromagnetic potential associated to $\w{F}$
such that $\Lie{\chi} \w{\tilde{A}} = 0$.
\end{proof}
