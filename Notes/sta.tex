...
\subsubsection{N+1 decomposition} \label{s:sta:np1_decomp}
...
that have $\Sp$ as inner boundary, are Lie-dragged to each other by $\w{\xi}$ and are \emph{maximal}\index{maximal!hypersurface},
i.e. have an extrinsic curvature\index{extrinsic curvature}\index{curvature!extrinsic --}
tensor $\w{K}$ of vanishing trace: $K^i_{\ \, i} = 0$. This last property allows
to simplify one of the Einstein equations in the $\mathcal{N}+1$ decomposition (cf.
p.~\pageref{s:sta:np1_decomp}),
namely the momemtum constraint (see e.g. Chap.~5 of Ref.~\cite{Gourg12}),
which reduces to $D^j K_{ij} = 8\pi p_i$.
\end{proof}
