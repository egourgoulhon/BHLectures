...
that a spacetime that contains a black hole cannot be strictly stationary.
We shall discuss in detail the two cases --- null or spacelike --- allowed
for $\w{\xi}$ on $\Hor$ in Sec.~\ref{s:sta:EH_KH}.

It is rather intuitive that a black hole event horizon $\Hor$ in a stationary
spactime must have a vanishing expansion $\theta_{(\wl)}$ along its null
normals $\wl$, otherwise the area of cross-sections would vary when dragged
along $\wl$ and this would define a privileged ``time direction'' along $\Hor$,
...
but the light cone is not invariant by any time translation isometry of
Minkowski spacetime. The rigorous proof that $\theta_{(\wl)} = 0$
for stationary event horizons can be found in Hawking \& Ellis' textbook
(Proposition~9.3.1 in Ref.~\cite{HawkiE73}).  Here, we shall simply state, following
the terminology introduced in Chap.~\ref{s:neh}:
...
In the definition of a non-expanding horizon given in Sec.~\ref{s:neh:def},
in addition to $\theta_{(\wl)} = 0$, the topology of $\Hor$ is
demanded to be $\R\times \Sp$ where $\Sp$ is a closed $(n-2)$-dimensional
manifold. Equivalently, the cross-sections of $\Hor$ must be closed manifolds,
i.e. compact without boundary. This is fulfilled for black holes
thanks to the topology theorem:

\begin{prop}[topology theorem \textnormal{(Hawking 1972 \cite{Hawki72},
Chru\'sciel \& Wald 1994 \cite{ChrusW94b})}]
Let $(\M, \w{g})$ be a 4-dimensional asymptotically flat stationary spacetime
containing a black hole region.
Let us assume that (i) the Ricci tensor $\w{R}$ of $\w{g}$ fulfills the
\defin{geometric null energy condition}, namely
\be \label{e:sta:geom_null_energy_cond}
    \w{R}(\wl, \wl) \geq 0 \quad \mbox{for any null vector $\wl$}
\ee
and (ii) the domain of outer communications is globally hyperbolic.
Then, the domain of outer communications is simply connected and
any connected component of a compact cross section of the black hole
event horizon is homeomorphic to a 2-sphere.
\end{prop}

\begin{remark}
If the Einstein equation [Eq.~(\ref{e:fra:Einstein_eq})] is fulfilled,
the geometric null energy condition (\ref{e:sta:geom_null_energy_cond})
is equivalent to the matter null energy condition (\ref{e:neh:null_energy_cond}).
\end{remark}

\begin{remark}
Generalization to $n>4$: Galloway \& Schoen \cite{GalloS06}.
\end{remark}
...
\cite{HawkiE73}), under the hypothesis of the
null dominant energy condition\index{null!dominant energy condition}\index{energy!condition!null dominant--} (cf. Sec.~\ref{s:neh:zeroth_law}).
...
exclude the 2-torus topology for the horizon's cross sections
...
The theorem was demonstrated by Piotr Chru\'sciel\index{Chrusciel, P.T.@Chru\'sciel, P.T.}
and Robert Wald\index{Wald, R.M.} in 1994 \cite{ChrusW94b} without
the analyticity assumption and assuming the null energy
condition (which is weaker than the dominant one).
\end{hist}
