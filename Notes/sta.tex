...
The quantities $M$ and $J_{ij}$ are called respectively the \defin{mass} and
the \defin{angular momentum} of the central source.
The coordinates $(x^\alpha)$ are correspond to the \emph{central source rest-frame}
...
\begin{proof}
Let $(\w{f},\w{h})$ obeys the Lorenz gauge and $(x^\alpha)$ be the corresponding
...
is nothing but $M$
as defined by Eq.~(\ref{e:sta:mass_weakly_relat}), while the second integral
...
Hence we may inverse Eq.~(\ref{e:sta:def_h_bar}) to
...
\[
    {\bar{h}}_{ii}(x)  = \frac{16\pi}{\Omega_{n-2}}
        ...
        + \frac{x^j}{r^{n-1}}  \int_\Sigma {x'}^j T_{ii}({x'}) \, \D^{n-1} {x'}
         \right)  + \bigO\left( \frac{1}{r^{n-1}} \right) .
\]
It turns out that the second integral vanishes identically, so that one is left
with
\be \label{e:sta:h_bar_ii}
    {\bar{h}}_{ii}(x)  = \frac{16\pi}{(n-3)\Omega_{n-2}}
        \frac{1}{r^{n-3}} \int_\Sigma T_{ii}({x'}) \, \D^{n-1} {x'}
            + \bigO\left( \frac{1}{r^{n-1}} \right) .
\ee
To show it, one can use the identity
\begin{align}
    ...
    & = x^j T^{ii} = x^j T_{ii} , \nonumber
\end{align}
where, as above for $\partial_k T^{0k} = 0$, $\partial_i T^{ij}=0$ follows from the equation of energy-momentum conservation (\ref{e:fra:divT}). Hence the integral of $x^j T_{ii}$ over $\Sigma$ is that of the divergence of a vector
