...
one gets immediately formula~(\ref{e:sta:Newtonian_mass}). This formula is
actually not restricted to a remote sphere, but is valid for any surface
...
volume integral (\ref{e:sta:mass_weakly_relat}) identifies the mass to the ``amount of matter'' constituting
the central body. The latter definition would yield $M=0$ for any vacuum spacetimes
...
demand $M > 0$ for the gravitating mass of stationary black holes such as Schwarzschild and Kerr ones, which are pure vacuum solutions of the Einstein equation.

Accordingly, let us focus on Gauss's law (\ref{e:sta:Newtonian_mass}) in order to
generalize the definition of mass to strongly relativistic spacetimes.
The mass appears as the coefficient of the dominant $1/r^{n-3}$ in the asymptotic
expansion of $g_{00}$ in Eq.~(\ref{e:sta:g00_weakly_relat}), so one could
recover it by considering the flux integral of $\partial g_{00}/\partial r$ over
a $(n-2)$-dimensional surface at large distance. In order to have a coordinate-independent
definition, it is more appropriate to focus on the
