...
If we integrate the above equation over $\Sigma_t$, we get,
using the Gauss-Ostrogradsky theorem for the divergence term
$D^j \left( K_{ij} \beta^i \right)$,
\[
...
\]
where we have used the fact that the boundary of $\Sigma_t$ is made
...
assumption). In the above surface integrals,
$\w{s}$ is the unit normal to $\Sp_\infty$ and $\Sp$ in $\Sigma_t$,
