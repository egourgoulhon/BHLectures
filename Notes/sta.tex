...
\be
    \w{g} = \w{\eta} + \w{h} ,
\ee
where $\w{\eta}$ is Minkowski metric on $\M$ and $\w{h}$ is small in the following
sense: in any Minkowskian coordinates $(x^\alpha)$, i.e. in coordinates
such that $(\eta_{\alpha\beta}) = \mathrm{diag}(-1,1,\ldots,1)$, the components
of $\w{h}$ obey $|h_{\alpha\beta}| \ll 1$. Moreover, we assume that $\w{g}$
...
Let us introduce
\be
    \w{\bar{h}} := \w{h} - \frac{1}{2} h \, \w{\eta} ,
\ee
where $h$ stands for the trace of $\w{h}$ with respect
to\footnote{$\eta^{\alpha\beta}$ denote the components of
the inverse metric $\w{\eta}^{-1}$; in Minkowskian coordinates,
the matrix $(\eta^{\alpha\beta})$ is identical to $(\eta_{\alpha\beta})$.}
$\w{\eta}$: $h := \eta^{\mu\nu} h_{\mu\nu}$.
Assuming the \defin{Lorenz gauge}\index{Lorenz gauge} (also called
