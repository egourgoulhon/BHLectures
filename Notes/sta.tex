...
\be \label{e:sta:Komar_mass_vol_integ}
    ...
    \int_{\Sigma} \left( \w{T}(\w{n}, \w{\xi}) - \frac{T}{2} \, \w{n}\cdot \w{\xi} \right)
...
as some flux over a closed 2-sphere $\Sp$ of quantities involving derivatives
...
\begin{prop}[Equality of Komar and ADM masses (Beig 1978)]
If $(\M,\w{g})$ is a stationary and asymptotically flat spacetime,
...
\end{prop}
The proof can be found in Beig's article \cite{Beig78}
(see also Ref.~\cite{AshteM79} for a different proof).
...
Let $(\M, \w{g})$ be an axisymmetric spacetime,
...
\begin{remark}
Besides the sign and the change $\w{\xi}\leftrightarrow\w{\eta}$, we notice a difference by a factor $-2$ between the r.h.s. of (\ref{e:sta:def_Komar_mass}) and (\ref{e:sta:def_Komar_J}).
This is known as \emph{Komar's anomalous factor} and is discussed further in
Ref.~\cite{Katz85}.
\end{remark}
...
and the factor $-1/(8\pi)$ by $1/(16\pi)$ to get the following expressions
...
\be \label{e:sta:Komar_angul_vol_integ}
    ...
    \int_{\Sigma} \left( \w{T}(\w{n}, \w{\eta}) - \frac{T}{2} \, \w{n}\cdot\w{\eta} \right)
    \sqrt{\gamma} \, \D x^1 \D x^2 \D x^3  , }
\ee
where $\w{n}$ be the future-directed unit normal to $\Sigma$,
$\sqrt{\gamma}\,\D x^1 \D x^2 \, \D x^3$ is the volume element on $\Sigma$
...
$\w{n}\cdot\w{\eta} = 0$, so that the second term in the integrand of Eq.~(\ref{e:sta:Komar_angul_vol_integ}) vanishes identically.
Moreover, if one expresses $\w{T}$ via the 3+1 decomposition (\ref{e:sta:T_3p1}),
...
\begin{prop}[Independence of the Komar angular momentum from the integration 2-surface]
Two 2-surfaces $\Sp_{\rm ext}$ and $\Sp_{\rm int}$ that are connected by
a spacelike hypersurface $\Sigma$ in a \emph{vacuum} region of an
