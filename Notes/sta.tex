...
\section{Black holes in stationary spacetimes} \label{s:sta:sta_st}

\subsection{Stationary spacetimes} \label{s:sta:def_station}

\begin{tdef}{stationary spacetime}{sta:stationary}
...
spacetime admits a conformal infinity $\scri$ --- which is required if a black hole
...
matter fields, they must respect the stationarity as well. For an electromagnetic
field $\w{F}$ (cf. Sec.~\ref{e:fra:electrovacuum}), this is expressed by
the vanishing of the Lie derivative along $\w{\xi}$:
\be
    \Lie{\xi} \w{F} = 0 .
\ee
This condition is similar to $\Lie{\xi} \w{g} = 0$ --- the Killing-vector property
[Eq.~(\ref{e:neh:Lie_xi_g})].

A stronger concept than stationarity is \emph{staticity}:

\begin{tdef}{static spacetime}{sta:static}
...
\begin{remark}
The same comment as in Remark~\ref{r:sta:pseudo-stationary} is appropriate: some authors
...
\begin{texample}{staticity of anti-de Sitter spacetime}{}
The anti-de Sitter spacetime\index{anti-de Sitter spacetime} AdS$_{4}$ considered in Example~\ref{x:glo:AdS}
is strictly static, with the coordinates $(\tau, r, \th,\ph)$ being
adapted to staticity (hence their name: \emph{global static}).
Indeed, the metric (\ref{e:glo:AdS4}) is of the form (\ref{e:sta:static_metric})
...
\begin{texample}{staticity of Schwarzschild spacetime}{}
The Schwarzschild spacetime\index{Schwarzschild!spacetime}, introduced in Example~\ref{x:def:Schw_hor} is static, but not
...
\subsection{Properties of black holes in stationary spacetimes}
\label{s:sta:BH_stationary}

Let us consider a spacetime $(\M,\w{g})$ that \emph{contains a black hole}, as defined in
Sec.~\ref{s:glo:def_BH}. In particular, $(\M,\w{g})$ admits a
conformal completion $(\tilde{\M},\w{\tilde{g}})$ with a future
infinity $\scri^+$.
Furthermore, we assume that $(\M,\w{g})$ is \emph{stationary},
as defined in Sec.~\ref{s:sta:def_station}.
$(\M,\w{g})$ is then invariant under the action of the isometry group $G = (\R,+)$
generated by the Killing vector $\w{\xi}$. In general $\w{\xi}$ can be extended
smoothly to the boundary $\scri$ of $\tilde{\M}$, thereby defining a
group action $\Phi: G \times \tilde{\M} \to \tilde{\M}$ (cf. Sec.~\ref{s:neh:symmetries})
on the whole of the conformal completion $\tilde{\M}$.
Let $t\in\R$ and $p\in \scri^+$.
Because $G$ is an isometry group of $\w{g}$ and $p$ is infinitely remote from points of $\M$,
there is no other possibility than
$\Phi_t(p)$ being infinitely remote from any point of $\M$. It follows that
$\Phi_t(\scri^+)\subset \scri$. Since $\Phi_0(\scri^+) = \scri^+$, we get
by continuity $\Phi_t(\scri^+)\subset \scri^+$. In other words, $\scri^+$
is globally invariant under the stationary group action.
Equivalently,
the extension of $\w{\xi}$ to $\scri^+$ is tangent to $\scri^+$. One can check
this last property in the example of the conformal completion of Minkowski spacetime
(cf. Fig.~\ref{f:sta:conf_diag_Mink_xi}). The set $\scri^+$ being globally
invariant under the action $G$, so is
its causal past $J^-(\scri^+)$. Since the event horizon $\Hor$
is the boundary of $J^-(\scri^+)$
inside $\M$ [Eq.~(\ref{e:glo:Hor_bound_past_scrip})], we get

\begin{tprop}{stationary event horizon}{sta:stationary_hor}
...
\begin{figure}
\centerline{\includegraphics[height=0.4\textheight]{sta_conf_diag_Mink_xi.pdf}}
\caption[]{\label{f:sta:conf_diag_Mink_xi} \footnotesize
Killing vector $\w{\xi} = \wpar_t$ of Minkowski spacetime $(\M,\w{g})$ extended to the
whole conformal completion $\tilde{\M}$ introduced in Sec.~\ref{s:glo:conf_Mink}
(cf. in particular Fig.~\ref{f:glo:conf_diag_Mink}).
In terms of the Einstein cylinder coordinates $(\tau,\chi,\th,\ph)$
used in the figure [cf. Eq.~(\ref{e:glo:tau_chi_t_r})], one has
$\w{\xi} = (1 + \cos\tau\cos\chi) \wpar_\tau -\sin\tau \sin\chi \wpar_\chi$,
so that $\w{\xi} \stackrel{\scri^+}{=} \sin^2\tau (\wpar_\tau - \wpar_\chi)$
and $\w{\xi} \stackrel{\scri^-}{=} \sin^2\tau (\wpar_\tau + \wpar_\chi)$.
It follows that $\w{\xi}$ is tangent
to $\scri^+$ and $\scri^-$.
...
it is then a null hypersurface (Property~\ref{p:glo:prop4}).
...
Lemma~\ref{p:def:tangent_to_null_hyp}), we conclude:

\begin{tprop}{stationary Killing vector tangent to the event horizon}{sta:xi_tangent_H}
...
according to Definition~\ref{d:sta:stationary}.
...
(cf. Example~\ref{x:def:light_cone6}),
...
\begin{tprop}{non-expanding horizons for stationary black holes}{sta:hor_non_expanding}
The event horizon $\Hor$ of a black hole in a stationary spacetime
...
according to Definition~\ref{d:neh:NEH}.
\end{tprop}

\subsection{Topology theorems} \label{s:sta:topology}

In dimension 4, one can strongly constrain the topology of the horizon cross-sections:

\begin{tprop}{topology theorem 1 \textnormal{(Hawking 1972)}}{sta:topology1}
...
$- \w{W}\cdot\w{k} \geq 0$ by Lemma~\ref{p:fra:lem2},
...
\begin{tprop}{topology theorem 2 \textnormal{(Chru\'sciel\index[pers]{Chrusciel, P.T.@Chru\'sciel, P.T.} \& Wald\index[pers]{Wald, R.M.} 1994)}}{sta:topology2}
...
and (iv) some technical condition is fulfilled (cf. Ref.~\cite{ChrusW94b} for the details).
...
obeys the Einstein equation (\ref{e:fra:Einstein_eq}) with $\Lambda=0$
...
\begin{tprop}{Lorenz gauge for the metric perturbation}{}
...
\begin{tprop}{asymptotic metric of a weakly relativistic system}{}
...
\begin{tprop}{asymptotic metric of a stationary asymptotically flat spacetime}{}
...
\begin{tdef}{Komar mass}{sta:Komar_mass}
...
and $\w{s}$ is a unit spacelike vector field normal to $\Sp$, orthogonal to $\w{n}$
(cf. Fig.~\ref{f:sta:surf_normal_plane}) and directed
...
\begin{figure}
\centerline{\includegraphics[width=0.5\textwidth]{sta_surf_normal_plane.pdf}}
\caption[]{\label{f:sta:surf_normal_plane} \footnotesize
Normal plane $T^\perp_p\Sp$ at a point $p$ of the spacelike $(n-2)$-dimensional surface $\Sp$.
Due to the graphic dimensional reduction, $\Sp$, as well as the tangent space $T_p\Sp$, are depicted as 1-dimensional objects, while they are
actually of dimension $n-2$. On the contrary, $T^\perp_p\Sp$, which is always 2-dimensional --- whatever the spacetime dimension $n$ --- is correctly depicted as a 2-plane.
The vector pairs $(\w{n},\w{s})$ and $(\w{n'},\w{s'})$ are orthonormal bases
of $T^\perp_p\Sp$, while the vector pair $(\w{\ell},\w{k})$ is a null basis
of $T^\perp_p\Sp$. The directions of $\w{\ell}$ and $\w{k}$
are uniquely defined as the intersections of $T_p^\perp\Sp$ with the
null cone emanating from $p$, whereas the directions of $\w{n}$ and $\w{s}$
can be changed by an arbitrary Lorentz boost in a direction normal to $\Sp$,
such a boost mapping $(\w{n},\w{s})$ to $(\w{n'},\w{s'})$, while preserving
the null directions of $\w{\ell}$ and $\w{k}$.
}
\end{figure}
...
\begin{tprop}{area element normal bivector to a codimension-2 surface}{sta:area_elem_bivector}
...
obeying the same properties as in item (iv) of Definition~\ref{d:sta:Komar_mass};
in particular at each point $p\in\Sp$, $(\left.\w{n}\right| _p,\left. \w{s}\right| _p)$ is an orthonormal basis
of the timelike plane $T^\perp_p\Sp$ normal to $\Sp$ (cf. Fig.~\ref{f:sta:surf_normal_plane});
...
2-dimensional Lorentz boost (cf. Fig.~\ref{f:sta:surf_normal_plane}):
\[
   \left\{\begin{array}{l}
...
\begin{tlemma}{flux integral of a 2-form}{sta:flux_integ_2form}
...
\begin{texample}{Komar mass in Schwarzschild spacetime}{sta:Komar_mass_Schw}
Let us consider the Schwarzschild spacetime $(\M,\w{g})$
introduced in
Example~\ref{x:def:Schw_hor}.
...
\begin{tprop}{Komar mass in the asymptotic expression of the metric}{sta:Komar_mass_asymp_metric}
...
\begin{tlemma}{flux integral of the divergence of a 2-form}{sta:flux_div_2_form}
...
\begin{tprop}{conservation of the Komar current}{}
...
\begin{tprop}{volume integral formula for the Komar mass}{sta:Komar_mass_vol_integ}
...
\begin{texample}{Komar mass of a static star}{sta:Komar_mass_star}
...
\begin{tprop}{independence of the Komar mass from the integration surface}{sta:Komar_mass_invariant}
...
\begin{tprop}{Komar mass at infinity}{sta:Komar_mass_inf}
...
\begin{texample}{total mass of Schwarzschild spacetime}{sta:Komar_mass_infty_Schwarz}
...
\begin{texample}{Vanishing total mass of cubic Galileon black holes}{}
...
\subsection{\texorpdfstring{$\mathcal{N}+1$}{N+1} decomposition} \label{s:sta:np1_decomp}
...
\begin{tprop}{$\bm{\mathcal{N}}$+1 expression of the Komar mass}{}
...
\begin{texample}{Komar mass of a weakly relativistic star}{sta:Komar_mass_star_dim_n}
...
\begin{tprop}{equality of Komar and ADM masses \textnormal{(Beig 1978)}}{sta:Komar_ADM_mass}
...
\begin{tdef}{axisymmetric spacetime}{sta:axisym}
...
\begin{tdef}{Komar angular momentum}{}
...
\begin{remark}
The Komar angular momentum is well defined for any spacetime dimension $n\geq 3$, while
Definition~\ref{d:sta:Komar_mass} of the Komar mass requires
$n\geq 4$.
\end{remark}
...
\begin{tprop}{Komar angular momentum in the asymptotic expression of the metric}{}
...
\begin{tprop}{volume integral formula for the Komar angular momentum}{}
Let $(\M, \w{g})$ be an axisymmetric spacetime of dimension $n\geq 3$,
with axisymmetric Killing vector $\w{\eta}$.
Let $\Sigma$ be a compact spacelike hypersurface bounded by two closed $(n-2)$-surfaces:
an ``internal'' one, $\Sp_{\rm int}$, and an ``external'' one,
...
\begin{tprop}{independence of the Komar angular momentum from the integration surface}{sta:invariance_Komar_angu}
Two $(n-2)$-surfaces $\Sp_{\rm ext}$ and $\Sp_{\rm int}$ that are connected by
a spacelike hypersurface $\Sigma$ in a \emph{vacuum} region of an
...
\begin{tprop}{Komar angular momentum at infinity}{sta:Komar_ang_mom_infty}
...
\subsection{Null stationary Killing field on \texorpdfstring{$\Hor$}{H}: the staticity theorem}\label{s:sta:staticity_thm}

By Lemma~\ref{p:def:tangent_to_null_hyp}, if the Killing vector
field $\w{\xi}$ is null on $\Hor$, it is necessarily tangent to the null geodesic generators
of $\Hor$ and therefore collinear to the null normals $\wl$ of $\Hor$.
From the Definition~\ref{d:neh:Killing_horizon} of a Killing horizon, we get immediately:

\begin{tprop}{event horizon as a Killing horizon for $\w{\xi}$ null on it}{sta:H_Killing_hor_xi_null}
...
i.e. that $(\M,\w{g})$ is \emph{static}, according to Definition~\ref{d:sta:static}:

\begin{tprop}{staticity theorem\index{staticity theorem} \textnormal{(Sudarsky\index[pers]{Sudarsky, D.} \& Wald\index[pers]{Wald, R.M.} 1993)}}{sta:staticity_thm}
Let $(\M,\w{g})$ be an asymptotically flat stationary spacetime of dimension $n\geq 4$
...
\subsection{Spacelike stationary Killing field on \texorpdfstring{$\Hor$}{H}: the strong rigidity theorem}
...
\begin{tprop}{strong rigidity theorem}{sta:strong_rigidity_thm}
...
so-called \emph{DHOST}\index{DHOST} family, for \emph{Degenerate Higher-Order Scalar-Tensor} \cite{Langl19,BenAc25}), the event horizon of which does not seem to be a Killing horizon
\cite{AnsonBCH21}.
\end{remark}
...
\begin{tprop}{axisymmetry\index{axisymmetric spacetime} of stationary black holes}{sta:axisymmetry_BH}
...
axisymmetry,
according to Definition~\ref{d:sta:axisym}.
...
By the very definition of stationarity (Definition~\ref{d:sta:stationary}),
the Killing vector field $\w{\xi}$ is
...
\begin{tdef}{extremal black hole}{}
A black hole in a stationary spacetime is said \defin{extremal}\index{extremal!black hole} (resp. \defin{non-extremal}\index{non-extremal black hole}) if, and
...
\begin{tprop}{generalized Smarr formula}{sta:Smarr_gen}
...
\begin{tprop}{4-dimensional generalized Smarr formula}{sta:Smarr_n4}
Let $(\M, \w{g})$ be a 4-dimensional stationary spacetime
...
\begin{texample}{Schwarzschild black hole}{}
...
for it. From Example~\ref{x:def:Schw_hor3}, we have
$\kappa = 1/(4 m)$ [Eq.~(\ref{e:def:kappa_Schw_hor})], while from
Example~\ref{x:neh:Schwarz_hor_area}  we have
...
that the parameter $m$ is nothing but the Komar mass
...
\begin{tprop}{electromagnetic field on a Killing horizon}{sta:electromag_Killing_hor}
...
\begin{tprop}{electric charge of a Killing horizon}{sta:electric_charge}
...
\begin{tprop}{generalized Smarr formula for charged black holes}{}
...
\begin{tprop}{4-dimensional Smarr formula for a connected charged black hole}{}
...
\begin{tprop}{generalized Israel uniqueness theorem (vacuum)
\index{Israel uniqueness theorem!vacuum}}{sta:Israel_uniqueness_thm}
...
\begin{tprop}{generalized Israel uniqueness theorem ($\bm{n = 4}$ electrovacuum)
\index{Israel uniqueness theorem!electrovacuum}}{sta:Israel_uniq_electrovac_n4}
...
\begin{tprop}{generalized Israel uniqueness theorem ($\bm{n\geq 5}$ electrovacuum)
\index{Israel uniqueness theorem!electrovacuum}}{sta:Israel_uniq_electrovac_n5}
...
\begin{texample}{five-dimensional Reissner-Nordström spacetime}{}
...
The black hole character of the Majumdar-Papapetrou solution has been truly recognized
...
\begin{tprop}{Carter-Robinson theorem
\textnormal{(Carter\index[pers]{Carter, B.} 1971, Robinson\index[pers]{Robinson, D.C.} 1975)}}{sta:Carter_Robinson_thm}
...
\begin{remark}
In their original publications, Carter \cite{Carte71} and Robinson \cite{Robin75} assumed
that $\Hor$ is a \emph{non-degenerate} Killing horizon (i.e. has non-zero surface gravity).
...
(Property~\ref{p:sta:Israel_uniqueness_thm}) because a static spacetime,
which by Definition~\ref{d:sta:static} has hypersurface-orthogonal timelike curves,
cannot contain any closed causal curve.
\end{remark}

Carter-Robinson theorem has been generalized to electrovacuum spacetimes
by Bunting \cite{Bunti83} and Mazur \cite{Mazur82}:

\begin{tprop}{Bunting-Mazur theorem
\textnormal{(Bunting\index[pers]{Bunting, G.L.} 1983, Mazur\index[pers]{Mazur, P.O.} 1982)}}{sta:Bunting_Mazur_thm}
...
\begin{tprop}{no-hair theorem\index{no-hair theorem}}{sta:no-hair_thm}
...
(among which the scalar field obeys the same symmetries as the spacetime ones), see the articles by
Capuano, Santoni \& Barausse (2023) \cite{CapuaSB23} and Ben Achour (2025) \cite{BenAc25} for details.
