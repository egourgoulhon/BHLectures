...
(\ref{e:fra:vac_Einstein}), refering
...
\subsection{Uniqueness theorems for static black holes}
...
Let $(\M,\w{g})$ is a $n$-dimensional static and asymptotically flat spacetime
...
to the domain of outer communications of a $n$-dimensional Schwarzschild spacetime\index{Schwarzschild!spacetime}, i.e. there exists a coordinate system
...
spanned by the coordinates $(\th_1,\ldots,\th_{n-3},\ph)$,
and the constant $\mu>0$ is related to the spacetime Komar (or ADM, cf. Property~\ref{p:sta:Komar_ADM_mass}) mass $M$ by
\be
    \mu = \frac{16\pi M}{(n-2) \Omega_{n-2}} .
\ee
$\Omega_{n-2}$ is the area of $\SS^{n-2}$ and is given by the formula
$\Omega_{n-2} = 2\pi^{(n-1)/2}/\Gamma((n-1)/2)$, which involves the
gamma function\index{gamma function}: $\Gamma(x) := \int_0^{+\infty} t^{x-1} e^{-t} \D t$,
...
The $n$-dimensional Schwarzschild metric (\ref{e:sta:Schwarz_Tang})
is also known as the
\defin{Schwarzschild-Tangherlini metric}\index{Schwarzschild-Tangherlini metric}
(cf. the historical note below).
\end{prop}
The proof can be found in Ref.~\cite{GibboIS02}, which relies on the assumption that
...
The proof in Ref.~\cite{GibboIS02} relies on the positive mass theorem\index{positive mass theorem}.
An elementary proof specific to the case $n=4$ can be found in Sec.~8.2 of the
textbook~\cite{Strau13} and well as in Chap.~9 of the textbook~\cite{Heusl96}.
...
has been first proven in the case $n=4$ by Werner Israel\index{Israel, W.}
in 1967 \cite{Israe67}
under the additional hypotheses that (i) the spacetime is analytic
(cf. Remark~\ref{r:bas:analytic} in Sec.~\ref{s:bas:def_manif}),
(ii) the event horizon $\Hor$ is connected,
(iii) $\Hor$  is non-degenerate
and (iv) $\Hor$'s cross-sections are topologically 2-spheres.
Property (iv)
actually follows from the topology theorem for $n=4$ (Properties~\ref{p:sta:topology1}
and \ref{p:sta:topology2}), which has been established 5 years later by Stephen Hawking\index{Hawking, S.W.} \cite{Hawki72}, so it can be removed from the hypotheses of the theorem. The requirement (ii) ($\Hor$ connected)
has been removed as unecessary by Gary Bunting\index{Bunting, G.} and
Abul Masood-ul-Alam\index{Masood-ul-Alam, A.K.M.} in 1987 \cite{BuntiM87}, while the requirement (iii)
($\Hor$ non-degenerate) has been removed by
Piotr Chru\'sciel\index{Chrusciel, P.T.@Chru\'sciel, P.T.},
Harvey Reall\index{Reall, H.S} and Paul Tod\index{Tod, P.} in 2006 \cite{ChrusRT06}.
Finally the analyticity hypothesis (i), which was implicit in Israel's study, has
been removed in 2010 by
Piotr Chru\'sciel\index{Chrusciel, P.T.@Chru\'sciel, P.T.} and
Gregory Galloway\index{Galloway, G.J.} \cite{ChrusG10}. The generalization to
dimensions $n\geq 4$ has been achieved by Seungsu Hwang\index{Hwang, S.} in 1998 \cite{Hwang98}
and by Gary Gibbons\index{Gibbons, G.W.}, Daisuke Ida\index{Ida, D.}
and Tetsuya Shiromizu\index{Shiromizu, T.} in 2002 \cite{GibboIS02}.
The generalization (\ref{e:sta:Schwarz_Tang}) of Schwarzschild metric to dimensions
$n\geq 4$ has been found by
Frank Tangherlini\index{Tangherlini, F.R.} in 1963 \cite{Tangh63}.
\end{hist}

\begin{prop}[Generalized Israel uniqueness theorem ($n = 4$ electrovacuum)
\index{Israel uniqueness theorem!electrovacuum}]
\label{p:sta:Israel_uniq_electrovac_n4}
Let $(\M,\w{g})$ is a $4$-dimensional static and asymptotically flat spacetime
...
to the the domain of outer communications of (i) a Reissner-Nordström black hole or (ii) a Majumdar-Papapetrou black hole.
The \defin{Reissner-Nordström black hole}\index{Reissner-Nordström!spacetime} is defined by the event horizon being connected and
the existence of a
...
\bea
    ...
    \nonumber \\
    & & + r^2 \left( \dd\th^2 + \sin^2\th\, \dd\ph^2 \right) \label{e:sta:ReiNor_n4}
\eea
and
\be
...
\ee
The three constants $m$, $Q$ and $P$ are respectively the Komar mass at infinity
(or ADM mass), $Q$ the black hole electric charge (cf. Property~\ref{p:sta:electric_charge})
and $P$ its magnetic monopole charge (cf. remark below); they fulfill
$\sqrt{Q^2 + P^2} \leq \sqrt{\frac{4\pi}{\mu_0}}\, m$.
The \defin{Majumdar-Papapetrou black hole}\index{Majumdar-Papapetrou!black hole} is defined by the event horizon being disconnected, with $K\geq 2$ connected components
...
\be \label{e:sta:MajPap_n4}
    \w{g} = - U^{-2} \dd t^2 + U^2 (\dd x^2 + \dd y^2 + \dd z^2)
    \qand
    \w{F} = \pm \sqrt{\frac{\mu_0}{4\pi}} U^{-2} \dd U \wedge \dd t,
\ee
where
\be  \label{e:sta:U_MajPap_n4}
    U := 1 + \sum_{k=1}^K \frac{\mu_k}{\sqrt{(x - x_k)^2 + (y - y_k)^2 + (z - z_k)^2}} .
\ee
This solution is characterized by $K$ constants $\mu_k$, which can be interpreted
as the mass of each ``black hole'' for widely separated configurations,
and $3K$ constants $(x_k, y_k, z_k)$, which are the locations in the $(x,y,z)$ coordinate
system of the $K$ degenerate Killing horizons $\Hor_k$.
Note that each $(x_k, y_k, z_k)$ correspond to
...
expression (\ref{e:sta:MajPap_n4}) for $\w{F}$; in particular it must be the same
for all the components $\Hor_k$'s, i.e. the charges $Q_k$ are either all positive or all negative.
\end{prop}
...
is a particle that
generates the radial magnetic field $\w{B} = \mu_0 q_{\rm m}/(4\pi r^2)) \wpar_r$
in its inertial rest frame, where $q_{\rm m}$ is the magnetic charge.
Although predicted by
some grand unified theories and string theories, no magnetic monopole has
been detected ever and standard electromagnetism postulates that magnetic monopoles
do not exist. From a formal point of view, magnetic monopoles would
...
\ee
where $\uu{j}_{\rm m}$ is the magnetic current density 1-form, which describes
the distribution of magnetic monopoles.
In the black hole framework, we are considering the source-free Maxwell equations
$\dd \w{F} = 0$ and $\dd\star\!\w{F} = 0$, which are perfectly symmetric
in terms of $\w{F}$ and $\star\!\w{F}$. As the Reissner-Nordström solution
(\ref{e:sta:ReiNor_n4})-(\ref{e:sta:F_ReiNor_n4}) shows, a black hole with a non-vanishing
magnetic monopole charge can exist even if magnetic monopoles are excluded
as elementary particles.
\end{remark}

\begin{remark}
The Majumdar-Papapetrou metric (\ref{e:sta:MajPap_n4})-(\ref{e:sta:U_MajPap_n4})
with $K=1$ is still a valid static solution of the electrovacuum Einstein equation. It is
actually a member of the first family given by the uniqueness theorem, namely
the Reissner-Nordström one. More precisely, it is a \defin{extremal Reissner-Nordström
spacetime}, i.e. a solution (\ref{e:sta:ReiNor_n4})-(\ref{e:sta:F_ReiNor_n4}) for which $Q = \sqrt{4\pi/\mu_0} \, m$ and $P=0$, or equivalently a
Reissner-Nordström solution with a degenerate event horizon.
\end{remark}

\begin{prop}[Generalized Israel uniqueness theorem ($n\geq 5$ electrovacuum)
\index{Israel uniqueness theorem!electrovacuum}]
...
\end{itemize}
then the domain of outer communications $\langle\langle \M\rangle\rangle$ of $(\M,\w{g})$ is isometric
to the domain of outer communications of a $n$-dimensional Reissner-Nordström spacetime\index{Reissner-Nordström!spacetime} with vanishing magnetic monopole, i.e. there exists a coordinate system ... TBC ...
\end{prop}
...
\textnormal{(Carter\index{Carter, B.} 1971 \cite{Carte71}, Robinson\index{Robinson, D.C.} 1975 \cite{Robin75})}]
...
solution of the vacuum Einstein equation (\ref{e:fra:vac_Einstein}) with a connected regular
...
\item is stationary
\item is a solution of the vacuum Einstein equation\index{Einstein!equation!vacuum --}\index{vacuum!Einstein equation} (\ref{e:fra:vac_Einstein})
