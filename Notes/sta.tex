...
spacetime stationary action $\M \to \M$, $p \mapsto \Phi_t(p)$
induces a 1-dimensional group action $\Sp \to \Sp$, $p \mapsto \Psi_t(p)$
such that each $\Psi_t$ is an isometry of $(\Sp,\w{g})$.
For $t\in\R$ and $p\in\Sp$, let us define $\Psi_t(p)$ as follows: the point
