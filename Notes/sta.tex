...
the \defin{Komar mass of}\index{Komar!mass}\index{mass!Komar --} $\Sp$ is
defined by
...
Eq.~(\ref{e:sta:M_Komar_partial_der}) to evaluate the Komar mass of $\Sp$.
...
\eea
In the second line, we have expressed the derivative of $\w{n}$ in terms of the extrinsic curvature
...
\bea
    \nabla_\mu V^\mu  & = & \frac{1}{\sqrt{-g}} \der{}{x^\mu} \left( \sqrt{-g} V^\mu \right)
    = \frac{1}{N \sqrt{\gamma}} \der{}{x^i} \left( N \sqrt{\gamma} V^i \right) \nonumber \\
    & = & \frac{1}{\sqrt{\gamma}} \der{}{x^i} \left( \sqrt{\gamma} V^i \right)
      + V^i \der{}{x^i} \ln N
    = D_i V^i + V^i \der{}{x^i} \ln N  ,  \nonumber
\eea
...
an ``internal'' one, $\Sp_{\rm int}$, and an ``external'' one,
$\Sp_{\rm ext}$, the exterior direction being that of the asymptotic flat end of $(\M, \w{g})$.
The Komar masses of $\Sp_{\rm ext}$ and $\Sp_{\rm int}$ are then
related by
...
\begin{example}[Komar mass of a fluid star]
We may use formula (\ref{e:sta:Komar_mass_vol_integ})
to check that at the Newtonian limit, the Komar mass of
a surface $\Sp_{\rm ext}$ surrounding a star
reduces to the volume integral of the matter mass density,
i.e. to the Newtonian expression of mass\index{Newtonian!mass}\index{mass!Newtonian --}.
To this aim, we assume that the spacetime is strictly static, i.e. that $\w{\xi}$
is timelike in all $\M$ and is hypersurface-orthogonal.
...
$\Delta \Phi = 4\pi \rho$ that is vanishing at infinity. We may then write
\be
...
term can be
neglected in front of the first term.
\end{example}
...
an asymptotically flat stationary spacetime share the same Komar mass:
\be
