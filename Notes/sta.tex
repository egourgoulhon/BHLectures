...
and $\sqrt{\gamma}\,\D x^1 \cdots \D x^{n-1}$ is the volume element on $\Sigma$
induced by $\w{g}$.
...
In an asymptotically flat and stationary 4-dimensional spacetime, if $\dd\uu{\xi}$
...
\be \label{e:sta:Komar_angul_vol_integ_R}
    ...
    \int_{\Sigma} \w{R}(\w{n}, \w{\eta}) \sqrt{\gamma} \, \D^{n-1} x  , }
\ee
where $\w{R}$ is the Ricci tensor of $\w{g}$,
$\w{n}$ is the future-directed unit normal to $\Sigma$
and $\sqrt{\gamma}\,\D x^1 \cdots \D x^{n-1}$ is the volume element on $\Sigma$
