...
give only a sketch here for $n=4$, refering to Ref.~\cite{HawkiE73} (Proposition~9.3.6)
...
two points on $\Hor$ if they belong to the same null geodesic generator. Given the
...
the spacetime metric $\w{g}$ on $\Sp_0$. Now the spacetime stationary action $(\R, +)$,
$\M \to \M$, $p \mapsto \Phi_t(p)$
induces an isometry of $(\hat{\Sp},\hat{\w{q}})$ as follows.
