...
John L. Friedman\index[pers]{Friedman, J.L.}, Kristin Schleich\index[pers]{Schleich, K.}
and Donald M. Witt\index[pers]{Witt, D.M.} in 1993 \cite{FriedSW93}.
...
by J. Ehlers\index[pers]{Ehlers, J.} and J.L. Stachel\index[pers]{Stachel, J.L.} and published in 1969 \cite{Boyer69}.
\end{hist}
...
and \ref{p:sta:topology2}), which has been established 5 years later by Stephen Hawking\index[pers]{Hawking, S.W.} \cite{Hawki72}, so it can be removed from the hypotheses of the theorem.
...
has been removed as unecessary by Gary Bunting\index[pers]{Bunting, G.L.} and
...
In this section, we restrict ourself to the standard spacetime dimension $n=4$.
Basically the strong ridigity theorem (Properties~\ref{p:sta:strong_rigidity_thm} and \ref{p:sta:axisymmetry_BH})
states that any stationary black hole spacetime obeying the (electro)vacuum Einstein
equation has to be axisymmetric.
A crucial step towards the no-hair theorem is then the following uniqueness theorem
for stationary and axisymmetric black holes:
...
Let $(\M,\w{g})$ be a 4-dimensional stationary and asymptotically flat spacetime
containing a black hole of event horizon $\Hor$. If
\begin{itemize}
\item $\w{g}$ fulfills the vacuum Einstein equation\index{Einstein!equation!vacuum --}\index{vacuum!Einstein equation} (\ref{e:fra:vac_Einstein}),
\item $\Hor$ is connected,
\item there is no closed causal (i.e. null or timelike) curve in the domain of outer communications $\langle\langle \M\rangle\rangle$,
\end{itemize}
...
However, this non-degeneracy
hypothesis can be relaxed \cite{ChrusN10} (see \cite{ChrusLH12} for an
extended discussion).
\end{remark}
...
\item $\Hor$ is connected,
\item $\Hor$ is rotating (i.e. the stationary Killing vector $\w{\xi}$ is
spacelike on some parts of $\Hor$, cf. Property~\ref{p:sta:xi_tangent_H}),
\item there is no closed causal (i.e. null or timelike) curve in the domain of outer communications $\langle\langle \M\rangle\rangle$,
\end{itemize}
then $\langle\langle \M\rangle\rangle$ is isometric
to the domain of outer communications of the
...
and $J$, the latter being the total angular momentum. We shall not give
...
\begin{remark}
The non-rotating case has been excluded from the hypotheses of Property~\ref{p:sta:Bunting_Mazur_thm} for it may lead to a solution that does not belong
to the Kerr-Newman family, namely the Majumdar-Papapetrou one
(cf. Property~\ref{p:sta:Israel_uniq_electrovac_n4}).
\end{remark}
...
Carter could reduce the 4-dimensional stationary-axisymmetric
...
Then Carter has been able to show that the solutions
...
such a family and the only one containing the Schwarzschild solution, but
Carter could not exclude that other families exist. This step was achieved
by David Robinson\index[pers]{Robinson, D.C.} in 1975 \cite{Robin75}, who
could show by some \emph{tour de force} \cite{Carte99}
...
has been treated by Piotr Chru\'sciel\index[pers]{Chrusciel, P.T.@Chru\'sciel, P.T.}
and Luc Nguyen\index[pers]{Nguyen, L.} in 2010 \cite{ChrusN10}.
They showed that electrovaccuum rotating black holes with a degenerate horizon
necessarily belong to the Kerr-Newman family (they are the so-called
\emph{extremal}\index{extremal!Kerr-Newman spactime} members of that family).
...
been discovered by Ezra Newman\index[pers]{Newman, E.T.} and his collaborators in 1965 \cite{Newma_al65}, two years after the discovery of the Kerr solution by Roy Kerr\index[pers]{Kerr, R.P.} \cite{Kerr63}. A historical note about the Kerr discovery is to be found
in Chap.~\ref{s:ker}, p.~\pageref{h:ker:Kerr_sol}.
\end{hist}

\subsection{The no-hair theorem} \label{s:sta:the_no-hair_thm}

We are now is position to state the famous
...
\item is stationary,
\item fulfils the electrovacuum Einstein equation\index{electrovacuum Einstein equation}\index{Einstein!equation!electrovacuum --} (\ref{e:fra:electrovac_Einstein}) (with the vacuum Einstein equation\index{Einstein!equation!vacuum --}\index{vacuum!Einstein equation} (\ref{e:fra:vac_Einstein}) being considered as a subcase),
...
\item is analytic,
\end{itemize}
has $\langle\langle \M\rangle\rangle$ isometric
...
In the case (i) ($\Hor$ rotating), we may invoke the strong rigidity
...
to the domain of outer communications of a Kerr-Newman spacetime.
In the case (ii) ($\Hor$ is non-rotating and
non-degenerate), we
...
is excluded since $\Hor$ is assumed non-degenerate).
\end{proof}

\begin{hist}
See the historical accounts by Carter \cite{Carte99} and Robinson \cite{Robin09}.
\end{hist}
