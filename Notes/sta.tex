...
the axisymmetric Killing vector $\w{\eta}_i^{(k)}$ over
any any cross-section of $\Hor_{(k)}$
...
were a unit timelike vector, $\w{E}$ would be a genuine electric field: the one measured
...
of the electric charge $Q$ (see e.g. Eq.~(18.40) of Ref.~\cite{Gourg13}), while the second equality results from Lemma~\ref{p:sta:flux_integ_2form}. Now, using expression (\ref{e:sta:dS_chi_k})
for $\D S^{\mu\nu}$, we have
\bea
    F_{\mu\nu} \, \D S^{\mu\nu}
    & = & F_{\mu\nu} (\chi^\mu k^\nu - k^\mu \chi^\nu) \sqrt{q}\, \D^{n-2} x
