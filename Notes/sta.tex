...
adapted to staticity (hence their name \emph{global static coordinates}),
since the metric (\ref{e:neh:AdS4}) is of the form (\ref{e:sta:static_metric})
...
The Schwarzschild spacetime\index{Schwarzschild!spacetime}, introduced in Example~\ref{x:def:Schw_hor} of Chap.~\ref{s:def}
and used throughout Chaps.~\ref{s:def} and \ref{s:neh}, is static, but not
strictly static. The staticity is not obvious from the metric components
...
given that $g_{tr} \neq 0$. However, we shall consider coordinates adapted to
staticity in Chap.~\ref{s:sch}: the so-called \emph{Schwarzschild-Droste coordinates}.
That Schwarzschild spacetime is not strictly static can be read directly on
