...
the metric dual of a vector field $\w{J}$ that is called the
...
we get the following identity, valid for any Killing vector:
\be
...
\be \label{e:sta:Komar_mass_vol_integ_R}
    ...
    \int_{\Sigma} \w{R}(\w{n}, \w{\xi}) \sqrt{\gamma} \, \D^{n-1} x , }
\ee
...
tensor $\w{T}$ and a vanishing cosmological constant, then we may rewrite
...
\begin{proof}
As for the proof of Property~\ref{p:sta:Komar_mass_asymp_metric}, we shall
use formula (\refl{e:sta:J_Komar_partial_der}) to evaluate
$J_{\Sp}$. The difference with the proof of Property~\ref{p:sta:Komar_mass_asymp_metric}
\end{proof}


The volume integral formula (\ref{e:sta:Komar_mass_vol_integ}) for the
