...
We shall call a \defin{weekly relativistic system}\index{weekly relativistic}
a $n$-dimensional spacetime $(\M,\w{g})$
such that $\M$ is diffeomorphic to $\R^n$, the metric tensor can be expressed as
\be \label{e:sta:weak_g_f_h}
    \w{g} = \w{f} + \w{h} ,
\ee
where $\w{f}$ is a flat Lorentzian metric on $\M$ and $\w{h}$ is small in the following
sense: in any \emph{Minkowskian} coordinates\index{Minkowskian coordinates} for $\w{f}$, i.e. coordinates
$(x^\alpha)$ such that\footnote{We keep the notation $\eta$ for the matrix $\mathrm{diag}(-1,1,\ldots,1)$, so that
$(\eta_{\alpha\beta})$ stands for the components of $\w{f}$ in Minkowskina coordinates only.}
$(f_{\alpha\beta}) = \eta := \mathrm{diag}(-1,1,\ldots,1)$, the components
of $\w{h}$ obey $|h_{\alpha\beta}| \ll 1$. Furthermore, we assume that $\w{g}$
is ruled by the Einstein equation (\ref{e:fra:Einstein_eq}) with $\Lambda=0$
and a compactly-supported energy momentum tensor $\w{T}$ in which the
spatial stresses are dominated by the energy and momentum densities, i.e.
such that $|T_{ij}| \ll \mathrm{min}(|T_{00}|,|T_{0k}|)$ in Minkowkian coordinates.
Given the flat metric $\w{f}$, the Minkowskian coordinates $(x^\alpha)$ on $\M$
are not unique: any Poincaré transformation\index{Poincaré!transformation}
...
The flat metric $\w{f}$ on $\M$, and hence the decomposition (\ref{e:sta:weak_g_f_h}), is
highly non-unique.
...
\]
where $\zeta_\alpha := f_{\alpha\mu} \zeta^\mu$.
We may recast the above expression of $\w{g}$ as
$\w{g} = \w{f'} + \w{h'}$,
where $\w{f'}$ is the metric whose the components in the coordinates $({x'}^\alpha)$ are\footnote{Note that
the components of $\w{f}$ with respect to the coordinates $({x'}^\alpha)$ are $\eta_{\alpha\beta} - \partial_\alpha \zeta_\beta - \partial_\beta \zeta_\alpha$.}
$\eta_{\alpha\beta}$ (hence $\w{f'}$ is flat) and $\w{h'}$ has the following components with respect to the coordinates $({x'}^\alpha)$:
...
$\w{h}$. This freedom arises from the freedom in the choice of the flat background metric $\w{f}$ in the
...
to ensure that the metric perturbation $\w{h}$ fulfills
\be \label{e:sta:Lorenz_h}
    \encadre{ \partial_\mu \left( f^{\mu\nu} \bar{h}_{\alpha\nu} \right)  = 0 },
\ee
where
\be
    \w{\bar{h}} := \w{h} - \frac{1}{2} h \, \w{f} ,
\ee
where $h$ stands for the trace of $\w{h}$ with respect
to\footnote{the notation $f^{\alpha\beta}$ stands for the components of
the inverse metric $\w{f}^{-1}$; in Minkowskian coordinates,
the matrix $(f^{\alpha\beta})$ is nothing but $\eta$.}
$\w{f}$: $h := f^{\mu\nu} h_{\mu\nu}$.
...
\begin{proof}
From Eq.~(\ref{e:sta:hprime_h_zeta}), we get $h' := f^{\mu\nu} {h'}_{\mu\nu} = h - 2 \partial_\mu \zeta^\mu$,
so that ${\bar{h}}'_{\alpha\beta} = \bar{h}_{\alpha\beta}
- \partial_\alpha \zeta_\beta - \partial_\beta \zeta_\alpha +  \partial_\mu \zeta^\mu f_{\alpha\beta}$.
It follows then that $\partial_\mu \left( f^{\mu\nu} {\bar{h}'}_{\alpha\nu} \right) =  \partial_\mu \left( f^{\mu\nu} \bar{h}_{\alpha\nu} \right) - \Box_{\w{f}} \zeta_\alpha$,
...
\be \label{e:sta:Box_zeta}
    \Box_{\w{f}} \zeta_\alpha = \partial_\mu \left( f^{\mu\nu} \bar{h}_{\alpha\nu} \right)
\ee
...
to the condition defining \defin{harmonic coordinates}\index{harmonic coordinates},
i.e.
\be
   \Box_{\w{g}} x^\alpha = 0 \iff  \partial_\mu \left( \sqrt{-g} \, g^{\mu\alpha} \right) = 0 ,
\ee
expanded at first order in $\w{h}$.
\end{remark}

\begin{remark}
The Lorenz gauge does not fully determine the pair $(\w{f},\w{h})$, for the solutions of the
d'Alembert equation (\ref{e:sta:Box_zeta}) are highly non unique: they depend on some initial and boundary data.
\end{remark}
...
\ee
where $\Box_{\w{f}}$ stands for the d'Alembertian operator for the metric $\w{f}$.
Let us consider that $(\M,\w{g})$ is a stationary spacetime, with Killing vector
$\w{\xi}$. We may then choose $\w{f}$ such that the MinkowskianO
coordinates $(x^\alpha)$ of $\w{f}$ are adapted to stationarity, i.e.
$x^0$ is an ignorable coordinate, or equivalently
$\wpar_0=\w{\xi}$. Let $\Sigma$ be a hypersurface $x^0 = \mathrm{const}$
and $\w{\gamma}$ the metric induced by $\w{f}$ in $\Sigma$. $(x^i)_{1\leq i \leq n-1}$ is
a Cartesian coordinate system with respect to $\w{\gamma}$:
$\w{\gamma} = \delta_{ij} \dd x^i \otimes \dd x^j = (\dd x^1)^2 + \cdots + (\dd x^{n-1})^2$.
The operator $\Box_{\w{f}}$ reduces then to the Laplace operator of $\w{\gamma}$,
...
\be \label{e:sta:hbar_Green_function}
    ...
    \int_\Sigma \frac{T_{\alpha\beta}(\bar{x})}{|x - \bar{x}|^{n-3}} \, \D^{n-1} \bar{x},
\ee
where $x:=(x^1, \ldots, x^{n-1})$, $\bar{x} :=(\bar{x}^1, \ldots, \bar{x}^{n-1})$,
$|x - \bar{x}| := \sqrt{(x^1 - \bar{x}^1)^2 + \cdots + (x^{n-1} - \bar{x}^{n-1})^2}$
and $\Omega_{n-2}$ is the area of the sphere $\SS^{n-2}$, which is given by the formula
$\Omega_{n-2} = 2\pi^{(n-1)/2}/\Gamma((n-1)/2)$, with $\Gamma(u) := \int_0^{+\infty} t^{u-1} \mathrm{e}^{-t} \D t$,
so that
\be
   \Omega_2 = 4\pi,\quad \Omega_3 = 2\pi^2 \qand  \Omega_4 = \frac{8}{3} \pi^2 .
\ee
Far from the matter sources (i.e. far from the region where $\w{T}\neq 0$), one may perform an expansion
of $1/|x - \bar{x}|^{n-3}$ in powers of $\bar{x}^i/r$, where $r:=\sqrt{(x^1)^2 + \cdots + (x^{n-1})^2}$:
\[
    \frac{1}{|x - \bar{x}|^{n-3}} = \frac{1}{r^{n-3}} \left( 1 + (n-3) \sum_{i=1}^{n-1} \frac{x^i}{r} \frac{\bar{x}^i}{r}
        + \cdots \right) .
\]
Formula~(\ref{e:sta:hbar_Green_function}) becomes then
\be
    {\bar{h}}_{\alpha\beta}(x)  = \frac{16\pi}{\Omega_{n-2}}
        \left( \frac{1}{(n-3)r^{n-3}} \int_\Sigma T_{\alpha\beta}(\bar{x}) \, \D^{n-1} \bar{x}
        + \frac{x^i}{r^{n-1}}  \int_\Sigma \bar{x}^i T_{\alpha\beta}(\bar{x}) \, \D^{n-1} \bar{x}
        + \cdots \right) .
\ee
Hence
\be
    {\bar{h}}_{00} = \frac{16\pi}{\Omega_{n-2}}
\ee
