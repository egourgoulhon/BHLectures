...
We shall call \defin{weakly relativistic}\index{weekly relativistic}
...
\subsection{Asymptotic expression of the metric in a stationary asymptotically flat spacetime}
...
the generic solution of $\Delta f = 0$ (harmonic function) and the Lorenz
...
Going beyong the first order expansion in $\w{h}$ does not spoil
the first terms in the expansions (\ref{e:sta:metric_weakly_relat}) of
the components of $\w{g}$. It simply adds terms with a higher power
of $1/r$. For instance, the second term in the expansion
of $g_{00}$ becomes of order $1/r^{n-2}$ and not $1/r^{n-1}$ as in
Eq.~(\ref{e:sta:g00_weakly_relat}) (see the above references for details).
We may then assert:
...
\end{align}
\end{subequations}
where $M$ is a constant and $(J_{ij})$ is an antisymmetric constant $(n-1)\times(n-1)$
matrix.
\end{prop}
...
a $(n-2)$-dimensional surface at large $r$. In order to have a coordinate-independent
...
We shall check below (Example~\ref{x:sta:Komar_mass_weakly_relat} on p.~\pageref{x:sta:Komar_mass_weakly_relat}) that, for a weakly relativistic system, the Komar mass
(\ref{e:sta:def_Komar_mass}) reduces to the mass defined by Eq.~(\ref{e:sta:mass_weakly_relat}).
...
\begin{example}[Komar mass of a weakly relativistic system]
\label{x:sta:Komar_mass_weakly_relat}
Let $(\M,\w{g})$ be a $n$-dimensional spacetime that is stationary
(Killing vector $\w{\xi}$) and weakly relativistic
in the sense defined in Sec.~\ref{s:sta:weakly_relativistic}. Let
$(\w{f},\w{h})$ be a decomposition of $\w{g}$ fulfilling the Lorenz gauge
and ($x^\alpha$) an associated $\w{f}$-Minkowskian coordinate system, adpated
to stationarity: $\w{\xi} = \wpar_0$.
Let us evaluate the Komar via the integral (\ref{e:sta:M_Komar_partial_der}) taken on
a sphere $\Sp$ of constant $(x^0,r)$ with $r := \sqrt{(x^1)^2 + \cdots + (x^{n-1})^2}$,
in the limit $r\to\infty$, denoting the surface element of $\Sp$ by $\sqrt{q} \, \D^{n-2} y$
instead of $\sqrt{q} \, \D^{n-2} x$ in Eq.~(\ref{e:sta:M_Komar_partial_der}) to avoid any
confusion with the $\w{f}$-Minkowskian coordinates $(x^\alpha)$.
We have $\xi_\alpha = g_{\alpha\mu} \xi^\mu = g_{0\alpha}$,
with $g_{0\alpha}$ given by formulas (\ref{e:sta:metric_weakly_relat}) for large $r$,
so that
\[
...
\]
where $M$ is the mass defined by the volume integral (\ref{e:sta:mass_weakly_relat}).
...
\begin{align}
    ...
    = \frac{M}{\Omega_{n-2}} \int_{\Sp} \underbrace{\frac{x^i}{r} \frac{x^i}{r}}_{1}
     ...
     & = \frac{M}{\Omega_{n-2}} \underbrace{ \int_{\Sp} \sqrt{\bar{q}} \, \D^{n-2} y }_{\Omega_{n-2}} .
       \nonumber
\end{align}
Hence, we get $M_{\Sp} = M$, which shows that for a weakly relativistic system the Komar mass reduces
to the integral of the matter energy density $T_{00}$, as given by formula (\ref{e:sta:mass_weakly_relat}).
\end{example}
