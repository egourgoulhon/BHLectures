...
for an electric charge: they are Majumdar-Papapetrou black holes\index{Majumdar-Papapetrou black hole} mentioned in
...
case is done explicitely in Hollands, Ishibashi \& Wald's work \cite{HollaIW07}.
\end{hist}
...
spacetime must be a Killing horizon. The \emph{strong ridigity theorem} is
...
tha coefficient such that the identity
...
Then the Komar mass (\ref{e:sta:def_Komar_mass}) over any cross section
of $\Hor_{(k)}$ is
...
(cf. Property~\ref{p:sta:Komar_mass_inf}) is
\bea
    M_\infty & = & \frac{n - 2}{2(n- 3)} \sum_{k = 1}^K
    \left(
    \frac{\kappa_{(k)} A_{(k)}}{4\pi}
    + 2  \sum_{i=1}^{L_{(k)}} \Omega_i^{(k)} J_{\Hor_{(k)}}^i \right) \nonumber \\
    & & + 2
...
It is worth to specialize the generalized Smarr formula to $n=4$
and to a connected event horizon:
...
or to the Killing vector $\w{\chi} = \w{\xi} + \Omega_H \w{\eta}$
...
an axisymmetric Killing vector. We shall combine the two cases by stating
that the former corresponds to $\Omega_H = 0$, so that $\w{\chi} = \w{\xi}$.
On $\Hor$, the following identity holds:
$\wnab_{\w{\chi}}\w{\chi} \equalH \kappa \w{\chi}$, which defines the surface
gravity $\kappa$ of $\Hor$. By Property~\ref{p:neh:zeroth_law}, $\kappa$
is a constant. Then the Komar mass (\ref{e:sta:def_Komar_mass}) over any cross section
of $\Hor$ obeys
\be
    M_{\Hor} = \frac{\kappa}{4\pi} A + 2 \Omega_H J_{\Hor} ,
\ee
where $A$ is the area of $\Hor$ (cf. Property~\ref{p:neh:invariance_area})
and $J_{\Hor}$ is the Komar angular momentum with respect to
the axisymmetric Killing vector $\w{\eta}$ as given by Eq.~(\ref{e:sta:def_Komar_J}).
\end{prop}
