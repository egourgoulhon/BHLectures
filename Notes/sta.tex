...
A stronger notion than stationarity is that of \emph{staticity}: an asymptotically
...
Lemma~\ref{p:def:tangent_to_null_hyp} in Sec.~\ref{s:def:spacelike_sections}), we conclude that
...
One has then:
\begin{prop}[staticity theorem\index{staticity theorem} \textnormal{(Sudarsky \& Wald, 1992 \cite{SudarW92})}]
Let us assume that the spacetime dimension is $n=4$ and the metric $\w{g}$
fulfills the Einstein equation. If the stationary Killing vector $\w{\xi}$
is null on $\Hor$, it must be
hypersurface-orthogonal in all $\M$, not only on $\Hor$.
It follows that the spacetime $(\M,\w{g})$ is
\emph{static}\index{static!spacetime}, according to the definition given in Sec.~\ref{s:sta:def_station}.
\end{prop}

Having that $(\M,\w{g})$ is static, we can go further and apply the
...
Assuming that $n=4$ and the metric $\w{g}$ obeys the vacuum Einstein equation,
