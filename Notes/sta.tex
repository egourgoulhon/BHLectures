...
the condition defining \defin{harmonic coordinates}\index{harmonic coordinates}
on $\M$, i.e.
\be
...
d'Alembert equation (\ref{e:sta:Box_zeta}) are highly non unique: they depend on initial and boundary data.
\end{remark}
...
$|x - {x'}| := \sqrt{(x^1 - {x'}^1)^2 + \cdots + (x^{n-1} - {x'}^{n-1})^2}$
...
so that
\be
   \Omega_2 = 4\pi,\quad \Omega_3 = 2\pi^2,\quad  \Omega_4 = \frac{8}{3} \pi^2
   \qand \Omega_5 = \pi^3 .
\ee
Far from the matter sources (i.e. far from the region where $\w{T}\neq 0$), one may perform an expansion
of $1/|x - {x'}|^{n-3}$ in powers of ${x'}^i/r$, where $r:=\sqrt{(x^1)^2 + \cdots + (x^{n-1})^2}$:
\[
    \frac{1}{|x - {x'}|^{n-3}} = \frac{1}{r^{n-3}} \left( 1 + (n-3) \sum_{i=1}^{n-1} \frac{x^i}{r} \frac{{x'}^i}{r}
        + \cdots \right) .
\]
...
\be
    {\bar{h}}_{\alpha\beta}(x)  = \frac{16\pi}{\Omega_{n-2}}
        ...
        + \frac{x^i}{r^{n-1}}  \int_\Sigma {x'}^i T_{\alpha\beta}({x'}) \, \D^{n-1} {x'}
        + \cdots \right) .
\ee
We may then use some of the degrees of freedom within the Lorenz gauge (cf. Remark~\ref{r:sta:freedom_Lorenz})
to consider that the coordinates $(x^\alpha)$ correspond to the \emph{central source rest-frame}
...
Definining the \defin{mass} of the central source as
\be
    M := \int_\Sigma T_{00}({x'}) \, \D^{n-1} {x'}
\ee
the component $00$ of the above equation yields
\be
