...
Here, we shall simply state, following
...
We shall assume that this holds for black holes.
In dimension 4, this is guaranted by the topology theorem:
...
and (ii) the following ``outermost'' property holds: when
displaced into the black hole region along the ingoing null normal $\w{k}$, the horizon cross-sections become surfaces with $\theta_{(\wl)} < 0$.
...
negative by hypothesis. By taking the limit $\eps\to 0$, we form the
...
Gaussian curvature\index{Gaussian curvature}.
The signs of the terms forming the integrand in the right-hand side
...
Equation~(\ref{e:sta:Euler_char}) yields $\chi > 0$. Given that the only orientable 2-manifolds
with positive Euler characteristic are homeomorphic to $\SS^2$ or
to some disjoint union of $\SS^2$'s, the theorem follows.
\end{proof}

Another version of the topology theorem relies on the null energy
condition, which is weaker than the dominant energy condition, and replaces the
assumption $\Lie{\w{k}} \theta_{(\wl)} < 0$ by other hypotheses:
...
Let $(\M, \w{g})$ be a 4-dimensional asymptotically flat stationary spacetime
containing a black hole region of event horizon $\Hor$.
...
any connected component of $\Sp$ is homeomorphic to a 2-sphere.
\end{prop}
...
version given by Hawking \cite{Hawki72}, for it adds the ``outermost'' hypothesis (ii).
...
for future outer trapping horizons.
The version of theorem obtained by Piotr Chru\'sciel\index{Chrusciel, P.T.@Chru\'sciel, P.T.}
