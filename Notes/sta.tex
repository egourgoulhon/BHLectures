...
$\w{\xi}$. We may then choose $\w{f}$ such that the Minkowskian
...
\be
    ...
    \int_\Sigma \frac{T_{\alpha\beta}(y)}{|x - y|^{n-3}} \, \D^{n-1} y ,
\ee
where $x:=(x^1, \ldots, x^{n-1})$, $y :=(y^1, \ldots, y^{n-1})$,
$|x - y| := \sqrt{(x^1 - y^1)^2 + \cdots + (x^{n-1} - y^{n-1})^2}$
and $\Omega_{n-2}$ is the area of the sphere $\SS^{n-2}$.
