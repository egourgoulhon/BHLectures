...
\be \label{e:sta:M_Komar_partial_der}
    \encadre{ M_{\Sp} = -\frac{1}{8\pi} \int_{\Sp} (s^\mu \partial_\mu \xi_\nu \, n^\nu - n^\mu \partial_\mu \xi_\nu \, s^\nu)
    \sqrt{q}\, \D x^2 \, \D x^3 } .
\ee
...
introduced in
Example~\ref{x:def:Schw_hor} of Sec.~\ref{s:def:geom_null_hypsurf} (p.~\pageref{x:def:Schw_hor}).
...
the vacuum case. To this aim, we shall make use of the following lemma:
\begin{lemma}[flux integral of the divergence of a 2-form]
In a 4-dimensional spacetime asymptotically flat spacetime $(\M, \w{g})$,
...
tensor $\w{K}$ of $\Sigma$ and the lapse function $N := - \w{n} \cdot \wpar_t$
