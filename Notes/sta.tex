...
is the unique pair $(N, \w{\beta})$ where $N$ is a scalar field on $\Sigma$
...
and $N$ is minus the scalar product of $\w{n}$ and $\w{\xi}$:
$\w{\beta} = \w{\xi} + (\w{n}\cdot\w{\xi}) \w{n}$ and
$N = - \w{n}\cdot\w{\xi}$. In the language of the 3+1 formalism of general
...
The 3+1 decomposition of the energy-momentum tensor $\w{T}$ with respect
to the hypersurface $\Sigma$ is constituted by the unique
3-tuple $(E, \w{p}, \w{S})$, where $E$ is a scalar field,
