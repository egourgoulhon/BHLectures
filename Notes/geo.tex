...
A less trivial example is the famous Killing tensor of Kerr spacetime, discussed in
Chap.~\ref{s:gek}.
\end{example}
...
\bea
 \frac{\D}{\D\lambda}  \left( \w{K}(\w{v},\ldots,\w{v}) \right) & = &
    \wnab_{\!\w{v}} \left( \w{K}(\w{v},\ldots,\w{v}) \right) = v^\mu \nabla_\mu (K_{\nu_1\ldots\nu_p} v^{\nu_1}\cdots v^{\nu_p}) \nonumber \\
            ...
            & = & 0 , \nonumber
\eea
where the first zero results from (\ref{e:geo:Kvv_const}), while the
...
For the Killing tensor $\w{K}$ of Kerr spacetime, the conserved quantity
$\w{K}(\w{v},\w{v})$ is Carter constant\index{Carter!constant} (cf. Chap.~\ref{s:gek}).
\end{example}
