...
$P_{\w{V}}(0) = p$.
If $t\not=0$, the nonzero vector $t\w{V}$ is colinear to $\w{V}$ and the uniqueness
...
By definition (cf. Sec.~\ref{s:bas:embed}), since $\exp_p: \mathcal{E}_p \subset T_p\M \to \M$
and $\exp_p(0) = p$, the differential $\left.\D\exp_p\right| _0$ carries an
...
Hence
\[
    X^\alpha(\lambda) = \langle \w{E}^\alpha, \lambda \w{V} \rangle
...
namely, in a pure \emph{Riemannian} manifold, geodesics are
...
the stationary length corresponds to a \emph{maximum}, according to
...
\end{greybox}
We shall not provide a full proof here, refering the reader to
