...
a \defin{curve}\index{curve}\footnote{As already mentioned in Remark~\ref{r:bas:curve_def} p.~\pageref{r:bas:curve_def}, in the mathematical literature, it is common to define
a curve as the parametrization itself, and not as its image.}
is the image $\mathcal{C}=P(I)$
...
\begin{greybox}
In a pseudo-Riemannian manifold $(\M,\w{g})$, a geodesic $\mathcal{C}$ belongs necessarily
...
$\wnab_{\!\w{v'}}\w{v'} = 0$. Since the accelerations of any two parametrizations of $\mathcal{C}$
...
The curve $\mathcal{C}$ is a geodesic iff there exists a parametrization of $\mathcal{C}$
...
of $\mathcal{C}$ fulfils the following
...
\ee
Moreover, this geodesic is unique, in the sense that any geodesic $\mathcal{C}'$
...
\be
    ...
    \left\{ \begin{array}{ll}
        p &  \mbox{if}\ \w{V}=0 \\
        P_{\w{V}}(1) \in \mathcal{C}_{\w{V}} & \mbox{if}\ \w{V}\not=0
        \end{array} \right.
    \end{array}
\ee
\end{greybox}
...
implies that $\mathcal{C}_{t\w{V}} = \mathcal{C}_{\w{V}}$. By virtue
of the transformation law (\ref{e:geo:change_tangent_vector}), $t\w{V}$ is
the tangent vector to $\mathcal{C}_{\w{V}}$ corresponding to the affine parameter
...
$\vp{P_{\w{V}}(0) P_{\w{V}}(\varepsilon)}$ along the geodesic $\mathcal{C}_{\w{V}}$
...
$\mathcal{C}_{\w{V}}$ through $p$ admitting $\w{V}\in T_p\M$ as tangent
vector at $p$ is (as long as $\mathcal{C}_{\w{V}}$ remains in the normal
...
\end{greybox}
\begin{proof}
Let $\lambda$ be the affine parameter of $\mathcal{C}_{\w{V}}$ corresponding
to the parametrization $P_{\w{V}}$. The coordinate equation of
$\mathcal{C}_{\w{V}}$ is then $x^\alpha = X^\alpha(\lambda)$ with [cf. Eq.~(\ref{e:geo:def_normal_coord})]
\[
...
The tangent vector field to the geodesic $\mathcal{C}_{\w{V}}$ corresponding
...
From the very definition of $L_{(p,q)}(\mathcal{C})$, it is obvious that
\be
    L_{(p,q)}(\mathcal{C}) \geq 0 .
\ee
...
\be \label{e:geo:C_null_L_zero}
    \mathcal{C}\mbox{ null } \Longrightarrow L_{(p,q)}(\mathcal{C}) = 0 .
\ee
...
variational problem, i.e. to consider $L_{(p,q)}(\mathcal{C})$ as an ``action''
...
but this time, $\mathcal{C}$ is assumed to be a spacelike geodesic from $p$ to $q$. Since $\mathcal{C}$ is necessarily a straight line
...
$(\mathcal{C}_h)_{h\in(-1,1)}$ lying in the orthogonal complement of $\Sigma$
...
a variational principle; one has to start from a different action, namely the
``energy'' functional:
\be \label{e:geo:energy_funct}
    E_{(p,q)}(P) := \int_{\lambda_p}^{\lambda_q}
        g_{\mu\nu}(X^\rho) \dot{X}^\mu \dot{X}^\nu \, \D\lambda ,
\ee
where $P$ is a parametrization of the curve $\mathcal{C}$, $\lambda$
the corresponding parameter
and $x^\alpha = X^\alpha(\lambda)$ the coordinate expression of $P$.

The Lagrangian in (\ref{e:geo:energy_funct}) is always differentiable, even
when $g_{\mu\nu}\dot{X}^\mu \dot{X}^\nu  = 0$, i.e. it allows for null curves.
However the price to pay is that,
contrary to the length (\ref{e:geo:def_length_X}),
the ``energy'' depends on the parametrization of the curve,
hence the notation $E_{(p,q)}(P)$ rather than $E_{(p,q)}(\mathcal{C})$.
For this reason, $E_{(p,q)}(P)$ is not expected to have any significant
physical meaning, contrary to $L_{(p,q)}(\mathcal{C})$, which is the proper
time along timelike curves. In particular, the name ``energy''
arises simply because of the quadratic dependencies in the ``velocity'' variables $\dot{X}^\alpha$,
as in a kinetic energy.

Searching for stationary points of the action
(\ref{e:geo:energy_funct}) is straightforward (cf. the computation below
Eq.~(\ref{e:geo:Euler-Lagrange})): the Euler-Lagrange equation
turns out to be equivalent to the geodesic equation (\ref{e:geo:eq_geod}).
We conclude that
\begin{greybox}
In a pseudo-Riemannian manifold $(\M,\w{g})$, a
curve $\mathcal{C}$ equipped with a parametrization $P$
is a stationary point of the ``energy'' functional (\ref{e:geo:energy_funct})
iff $\mathcal{C}$ is a geodesic and $P$ an affine parametrization of it.
\end{greybox}
...
vector field of $(\M,\w{g})$, then along any geodesic $\mathcal{C}$,
the $\w{g}$-scalar product of $\w{\xi}$ by any tangent vector field
$\w{v}=\D\w{x}/\D\lambda$ associated with an
affine parameter $\lambda$ of $\mathcal{C}$ is constant:
\be
...
\begin{proof}
The variation of $\w{g}(\w{\xi},\w{v})$ along $\mathcal{C}$ is,
...
Eq.~(\ref{e:geo:geod_eq_v}), which expresses that $\mathcal{C}$ is a geodesic
...
parameter of $\mathcal{C}$, the second zero in Eq.~(\ref{e:geo:dem_conservation})
must be replaced by $\kappa v^\nu$, where $\kappa$ is the non-affinity
coefficient of $\w{v}$ [cf. Eq.~(\ref{e:geo:v_pregeodesic})]. Accordingly
$\w{g}(\w{\xi},\w{v})$ is no longer constant along $\mathcal{C}$ but rather
evolves according to
\be
    \frac{\D}{\D\lambda} \, \w{g}(\w{\xi},\w{v}) = \kappa \, \w{g}(\w{\xi},\w{v}) .
\ee
Note that $\kappa$ a priori varies along $\mathcal{C}$, so that the integration of
