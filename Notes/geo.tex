...
\subsection{All geodesics as stationary points of the ``energy'' functional}


\begin{greybox}
In any pseudo-Riemannian manifold $(\M,\w{g})$, we shall define
the \defin{energy}\index{energy!of a curve parametrization} of the
parametrization $P$ of a curve
$\mathcal{C}$ connecting two points $p$ and $q$ by
\be \label{e:geo:def_length}
    E_{(p,q)}(P) := \int_{\lambda_p}^{\lambda_q}
        \w{g}(\w{v},\w{v}) \, \D\lambda ,
\ee
where $\lambda$ is the parameter of $P$, $\lambda_p$
(resp. $\lambda_q$) being its value at $p$ (resp. $q$) and
$\w{v} = \D\w{x}/\D\lambda$ is the tangent vector field of $P$,
and we assume $\lambda_q \geq \lambda_p$.
\end{greybox}
Contrary to the length introduced in Sec.~\ref{s:geo:length},
