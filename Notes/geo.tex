...
an \defin{autoparallel curve}\index{autoparallel}; it is also often called
...
Note that if a solution of (\ref{e:geo:eq_geod}) is found, the parameter
$\lambda$ is necessarily an affine parameter.
For a generic parameter,
of the (pre)geodesic equation becomes (\ref{e:geo:eq_geod}) with the right-hand side
replaced by $\kappa \D X^\alpha /\D\lambda$, which is the coordinate
expression of the right-hand side $\kappa \, \w{v}$ of
...
of $g_{\alpha\beta}$, we can write the second terms as
\[
    \der{g_{\alpha\mu}}{x^\nu} \dot{X}^\mu \dot{X}^\nu
...
the stationary length corresponds to a \emph{maximum}: any over timelike curve
connecting $p$ and $q$ will have a smaller length. If one interprets timelike
...
For a spacelike geodesic, the stationary length corresponds neither to a
