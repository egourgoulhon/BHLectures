...
of $\Li_s$ associated to $\lambda$ (cf. Sec.~\ref{s:geo:def}). Note that the union of all $\Li_s$ forms a
2-dimensional surface $\Sp$ of $\M$, which is parametrized by $(\lambda,s)$
($\Sp$ is the image of the map $\Phi$ defined above). On $\Sp$,
let us denote by $\w{v}$ and $\w{z}$ the vector fields of the natural basis
associated with the coordinates $(\lambda,s)$:
\be
...
(cf. Fig.~??). Note that the separation vector depends upon the choice of the
...
the \emph{relative velocity} of nearby geodesics and the second order derivative
$\wnab_{\w{v}} \wnab_{\w{v}} \w{z}$ as the \emph{relative acceleration} of nearby
...
and $\w{z}$ is the separation vector at fixed $\lambda$
and $\mathrm{\bf Riem}$ is the
Riemann curvature tensor\index{Riemann!curvature}\index{curvature!tensor} of
$\w{g}$ (cf. Sec.~\ref{s:bas:curvat}).
