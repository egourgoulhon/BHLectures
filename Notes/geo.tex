...
Another way to convince ourselves that null geodesics would not have fit
in the analysis is the division by $\mathcal{L}$ in Eq.~(\ref{e:geo:kappa_from_L}),
which excludes $\mathcal{L} = 0$.
However, it is possible to get all geodesics, including the null ones, from
a variational principle, but starting from a different action, namely the
``energy'' functional:
\be \label{e:geo:energy_funct}
    E_{(p,q)}(P) := \int_{\lambda_p}^{\lambda_q}
        \w{g}(\w{v},\w{v}) \, \D\lambda ,
\ee
where $P$ is a parametrization of the curve $\mathcal{C}$ and
$\lambda$ and $\w{v}$ are the associated parameter and tangent vector,
respectively.

The Lagrangian in (\ref{e:geo:energy_funct}) is always differentiable, even
for null curves. However the price to pay is that,
contrary to the length (\ref{e:geo:def_length}),
the energy depends on the parametrization of the curve $\mathcal{C}$,
hence the notation $E_{(p,q)}(P)$ instead of $E_{(p,q)}(\mathcal{C})$.
\end{remark}
