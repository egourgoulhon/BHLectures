\chapter{Non-expanding horizons and Killing horizons}
\label{s:neh}
...
To model the event horizon of a black hole in equilibrium, it is quite
natural to consider a null hypersurface with a vanishing expansion along
its null normals (cf. Property~\ref{p:def:expansion_indep_cs}) and whose cross-sections are
closed (compact without boundary\footnote{Cf. the discussion in Sec.~\ref{s:def:spacelike_sections}.}, like the sphere or the torus). The vanishing expansion reflects the equilibrium state, while
the closed character of cross-sections enforces the ``localized'' facet
in the naive definition of a black hole (Definition~\ref{d:glo:naive_BH}).

\begin{tdef}{non-expanding horizon}{neh:NEH}
A \defin{non-expanding horizon}\index{non-expanding horizon}\index{horizon!non-expanding --} in a $n$-dimensional spacetime is a null hypersurface $\Hor$ such that
\begin{itemize}
\item[(i)] the expansion of $\Hor$ along any null normal $\wl$ vanishes
identically:
\be
    \theta_{(\wl)} = 0 ;
\ee
\item[(ii)] the topology of $\Hor$ is
\be \label{e:neh:H_topology}
    \Hor \sim \R \times \Sp_0 ,
\ee
where $\Sp_0$ is a closed manifold\index{closed!manifold} of dimension $n-2$,
and all the complete cross-sections of $\Hor$ have the topology of $\Sp_0$.
\end{itemize}
\end{tdef}

\begin{remark}
As stated in Property~\ref{p:def:rescaling_expansion}, any two null normals $\wl$ and $\wl'$ to
$\Hor$ are necessary collinear: $\wl' = \alpha \wl$, with $\alpha\neq 0$, and the expansions of $\Hor$ along
them scale as $\theta_{(\wl')} = \alpha \theta_{(\wl)}$.
Hence $\theta_{(\wl)} = 0$ is equivalent to $\theta_{(\wl')} = 0$. It follows that
the definition of a non-expanding horizon
does not depend on the choice of the null normal.
\end{remark}

\begin{remark}
In their review article \cite{AshteK25},
Ashtekar and Krishnan define a non-expanding horizon by adding a
third condition to (i) and (ii) above, namely (iii) the null normals $\wl$ to $\Hor$
obey
\be
    \w{R}(\wl, .) = f \uu{\el}
    \qquad \left[\mbox{or}\  R_{\mu\alpha} \el^\mu = f \el_\alpha \right],
\ee
where $\w{R}$ is the spacetime Ricci tensor and $f$ is some scalar field on $\Hor$.
This condition ensures that some ``energy conditions'' are fulfilled; for instance, it
implies $\w{R}(\wl, \wl) = 0$ and hence that $\wl$ obeys the null convergence condition (\ref{e:neh:null_energy_cond}). We prefer
the more general definition~\ref{d:neh:NEH} (which sticks to the original
one by H\'aj\'{\i}\v{c}ek, cf. the historical note below) and
to add the appropriate energy condition as an explicit hypothesis
when establishing various properties of non-expanding horizons. Besides,
Definition~\ref{d:neh:NEH} allows Killing horizons (with closed-manifold cross-sections)
to be counted as non-expanding horizons (Property~\ref{p:neh:KH_as_NEH} below).
\end{remark}

Let us stress that Definition~\ref{d:neh:NEH} captures only
the event horizon of black holes in equilibrium,
as we shall discuss in Chap.~\ref{s:sta}.
For a black hole out of equilibrium, one has generically $\theta_{(\wl)} > 0$,
as we shall see in Chap.~\ref{s:evo}.

\begin{texample}{Schwarzschild horizon}{}
The Schwarzschild horizon $\Hor$ considered in many examples of Chap.~\ref{s:def}
has $\theta_{(\wl)} = 0$ [Eq.~(\ref{e:def:theta_Schw_hor})]
and has the topology $\Hor \sim \R\times \SS^2$ (cf. Fig.~\ref{f:def:Schwarz_horizon}),
with its complete cross-sections having the topology $\SS^2$ (cf. Example~\ref{x:def:Schw_hor4}).
Hence $\Hor$ is a non-expanding horizon.
\end{texample}

\begin{texample}{null hyperplane and light cone as counter-examples}{}
...
having non-compact cross-sections (null hyperplane) or nonzero expansions
(light cone).
\end{texample}

\begin{hist}\label{h:neh:NEH}
The concept of non-expanding horizon has been introduced by
Petr~H\'aj\'{\i}\v{c}ek\index[pers]{Hajicek, P.@H\'aj\'{\i}\v{c}ek, P.}
...
\begin{tprop}{area of a non-expanding horizon}{neh:invariance_area}
...
\begin{texample}{Schwarzschild horizon}{neh:Schwarz_hor_area}
...
$\sqrt{q}=4m^2 \sin\th$; formula~(\ref{e:neh:total_area}) then
results in $A = 4 m^2 \times 4\pi$, i.e.
...
\begin{texample}{marginally trapped surfaces on the Schwarzschild horizon}{}
...
\begin{tprop}{invariance of the cross-section metric along the null generators}{neh:invariance_of_2metric}
...
is a priori no unique affine connection on $\Hor$ associated with
...
the spacetime Levi-Civita connection $\wnab$ induces a unique
torsion-free connection ${}^\Hor\!\wnab$ on $\Hor$.
Indeed, let $\w{u}$ and $\w{v}$ be two vector fields on $\Hor$. We have, using
...
We conclude:
\begin{tprop}{induced affine connection}{}
Let $\mathfrak{X}(\Hor)$ be the space of vector fields tangent to the non-expanding horizon $\Hor$.
...
\be \label{e:neh:def_induced_connection}
    ...
    & (\w{u},\w{v}) & \longmapsto & {}^\Hor\!\wnab_{\w{u}}\w{v} := \wnab_{\w{u}} \,\w{v} ,
    \end{array}
\ee
is well-defined, i.e. ${}^\Hor\!\wnab_{\w{u}}\w{v}$  belongs to
$\mathfrak{X}(\Hor)$.
...
\begin{tprop}{horizon-intrinsic derivative of the null normal}{neh:Hnab_l}
Under the hypotheses of Property~\ref{p:neh:invariance_of_2metric},
...
along $\w{k}$ [Eq.~(\ref{e:def:def_proj_H_along_k})]. Since $\wnab_{\w{u}} \wl$
is tangent to $\Hor$, we have
$\langle \w{a}, {}^\Hor\!\wnab_{\w{u}} \wl \rangle = \langle \w{\bar{a}}, \wnab_{\w{u}} \wl \rangle$.
...
${}^\Hor\!\w{\omega}$ remains constant up to the addition of an exact 1-form:
...
stems from the angular momentum of a non-expanding horizon
being given by an integral involving ${}^\Hor\!\w{\omega}$, as we shall see
in Chap.~\ref{s:loc}.

\subsection{Going further}

See Refs.~\cite{AshteK25,GourgJ06,Jaram13,AshteKKL22a} for more
...
\end{itemize}
The \defin{orbit}\index{orbit!under a group action} of a point $p\in\M$ under the group action $\Phi$
is the set $\{\Phi_g(p),\ g\in G\}\subset\M$, i.e. the set of points which are connected to $p$ by some transformation belonging to $G$. One says that $p$ is a
...
over $\M$ and setting $\w{\xi} = 0$ at fixed points
of the group action.
...
\begin{tprop}{infinitesimal transformations as displacements along the generator}{}
A 1-dimensional group action limited to infinitesimal transformations of parameter
$\D t$ around the identity ($\D t =0$) amounts to displacements of points of $\M$ by the infinitesimal vector $\D t\, \w{\xi}$, where $\w{\xi}$ is the generator of the group.
\end{tprop}
...
generator associated with the parameter $t$.}
\end{figure}
...
\begin{tprop}{characterization of continuous spacetime isometries}{def_Killing_vector}
...
\begin{tprop}{isometry in adapted coordinates}{neh:isometry_adapt_coord}
...
\begin{tdef}{Killing horizon}{neh:Killing_horizon}
...
It immediately follows from the definition that the Killing vector $\w{\xi}$ is non-vanishing
...
We shall see in Chap.~\ref{s:sta} that in a stationary spacetime ruled by general relativity and under some rather generic
...
\begin{texample}{null hyperplane as a translation-Killing horizon}{neh:transKH}
...
\begin{texample}{null half-hyperplane as a boost-Killing horizon}{neh:boostKH}
...
\begin{texample}{null hyperplane as a null-rotation-Killing horizon}{neh:nullrotKH}
...
\begin{texample}{light cone as a counter-example}{}
The future light cone introduced in Example~\ref{x:def:light_cone} is \emph{not} a
...
\begin{texample}{Schwarzschild horizon}{neh:Schwarz_KH}
...
\begin{tprop}{vanishing of the deformation rate of cross-sections of a Killing horizon}{}
...
\ee
\end{tprop}
In view of Definition~\ref{d:neh:NEH} of a non-expanding horizon,
an immediate corollary is
\begin{tprop}{Killing horizons as non-expanding horizons}{neh:KH_as_NEH}
...
$\wl$ is then a symmetry generator on $\Hor$ and one may expect that $\kappa$
is invariant along $\wl$; this is indeed the case:
\begin{tlemma}{constancy of $\kappa$ along the generators of $\Hor$}{neh:kappa_along_generators}
On any Killing horizon $\Hor$, the non-affinity coefficient $\kappa$ associated
to the Killing vector [cf. Eq.~(\ref{e:neh:xi_nab_xi_kappa})] is constant along the null geodesic generators of $\Hor$, i.e.
...
\begin{proof}
Since $\w{\xi}$ is tangent to $\Hor$, we may take
the Lie derivative along $\w{\xi}$ of the 1-form identity (\ref{e:neh:dxi2_kappa}):
$\Lie{\xi} \left( \dd (\w{\xi}\cdot\w{\xi}) \right) \equalH - 2 \Lie{\xi} ( \kappa \, \uu{\xi})$. Using the Cartan identity (\ref{e:bas:Cartan}) to express the left-hand side and
the Leibniz rule to expand the right-hand side, we get
\[
    \w{\xi} \cdot \underbrace{\dd \dd (\w{\xi}\cdot\w{\xi})}_{0}
    + \dd ( \underbrace{\w{\xi}\cdot\dd(\w{\xi}\cdot\w{\xi})}_{0} )
    \equalH - 2 \Lie{\xi} \kappa \, \uu{\xi} - 2 \kappa \underbrace{\Lie{\xi} \uu{\xi}}_{0}  .
\]
The first zero arises from $\dd\dd = 0$ [Eq.~(\ref{e:ext_der_nilpot})], the second
one from $\w{\xi}$ tangent to $\Hor$ and $\w{\xi}\cdot\w{\xi} \equalH 0$ and
the third one from
$\Lie{\xi} \uu{\xi} = \Lie{\xi}(\w{g}(.,\w{\xi})) = \Lie{\xi}\w{g}(.,\w{\xi}) +
\w{g}(., \Lie{\xi}\w{\xi})=0$, since $\Lie{\xi}\w{g} = 0$ by definition of a Killing vector
and $\Lie{\xi}\w{\xi}=0$ by definition of the Lie derivative. Given that
$\wl \equalH \w{\xi}$ and $\uu{\xi}\neq 0$, this proves (\ref{e:neh:kappa_const_along_L}).
\end{proof}

\begin{remark}
An alternative proof of Lemma~\ref{p:neh:kappa_along_generators} is based on the identity (\ref{e:neh:v_nab_kappa}) below (which follows from expresion~(\ref{e:neh:kappa2}) of $\kappa^2$),
as indicated in the proof of Property~\ref{p:neh:zeroth_law_bifur}.
\end{remark}

Lemma~\ref{p:neh:kappa_along_generators} establishes that $\kappa$ is constant
along any null geodesic generator of $\Hor$, but it may vary from one generator to another.
To prove that this is not the case,
let us consider a complete cross-section
...
for (\ref{e:neh:null_dominant_cond}) is not standard. We are introducing it to
distinguish from the \emph{null dominant energy condition} (\ref{e:neh:null_dominant_cond_T}),
which is a condition on the matter energy-momentum tensor, while (\ref{e:neh:null_dominant_cond})
...
either collinear to $\wl$ or spacelike (cf. Lemma~\ref{p:def:tangent_to_null_hyp}).
...
constant along each null geodesic generator of $\Hor$ (Lemma~\ref{p:neh:kappa_along_generators}), this completes the proof of the constancy
of $\kappa$ over $\Hor$. More precisely, we have established
the following property:

\begin{tprop}{zeroth law of black hole dynamics}{neh:zeroth_law}
...
\begin{texample}{null hyperplane as a translation-Killing horizon}{neh:transKH_kappa}
...
$\kappa = 0$, as already noticed in Example~\ref{x:def:null_hyp3},
which is obviously constant over $\Hor$.
\end{texample}

\begin{texample}{null hyperplane as a boost-Killing horizon}{neh:boostKH_kappa}
...
\begin{texample}{null hyperplane as a null-rotation-Killing horizon}{neh:nullrotKH_kappa}
...
\begin{texample}{Schwarzschild and Kerr horizons}{neh:Schw_Kerr_kappa}
We have found in Example~\ref{x:def:Schw_hor3}
that on a Schwarzschild horizon $\kappa = 1/(4m)$ [Eq.~(\ref{e:def:kappa_Schw_hor})],
which is clearly constant. But
this last feature is rather trivial since the Schwarzschild horizon is spherically
symmetric, so that no dependence of $\kappa$ on $\th$ nor $\ph$ is expected.
...
\begin{texample}{Cubic Galileon black hole as a counter-example}{}
It has been found recently that the surface gravity of stationary rotating black holes
...
\begin{texample}{Killing horizons in Minkowski spacetime}{}
...
\begin{texample}{Schwarzschild and Kerr horizons}{}
...
\begin{texample}{Poincaré horizon in AdS$_{4}$}{neh:AdS}
The 4-dimensional anti-de Sitter spacetime\index{anti-de Sitter spacetime}
$(\M,\w{g})$
has been introduced in Example~\ref{x:glo:AdS}.
The metric tensor expressed in the conformal coordinates
$(\tau,\chi,\th,\ph)$ is given by Eq.~(\ref{e:glo:metrix_AdS_conformal}):
...
Another set of coordinates commonly used in AdS$_{4}$ is \defin{Poincaré coordinates}\index{Poincaré!coordinates on AdS$_n$} $(t,x,y,u)$. They cover only a subpart
...
(\ref{e:neh:def_Poincare_patch}) of $\M_{\rm P}$, $\Hor$ appears to be the level set $u=0$:
...
\subsection{Interpretation of \texorpdfstring{$\kappa$}{κ} as a ``surface gravity''}
...
\begin{tdef}{bifurcate Killing horizon}{}
...
\begin{tprop}{vanishing of the Killing vector on the bifurcation surface}{sta:xi_S_zero}
...
a Killing horizon. Indeed, according to Definition~\ref{d:neh:Killing_horizon} (Sec.~\ref{s:neh:def_Killing_hor}), a Killing horizon
is a regular (i.e. embedded) hypersurface
of $\M$ (cf. Sec.~\ref{s:bas:embed}), while
...
\begin{texample}{Lorentz-boost bifurcate Killing horizon}{sta:bif-KH-boost}
...
\begin{tprop}{affine parametrization of a non-degenerate Killing horizon}{}
...
More precisely, Boyer\index[pers]{Boyer, R.H.} \cite{Boyer69}  proved the following:

\begin{tprop}{Boyer's theorem\index{Boyer's theorem} \textnormal{(1969)}}{sta:Boyer_thm}
...
\begin{tprop}{non-degenerate Killing horizons and bifurcate Killing horizons}{sta:non_degen_bifurcate}
...
as Examples~\ref{x:neh:nullrotKH} and \ref{x:neh:nullrotKH_kappa} (cf. Fig.~\ref{f:neh:hplaneKilling-nullrot}): $\w{\xi}=0$
...
\begin{tprop}{zeroth law\index{zeroth law!of BH dynamics} for bifurcate Killing horizons}{neh:zeroth_law_bifur}
...
\begin{tlemma}{second derivative of a Killing vector}{}
...
\begin{texample}{Lorentz-boost bifurcate Killing horizon}{}
