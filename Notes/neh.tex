...
Killing vector field $\w{\xi}$ such that $\left.\w{\xi}\right|_{\Hor}$ is a
normal to $\Hor$.
\end{greybox}
...
(\ref{e:def:wl_geod_kappa}), we have then
\be
...
\[
    \uu{\xi} \equalH - e^\rho \, \dd u ,
\]
$u$ being a scalar field defining $\Hor$ by $u=0$.
...
field $\w{\xi}$.
However, in actual calculations, one generally prefers to employ
(\ref{e:neh:dxi2_kappa}) for evaluating $\kappa$, because it does not involve
...
the non-affinity coefficient $\kappa$ discussed above is constant.
...
and the latter is called a \defin{degenerate Killing horizon}\index{degenerate!Killing horizon}\index{Killing!horizon!degenerate --};
\item if $\kappa \not=0$, $\w{\xi}$ is only a pregeodesic vector on $\Hor$
(cf. Sec.~\ref{s:def:geod_gener})
and the latter is called a \defin{non-degenerate Killing horizon}\index{non-degenerate!Killing horizon}\index{Killing!horizon!non-degenerate --}.
\end{itemize}
...
i.e. that $\kappa\not=0$.
Let $p\in\Hor$ and $\w{v}\in T_p\M$ be a vector transverse to $\Hor$, i.e.
...
along any direction transerve to $\Hor$ does not vanish. Since
...
of $ \xi_{\alpha}\nabla_\beta\xi_{\gamma}$ denoted by square brackets in the first line.
...
\be \label{e:neh:omega_square2}
    \xi_\mu  \nabla_\nu \xi_\rho \, \xi^\nu  \nabla^\rho \xi^\mu
        ...
        = - V^2 \nabla^\rho V \, \nabla_\rho V  .
\ee
Actually, we notice that each line has one term of type (\ref{e:neh:omega_square1})
...
\[
    ...
    \frac{1}{6} \left[
        ...
        \right] .
\]
We notice that all terms canceal two by two:
\be
    \omega_{\alpha\beta\gamma} \equalH 0 .
\ee
We recover a variant of Frobenius theorem\index{Frobenius!theorem},
...
Let us evaluate the gradient of (\ref{e:neh:omega_square_V}) and take the limit
on $\Hor$:
\bea
    \nabla_\alpha \omega_{\mu\nu\rho} \, \underbrace{\omega^{\mu\nu\rho}}_{0}
 + \underbrace{\omega_{\mu\nu\rho}}_{0} \nabla_\alpha  \omega^{\mu\nu\rho} & \equalH &
  - \frac{1}{3} \underbrace{\nabla_\alpha V^2}_{2\kappa \xi_\alpha}
    \left( \underbrace{\nabla_\mu \xi_\nu \nabla^\mu \xi^\nu}_{-2\kappa^2} + 2 \nabla_\mu V \nabla^\mu V \right)
        \nonumber \\
 &  & -  \frac{1}{3} \underbrace{V^2}_{0}  \nabla_\alpha \left( \nabla_\mu \xi_\nu \nabla^\mu \xi^\nu + 2 \nabla_\mu V \nabla^\mu V \right) ,  \nonumber
\eea
where we have used Eq.~(\ref{e:neh:dxi2_kappa}) in the form $\nabla_\alpha V^2 \equalH 2 \kappa \xi_\alpha$ and expression~(\ref{e:neh:kappa2}) of $\kappa^2$. Hence we are left with
\[
...
The above limit implies then (\ref{e:neh:kappa2_nabV}).
\end{proof}
...
The unit vector field $\w{u}$ can be considered as the 4-velocity
...
\ee
where $\Obs\rightarrow\Hor$ means that the limit is acheived by choosing
the worldline of observer $\Obs$ arbitrarly close to $\Hor$.
...
diverges as $\Obs$ is placed as close as possible to $\Hor$. In that sense,
