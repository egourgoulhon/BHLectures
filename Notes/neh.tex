...
\be \label{e:neh:sigma_square_si}
   \sigma_{ab} \sigma^{ab}
   = \sum_{a=1}^{n-2} \sum_{b=1}^{n-2} (\sigma_{ab})^2 \geq 0 .
\ee
\end{proof}
...
\begin{remark}
The condition (\ref{e:neh:null_energy_cond}) is also called
the \defin{null convergence condition}\index{null!convergence condition}\index{convergence!null -- condition} (e.g. \cite{Senov22,SenovG15}), which reflects better its geometrical
significance, while the connection with the positivity of energy [Eq.~(\ref{e:neh:null_energy_cond_matter})] requires
...
\ee
We conclude:
\begin{prop}[invariance of the cross-section 2-metric along the null generators]
...
holds and we have, according to Eqs.~(\ref{e:def:Rll_Tll}) and (\ref{e:neh:R_l_l_zero}),
\[
    \wl \cdot \w{W} = - \w{T}(\wl, \wl) = - \frac{1}{8\pi} \w{R}(\wl, \wl) = 0 .
\]
...
the zeroth law states that the surface gravity of a black hole in equilibrium is
