...
is usually called the \defin{rotation 1-form}\index{rotation!1-form} or
\defin{connection 1-form}\index{connection!1-form}
...
\be \label{e:neh:zeroth_law_step1}
    ...
    + \kappa  \, \omega_\nu q^\nu_{\ \, \alpha} = R_{\mu\nu} \el^\mu q^\nu_{\ \, \alpha} .
\ee
Let us make the Lie derivative
of the 1-form $\w{\omega}$ along $\wl$
appear via formula (\ref{e:bas:Lie_der_comp_nab})
with $(k,\ell) = (0,1)$:
\bea
    \Liec{\el}\omega_\nu & =  & \el^\mu \nabla_\mu \omega_\nu + \omega_\mu \nabla_\nu \el^\mu
     = \el^\mu \nabla_\mu \omega_\nu + \omega_\mu \left( \Theta_\nu^{\ \, \mu}
     + \omega_\nu \el^\mu - \el_\nu k^\sigma \nabla_\sigma \el^\mu \right) \nonumber \\
     & = & \el^\mu \nabla_\mu \omega_\nu  + \kappa \omega_\nu - \omega_\mu \el_\nu k^\sigma \nabla_\sigma \el^\mu , \nonumber
\eea
where we have expressed $\nabla_\nu \el^\mu$ via Eq.~(\ref{e:def:nab_l_Theta}) and used $\Theta_\nu^{\ \, \mu} = 0$ and $\omega_\mu \el^\mu = \kappa$
[Eq.~(\ref{e:def:omega_l_kappa})].
Given that $\el_\nu q^\nu_{\ \, \alpha} = 0$, Eq.~(\ref{e:neh:zeroth_law_step1})
becomes
\be  \label{e:neh:zeroth_law_step2}
    \Liec{\el}\omega_\nu \, q^\nu_{\ \, \alpha}  - \nabla_\nu \kappa \, q^\nu_{\ \, \alpha}
    = R_{\mu\nu} \el^\mu q^\nu_{\ \, \alpha} .
\ee
It is then judicious to
express $\Lie{\el}\w{\omega}$ in terms of $\Lie{\el}{}^{\Hor}\!\w{\omega}$,
where ${}^{\Hor}\!\w{\omega}$ is the restriction of $\w{\omega}$ to tangent vectors
to $\Hor$ introduced in Sec.~\ref{e:neh:induced_connection}.
Indeed, it has been shown in Property~\ref{p:neh:Hnab_l}
that ${}^{\Hor}\!\w{\omega}$ is a geometric quantity intrinsic to
...
to $\w{v} = \w{v}^\parallel - \langle \uu{\ell}, \w{v} \rangle \w{k}$.
Actually $\w{v}^\parallel = \w{\Pi}(\w{v})$, where $\w{\Pi}$ is the
the projector onto $\Hor$ along $\w{k}$ introduced by Eq.~(\ref{e:def:def_proj_H_along_k}).
We have then
\[
...
\ee
Given that $\el_\nu  q^\nu_{\ \, \alpha}=0$, it follows that the first
term in the left-hand side of Eq.~(\ref{e:neh:zeroth_law_step2}) is zero and we are
left with
\be \label{e:neh:DS_kappa_W}
   \DSc_\alpha \kappa = - R_{\mu\nu} \el^\mu q^\nu_{\ \, \alpha} ,
\ee
where $\DS$ stands for
the covariant derivative in the Riemannian manifold $(\Sp,\w{q})$:
$\DSc_\alpha \kappa := \nabla_\nu \kappa \, q^\nu_{\ \, \alpha}$.
