...
Trapped surface (left) and untrapped surface (right).
}
\end{figure}
...
$\chi$ tend to $\Hor$ when $t\to \pm\infty$, which is expected as well since
...
and $\w{T}=0$ (vacuum). Using the so-called \emph{conformal coordinates} $(x^\alpha)=(\tau,\chi,\th,\ph)$ with
...
Yet another coordinate system commonly used in AdS$_{4}$ is the \defin{Poincaré coordinates}\index{Poincaré!coordinates on AdS$_4$} $(t,x,y,u)$. These coordinates cover only a subpart
...
$z:=\ell^2/u$ instead of $u$, so that the line element becomes
$g_{\mu\nu} \D x^\mu \D x^\nu = \ell^2 \left( - \D t^2 + \D x^2 + \D y^2 + \D z^2 \right)/z^2$,
which is similar to the line element of the Poincaré half-space\index{Poincaré!half-space} model of
...
\be \label{e:neh:AdS:metric_Poincare}
   g_{\mu\nu} \D x^\mu \D x^\nu = \frac{u^2}{\ell^2} \left( - \D t^2 + \D x^2 + \D y^2 \right)
   + \frac{\ell^2}{u^2} \, \D u^2 .
\ee
...
Its normal is given by the gradient of $u$ and we define the vector field
$\w{k} := \vec{\wnab} u$ in all $\M$, so that, on $\Hor$, $\w{k}$ is normal
to $\Hor$.
...
scalar square of $\w{\xi}$ via the components (\ref{e:neh:metrix_AdS_conformal})
and (\ref{e:neh:AdS:def_xi}) of respectively $\w{g}$ and $\w{\xi}$,
getting the simple formula:
\be
    \w{\xi}\cdot\w{\xi} = - \frac{u^2}{\ell^2} .
\ee
One has then $\dd(\w{\xi}\cdot\w{\xi}) = - 2 \ell^{-2} u\, \dd u$. Since
