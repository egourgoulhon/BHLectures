...
$\Sp$ of $\Hor$ and show that $\kappa$ is constant on each connected component of $\Sp$. The starting
...
We note that right-hand side of Eq.~(\ref{e:neh:DS_kappa_W}) is nothing but the
...
vector field tangent to $\Sp$, this implies that
$\kappa$ is constant over $\Sp$ if the latter is connected. If $\Sp$ is
disconnected, the conclusion is that $\kappa$ is constant over any connected component of $\Sp$. Given that $\kappa$ is
constant along each null geodesic generator of $\Hor$, this completes the demonstration
that $\kappa$ is constant over connected components of $\Hor$. More precisely, we have established
...
$\Hor$ is constant over any connected component of $\Hor$:
...
constant and we shall see in Chap.~\ref{s:sta} that (a connected part of)
...
Since $\kappa$ is constant on a connected Killing horizon $\Hor$ (assuming the dominant
energy condition), we may use it to classify connected Killing horizons in two
