...
\be \label{e:neh:isometry_pullback}
    \Phi_t^*\w{g} = \w{g} .
\ee
We may re-express this condition in terms of the Lie derivative as follows:
...
of $(\M,\w{g})$. Equation~(\ref{e:neh:Lie_xi_g}) is equivalent to the
so-called
...
\be \label{e:neh:null_dominant_cond_T}
  \encadre{ \begin{array}{ll}
    \mbox{The vector} \ \w{W} := - \vw{T}(\wl) \ & \mbox{is future-directed null or timelike} \\
    & \mbox{for any future-directed null vector $\wl$}
    \end{array} }_{\rm \, GR},
\ee
...
\be
   \begin{array}{ll}
     \mbox{The vector} \ \w{W} := - \vw{T}(\w{u}) \ & \mbox{is future-directed null or timelike} \\
...
\subsection{Zeroth law for bifurcate Killing horizons}
...
The surface gravity is constant over a bifurcate Killing horizon:
\be
    \encadre{\kappa = \mathrm{const}.}
\ee
\end{prop}
\begin{proof}
Let $\Hor$ be a Killing horizon with respect to a Killing vector $\w{\xi}$
and $\w{v}$ be a generic non-vanishing vector field tangent to $\Hor$. Let us
evaluate the derivative of $\kappa$ along $\w{v}$ from expression (\ref{e:neh:kappa2}):
\[ 2 \kappa v^\rho \nabla_\rho \kappa =
    - \frac{1}{2} v^\rho \nabla_\rho \left( \nabla_\mu \xi_\nu \nabla^\mu \xi^\nu \right) =
    - v^\rho \nabla_\rho \nabla_\mu \xi_\nu \nabla^\mu \xi^\nu .\]
Now a standard identity valid for any Killing vector $\w{\xi}$ is\footnote{To prove it, use
the covariant version of the Ricci identity (\ref{e:bas:Ricci_ident}):
$\nabla_\alpha\nabla_\beta \xi_\gamma - \nabla_\beta\nabla_\alpha \xi_\gamma
        = R_{\gamma\mu \alpha\beta} \, \xi^\mu$, calling it equation $(1)$.
...
Now, using the antisymmetry properties (\ref{e:bas:Riemann_antisym34}) and (\ref{e:bas:Riemann_antisym12}) of the Riemann tensor and then the cyclic symmetry (\ref{e:bas:Riemann_cyclic}),
$- R_{\alpha\mu\beta\gamma} - R_{\beta\mu\gamma\alpha} = - R_{\mu\alpha\gamma\beta} - R_{\mu\beta\alpha\gamma}
= R_{\mu\gamma\beta\alpha}$. This turns the right-hand side into $(R_{\gamma\mu \alpha\beta} + R_{\mu\gamma\beta\alpha})\xi^\mu = 2 R_{\gamma\mu \alpha\beta} \xi^\mu = 2 R_{\alpha\beta\gamma\mu} \xi^\mu$, the last equality following from
property (\ref{e:bas:Riemann_sym}). This establishes Eq.~(\ref{e:neh:nabnab_xi_Riem}).}
...
$\w{\Theta}=0$ \\
NDC $\Longrightarrow \kappa=\mathrm{const}$ (Zeroth Law)};
\draw[inherits] (H)--(N);
\draw[inherits] (K)--(H);
\end{tikzpicture}
