...
by John L. Synge in 1950 \cite{Synge50}. He used coordinates\index{Synge!coordinates} whose relation to
Schwarzschild-Droste coordinates is more complicated than Kruskal-Szekeres one\footnote{
For $r\geq 2m$, Synge coordinates are
$T' = 2m \left[ \mathrm{acosh}\sqrt{\frac{r}{2m}} + \sqrt{ \frac{r}{2m}\left(\frac{r}{2m} - 1 \right) }\right]
\sinh \left(\frac{t}{4m}\right)$
and $X' = 2m \left[ \mathrm{acosh}\sqrt{\frac{r}{2m}} + \sqrt{ \frac{r}{2m}\left(\frac{r}{2m} - 1 \right) }\right]
\cosh \left(\frac{t}{4m}\right)$; compare with (\ref{e:sch:KS_SD_I}). Interestingly, in his
article \cite{Szeke60}, Szekeres says that the transformations (\ref{e:sch:KS_SD_I})
``are essentially due to Synge''.},
albeit both coordinate systems share some similarities: in Synge diagram\index{Synge!diagram} (Figs.~8 and 9 in Ref.~\cite{Synge50}),
...
Let us recall that the hypersurface $T=0$ considered here coincide with the
