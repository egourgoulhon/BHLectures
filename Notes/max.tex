...
the \defin{Lambert function}\index{Lambert function} $W_0$ \cite{AbramS72}, which is defined as
...
(\ref{e:sch:metric_KS});
using Eq.~(\ref{e:max:exp_tW0}) to get rid of $\mathrm{e}^{W(X^2 - T^2)}$, we
arrive at a fully explicit expression of Schwarzschild metric in terms
of the Kruskal-Szekeres coordinates:
\be \label{e:sch:metric_KS_TX_partial}
   \encadre{
    \w{g}  =  4m^2 \left[
    \frac{4  (W(X^2-T^2) - 1)}{(X^2-T^2) W(X^2-T^2)}\left( - \dd T^2 + \dd X^2 \right)
...
\be
    \encadre{
      \w{g} = 4m^2 \left[
      \frac{4(W(-UV) - 1)}{UV \, W(-UV)} \, \dd U \, \dd V
