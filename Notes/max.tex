...
\be \label{e:max:conf_metric_scri}
   ...
   - 64 m^2 \D \hat{U} \, \D \hat{V} .
\ee
This bilinear form is clearly degenerate. Therefore $\tilde{\w{g}}$ is not a
...
In order to get a regular conformal completion of the maximal extension
of Schwarzschild spacetime $(\M,\w{g})$, a finite-range coordinate system has
been proposed by Frolov \& Novikov \cite{FroloN98}: instead of (\ref{e:max:def_hUhV}),
the finite-range coordinates $(\tilde{U},\tilde{V})$ are defined by
...
The presence of the $\sinh$ function in the expression (\ref{e:max:def_tUtV})
...
We have from (\ref{e:max:def_tUtV})
\[
...
\be \label{e:max:def_Omega_tUtV}
    \Omega := \cos\tilde{U}\cos\tilde{V} .
\ee
The corresponding conformal metric is
\bea
    {\tilde g}_{\mu\nu} \, \D x^\mu \, \D x^\nu & =&
    16 m^2 \left( 1 - \frac{2m}{r} \right)
    \frac{\D\tilde{U} \, \D\tilde{V}}{\tanh(\tan\tilde{U}) \tanh(\tan\tilde{V}) } \nonumber\\
    & &
     +  r^2 \cos^2\tilde{U}\cos^2\tilde{V} \left( \D\th^2 + \sin^2\th\, \D\ph^2 \right) . \label{e:max:conf_metric_tUtV}
\eea
...
given in Sec.~\ref{s:glo:conf_compl} are obeyed. There remains to check
