...
be computed numerically (see Sec.~?? for the computation with SageMath).
...
\begin{remark}
There are many inexact figures regarding embeddings of spatial sections
of Schwarz\-schild spacetime in the literature, including in renown textbooks.
A first common error is to draw the two ends of the embedded surface as asymptotic to flat
planes, which a paraboloid is not. This is correct from a topological point of view,
but not from the geometrical one, i.e. the embedding depicted in this way
is not isometric. A second error regards the embeddings for $|T_0|> 1$, which
