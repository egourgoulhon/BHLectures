...
\ee
Note that $\tilde{W}_0$ is a bijection $(-1,+\infty) \to (0, +\infty)$. Its
...
\bea
    \hat{\Hor}:& & U = 0 \quad\mbox{or}\quad V = 0 \\
    \Hor: & &  U = 0 \quad\mbox{and}\quad V > 0 .
\eea
...
\be
    ...
    - \frac{32 m^3}{r} \, \mathrm{e}^{-r/2m} \,
     ...
     +  r^2 \left( \D\th^2 + \sin^2\th\, \D\ph^2 \right) ,
\ee
where $r = 2 m \tilde{W}_0(-\tan \hat{U} \tan \hat{V})$ .

To depict $\M$, let us introduce  ``time+space'' coordinates $(\hat{T},\hat{X})$,
which are related to $(\hat{U},\hat{V})$ is exactly the same way
as the coordinates as the  coordinates $(\tau,\chi)$ were related
...
to the coordinates $(\hat{T},\hat{X})$ in Fig.~\ref{f:sch:sch_carter-penrose}.
...
Fig.~\ref{f:sch:sch_carter-penrose} are not related to any regular conformal
completion, as defined in Sec.~\ref{s:glo:conf_compl},
contrary to the coordinates $(\tau,\chi)$ used for Minkowski spacetime.
 To see this, let us
introduce finite-range null coordinates $(\tilde{U},\tilde{V})$ by
\be \label{e:max:tUtV}
    \left\{\begin{array}{l}
    \tilde{U} := \tilde{T} - \tilde{X}\\
    \tilde{V} := \tilde{T} + \tilde{X}.
    \end{array}\right.
\ee
The range of $(\tilde{U},\tilde{V})$ is deduced from (\ref{e:max:range_tTtX}):
\be
    -\pi < \tilde{U} < \pi, \quad
    -\pi < \tilde{V} < \pi, \quad
    -\pi < \tilde{U}+\tilde{V} < \pi.
...
The advantage of coordinates $(\tilde{U},\tilde{V})$ over
$(\tilde{T},\tilde{X})$ is that they are better adapted to the
spacetime asymptotics.
Indeed by combining (\ref{e:max:tUtV}) and (\ref{e:max:tTtX}), we have
\[
    \left\{\begin{array}{l}
    \tilde{U} = 2 \arctan(T-X)\\
    \tilde{V} = 2 \arctan(T+X).
    \end{array}\right.
\]
Combining further with (\ref{e:sch:def_T_X}), we get
\be \label{e:max:tUtV_UV}
    \left\{\begin{array}{l}
    \tilde{U} = 2 \arctan U\\
    \tilde{V} = 2 \arctan V
    \end{array}\right.
...
\ee
\begin{remark}
These relations can be compared with Eq.~(\ref{e:glo:UV_uv}): they are
identical, up to a factor $2$.
\end{remark}
...
\be
    \scri^+:\ V\rightarrow +\infty \qquad\mbox{and}\qquad
    \scri^-:\ U\rightarrow -\infty.
\ee
In view of (\ref{e:max:tUtV_UV}) it is then clear that the coordinates
$(\tilde{U},\tilde{V})$ are well adapted to the spacetime asymptotics:
\be \label{e:max:scri_tUtV}
    \scri^+:\ \tilde{V}\rightarrow \pi \qquad\mbox{and}\qquad
    \scri^-:\ \tilde{U}\rightarrow -\pi.
\ee
\begin{remark}
The coordinates $(\tilde{U},\tilde{V})$ are also well adapted to the
bifurcate Killing horizon:
\be
    \hat{\Hor}:\ \tilde{U} = 0 \ \mbox{or}\ \tilde{V} = 0 ,
\ee
with the Schwarzschild horizon and the bifurcation sphere being the following
subsets:
\bea
    \Hor:& \ \tilde{U} = 0 \ \mbox{and}\ \tilde{V} > 0 \\
    \Sp:& \ \tilde{U} = 0 \ \mbox{and}\ \tilde{V} = 0 .
\eea

\end{remark}

The relation (\ref{e:max:tUtV_UV}) leads immediately to
\[
    \D U = \frac{\D \tilde{U}}{2\cos^2(\tilde{U}/2)}
    \qquad\mbox{and}\qquad
    \D V = \frac{\D \tilde{V}}{2\cos^2(\tilde{V}/2)} .
\]
Substituting into (\ref{e:sch:metric_UV}), we obtain the expression of
the metric components with respect to the coordinates
$x^\alpha=(\tilde{U}, \tilde{V}, \th,\ph)$:
\[
    g_{\mu\nu} \, \D x^\mu \, \D x^\nu =
    - \frac{8 m^3}{r} \,
    \frac{\mathrm{e}^{-r/2m}}{\cos^2(\tilde{U}/2)\cos^2(\tilde{V}/2)}
    \,  \D \tilde{U} \, \D \tilde{V}
     +  r^2 \left( \D\th^2 + \sin^2\th\, \D\ph^2 \right) .
\]
Now, from Eq.~(\ref{e:sch:r_UV}),
\bea
    \frac{\mathrm{e}^{-r/2m}}{\cos^2(\tilde{U}/2)\cos^2(\tilde{V}/2)} & =&
    - \left( \frac{r}{2m} - 1 \right) \frac{1}{UV \cos^2(\tilde{U}/2)\cos^2(\tilde{V}/2)} \nonumber \\
    & = &
    - \left( \frac{r}{2m} - 1 \right) \frac{1}{\tan(\tilde{U}/2) \cos^2(\tilde{U}/2)\tan(\tilde{V}/2)\cos^2(\tilde{V}/2)} \nonumber \\
    & = &
    - \left( \frac{r}{2m} - 1 \right) \frac{1}{\sin(\tilde{U}/2) \cos(\tilde{U}/2)\sin(\tilde{V}/2)\cos(\tilde{V}/2)} \nonumber \\
    & = &
    - \left( \frac{r}{2m} - 1 \right) \frac{4}{\sin\tilde{U} \, \sin\tilde{V}} .
        \nonumber
\eea
Hence
\be \label{e:max:g_tUtV}
    \encadre{ g_{\mu\nu} \, \D x^\mu \, \D x^\nu =
     16 m^2 \left( 1 - \frac{2m}{r} \right)\,
    \frac{\D \tilde{U} \, \D \tilde{V}}{\sin\tilde{U} \, \sin\tilde{V}}
     +  r^2 \left( \D\th^2 + \sin^2\th\, \D\ph^2 \right) }.
\ee
In this expression, $r$ is the function of $(\tilde{U},\tilde{V})$ defined
by [cf. Eqs.~(\ref{e:sch:KS_SD_I}), (\ref{e:sch:def_T_X}) and (\ref{e:max:tUtV_UV})]
\be
    r = 2 m \tilde{W}_0\left(-\tan\frac{\tilde{U}}{2} \tan\frac{\tilde{V}}{2} \right).
\ee
...
Since in Eq.~(\ref{e:max:g_tUtV}), the term
$\sin\tilde{U} \, \sin\tilde{V}$ vanishes both at $\scri^+$ and $\scri^-$
[cf. Eq.~(\ref{e:max:scri_tUtV})],
we would have, up to some constant factor,
\be \label{e:max:Omega_tUtV}
    \Omega = \sqrt{- \sin\tilde{U} \, \sin\tilde{V}} .
\ee
Hence the conformal metric should be
\be \label{e:max:conf_metric_tUtV}
   {\tilde g}_{\mu\nu} \, \D x^\mu \, \D x^\nu =
     - 16 m^2 \left( 1 - \frac{2m}{r} \right)
    \D \tilde{U} \, \D \tilde{V}
     -  r^2 \sin\tilde{U} \, \sin\tilde{V}
     \left( \D\th^2 + \sin^2\th\, \D\ph^2 \right) .
\ee
At $\scri^+$, $r\rightarrow +\infty$, $2m/r\rightarrow 0$ and
$r^2 \sin\tilde{V} = 2 r^2 \sin(\tilde{V}/2) \cos(\tilde{V}/2) \sim
2 r^2 \cos(\tilde{V}/2)$, since $\tilde{V} = \pi$ at $\scri^+$
[Eq.~(\ref{e:max:scri_tUtV})]. Hence the conformal metric at $\scri^+$ is
\be \label{e:max:conf_g_scrip}
   \left. {\tilde g}_{\mu\nu} \right| _{\scri^+} \, \D x^\mu \, \D x^\nu =
     - 16 m^2 \D \tilde{U} \, \D \tilde{V}
     -  2 r^2 \cos(\tilde{V}/2) \sin\tilde{U}
     \left( \D\th^2 + \sin^2\th\, \D\ph^2 \right) .
\ee
Now, by combining Eqs.~(\ref{e:sch:r_UV}) and (\ref{e:max:tUtV_UV}),
we have
\[
    - \tan(\tilde{U}/2) \tan(\tilde{V}/2) =  \mathrm{e}^{r/2m} \left( \frac{r}{2m} - 1 \right)
\]
Hence on $\scri^+$,
\[
    - \tan(\tilde{U}/2) \frac{1}{\cos(\tilde{V}/2)} \sim \frac{r}{2m} \mathrm{e}^{r/2m} ,
\]
from which
\[
    r^2 \cos(\tilde{V}/2) \sim - 2m \tan(\tilde{U}/2) \times
        \underbrace{r \mathrm{e}^{-r/2m}}_{\rightarrow 0}
        \longrightarrow 0 .
\]
Accordingly the conformal metric at $\scri^+$ (\ref{e:max:conf_g_scrip})
reduces to
\be
   \left. {\tilde g}_{\mu\nu} \right| _{\scri^+} \, \D x^\mu \, \D x^\nu =
     - 16 m^2 \D \tilde{U} \, \D \tilde{V} .
\ee
This metric is clearly degenerate. It is therefore not a regular metric.
We conclude that (\ref{e:max:Omega_tUtV})-(\ref{e:max:conf_metric_tUtV}) does
