...
On $\M_{\rm I}$, $r>2m$ and (\ref{e:sch:u_v_r_t}) yields
...
\ee
This relation takes a very simple form in terms of the tortoise coordinate\index{tortoise coordinate}
...
The coordinates $U$ and $V$ are null coordinates, since $u$ and $v$ are
...
the \defin{Lambert function}\index{Lambert function} $W_0$ \cite{AbramS72,CorleGHJK965}, which is defined as
...
\be \label{e:sch:metric_KS_TX_partial}
   \encadre{
    \w{g}  =  4m^2 \left[
    \frac{4}{X^2-T^2 + \mathrm{e}^{W(X^2 - T^2)}}\left( - \dd T^2 + \dd X^2 \right)
    +  W(X^2-T^2)^2 \left( \dd\th^2 + \sin^2\th\, \dd\ph^2 \right)
    \right] } .
\ee
...
\be \label{e:sch:KS_IEF}
    ...
    \iff
    \encadre{\left\{\begin{array}{l}
     \ti =  2 m \left[ 2 \ln (T+X) - W(X^2 - T^2) \right] \\[1ex]
    r = 2 m \, W(X^2 - T^2)
    \end{array}\right. }
\ee
...
has been computed in Example~\ref{x:def:Schw_hor3}:
...
\be
    \encadre{
      \w{g} = 4m^2 \left[
      \frac{4}{UV - \mathrm{e}^{W(-UV)}} \, \dd U \, \dd V
      + W(-UV)^2 \left( \dd\th^2 + \sin^2\th\, \dd\ph^2 \right) \right] .
    }
\ee
...
by Martin Kruskal\index[pers]{Kruskal, M.D.} \cite{Krusk60} and
George Szekeres\index[pers]{Szekeres, G.} \cite{Szeke60}.
...
John Synge\index[pers]{Synge, J.L.} in 1950 \cite{Synge50}, i.e. 10 years before Kruskal and Szekeres.
Synge used coordinates\index{Synge!coordinates} $(T',X')$
whose relation to
...
has also been found by Christian Fronsdal\index[pers]{Fronsdal, C.} \cite{Frons59} in 1959, not via any explicit change of coordinates but rather via
...
we recover Property~\ref{p:sta:xi_S_zero}: the Killing
vector field vanishes on the bifurcation sphere:
...
See also Fig.~\ref{f:glo:Schwarz_BH}, where $\M_{\rm II}$ is denoted $\mathscr{B}$
and $\M_{\rm IV}$ is denoted $\mathscr{W}$.
...
It was used in Sec.~\ref{s:glo:def_BH} to illustrate the black hole definition
(cf. Fig.~\ref{f:glo:Schwarz_BH}).
As the Kruskal diagram (Fig.~\ref{f:sch:kruskal_diag}), the Carter-Penrose diagram
displays radial null geodesics as straight lines with slope $\pm 45^\circ$.
This holds
...
to introduce a slight deviation from Definition~\ref{d:glo:conformal_completion}
(given that $\Omega^2$, which is involved in (\ref{e:max:g_Omega2_tg}), is smooth) .
...
spacetime has been constructed by Brandon Carter\index[pers]{Carter, B.} in 1966 \cite{Carte66}, using the same coordinates $(\hat{T},\hat{X})$ as here\footnote{
...
coordinate transformations recalls the conformal diagrams used by R. Penrose''\index[pers]{Penrose, R.}
in 1964 \cite{Penro64}, the diagrams in Penrose's article \cite{Penro64}
regarding Minkowski and de Sitter spacetimes only.
This justifies the name \emph{Carter-Penrose diagram}\index{Carter-Penrose diagram}
used in the literature (e.g. \cite{FroloZ11,Chrus20}) for the graphical representation shown in Fig.~\ref{f:max:carter-penrose-std}, while the mere
\emph{Penrose diagram}\index{Penrose!diagram} or \emph{conformal diagram}\index{conformal!diagram}
is quite common;
one encounters as well the name \emph{Penrose-Carter diagram}\index{Penrose-Carter diagram}
(e.g. \cite{FroloN98,Poiss04}).
\end{hist}

\subsection{A regular conformal completion based on Penrose-Frolov-Novikov coordinates}
...
\defin{Penrose-Frolov-Novikov coordinates}\index{Penrose-Frolov-Novikov coordinates}
...
\be \label{e:max:r_tUtV}
    r = 2 m\,  W\!\left(-\sinh(\tan \tilde{U}) \sinh(\tan \tilde{V})\right) .
\ee
...
$\left.\dd\Omega\right|_{{\scri'}^+} = -\cos\tilde{V}\, \dd\tilde{U}\not=0$ and
$\left.\dd\Omega\right|_{{\scri'}^-} = \cos\tilde{U}\, \dd\tilde{V}\not=0$.
Hence conditions 1, 3 and 4 of Definition~\ref{d:glo:conformal_completion}
of a conformal completion are fulfilled. There remains to check
condition 2, namely that the tensor $\w{\tilde{g}}$ defined
by (\ref{e:max:conf_metric_tUtV}) is a regular metric on the whole of $\tilde{\M}$.
...
\begin{tprop}{conformal completion of Schwarzschild spacetime}{}
The pair
...
has been suggested by Roger Penrose\index[pers]{Penrose, R.} in 1967 (p.~209 of Ref.~\cite{Penro68})
...
discussed in details by Valeri P. Frolov\index[pers]{Frolov, V.P.} and Igor D. Novikov\index[pers]{Novikov, I.D.} in their 1998 textbook \cite{FroloN98} (Sec.~5.1.3), stressing that it leads to
...
matches Definition~\ref{d:glo:fut_past_inf} of a conformal
...
given in Sec.~\ref{s:glo:def_BH} and to justify rigorously
Examples~\ref{x:glo:Schwarzschild_BH} and \ref{x:glo:Schwatz_white_hole}
given there for illustrative purposes.
The causal past of $\scri^+_*$ is $J^-(\scri^+_*) = \M_{\rm I}\cup \M_{\rm III}\cup \M_{\rm IV}$
(cf. Fig.~\ref{f:max:carter-penrose-FN}). In view of the definition
(\ref{e:glo:def_BH}), we get
\begin{tprop}{black hole in the maximal Schwarzschild spacetime}{}
...
\begin{tprop}{white hole in the maximal Schwarzschild spacetime}{}
...
\be \label{e:max:isometric_emb}
    \forall p\in \Sp,\quad  \forall (\w{u},\w{v})\in (T_p \Sp)^2,\quad
        \w{f}(\Phi_*\w{u}, \Phi_*\w{v}) = \w{g}(\w{u},\w{v}) ,
\ee
where $\Phi_*\w{u}$ is the vector of $\R^3$ that is the
the pushforward\index{pushforward} of $\w{u}$ by $\Phi$, as defined in
Sec.~\ref{s:bas:push_pull}. Another phrasing of the
isometry property (\ref{e:max:isometric_emb}) is: the pullback\index{pullback} of $\w{f}$
...
\be
    r_{\rm min}(T_0) = \left\{ \begin{array}{ll}
        2m\,  W(-T_0^2) & \ \mbox{if}\quad  |T_0| \leq 1 \\
...
\cite{Schwa1916}, the Austrian physicist Ludwig Flamm\index[pers]{Flamm, L.} (1885-1964) showed that the slice $t=\mathrm{const}$
...
At this stage, we have introduced many coordinate systems on Schwarzschild spacetime:
\begin{itemize}
...
is sometimes called the \emph{eternal Schwarzschild black hole}\index{eternal Schwarzschild black hole}.
