...
\subsection{Hypersurface of constant Kruskal-Szekeres time}
...
\be
    X_0 = \sqrt{\max\left(0, T_0^2-1\right)} \qquad \mbox{and}\qquad
...
metric on $\Sigma_0$ in terms of the coordinate $(x^i) = (r,\th,\ph)$:
...
Coordinates on $\Sigma_{T_0}^{\rm eq}$ are $(x^a)=(X,\ph)$, or the two patches
$(x^a)=(r,\ph)$ (one for $X>0$, the other for $X<0$).
If $|T_0| < 1$, the topology of $\Sigma_{T_0}^{\rm eq}$ is
$\R\times\SS^1$, i.e. that of a cylinder, while for $|T_0| \geq 1$, it has
two connected components, reflecting the two connected components of
$\Sigma_{T_0}$, each of them having the topology of a cylinder.
...
\be \label{e:max:metric_q}
    ...
    1 - \frac{2m}{r} \left(1 -  T_0^2 \mathrm{e}^{-r/2m} \right) \right) ^{-1}
     \, \D r^2 +  r^2 \D\ph^2 .
\ee
Given the invariance in $\ph$, it's
...
\be \label{e:max:metric_h}
    h_{ab}\,  \D x^a \D x^b = \left( 1 + Z'(r)^2 \right) \D r^2
            + r^2 \D\ph^2 .
\ee
Comparing (\ref{e:max:metric_h}) with (\ref{e:max:metric_q}), we see that
the isometric embedding of $\Sigma_{T_0}^{\rm eq}$ into $\R^3$
must be
\be
    \begin{array}{clcl}
    \Phi: & \Sigma_{T_0}^{\rm eq} & \longrightarrow & \R^3 \\
        & (X,\ph) & \longmapsto & (r,z,\ph) = \left(r(X), Z(r(X)), \ph \right) ,
    \end{array}
\ee
with the function $Z$ obeying
\[
   1 + Z'(r)^2 =  \left(
    1 - \frac{2m}{r} \left(1 -  T_0^2 \mathrm{e}^{-r/2m} \right) \right) ^{-1} .
\]
...
\]
In order for the isometric embedding to be well defined, the right-hand side
of the above equation must be non-negative. Since the denominator of the last
...
of $r$,
\be
    r_{\rm min}(T_0) :=  4 m \ln |T_0|,
\ee
such that the part of $\Sigma_{T_0}^{\rm eq}$ with $r< r_{\rm min}(T_0)$
...
on $X$:
\be
    |X| \geq X_{\rm min}(T_0) := |T_0| \sqrt{2\ln |T_0|} .
\ee
