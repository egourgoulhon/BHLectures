...
based on the coordinates $(\tau,\chi)$ only. The near-horizon region in
...
curves ranging from $T=-2$ to $T=2$ (grey curves) in the right figure.
\textsl{[Figures generated by the notebook \ref{s:sam:Kerr_extremal_extended} (left)
...
\ee
so that\footnote{In Example~\ref{x:neh:AdS}, we had $\chi := \arctan r \in (0,\pi/2)$ only
because of the constraint $r\ge 0$ for AdS$_4$ spacetime, while $y$ spans the whole of $\R$
in NHEK spacetime.}  $\chi\in (-\pi/2, \pi/2)$.
...
\be \label{e:exk:T_R_Phi_tau_chi_psi}
    ...
    \end{array}\right.
\ee
These transformations can be used to map the
...
so that $(T,R,\th,\Phi)$ constitute a regular coordinate system of
...
They appear as the interiors of triangles with one vertical edge at the NHEK
...
``points'' towards the right, while in Fig.~\ref{f:exk:compar_Kerr_NHEK},
$\mathscr{N}_{\rm P}^+$ (where $R>0$) points towards the left, but this results simply
...
\be
    \encadre{
    \begin{array}{lll}
    h_{\mu\nu}\,  \D \hat{Y}^\mu \D \hat{Y}^\nu  = &
    m^2 (1 + \cos^2\th) \Bigg[
    & \displaystyle \!\!\! \frac{1}{\cos^2\chi} \left( - \D \tau^2 + \D \chi^2 \right)
    + \D\th^2 \\[2ex]
    & & \displaystyle \!\!\!  + \frac{4 \sin^2\th}{(1 + \cos^2\th)^2}
    \left( \D\psi + \tan\chi\,  \D \tau\right)^2 \Bigg]
    \end{array}
    }
\ee
...
metric, so it cannot be considered as a proper conformal factor. Since $\chi\in(-\pi/2, \pi/2)$,
...
around the horizon. This narrow strip, endowed with the Kerr metric $\w{g}$, becomes more and
more isometric to the fixed NHEK strip $R\in[-1, 1]$ endowed with the metric $\w{h}$.
...
be approximated arbitrarily closely by the metric $\w{h}$
...
The NHEK geometry is at the core of the so-called \emph{Kerr/CFT correspondence}\index{Kerr/CFT correspondence}, initiated by M.~Guica, T.~Hartman, W. Song and A. Strominger in 2009 \cite{GuicaHSS09}, which conjectures that quantum gravity\index{quantum!gravity}
