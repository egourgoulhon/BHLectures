...
That $i^{\rm int}$ is located infinitely far from any point at $r=r_0>m$ on
a hypersurface $\Sigma_t$ is clear on the line element (\ref{e:exk:metric_BL}).
According to the latter, the distance along a
curve $(t,\th,\ph)=\mathrm{const}$ between the point
at $r=r_0$ and $r=m$ is indeed
\[
...
Similarly, the coordinate $\ph$ diverges along the worldline of any particle that reaches $\Hor$.
