...
\ee
The Boyer-Lindquist expression of $\w{\xi}_3$ is
\be
...
taking into account Carter's notations $(\tau,\lambda,\varphi)$ for our
$(T,-R,\Phi)$.
...
The NHEK metric has been rediscovered by James Bardeen\index{Bardeen, J.M.} and Gary Horowitz\index{Horowitz, G.T.} in 1999 \cite{BardeH99}. They deduced it from the Boyer-Lindquist expression of the extremal Kerr metric by (i) performing the change of coordinates (\ref{e:exk:def_R_T_Phi})-(\ref{e:exk:R_T_Phi_BL}) and (ii) taking the limit $\veps\to 0$, as presented in
Sec.~\ref{s:exk:NHEK_metric}. Note that the coordinates actually used by
Bardeen and Horowitz were $\tilde{T} := 2m T$ and $\tilde{R} = m R$, denoted
respectively $t$ and $r$ by them. In the same article~\cite{BardeH99},
Bardeen and Horowitz have introduced
...
and have obtained expression (\ref{e:exk:global_NEHK_metric}) for $\w{h}$ [their Eq.~(2.9)]. They have
then analyzed the global properties of the NHEK spacetime.
