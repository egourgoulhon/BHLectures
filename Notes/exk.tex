...
its group of isometries has the maximum possible dimension, which is $S = n(n+1)/2$, where $n$ is the spacetime dimension.
It is equivalent to say that there are $n(n+1)/2$ linearly independent Killing vector fields, $n$ of them generating
translations and $n(n-1)/2$ of them generating rotations.
For $n=4$, one has $S=10$, which, among others, is the dimension of the Poincaré group (case of Minkowski spacetime).},
it should come as no surprise that the NHEK spacetime $(\mathscr{N},\w{h})$
has more symmetries than the extreme Kerr spacetime $(\M,\w{g})$.
