...
consider the tensor field $\w{h}$ whose components with respect to the
...
independent from $\veps$. This is of course because $\veps$ disappears in
the combinations (\ref{e:exk:BH_to_BL}).
\end{remark}

In the limit $\veps\to 0$, the Bardeen-Horowitz coordinates $(T,R,\th,\Phi)$
do not constitute a regular coordinate system on $\M_{\rm BL}$ --- the part
of Kerr spacetime covered by Boyer-Lindquist coordinates. However,
expression~(\ref{e:exk:NEHK_metric_BL}) of $\w{h}$ is independent from $\veps$
and thus perfectly regular at the limit $\veps\to 0$. We may thus consider
that
\begin{greybox}
$\w{h}$ is a Lorentzian metric tensor on the whole of $\M_{\rm BL}$, the expression of which in
terms of Boyer-Lindquist coordinates is given by Eq.~(\ref{e:exk:NEHK_metric_BL})
\end{greybox}
In Sec.~\ref{s:exk:NHEK_spacetime}, we shall consider instead that $\w{h}$ is defined
on a manifold distinct from the Kerr manifold $\M$.

\subsection{Anti-de Sitter features}

It appears from (\ref{e:exk:NEHK_metric}) that the NEHK
metric $\w{h}$ has striking similiraties with
...
the isometry group of Kerr spacetime, which $\mathbb{R}\times\mathrm{U}(1)$.
...
in axisymmetric spacetimes \cite{KunduL13,Compe17}.
However, generically
\end{remark}
