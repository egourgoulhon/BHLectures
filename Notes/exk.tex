...
the \defin{Bardeen-Horowitz coordinates}\index{Bardeen-Horowitz coordinates}\footnote{Cf. the historical note on page~??}
$(T,R,\th,\Phi)$, which are defined on $\M_{\rm BL} = \M_{\rm I} \cup \M_{\rm III}$
...
the commutator\footnote{See Eq.~(\ref{e:bas:def_commutator}) in Appendix~\ref{s:bas} for the definition of the commutator
...
\begin{remark}
$\mathrm{SL}(2,\R)$ is isomorphic to the pseudo-orthogonal group $\mathrm{SO}(2,1)$, which is the isometry group of AdS$_2$.
\end{remark}
...
Pure geometric arguments suffice to show that a \emph{degenerate} Killing horizon
...
leads necessarily to the existence of the fourth Killing vector $\w{\xi}_3$
\cite{KunduLR07}.
\end{remark}
