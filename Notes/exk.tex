...
The inverse metric takes a rather simple form (cf. the notebook~\ref{s:sam:NEHK_spacetime}
for the computation):
\be
...
and (\ref{e:exk:NEHK_metric_BL}). The parameter $m$ is of course the mass of the extremal
Kerr spacetime from which the NHEK spacetime $(\mathscr{N},\w{h})$ arises. However, one cannot
associate a mass to the latter. Indeed, we cannot use the
...
\defin{total angular momentum} can be defined  unambiguously from the axisymmetry Killing vector
$\w{\eta} = \wpar_{\ph} = \wpar_{\Phi} = \wpar_{\psi}$, via
the Komar formula\index{Komar!angular momentum}\index{angular!momentum!Komar --} (\ref{e:ker:def_Komar_J}):
\be
     J := \frac{1}{16\pi} \int_{\Sp} \star(\dd \uu{\eta})  ,
\ee
where $\uu{\eta}$ is the 1-form $h$-dual to $\w{\eta}$ (i.e. obtained as $\eta_\alpha := h_{\alpha\mu} \eta^\mu$)
and the Hodge dual $\star$
is taken by means of the Levi-Civita tensor associated to $\w{h}$.
...
total angular momentum, which is nothing but the total angular momentum $J = m^2$
