...
a small parameter $\veps$ and
the so-called \defin{Bardeen-Horowitz coordinates}\index{Bardeen-Horowitz coordinates}
$(T,R,\th,\Phi)$, which are related to the Boyer-Lindquist coordinates
$(t,r,\th,\ph)$ by
...
For $\veps\to 0$,
the coordinate $R$ clearly implements the zoom-in on the region $r\simeq m$.
Regarding the change $t\to T$, we may notice that the Boyer-Lindquist coordinate
$t$ diverges near the horizon $\Hor$ --- more precisely,
...
coordinates $(X^\alpha):=(T,R,\th,\Phi)$. Their expressions is rather complicated and can be found in
the notebook \ref{s:sam:NEHK}.
One defines the \defin{near-horizon extremal Kerr (NEHK) metric} as the
metric tensor $\w{h}$ whose components with respect to the
...
form. But we check immediately on (\ref{e:exk:NEHK_metric}) that
$\w{h}$ is non-degenerate and has signature $(-,+,+,+)$. Hence it is
a proper metric tensor (cf. Sec.~\ref{s:bas:metric} in Appendix~\ref{s:bas}).
\end{remark}
The metric (\ref{e:exk:NEHK_metric}) has striking similiraties with
the metric of the anti-de Sitter spacetime\index{anti-de Sitter spacetime} (AdS):
...
of Chap.~\ref{s:neh}. More precisely, the metric induced by $\w{h}$ on
...
To prove that (\ref{e:exk:h_th_star}) is the metric of AdS$_{3}$,
...
are linked to AdS$_3$ being a bundle over AdS$_2$ with the real line as fiber
(in the same way as $\mathbb{S}^3$ being a bundle over $\mathbb{S}^2$ with
...
It is instructive to express the NHEK metric $\w{h}$ in terms of the Boyer-Lindquist coordinates
...
independent from $\veps$. This is of course because $\veps$ has disappeared from
the combinations (\ref{e:exk:BH_to_BL}).
\end{remark}

\subsection{NHEK spacetime}

The Bardeen-Horowicz components (\ref{e:exk:NEHK_metric}) of the NHEK metric $\w{h}$
...
of $\w{h}$ with respect to these coordinates,
are regular there.
They are actually regular in all the range $t\in\R$, $y\in\R$, $\th\in(0,\pi)$
and $\psi\in(0,2\pi)$. Because of the $\arctan$ in Eq.~(\ref{e:exk:def_global_NHEK}),
on $\M$ the range of $\tau$ is restricted to $(-\pi/2, \pi/2)$. We may therefore
...
Ricci tensor, since it is nothing but the extremal Kerr metric, expressed in the Bardeen-Horowicz
...
\begin{remark}
The quantity $m^2$ appears as global scale of the metric $\w{h}$ in
...
$(\mathscr{N},\w{h})$ is not asymptotically flat. Incidently, this would
anyway preclude any unambiguous definition of the Komar mass, since asymptotic flatness
is required to normalize the Killing field to a scalar square $-1$ at spatial
infinity.
...
is the dimension of the Poincaré group, ruling the isometries of Minkowski spacetime.
...
has more symmetries than the extreme Kerr spacetime $(\M,\w{g})$. First of all, it is
obvious on the expression (\ref{e:exk:NEHK_metric}) of the NEHK metric
in Bardeen-Horowicz coordinates that $T$ and $\Phi$ are ignorable coordinates,
...
We notice that this expression is independent from $\veps$,
...
are independent from $\tau$. Hence $\wpar_\tau$ is a Killing vector of
$\w{h}$. In what follows, we will instead the linear combination
\be
    \w{\xi}_3 := \wpar_\tau - 1/2\, \w{\xi}_1,
\ee
since it has slightly simpler
components with respect to Bardeen-Horowicz coordinates. Given the
...
of the NEHK spacetime is thus 4-dimensional. This is 2 dimensions more than
the isometry group of Kerr spacetime, which $\mathbb{R}\times\mathrm{SO}(2)$.
In order to determine $G$, let us evaluate the commutator\footnote{See Eq.~(\ref{e:bas:def_commutator}) in Appendix~\ref{s:bas} for the definition of the commutator
of two vector fields.} of each pair
of the four vector fields $\w{\eta}$, $\w{\xi}_1$, $\w{\xi}_2$ and  $\w{\xi}_3$.
...
generator of the axisymmetry group $\mathrm{SO}(2)$, we may write
$G = \mathrm{SO}(2)\times G_3$, where $G_3$ is a
...
special linear group $\mathrm{SL}(2,\R)$. Hence the full isometry group
of the NEHK spacetime is
\be
    \encadre{ G = \mathrm{SO}(2)\times \mathrm{SL}(2,\R) }.
\ee

\begin{remark}
The enhanced symmetry group near the horizon is not specific to the
extremal Kerr black hole;
it is actually a generic feature, which occurs near any \emph{degenerate} Killing horizon,
as the extremal Kerr event horizon is (cf. Sec.~\ref{s:exk:horizon}) \cite{KunduL13,Comper17}.
However, generically
\end{remark}
