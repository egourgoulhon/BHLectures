...
far in the past of any of the geodesics $\Li^{{\rm out},\Hor}_{(\th,\psi)}$.
...
new coordinates $(T,R,\th,\Phi)$ related to the Boyer-Lindquist coordinates
$(t,r,\th,\ph)$ by
\be \label{e:exk:def_R_T_Phi}
    t = \frac{T}{\veps},\quad
    r = m + \veps R,\quad
    \ph = \Phi + \frac{T}{2m\veps} .
\ee
This choice ensures, for $T>0$, $R\geq 0$ and $\Phi\in [0,2\pi)$,  %]$
\be
...
The coordinate $t$ diverges near the horizon $\Hor$ --- more precisely,
...
So, we may keep $T = \veps t$ finite in the vicinity of $\Hor$, while having
$t$ diverging.
Similarly, the coordinate $\ph$ diverges along the worldline of any particle that reaches $\Hor$.
For geodesics, this divergence occurs at the rate $\D\phi/\D t = \Omega_H = 1/(2m)$
[Eqs.~(\ref{e:gek:lim_dphdt_Hor}) and (\ref{e:exk:def_chi})], which is ensured
at fixed $\Phi$ by the choice (\ref{e:exk:def_R_T_Phi}).
Finally, the coordinate $R = (r - m)/\veps$ is better adapted to describe the
extremal Kerr throat, since points with almost the same value of $r$ close
to $m$ can be separated by huge distances.
