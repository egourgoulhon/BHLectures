...
In various occasions during our study of Kerr spacetime in Chap.~\ref{s:ker}, we have
...
(i.e. timelike or null) geodesics in Kerr spacetime.
We shall start by investigating the first integrals of geodesic motion
(Sec.~\ref{s:gek:integrals_motion}), among which the Carter constant.
The latter arises from an algebraic particularity of Kerr spacetime, which gives
birth to a non-trivial Killing tensor. Along with the conserved energy $E$, conserved angular
momentum $L$ and the particle mass $\mu$, it provides a fourth constant of motion,
which allows us to reduce the geodesic motion to a system of first order equations,
the solution of which can be obtained by quadrature, as we shall see
in Sec.~\ref{s:ges:equat_geod_motion}.
...
in Chap.~\ref{s:ges}. As already suggested in the introduction of Chap.~\ref{s:ges}, before moving on, the reader might want to take
a look at Appendix~\ref{s:geo}, which recaps the properties of geodesics in
pseudo-Riemannian manifolds.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{First integrals of motion} \label{s:gek:integrals_motion}
...
This makes a total of four integral of motions and renders the problem integrable (Sec.~\ref{s:ges:equat_geod_motion}).
...
property:
\begin{tprop}{conserved quantities along causal geodesics}{gek:conserved_E_L}
...
\begin{texample}{generators of the event and inner horizons}{gek:null_generator_hor}
...
\begin{texample}{ingoing principal null geodesics}{gek:ingoing_null_E_L}
...
\begin{texample}{outgoing principal null geodesics}{gek:outgoing_null_E_L}
...
\begin{texample}{Carter constant for equatorial motion in Schwarzschild spacetime}{x:gek:K_Schwarz}
For $a=0$ (Schwarzschild limit), $\Delta = r(r - 2m)$, $\rho = r$
and Eq.~\eqref{e:gek:Kcarter_first_int} reduces to
\[
    \mathscr{K} = r^2 \left[ \left( 1 - \frac{2m}{r} \right) ^{-1} \left( E^2 -  (p^r)^2 \right)
        - \mu^2 \right] .
\]
Now, for a motion in the equatorial plane ($\th=\pi/2$), Eq.~\eqref{e:ges:dot_r_square} gives
$E^2 -  (p^r)^2 =  ( 1 - {2m}/{r} ) \left( \mu^2 + {L^2}/{r^2} \right)$.
Hence there comes a very simple expression for the Carter constant:
\be \label{e:gek:K_Schwarz}
    \encadre{ \mathscr{K} = L^2}_{\; a=0,\, \th=\frac{\pi}{2}} \ .
\ee
Since in Schwarzschild spacetime there is no loss of generality in considering that
a geodesic lies in the equatorial plane (Property~\ref{p:ges:planar_motion}), we conclude
that the Carter constant does not bring any new piece of information for
Schwarzschild geodesics. This is of course not the case for $a\neq 0$.
\end{texample}

Beside the Schwarzschild limit, some physical interpretation of the Carter constant
can be inferred from
expression~\eqref{e:gek:Kcarter_first_int}, as soon as the particle $\mathscr{P}$ of worldline $\Li$ visits the asymptotic region $r\to+\infty$. Indeed, given
...
\ee
Of course, at the Schwarzschild limit and for $\th=\pi/2$,
we recover the value \eqref{e:gek:K_Schwarz} since
$\w{L}_{\rm tot} = L \w{e}_{(z)}$ with $\w{e}_{(z)} = - \w{e}_{(\th)}$
...
\begin{tprop}{null geodesics with vanishing Carter constant}{gek:null_geod_K_0}
...
Now, according to Property~\ref{p:fra:corol2},
...
\begin{texample}{generators of the event and inner horizons}{}
We have seen in Example~\ref{x:gek:null_generator_hor}
...
\begin{tprop}{characterization of the null generators of the two horizons}{}
...
\section{First order system for geodesic motion} \label{s:ges:equat_geod_motion}

\subsection{Reduction to first order equations} \label{s:gek:first_order_system}

We have obtained in Sec.~\ref{s:gek:integrals_motion} four first integrals of the geodesic equation
(\ref{e:gek:eq_geod}):
...
\ee
Following standard usage, we call $Q$ the \defin{Carter constant}\index{Carter!constant}
...
\begin{texample}{Carter constant $Q$ in Schwarzschild spacetime}{}
If $a=0$ (Schwarzschild limit), Eq.~\eqref{e:gek:def_Q} reduces to
$Q = \mathscr{K} - L^2$ and
Eq.~\eqref{e:gek:Q_Ltot2_L2} shows that
$Q$ can be interpreted as the square of
the part of $\mathscr{P}$'s angular momentum (measured by the asymptotic inertial
observer) that is not in $L$.
Moreover, for a geodesic in the equatorial plane, Eq.~\eqref{e:gek:K_Schwarz} yields
\be
    \encadre{ Q = 0}_{\; a=0,\, \th=\frac{\pi}{2}} \ .
\ee
Actually we shall see that $Q=0$ for equatorial geodesics
even for $a \neq 0$
(Property~\ref{p:gek:latitudinal_motion} below).
\end{texample}

\begin{texample}{Carter constant $Q$ of the principal null geodesics}{}
...
(cf. Example~\ref{x:geo:trivial_Killing_tensors}),
...
\begin{tprop}{system of first order equations for geodesic motion}{}
The geodesic motion in Kerr spacetime is
governed by four first-order equations expressing the derivatives of
the Boyer-Lindquist coordinates $(t,r,\th,\ph)$ with respect to the affine
parameter $\lambda$ (associated to the 4-momentum $\w{p}$, cf. Eq.~\eqref{e:gek:pa_der_xa}) along the geodesic:
\begin{subequations}
...
coordinate $r$.
We define a \defin{$r$-turning point}\index{turning point!$r$-turning point} of $\Li$ as a point $p_0\in\Li$, the $r$-coordinate $r_0$ of which is a simple root of the polynomial $R$:
...
it is natural to introduce a new
parameter $\lambda'$
along the geodesic $\Li$ that is related to the affine parameter $\lambda$ by
...
the two parameters is that $\lambda'$ is not an affine parameter
...
The quantity $\lambda'$ is called the \defin{Mino parameter}\index{Mino!parameter}\index{parameter!Mino --} \cite{Mino03}.

In terms of the Mino parameter, the system (\ref{e:gek:eom_first_order}) simplifies to
\begin{subequations}
...
\begin{tprop}{solution for geodesic motion}{}
...
\subsection{Sign of \texorpdfstring{$E$}{E}} \label{s:gek:sign_E}

We have:
\begin{tprop}{positivity of the conserved energy outside the ergoregion}{gek:E_positive}
...
\begin{texample}{generators of the event and inner horizons}{}
We have already encountered zero-energy particles in Example~\ref{x:gek:null_generator_hor}: the photons whose worldlines are the null generators of the horizons $\Hor$
and $\Hor_{\rm in}$ [cf. Eq.~(\ref{e:gek:generator_hor_E_L_zero})].
\end{texample}

\begin{texample}{outgoing principal null geodesics in $\M_{\rm II}$}{}
We have seen in Example~\ref{x:gek:outgoing_null_E_L}
...
\begin{tprop}{}{gek:future_directed_Carter}
...
\begin{tprop}{}{gek:E_neg_implies_L_neg}
...
Hence we conclude:
\begin{tprop}{behavior of $\ph$ near the event horizon}{}
...
Hence
\begin{tprop}{behavior of $\ph$ near the inner horizon}{}
...
\subsection{Asymptotic \texorpdfstring{$r$-values}{r-values} and \texorpdfstring{$\th$-values}{θ-values}} \label{s:gek:asymptotic_values}
...
Hence we conclude:
\begin{tprop}{asymptotic $r$-value}{}
...
\begin{tprop}{asymptotic $\th$-value}{}
...
\begin{tprop}{latitudinal motion of Kerr geodesics}{gek:latitudinal_motion}
\begin{itemize}
...
\begin{texample}{Schwarzschild geodesics}{}
...
\begin{tprop}{bound orbits}{gek:bound_orbits}
...
provided it has $E=0$
(cf. Example~\ref{x:gek:null_generator_hor}).
We have
\begin{tprop}{}{}
...
\begin{texample}{null geodesic generators of the horizons}{}
The null geodesics generating the horizons $\Hor$ and $\Hor_{\rm in}$
considered in
Example~\ref{x:gek:null_generator_hor}
...
is
\begin{tprop}{necessary condition to visit the negative-$r$ region}{}
...
\begin{tprop}{decreasing of $r$ in $\M_{\rm II}$}{}
...
\begin{texample}{principal null geodesics}{}
For the ingoing principal null geodesics $\Li^{\rm in}_{(v,\th,\tph)}$,
...
\begin{tprop}{vanishing of $Q$ required to approach the ring singularity}{}
...
\begin{tprop}{ring singularity reachable only from the equatorial plane}{gek:ring_reach_from_equat}
...
\subsection{Moving to the \texorpdfstring{negative-$r$}{negative-r} side}
...
\begin{tprop}{crossing $r=0$ only possible for vortical geodesics}{}
...
\begin{texample}{vortical ingoing principal null geodesics}{}
...
\be \label{e:gek:veps_lt_one}
    |\veps| < 1 .
\ee
According to Property~\ref{p:gek:bound_orbits}, such a geodesic has necessarily a non-negative
Carter constant:
\be
    q \geq 0
\ee
...
\begin{tprop}{periodicities of bound timelike orbits}{}
...
\begin{tprop}{latitudinal stability of equatorial circular timelike orbits}{}
...
\begin{tprop}{criterion for radial stability of circular orbits}{}
...
\begin{tprop}{stability of circular orbits}{}
\be
...
\begin{tprop}{characterization of the ISCO}{gek:ISCO_extremum_eps_ell}
...
\begin{tprop}{existence and stability of equatorial circular timelike orbits}{}
...
\begin{tprop}{specific conserved energy and angular momentum at the ISCO}{}
...
\begin{tprop}{energy release from an accretion disk around a black hole}{}
...
\begin{tprop}{timelike circular orbits in the ergoregion}{}
...
\begin{tprop}{stable timelike circular orbits in the ergoregion}{}
