...
$\ell$ ,
and \defin{reduced Carter constant}\index{reduced!Carter constant}\index{Carter!constant!reduced --} $\mathcal{Q}$:
\be \label{e:gek:def_eps_ell_mQ}
  \encadre{ \veps := \frac{E}{\mu}}, \qquad
  \encadre{ \ell :=  \frac{L}{\mu}} \qquad\mbox{and}\qquad
  \encadre{ \mathcal{Q} := \frac{Q}{\mu^2}} .
\ee

The first-order equation of radial motion (\ref{e:gek:r_eff_potential})
can be rewritten in term of the above quantities:
\be \label{e:gek:r_eff_pot_timelike}
   \encadre{ \left( \derd{r}{\tau'} \right)^2 + \mathcal{U}(r) = - a^2  \mathcal{Q} },
\ee
where
\be \label{e:gek:U_timelike}
    \mathcal{U}(r) := (1-\veps^2) r^4 - 2 m r^3
    + \left[\mathcal{Q} + \ell^2 + a^2 (1-\veps^2)\right] r^2
    - 2m\left[ \mathcal{Q} + (\ell -a \veps)^2 \right] r
\ee
and $\tau'$ is related to the Mino parameter $\lambda'$ by
\be
   \tau' = \mu \lambda'.
\ee
The function $\mathcal{U}(r)$ is simply a rescaling of the effective
$r$-potential $U(r)$ by $\mu^2$: $\mathcal{U}(r) = U(r)/\mu^2$ [cf. Eq.~(\ref{e:gek:U_r_powers})]. We shall call $\mathcal{U}(r)$ the
\defin{effective $r$-potential}\index{effective!$r$-potential} as well.
