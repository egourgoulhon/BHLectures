...
and (\ref{e:gek:th_eff_potential}) can be rewritten in terms of the above quantities:
\begin{subequations}
\begin{align}
& \encadre{ \derd{t}{\tau'} = a\ell + T_1(r) + T_2(r) } \label{e:gek:dtdmino_timelike}\\
...
and
\be
    T_1(r) := \frac{r^2 + a^2}{r^2 - 2 m r + a^2} [(r^2 + a^2) \veps - a \ell]
    \qquad\mbox{and}\qquad
    T_2(\th) := - a^2 \veps \ell \sin^2\th,
\ee
\be
    \Phi_1(r) := \frac{a}{r^2 - 2 m r + a^2}\left[(r^2 + a^2) \veps - a \ell \right]
    \qquad\mbox{and}\qquad
...
From the viewpoint of Eq.~(\ref{e:gek:th_eff_pot_timelike}) above, they
correspond to a periodic motion in the potential well $-\tilde{\Theta}(\th)$:
\be
...
For non-polar orbits, the motion is restricted by $r_{\rm p}\leq r \leq r_{\rm a}$
and $\th_{\rm min} \leq \th \leq \pi/2 - \th_{\rm min}$. This means that
in terms of the ``Cartesian'' Boyer-Lindquist coordinates $(x,y,z)$, the
...
\begin{align}
  ...
  &  \Phi_2(\th) = \sum_{k=-\infty}^{+\infty} \hat{\Phi}_{2k} \, \mathrm{e}^{\mathrm{i} k \Upsilon_\th \, \tau'} = \langle \Phi_2(r) \rangle + \sum_{k=1}^{+\infty} \left( \hat{\Phi}_{2k} \, \mathrm{e}^{\mathrm{i} k \Upsilon_\th \, \tau'} + \mathrm{c.c.} \right) ,
\end{align}
\end{subequations}
...
Trace with respect to the Cartesian Boyer-Lindquist coordinates
$(x,y,z)$ of the bound timelike geodesic of parameters
...
Trace in the $(x,y)$ plane (left panel) and the $(x,z)$ plane (right panel)
of the bound timelike geodesic considered in Fig.~\ref{f:gek:timelike_xyz}.
The grey disk depicts the black hole region.
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_geod_plots}]}
}
\end{figure}
...
$(t,x,y)$ depicting the bound timelike geodesic considered in
Figs.~\ref{f:gek:timelike_xyz} and \ref{f:gek:timelike_plane}. The black
...
show a bound timelike geodesic $\Li$ with $\veps=0.9$, $\ell=2 m$ and $q=1.3\, m^2$
orbiting around a Kerr black hole with $a=0.998\, m$. The effective radial potential
$-\mathcal{R}(r)$
of this geodesic is shown in Fig.~\ref{f:gek:R_potential}. In particular,
$\Li$'s periastron is $r_p \simeq 2.175\, m$ and its apoastron is
$r_a \simeq 6.853\, m$. Moreover it has $\th_{\rm min} = 29.26^\circ$.
