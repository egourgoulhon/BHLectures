...
We have immediately $\ell_+ > 0$, while the sign of $\ell_-$ is the opposite of that
of $r_0^2 + a^2 - 2 a \sqrt{mr_0}$.
Now, for $\ell_-$, one has necessarily $r_0 \geq r_{\rm min}^- > m$ [Eq.~(\ref{e:gek:range_circ_minus})], so that
$r_0^2 + a^2 - 2 a \sqrt{mr_0} > r_0^2 + a^2 - 2 a r_0 = (r_0 - a)^2 > 0$.
Accordingly, $\ell_- < 0$ always holds. Therefore, we conclude
\be
    \encadre{\ell_+ > 0} \qand \encadre{\ell_- < 0} .
\ee
