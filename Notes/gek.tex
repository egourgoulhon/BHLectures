...
[Eq.~(\ref{e:gek:Theta_non_neg})] along $\Li$.
...
\end{align}
\end{subequations}
where $R$ is the quartic polynomial defined by Eq.~(\ref{e:gek:def_R}) (see also Eq.~(\ref{e:gek:R_r_powers}) below) and $\Theta$ is the function defined by Eq.~(\ref{e:gek:Theta_Q}).
\end{greybox}
\begin{proof}
...
(\emph{NB:} Carter's $u$ is our $v$) and not the Boyer-Lindquist ones.
...
In particular, this allows to identify $\lambda$ with the proper
...
\encadre{ \frac{1}{\Delta} \left( E - \frac{a}{r^2 + a^2} L
+ \frac{\eps_r}{r^2 + a^2} \sqrt{R(r)} \right) > 0 } .
\ee
Given expression (\ref{e:gek:def_R}) for $R(r)$ and
...
using the \defin{Cartesian Boyer-Lindquist coordinates} $(t,x,y,z)$, which are
...
(\ref{e:gek:def_R}) for the quartic polynomial $R(r)$ in terms of $Q$
...
\subsection{Equatorial timelike geodesics}
...
\subsection{Circular timelike orbits in the equatorial plane}
...
prograde orbits ($\ell > 0$) exist in both the light and dark grey regions,
while retrograde orbits ($\ell > 0$) exist only in the dark grey region. The orange line marks
...
\ee
\begin{figure}
\centerline{\includegraphics[width=0.7\textwidth]{gek_eps_circ_orb.pdf}}
\caption[]{\label{f:gek:eps_circ_orb} \footnotesize
Specific conserved energy $\veps_+$ and $\veps_-$ along circular timelike
orbits in the equatorial plane, as a function of the orbital radius $r_0$
and for selected values of the Kerr spin parameter $a$
[Eq.~(\ref{e:gek:veps_circ})].
Solid lines corresponds to $\veps_+$ and dashed ones to $\veps_-$.
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_circular_orbits}]}
}
\end{figure}
...
\begin{figure}
\centerline{\includegraphics[width=0.7\textwidth]{gek_ell_circ_orb.pdf}}
\caption[]{\label{f:gek:ell_circ_orb} \footnotesize
Specific conserved angular momentum $\ell_+$ and $\ell_-$ along circular timelike
orbits in the equatorial plane, as a function of the orbital radius $r_0$ and for
selected values of the Kerr spin parameter $a$
[Eq.~(\ref{e:gek:ell_circ})].
Solid lines corresponds to $\ell_+$ and dashed ones to $\ell_-$.
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_circular_orbits}]}
}
\end{figure}
...
\[
    \mathcal{V}''(r_0) = \frac{2m}{r_0^3} \left( 1 - \frac{\tilde{\ell}^2}{r_0^2} \right) .
\]
...
considered and \emph{ISCO} stands for \defin{innermost stable circular orbit}.
...
\ee
Note that $r_{\rm ISCO}^\pm  / m$ is a function of $\bar{a} := a/m$ only, via
the expressions (\ref{e:gek:Z1}) and (\ref{e:gek:Z2}) of $Z_1$ and $Z_2$; this
...
Radius of the innermost stable circular orbit $r_{\rm ISCO}^\pm$ [Eq.~(\ref{e:gek:r_ISCO})], as
a function of the Kerr spin parameter $a$ (red curves); the other curves are the same as in
Fig.~\ref{f:gek:circ_orb_lim}.
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_circular_orbits}]}
}
\end{figure}
...
\begin{greybox}
Three kinds of equatorial circular timelike orbits exist in Kerr spacetime:
\begin{enumerate}
\item The \emph{prograde outer circular orbits}: they are located outside
the black hole event horizon, having a Boyer-Lindquist radius $r_0$ ranging
...
where $r_{\rm ISCO}^+$ is given by Eqs.~(\ref{e:gek:r_ISCO}), (\ref{e:gek:Z1}) and (\ref{e:gek:Z2}),
...
that cross the various blocks of the maximal analytic extension, see
Chap.~4 of O'Neill textbook \cite{ONeil95}.
