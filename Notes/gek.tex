...
\bea
    ...
    & = & \eps_\th^{(0)} \int_{\th_0}^{\th_1} \frac{\D \bar{\th}}{\sqrt{\Theta(\bar{\th})}}
        + \sum_{j=1}^{N-1} \eps_{\th}^{(j)}
        \int_{\th_j}^{\th_{j+1}} \frac{\D \bar{\th}}{\sqrt{\Theta(\bar{\th})}}
        + \eps_\th^{(N)}  \int_{\th_N}^{\th} \frac{\D \bar{\th}}{\sqrt{\Theta(\bar{\th})}} ,
...
\begin{greybox}
A null or timelike geodesic that approaches the ring singularity lies
...
\[
    ...
    - \frac{L^2}{\sin^2\th} \right] .
\]
The $\cos^2\th$ term makes the ingetral on $\th$ in
Eq.~(\ref{e:gek:dashint_r_theta}) divergent, which is incompatible with the
finite value of the integral on $r$. Hence only (i) is possible, which completes
the proof.
\end{proof}
