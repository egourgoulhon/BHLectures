...
The first-order equation of radial motion (\ref{e:gek:drdl_Mino})
...
\ee
and
$\mathcal{R}(r) := R(r) / \mu^2$. In view of Eq.~(\ref{e:gek:R_r_powers}),
we have
\be \label{e:gek:R_timelike}
    \mathcal{R}(r) := (\veps^2 - 1) r^4 + 2 m r^3
    + \left[a^2 (\veps^2 - 1) - \mathcal{Q} -  \ell^2\right] r^2
    + 2m\left[ \mathcal{Q} + (\ell -a \veps)^2 \right] r  - a^2  \mathcal{Q} .
\ee
...
Following the definition given in Sec.~\ref{s:gek:r-motion}, a geodesic
$\Li$ has a bound orbit iff $|E|< \mu$, or equivalently
\be
    |\veps| < 1 .
\ee
