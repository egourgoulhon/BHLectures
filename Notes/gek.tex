...
\begin{figure}
\centerline{\includegraphics[width=0.49\textwidth]{gek_th_pot_L_0_low_E.pdf}
\ \includegraphics[width=0.49\textwidth]{gek_th_pot_L_0_high_E.pdf}}
...
The left figure is for $E^2 < \mu^2$, with horizontal trajectories corresponding
to three values of $Q$: $0$, $Q_1 = 0.75 V_0$ and $Q_2 = 1.2 V_0$.
The right figure is for $E^2 > \mu^2$, with horizontal trajectories corresponding
to three values of $Q$: $Q_1 = -0.3 V_0$, $0$ and $Q_2 = 0.2 V_0$.
...
and $\pi/2-\th_0$.
\item $Q = a^2(\mu^2 - E^2)$: the geodesic reaches the rotation axis at
$\th=0$ or $\th=\pi$, with a vanishing velocity $\D\th / \D\lambda'$.
\item $Q > a^2(\mu^2 - E^2)$:
...
\item $Q=0$: the geodesic approaches asymptotically the equator, which is
reached with a vanishing velocity $\D\th/\D\lambda'$.
...
The left figure is for $a^2(E^2 - \mu^2) \leq L^2$, with horizontal trajectories corresponding
to two values of $Q$: $0$ and $Q_1= 4 L^2$.
The right figure is for $a^2(E^2 - \mu^2) > L^2$, with horizontal trajectories corresponding
...
\begin{itemize}
\item $a^2(E^2 - \mu^2) \leq L^2$: $V(\th)$ has a single extremum, which is
necessarily a miminum, at $\th = \pi/2$.
\item $a^2(E^2 - \mu^2) > L^2$: $V(\th)$ has three extrema, located at
$\th=\th_*$, $\pi/2$ and $\pi-\th_*$ with
\be
    \th_* := \arcsin\sqrt{\frac{|L|}{a\sqrt{E^2 - \mu^2}}}
    \quad\mbox{and}\quad 0 < \th_* < \frac{\pi}{2} .
\ee
$\th_*$ and $\pi-\th_*$ correspond to a minimum, while $\pi/2$ to a local maximum
of $V(\th)$.
\end{itemize}
...
For $\th=\pi/2$ (the equator), the constraint~(\ref{e:gek:cond_th_motion}) reduces to $0 \geq -Q$.
Therefore, if $Q > 0$, there always exists an interval around $\pi/2$,
$[\pi/2-\alpha, \pi/2+\alpha]$ say, with $0<\alpha \leq\pi/2$,
such that (\ref{e:gek:cond_th_motion}) is fulfilled for
$\th$ in that interval, whatever the values
of $E$, $\mu$ and $L$. If $L\not=0$, this interval cannot be extended to the
...
If $Q=0$, condition (\ref{e:gek:cond_th_motion}) is equivalent to
(i) $\cos^2\th = 0$, i.e. $\th=\pi/2$ or (ii)
$a^2 (E^2 - \mu^2) - L^2 / \sin^2\th \geq 0$, i.e.
\be \label{e:gek:sin2th_ii}
    \sin^2\th \geq \frac{L^2}{a^2(E^2-\mu^2)} ,
\ee
which implies $a^2 (E^2 - \mu^2) \geq L^2$ and $\th\in[\th_0,\pi/2]$ or
$\th\in[\pi/2,\pi-\th_0]$, with
\be \label{e:gek:def_th_0_Q_0}
    \th_0 := \arcsin\left( \frac{|L|}{a\sqrt{E^2 - \mu^2}} \right) .
\ee
In case (i), one has $\Theta(\th) = 0$ so that the equation of motion
(\ref{e:gek:dthdl}) is fulfilled. In case (ii), a motion at
constant $\th$ is possible as well, the constant value
being either $\th_0$ as given by (\ref{e:gek:def_th_0_Q_0}) or $\pi-\th_0$.
Indeed, $\Theta(\th_0) = 0$ so that (\ref{e:gek:dthdl}) is fulfilled by
$\th(\lambda)=\th_0$. When $\th$ varies in case (ii), we have $\Theta(\th) = 0$
for $\th=\pi/2$, so that the equator is a turning point in the $\th$-motion.
Accordingly, the particle never crosses it. To summarize:
\begin{greybox}
A geodesic with a vanishing Carter constant $Q$ is either a geodesic confined
to the equatorial plane ($\th=\pi/2$) or a geodesic whose constants of motion
$E$, $L$ and $\mu$ obey
\be
    a^2 (E^2 - \mu^2) \geq L^2 .
\ee
In the latter case, the geodesic is located at a constant value
of $\th$, which is $\th_0$ as given by Eq.~(\ref{e:gek:def_th_0_Q_0}) or
$\pi-\th_0$, or has a $\th$-motion on one side of equatorial
plane, with $\th$ ranging from either $\th_0$ to $\pi/2$ or
$\pi/2$ to $\pi-\th_0$. The $\th$-motion extends to the rotation axis
(i.e. $\th_0=0$) if, and only if, $L=0$.
\end{greybox}
