...
accretion of matter onto black holes proceeds via an accretion disk\index{accretion!disk} and
the inner edge of the disk is located at the ISCO, either exactly (low luminosity thin disk)
or very close to it \cite{AbramF13,King23}.
Matter slowly inspiral in the accretion disk, losing energy and angular momentum due to the
friction induced by the disk's differential rotation. The friction heats the disk and the
energy is taken away by electromagnetic radiation. At a given orbit in the disk,
...
orbit (\emph{binding energy}\index{binding energy}) starting from rest far from the
black hole (its energy was then $E_\infty=\mu$).
According to Property~\ref{p:gek:ISCO_extremum_eps_ell}, this radiated energy
is maximal when the particle has reached the ISCO. Let us then evaluate $\veps$
at the ISCO.
...
specific conserved energy and angular momentum of a particle orbiting at the prograde ISCO
...
at the prograde ISCO (solid curve) and retrograde ISCO (dashed curves),
as a function of the Kerr spin parameter $a$.
\textsl{[Figure generated by the notebook \ref{s:sam:Kerr_circular_orbits}]}
}
\end{figure}

\begin{proof}
Let us set $u := r_{\rm ISCO}^\pm/m$. Equation~(\ref{e:gek:a_r_ISCO})
becomes then $a = \pm m \sqrt{u}(4 - \sqrt{3u - 2})/3$.
...
Similarly, after the substitution of $a$, Eq.~(\ref{e:gek:ell_circ}) yields
\[
...
\]
which, after multiplication of both the numerator and denominator
by $\sqrt{3u - 2} + 1$ and simplification, results in the second formula in Eq.~(\ref{e:gek:veps_ell_ISCO}).
\end{proof}
...
and $0.423$ for $a=m$, which leads to the important conclusion:

\begin{prop}[Released energy in an accretion disk around a black hole]
Accretion onto a black hole via an accretion disk releases a significant
