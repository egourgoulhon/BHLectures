...
\]
Using $r_0^2 - 2m r_0 + a^2 - A_\pm  = m r_0 + a^2 \mp 2a \sqrt{m r_0} =
(\sqrt{m r_0} \mp a)^2$, we obtain
\be \label{e:gek:tell2_circ}
    \tilde{\ell}^2_{\pm} = \frac{r_0^2 (\sqrt{m r_0} \mp a)^2}{r_0 (r_0 - 3m) \pm 2 a \sqrt{m r_0}} .
\ee
Since obviously $\tilde{\ell}^2_{\pm} \geq 0$, this expression gives birth to
the constraint $r_0 (r_0 - 3m) \pm 2 a \sqrt{m r_0} > 0$,
...
\begin{align}
    ...
    0 < r_0 < r_* \qquad\mbox{or}\qquad r_0 > r_{\rm min}^+ \\
    & \mbox{for}\ \tilde{\ell}^2_-:\qquad r_0 > r_{\rm min}^- ,
\end{align}
\end{subequations}
with
\be
...
\be \label{e:gek:eps2_circ}
    \veps^2_{\pm} = \frac{\left[ r_0(r_0-2m) \pm a \sqrt{mr_0} \right] ^2}{r^2\left[r_0 (r_0 - 3m) \pm 2 a \sqrt{m r_0}\right]} .
\ee
...
so that the pair $(\veps_+^2, \tilde{\ell}_+^2)$ could generate four solutions
for $(\veps, \tilde{\ell})$, and similarly the pair $(\veps_-^2, \tilde{\ell}_-^2)$ could generate four extra solutions for $(\veps, \tilde{\ell})$, leading to a total of eight solutions.
