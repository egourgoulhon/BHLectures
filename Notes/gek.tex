...
here, refering the interested reader to the article~\cite{FujitW09}.
...
For (i) $\ell\neq 0$ or (ii) $\ell=0$ and $q < a^2 ( 1-\veps^2)$, it follows that
the geodesic oscillates symmetrically about the equatorial plane between
two $\th$-turning points $\th_{\rm min}$ and $\pi-\th_{\rm min}$,
with $\th_{\rm min} \in (0,\pi /2)$ being a zero of $\Theta(\th)$.
From the viewpoint of Eq.~(\ref{e:gek:th_eff_pot_timelike}) above, this
corresponds to a periodic motion in the potential well $-\tilde{\Theta}(\th)$:
\be
...
\end{align}
\end{subequations}
where $\langle T_1(r) \rangle = \hat{T}_{10}$, $\langle T_2(\th) \rangle = \hat{T}_{20}$,
...
\ee
$\Gamma$ and $\Upsilon_\ph$ represent the constant parts in
respectively $\D t/\D\tau'$ and $\D \ph / \D\tau'$, while the oscillatory parts
are the terms in the sums over $k$.
