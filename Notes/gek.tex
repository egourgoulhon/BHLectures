...
\ee
\end{greybox}
Not that if $\Li$ is not a principal null geodesic, (\ref{e:gek:future_directed_Carter}) is
...
\begin{itemize}
\item orbits of the $(\veps_+,\ell_+)$ family with $r_0 >  r_{\rm min}^+$
are called \defin{prograde outer circular orbits}\index{prograde!outer circular orbit};
\item orbits of the $(\veps_+,\ell_+)$ family with $0 < r_0 < r_*$
...
the minimal radius
$r_{\rm min}^+$ decreases with $a$ monotically from $3m$ ($a=0$) to $m$ ($a=m$).
...
$r_{\rm min}^-$ increases with $a$ monotically from $3m$ ($a=0$) to $4m$ ($a=m$).
These orbits are radially unstable for
$r_{\rm min}^- < r_0 \leq r_{\rm ISCO}^-$ and stable for $r_0 > r_{\rm ISCO}^-$,
where $r_{\rm ISCO}^-$ is given by Eq.~(\ref{e:gek:r_ISCO}),
...
the maximal radius $r_*$ increases with $a$ monotically from $0$ ($a=0$) to $m$ ($a=m$).
...
a circular orbit in the equatorial plane is thus a \emph{stationary observer}\index{stationary!observer in Kerr spacetime}\index{observer!stationary --} as defined in Sec.~\ref{s:ker:station_obs}
...
\be
    \lim_{r_0 \to 0} V^{(\ph)}_+ = 1, \quad \lim_{r_0 \to r_*} V^{(\ph)}_+ = - 1,
    \quad \lim_{r_0 \to r_{\rm min}^+} V^{(\ph)}_+ = 1,
    \quad \lim_{r_0 \to r_{\rm min}^-} V^{(\ph)}_- = -1 .
\ee
...
of their domain of existence ($r_0 \to r_*$), this motion being
...
in Sec.~\ref{s:gek:existence_circ_orb}. We shall see in Sec.~?? that
these boundaries correspond to photon orbits.
