...
but that are not solutions of the
geodesic equation (\ref{e:gek:eq_geod})
(see Ref.~\cite{Vanae20} for details). One must add a second condition, which
is obtained as follows. For any timelike equatorial geodesic (not necessarily
circular), the following relation must holds:
\[
...
stationary (e.g. outside possible $r$-turning point), we have $\D r/\D\tau \neq 0$
...
\end{subnumcases}
This is a system of 2 equations for 3 unknowns: $r_0$, $\veps$ and $\ell$.
...
\ee
Substituting (\ref{e:gek:r03_eps2}) for $\veps^2$, we get
\[
...
\ee
The denominator of the above expression can be written as
\[
...
\]
In the numerator of Eq.~(\ref{e:gek:sol_tell2}), we may use the identity
$\pm 2 a \sqrt{m r_0} = A_\pm - r_0 (r_0 - 3m)$ and get, after some
simplifications,
\[
