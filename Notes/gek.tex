...
which is asympotically at rest (marginally bound particle, having $E=\mu$,
...
two $\th$-turning points, which are symmetric about the equatorial plane:
...
$[0,\pi]$;
\item for $Q>0$, $\th$ varies monotically along the geodesic, which
...
The $\th$-turning points $\th_0$ and $\th_1$ mentionned above are solutions of
...
\be \label{e:gek:th0_a2E2mu2_zero}
     \encadre{\theta_0 = \arccos\sqrt{\frac{Q}{L^2 + Q}} . }_{\, a^2 (E^2 - \mu^2) = 0}
\ee
...
$L=0$ case discussed above:
\be \label{e:gek:th0_L_zero}
   \encadre{ \th_0 = \arccos \sqrt{\frac{Q}{a^2 (\mu^2 - E^2)}} . }_{\, L=0}
\ee
This formula assumes that $0 < Q/(a^2 (\mu^2 - E^2)) < 1$, otherwise there
is no $\th$-turning point (curves marked $Q=Q_2$ in Fig.~\ref{f:gek:th_pot_L_0}).

For $L\neq 0$, one shall distinguish two cases:
\begin{itemize}
\item if $|E| < \mu$, then only $x_-$ fulfills the criterium (\ref{e:gek:th_turn_x_pm_range}),
leading to the turning point value:
\be \label{e:gek:th0_E_lt_mu}
    \encadre{ \th_0 = \arccos \left[ \frac{1}{2} \left( 1 + \frac{L^2 + Q}{a^2 (\mu^2 - E^2)}
        - \sqrt{ \left[1 + \frac{L^2 + Q}{a^2 (\mu^2 - E^2)}\right] ^2
        - \frac{4Q}{a^2 (\mu^2 - E^2)}} \right) \right] ^{1/2}}
\ee
\item if $|E| > \mu$, then $x_+$ always fulfills the criterium (\ref{e:gek:th_turn_x_pm_range}),
leading to the turning point value:
\be \label{e:gek:th0_E_gt_mu}
    \encadre{\th_0 =  \arccos  \left[  \frac{1}{2} \left( 1 - \frac{L^2 + Q}{a^2 (E^2 - \mu^2)}
        + \sqrt{ \left[1 - \frac{L^2 + Q}{a^2 (E^2 - \mu^2)}\right] ^2
        + \frac{4Q}{a^2 (E^2 - \mu^2)}} \right)  \right] ^{1/2} } .
\ee
If, in addition $Q<0$, then $x_-$ also fulfills (\ref{e:gek:th_turn_x_pm_range})
and we get a second turning point value:
\be \label{e:gek:th1}
    \encadre{\th_1 =  \arccos  \left[  \frac{1}{2} \left( 1 - \frac{L^2 + Q}{a^2 (E^2 - \mu^2)}
        - \sqrt{ \left[1 - \frac{L^2 + Q}{a^2 (E^2 - \mu^2)}\right] ^2
        + \frac{4Q}{a^2 (E^2 - \mu^2)}} \right)  \right] ^{1/2} } .
\ee
...
between two $\th$-turning points, at $\th=\th_0$ and $\th=\pi-\th_0$, where $\th_0\in(0,\pi/2)$
...
two $\th$-turning points at $\th = \th_0 \in (0,\pi /2)$ and $\th=\pi-\th_0$;
...
One has then $\th_0 \leq \th \leq \pi - \th_0$, with $\th_0$ given by
Eq.~(\ref{e:gek:th0_E_lt_mu}):
\be \label{e:gek:th_min_arccos}
    \encadre{ \th_0 = \arccos \sqrt{ \frac{1}{2} \left( 1 + \frac{\ell^2 + q}{a^2 (1 - \veps^2)}
...
\be
   \encadre{ \th_0 = \arccos \sqrt{\frac{q}{a^2 (1 - \veps^2)}} . }_{\, \ell=0}
\ee
...
that $\th_0 \leq \th \leq \pi -\th_0$ with
$\th_0 \sim 60^\circ$, in agreement with the generic behaviour
...
$r_a \simeq 6.853\, m$ and $\th_0 \simeq 1.060\, {\rm rad} \simeq 60.75^\circ$.
...
$[\th_0, \pi-\th_0] \sim [60^\circ, 120^\circ]$ appears clearly in the
...
\begin{align}
    ...
    & \langle T_2(\th) \rangle = \frac{1}{\Lambda_\th} \int_{0}^{\Lambda_\th}
        T_2(\th(\tau'))\, \D \tau'
        = \frac{4}{\Lambda_\th} \int_{\th_0}^{\pi/2}
...
\be \label{e:gek:def_th_inc}
    \th_{\rm inc} := \frac{\pi}{2} - (\operatorname{sgn}\ell) \th_0 .
\ee
...
(ii) $\th_0$ in the right-hand side of expression~(\ref{e:gek:def_th_inc}) for
...
by Eq.~(\ref{e:gek:rp_ra_p_e}) and $\th_0$ is the function
...
$r_p = 2.175\, m$,
$r_a = 6.853\, m$ and $\th_0 = 1.060\, {\rm rad} = 60.75^\circ$.
...
\be
    ...
    \th(\tau) =: \arccos\left( \cos\th_0 \, \cos\chi(\tau) \right) .
\ee
