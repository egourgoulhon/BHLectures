...
The basic properties of geodesics that one can infer from this system are derived
in Sec.~\ref{s:ges:general_prop}.
Then we focus on timelike geodesics (Sec.~\ref{s:ges:timelike}) and in particular
on those that are bound, which represent orbits of massive particles or bodies around a
...
null Kerr coordinates (Sec.~\ref{s:ker:Kerr_coord}) or Kerr coordinates
...
By Eq.~(\ref{e:fra:orient_time1}) in Lemma~\ref{p:fra:lem1} of Sec.~\ref{s:fra:time_orientation}, one has then necessarily $\w{\xi}\cdot\w{p} < 0$; hence
...
By virtue of Lemma~\ref{p:fra:lem2}  of Sec.~\ref{s:fra:time_orientation}
...
$\wl$ to the outgoing principal null geodesics, introduced in Sec.~\ref{s:ker:principal_geod} [Eq.~(\ref{e:ker:def_ell_outgoing})]. If we apply Lemma~\ref{p:fra:lem2}  of Sec.~\ref{s:fra:time_orientation} with $\wl$ instead
...
We can then apply Lemma~\ref{p:fra:lem1}  of Sec.~\ref{s:fra:time_orientation}, using
...
of Kerr spacetime. Lemma~\ref{p:fra:lem1} of Sec.~\ref{s:fra:time_orientation} [cf. Eq.~(\ref{e:fra:orient_time1})]
then yields
\[
...
has been shown for equatorial geodesics by George Contopoulos\index{Contopoulos, G.} in 1984 \cite{Conto84}.
\end{hist}
...
\begin{remark}
The winding property does not hold for the Kerr or null Kerr coordinates. Indeed,
...
of neither the Kerr coordinates $(\ti,r,\th,\tph)$ nor the null Kerr ones $(v,r,\th,\tph)$.
