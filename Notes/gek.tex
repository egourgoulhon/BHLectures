...
as we did for Eq.~(\ref{e:ges:1d_motion_timelike}) to make it resemble
...
\be \label{e:gek:sol_tell2}
    \tilde{\ell}^2 = r_0 \frac{ (a^2 - 3m^2) r_0^2 + m r_0 (r_0^2 + a^2)
    \pm 2 a \sqrt{m r_0} (r_0^2 - 2m r_0 + a^2)}{r_0 (r_0 - 3m)^2  - 4 a^2 m} .
\ee
...
$\pm 2 a \sqrt{m r_0} = A_\pm - r_0 (r_0 - 3m)$ and get, after
simplification,
\[
    \tilde{\ell}^2 = \frac{r_0^2}{A_+ A_-} \left[ A_\pm (r_0^2 - 2m r_0 + a^2)
     - A_+ A_- \right] = \frac{r_0^2}{A_\mp} \left(
        r_0^2 - 2m r_0 + a^2 - A_\mp \right) .
\]
Using $r_0^2 - 2m r_0 + a^2 - A_\mp  = m r_0 + a^2 \pm 2a \sqrt{m r_0} =
(\sqrt{m r_0} \pm a)^2$, we obtain
\be \label{e:gek:tell2_circ}
    \tilde{\ell}^2 = \frac{r_0^2 (\sqrt{m r_0} \pm a)^2}{r_0 (r_0 - 3m) \mp 2 a \sqrt{m r_0}} .
\ee
Substituting this expression for $\tilde{\ell}^2$ in Eq.~(\ref{e:gek:r03_eps2}), we get,
after simplification,
\be \label{e:gek:eps2_circ}
    \veps^2 = \frac{\left[ r_0(r_0-2m) \mp a \sqrt{mr_0} \right] ^2}{r^2\left[r_0 (r_0 - 3m) \mp 2 a \sqrt{m r_0}\right]} .
\ee
The $\pm$ and $\mp$ in Eqs.~(\ref{e:gek:tell2_circ})-(\ref{e:gek:eps2_circ}) have
to be consistent. In other words, we have two pairs of solutions for
$(\veps^2, \tilde{\ell}^2)$, namely $(\veps_1^2, \tilde{\ell}_1^2)$
and $(\veps_2^2, \tilde{\ell}_2^2)$ with
\be
    \veps_1^2 = \frac{\left[ r_0(r_0-2m) - a \sqrt{mr_0} \right] ^2}{r^2\left[r_0 (r_0 - 3m) - 2 a \sqrt{m r_0}\right]}
    \qand
    \tilde{\ell}_1^2 = \frac{r_0^2 (\sqrt{m r_0} + a)^2}{r_0 (r_0 - 3m) - 2 a \sqrt{m r_0}} ,
\ee
\be
    \veps_2^2 = \frac{\left[ r_0(r_0-2m) + a \sqrt{mr_0} \right] ^2}{r^2\left[r_0 (r_0 - 3m) + 2 a \sqrt{m r_0}\right]}
    \qand
    \tilde{\ell}_2^2 = \frac{r_0^2 (\sqrt{m r_0} - a)^2}{r_0 (r_0 - 3m) + 2 a \sqrt{m r_0}} .
\ee
A priori, $\veps_1^2$ leads to two values for $\veps$, namely $\pm\sqrt{\veps_1^2}$
and $\tilde{\ell}_1^2$ leads to two values for $\tilde{\ell}$, namely $\pm\sqrt{\tilde{\ell}_1^2}$,
so that the pair $(\veps_1^2, \tilde{\ell}_1^2)$ could generate four solutions
for $(\veps, \tilde{\ell})$, and similarly the pair $(\veps_2^2, \tilde{\ell}_2^2)$ could generate four extra solutions for $(\veps, \tilde{\ell})$, leading to a total of eight solutions.
However, these solutions satisfy the ``squared'' equation (\ref{e:gek:ar0_eps_ell_square})
but not all of them satisfy the original equation (\ref{e:gek:ar0_eps_ell}).
Moreover, to be physically acceptable, a solution for $(\veps,\ell)$ must obey
the future-directed geodesic condition (\ref{e:gek:future_directed}), which
for circular orbits reduces to
\be
\frac{1}{r_0^2 - 2 m r_0 + a^2} \left( \veps - \frac{a}{r_0^2 + a^2} \ell \right) > 0 ,
\ee
since $R(r_0) = 0$ by virtue of Eq.~(\ref{e:gek:circ_V_zero}).
