...
is a null geodesic, the term $\w{g}(\w{p},\w{p})$ vanished identically, so that
one is left with
\[
...
with $M\geq 1$ $r$-turning points $r_1,\ldots, r_M$ between $\lambda'_0$ and $\lambda'$
is
...
$\th$-turning points between $\lambda'_0$ and $\lambda'$
...
directly from the constraint $\Theta(\th)\geq 0$, by noticing the identity
...
Following the above definition, a geodesic $\Li$ with $|E| = \mu$ is naturally
called a \defin{geodesic with a marginally bound orbit}\index{geodesic!with a
marginally bound orbit}\index{marginally!bound!orbit}, or a
\defin{marginally bound geodesic}\index{marginally!bound!geodesic}.
Note that such a geodesic can be null, provided it has $E=0$
(cf. Example~\ref{x:gek:null_generator_hor} on page~\pageref{x:gek:null_generator_hor}).
We have
\begin{greybox}
Any marginally bound geodesic $\Li$ has $Q\geq 0$ and is necessarily confined
...
$\mathscr{K} = Q + (L - a E)^2$ [Eq.~(\ref{e:gek:def_Q})], with $Q=L=E=0$.
Then, from the result (\ref{e:gek:K_zero_null}), $\Li$ is a principal null
geodesic. Moreover, $Q+L^2=0$ implies that $U(r)$ vanishes identically,
so that Eq.~(\ref{e:gek:r_eff_potential}) along with $Q=0$ yields
$r(\lambda') = \mathrm{const}$. Now, the only principal null geodesics lying
at a fixed value of $r$ are the ones from the outgoing family that generates
the horizons $\Hor$ and $\Hor_{\rm in}$ (cf. Figs.~\ref{f:ker:princ_null_geod_a90} and \ref{f:ker:princ_null_geod_a50}). They do have $E=0$ and $L=0$
(cf. Example~\ref{x:gek:null_generator_hor} on page~\pageref{x:gek:null_generator_hor})
and lie at $r=r_+$ or $r=r_-$. Since both $r_+$ and $r_-$ are positive
[Eq.~(\ref{e:ker:order_r_pm})], we cannot have $r<0$ in this case either.
\end{proof}

A timelike marginally bound geodesic can reach the asymptotic region
$r\to +\infty$. It has then $E=\mu$, which implies that the Lorentz factor
with respect to the asymptotic static observer of 4-velocity $\w{\xi}$ is
$\Gamma=1$ (cf. Remark~\ref{r:gek:bound_geod} above), which implies that
$\w{p}$ is collinear to $\w{\xi}$. In that sense, a timelike marginally
bound geodesic is asymptotically ``at rest''.

On the opposite, a null marginally bound geodesic cannot reach the asymptotic region
$r\to +\infty$. Actually it cannot exist outside the ergoregion of Kerr spacetime.
Indeed such a geodesic has necessarily $E=0$, which is not possible to
achieve from the definition $E = -\w{\xi}\cdot\w{p}$ [Eq.~(\ref{e:gek:def_E})]
with $\w{\xi}$ timelike
(exterior of the ergoregion) and $\w{p}$ null, given that a timelike vector
can never be orthogonal to a null one (Corollary~1 in Sec.~\ref{s:fra:time_orientation}).
...
are marginally bound [Eq.~(\ref{e:gek:generator_hor_E_L_zero})]
...
A timelike or null geodesic with $|E| > \mu$ is called a
\defin{geodesic with a unbound orbit}\index{geodesic!with a unbound orbit}\index{unbound!orbit},
or in short, a \defin{unbound geodesic}\index{unbound!geodesic}.
An immediate corollary of the above two properties is
\begin{greybox}
Only unbound geodesics can have some part in the region $r<0$ of Kerr spacetime, $\M_-$:
\be
