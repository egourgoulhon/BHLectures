...
\begin{subequations}
\begin{align}
& \encadre{ \rho^2 \derd{t}{\lambda} = \frac{r^2 + a^2}{\Delta} [(r^2 + a^2) E - a L] + a ( L - a E \sin^2\th ) }\\
& \encadre{ \rho^2 \derd{r}{\lambda} = \pm \sqrt{ R(r) } }\\
& \encadre{ \rho^2 \derd{\th}{\lambda} = \pm \sqrt{\Theta(\th)} }\\
...
in 1968 \cite{Carte68}. He actually did not get $\mathscr{K}$ from the Killing tensor $\w{K}$,
...
the constant $Q$ via Eq.~(\ref{e:gek:def_Q}).
\end{hist}
