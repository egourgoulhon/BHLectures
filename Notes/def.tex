...
basis of $T_p^\perp\Sp$. One can construct from it an \emph{orthonormal}
basis $(\w{n},\w{s})$ as follows:
\be
...
Lie dragging of the surface $\Sp$ along $\wl$ by a small parameter $\varepsilon$.}
\end{figure}

Let us define the expansion of the spacelike section $\Sp$ along the vector
field $\wl$ as follows. Given an infinitesimal parameter $\varepsilon>0$, take a point
$p\in \Sp$ and displace it by the infinitesimal vector $\varepsilon \wl$, thereby getting
a nearby point $p'$ (cf. Fig.~\ref{f:def:expansion}).
Since $\wl$ is tangent to $\Hor$ and $p\in\Hor$, we have $p'\in\Hor$.
By repeating this for each point in $\Sp$,
keeping the value of $\varepsilon$ fixed, we define a new codimension-2 surface
$\Sp'$ (cf. Fig.~\ref{f:def:expansion}). One says that $\Sp'$ is obtained
from $\Sp$ by \defin{Lie dragging along $\wl$ by the parameter $\varepsilon$}\index{Lie!dragging}.
Since $p'\in\Hor$ for every $p\in\Sp$, we have $\Sp'\subset \Hor$.

At each point $p\in\Sp$, the \defin{expansion of $\Sp$ along $\wl$} is defined from the
rate of change $\theta_{(\wl)}$ of the area $\delta A$ of an element of surface of
$\Sp$ around $p$:
\be
    \theta_{(\wl)} := \lim_{\varepsilon\rightarrow 0} \frac{1}{\varepsilon}
    \frac{\delta A' - \delta A}{\delta A} .
\ee
In the above formula, $\delta A'$ stands for the area of the
surface element $\delta S'\subset \Sp'$ that is obtained from $\delta S$ by
