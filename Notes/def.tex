...
as a submanifold $\Sp$ of $\Hor$ of codimension\footnote{Considered as a submanifold
of $\M$, the codimension of $\Sp$ is 2, not 1.} 1 (i.e. $\dim \Sp=n-2$),
...
stationary black holes, as we shall see in Sec.~\ref{s:sta:BH_stationary}.
But for $n > 4$, other
closed-manifold topologies, like that of a torus, are allowed for
cross-sections of a stationary black hole.

A first important property of cross-sections is the following one.
...
The complete cross-sections are then compact and a natural choice
for them is that of the spheres defined by $t=t_0$ for some positive constant $t_0$:
\[
