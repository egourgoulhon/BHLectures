...
\subsection{Spacetime symmetries} \label{s:def:symmetries}
...
$t$ is a natural parameter along the curve (cf. Fig.~\ref{f:def:orbit_group}). The tangent vector corresponding to that parameter is called the \defin{generator of the group} $G$
...
$\Phi_{\D t}(p)$ (cf. Sec.~\ref{s:bas:vectors} and Fig.~\ref{f:def:orbit_group}).
We have then
\begin{greybox}
...
a bilinear form, we may reexpress the isometry condition (\ref{e:def:isometry})
in terms of the
pullback of $\w{g}$ by $\Phi_t$:
\be \label{e:def:isometry_pullback}
    \Phi_t^*\w{g} = \w{g} .
\ee
...
(\ref{e:def:isometry_pullback}) leads then to
...
we see immediately that Eq.~(\ref{e:def:Lie_xi_g}) is equivalent to the so-called
\defin{Killing equation}\index{Killing!equation}:
\be \label{e:def:Killing_equation}
    \encadre{ \nabla_\alpha \xi_\beta + \nabla_\beta \xi_\alpha = 0 }.
\ee

\subsection{Definition and examples of Killing horizons} \label{s:def:def_Killing_hor}
...
1-parameter group, as described in Sec.~\ref{s:def:symmetries}.
...
\caption[]{\label{f:def:hplaneKilling-boost} \footnotesize
...
On $\Hor$ we have (cf. Fig.~\ref{f:def:hplaneKilling-boost}):
\[
...
$\Hor\setminus\{t=0\}$ in two open half-hyperplanes (cf. Fig.~\ref{f:def:hplaneKilling-boost}):
\[
...
\be \label{e:def:xi_null_rotation}
...
the vector $\w{\xi}$ defined by (\ref{e:def:xi_null_rotation}) obeys
Killing equation [Eq.~(\ref{e:def:Killing_equation})].
On $\Hor$, $t-x=0$, so that
(\ref{e:def:xi_null_rotation}) reduces to
\[
    \w{\xi}  \equalH y ( \wpar_t + \wpar_x )  \equalH y \, \wl .
\]
It follows that $\w{\xi}$ is a null normal to $\Hor$ as soon as $y\not=0$.
We may then split $\Hor\setminus\{y=0\}$ in two open half-hyperplanes (cf. Fig.~\ref{f:def:hplaneKilling-nullrot}):
\[
...
\caption[]{\label{f:def:hplaneKilling-nullrot} \footnotesize
...
$\wl \equalH \w{\xi}$.
Equation~(\ref{e:def:Lie_xi_g}) then implies:
\[
    \Lie{\el} \w{g} \equalH 0 .
\]
...
\begin{example}[null hyperplane as a translation-Killing horizon]
For the null hyperplane $\Hor$ considered in Example~\ref{x:def:transKH} as a Killing horizon with respect to the translation group along its normal, we have
...
and $\Hor^-$ of Example~\ref{x:def:boostKH}, which are Killing horizons with
...
\[
    \kappa_+ = 1 .
\]
On $\Hor^-$, $\w{\xi}$ is past-directed (cf. Fig.~\ref{f:def:hplaneKilling-boost}).
...
\begin{example}[null hyperplane as a null-rotation-Killing horizon]
In Example~\ref{x:def:nullrotKH}, we have introduced the Killing horizons
$\Hor_1$ and $\Hor_2$ with respect to the null-rotation Killing vector
$\w{\xi} = y( \wpar_t + \wpar_x ) + (t-x)\wpar_y$ of Minkowski spacetime.
On $\Hor_1$, $\w{\xi}$ is past-directed (cf. Fig.~\ref{f:def:hplaneKilling-nullrot}),
...
\[
    \kappa_1 = 0 .
\]
On $\Hor_2$, $\w{\xi}$ is future-directed (cf. Fig.~\ref{f:def:hplaneKilling-nullrot})
