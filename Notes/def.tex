\chapter{The concept of black hole 1: Horizons and null hypersurfaces}
...
\caption[]{\label{f:def:membranes} \footnotesize
A timelike hypersurface is a two-way membrane, while null and spacelike hypersurfaces
are one-way membranes.}
\end{figure}
...
\end{itemize}
The hypersurface type can also be deduced from any normal vector
...
regarding timelike and null worldlines (cf. Fig.~\ref{f:def:membranes}).
...
Since $\wl$ is colinear to $\wl'$, it obviously shares the same field lines,
...
\end{quote}
As a byproduct of (\ref{e:def:nab_lp_lp}), we get the behavior of the
...
in the Minkowksian coordinates $(t,x,y,z)$, we get
...
as a submanifold $\Sp$ of $\Hor$ of codimension 2,
such that each null geodesic generator of $\Hor$ intersect $\Sp$ once, and only once.
...
On its side, the future sheet $\Hor$ of the light cone of Minkowkski spacetime considered in Examples~\ref{x:def:light_cone},
\ref{x:def:light_cone2} and \ref{x:def:light_cone3} does obey (\ref{e:def:H_topology}),
since we have excluded the origin from $\Hor$.
...
has exactly the topology (\ref{e:def:H_topology}). Since its is defined
by $r=2m$ in terms of the 3+1 Eddington-Finkelstein coordinates $(t,r,\th,\ph)$,
a natural coordinate system on $\Hor$ is $x^A = (t,\th,\ph)$. Morevover, we
...
\[
    \Sp = \left\{ p \in \Hor,\  t(p) = t_0 \right\} .
\]
$x^a=(\th,\ph)$ constitutes then a coordinate system on $\Sp$.
...
For the light cone in Minkowkski spacetime,
...
specific case of the Schwarzschild horizon, $r=2m$ and we get a vanishing
expansion:
...
$\delta S$ around $p$ that are enclosed in tube made of null geodesic generators have the same
...
case of the Schwarzschild horizon $\Hor$, $r=2m$ and we get
...
The Schwarzschild horizon considered in Examples~\ref{x:def:Schw_hor}, \ref{x:def:Schw_hor2},
...
[cf. Eq.~(\ref{e:def:theta_Schw_hor})].
\end{example}
...
\ee
where $x^a = (x^2, \ldots, x^{n-1})$ is a coordinate system on $\Sp$ and $q$
...
\begin{example}[Schwarzschild horizon]
The area of the Schwarzchild horizon is readily computed from
...
horizon $\Hor$ is defined by $r=2ùm$, we have
...
A much less trivial example is that of the event horizon of the Kerr black hole,
which we shall discuss in Chap.~\ref{s:ker}. This horizon is only axisymmetric,
so that a priori $\kappa$ could depend on $\th$. But it does not:
\[
    \kappa = \frac{\sqrt{m^2 - a^2}}{2m(m + \sqrt{m^2-a^2})} ,
\]
where $(m,a)$ are the two parameters of the Kerr solution. Note that for $a=0$,
