...
This requires the concepts of Killing vector and 1-dimensional group of
...
\ee
is a Killing vector field of Minkowski spacetime: $\w{\xi}$ is indeed the
...
\begin{example}[light cone (counter-example)]
...
Let $\Hor$ be a Killing horizon with closed cross-sections, i.e. $\Hor$ has the
topology (\ref{e:def:H_topology}). Let us select the null normal $\wl$
...
\textbf{\small with closed cross-sections}\\
$\w{\Theta}=0$ \\
