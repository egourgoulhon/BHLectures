...
$\Sp_\varepsilon$ is defined as the set of points $p$
such that $(x^0(p), x^1(p)) = (\varepsilon,0)$
...
case of the Schwarzschild horizon, $r=2m$ and (\ref{e:def:theta_Schw_hor_u})
...
covariant derivative $\wnab$ via Eq.~(\ref{e:bas:Lie_der_comp_nab}), we get
\[
...
(i.e. contracting with $\el^\alpha$), we get, since
$\omega_\alpha \el^\alpha = \kappa$ [Eq.~(\ref{e:def:omega_l_kappa})]
and $\el_\alpha \el^\alpha =0$,
...
the \defin{null Raychaudhuri equation}\index{null!Raychaudhuri equation}\index{Raychaudhuri!null --  equation}:
