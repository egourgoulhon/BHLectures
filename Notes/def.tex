...
leaves the metric tensor invariant:
...
\ee
where $\Phi_t^*\w{u}$ is the pushforward of the vector $\w{u}\in T_p\M$
to the tangent space $T_{\Phi_t(p)}\M$ (cf. Sec.~\ref{s:bas:Lie_der_vector}
in Appendix~\ref{s:bas}).
...
Thus a Killing horizon requires that the spacetime $(\M,\w{g})$ has some
continuous symmetry
