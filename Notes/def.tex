...
\be \label{e:def:v_u_zero}
    \w{v}(u) = v^\mu \der{u}{x^\mu} = 0 .
\ee
[cf. Eqs.~(\ref{e:bas:dell_f}), (\ref{e:bas:dell_v_dlamb}) and (\ref{e:bas:v_va_wpar_a})
in Apppendix~\ref{s:bas}].
...
terms of components:
\be
    n^\alpha = \nabla^\alpha u = g^{\alpha\mu} \der{u}{x^\mu} .
\ee
