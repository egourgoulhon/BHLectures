...
on the extension of $\wl$ outside $\Hor$ (cf. the discussion in \S~\ref{s:def:null_gener}).
...
The dependency of the expansion on $\wl$ is explicited by the following
...
by taking the derivative of $\theta_{(\wl)}$ along $\wl$. Starting from
$\theta_{(\wl)} = \wnab\cdot\wl - \kappa$ [Eq.~(\ref{e:def:theta_div_l})], we have
\be \label{e:def:evol_theta_debut}
    \el^\mu \nabla_\mu \theta_{(\wl)} = \el^\mu \nabla_\mu \left(
        \nabla_\nu \el^\nu - \kappa \right)
            = \el^\mu \nabla_\mu \nabla_\nu \el^\nu - \el^\mu \nabla_\mu  \kappa .
\ee
Let us permute the indices $(\mu,\nu)$ in $\nabla_\mu \nabla_\nu \el^\nu$
via the Ricci identity [Eq.~(\ref{e:bas:Ricci_ident}) in Appendix~\ref{s:bas}]
applied to $\wl$:
\[
   \left(\nabla_\alpha\nabla_\beta
...
$\w{g}$.
Taking the trace on the indices $(\alpha,\gamma)$ and relabeling $\beta\rightarrow\mu$ yields
\[
    \nabla_\nu \nabla_\mu \el^\nu - \nabla_\mu \nabla_\nu \el^\nu =
        R_{\mu\nu} \el^\nu ,
\]
where $R_{\mu\nu} = R_{\nu\mu} = R^\sigma_{\ \  \nu \sigma\mu}$ is
the Ricci tensor of $\w{g}$. Hence
$\nabla_\mu \nabla_\nu \el^\nu = \nabla_\nu \nabla_\mu \el^\nu - R_{\mu\nu} \el^\nu$,
so that (\ref{e:def:evol_theta_debut}) can be rewritten as
\bea
    \el^\mu \nabla_\mu \theta_{(\wl)} &= &\el^\mu  \nabla_\nu \nabla_\mu \el^\nu
        - R_{\mu\nu} \el^\mu \el^\nu - \el^\mu \nabla_\mu  \kappa \nonumber \\
    & = & \nabla_\nu (\underbrace{\el^\mu \nabla_\mu \el^\nu}_{\kappa \el^\nu})
        - \nabla_\nu \el^\mu \nabla_\mu \el^\nu
        - R_{\mu\nu} \el^\mu \el^\nu - \el^\mu \nabla_\mu  \kappa \nonumber \\
    & = & \el^\nu \nabla_\nu \kappa + \kappa \underbrace{\nabla_\nu \el^\nu}_{\theta_{(\wl)} + \kappa}
    - \nabla_\nu \el^\mu \nabla_\mu \el^\nu
        - R_{\mu\nu} \el^\mu \el^\nu - \el^\mu \nabla_\mu  \kappa \nonumber \\
    & = & \kappa(\theta_{(\wl)} + \kappa) - \nabla_\nu \el^\mu \nabla_\mu \el^\nu
        - R_{\mu\nu} \el^\mu \el^\nu  . \label{e:def:null_Raych_prov}
\eea
Now, from Eq.~(\ref{e:def:nab_l_Theta}), we have
\[
    \nabla_\nu \el^\mu \nabla_\mu \el^\nu =
    \left(\Theta_\nu^{\  \, \mu} + \omega_\nu \el^\mu - \el_\nu k^\rho \nabla_\rho \el^\mu\right)
    \left(\Theta^\nu_{\ \, \mu} + \omega_\mu \el^\nu - \el_\mu k^\sigma \nabla_\sigma \el^\nu\right) .
\]
Expanding and using $\Theta_\nu^{\  \, \mu} \el^\nu = 0$ [Eq.~(\ref{e:def:Theta_sigma_tangent})],
$\el^\mu \el_\mu = 0$ and the symmetry of $\w{\Theta}$ results in
\[
    \nabla_\nu \el^\mu \nabla_\mu \el^\nu = \Theta_{\mu\nu} \Theta^{\mu\nu}
        + (\omega_\mu \el^\mu)^2 .
\]
The first term in the right-hand side can be re-expressed by the decomposition
...
\eea
while the second term is easily evaluated via Eq.~(\ref{e:def:def_omega}):
\[
    \omega_\mu \el^\mu = - k^\nu \underbrace{\el^\mu \nabla_\mu \el_\nu}_{\kappa \el_\nu}
...
\[
    \nabla_\nu \el^\mu \nabla_\mu \el^\nu =
    \sigma_{ab} \sigma^{ab} + \frac{1}{n-2} \, \theta_{(\wl)}^2 + \kappa^2
\]
and Eq.~(\ref{e:def:null_Raych_prov}) becomes
\be
    \encadre{ \wnab_{\el}\,  \theta_{(\wl)} = \kappa \theta
        - \frac{1}{n-2} \, \theta_{(\wl)}^2 - \sigma_{ab} \sigma^{ab}
        - \w{R}(\wl, \wl) }.
\ee
This evolution equation of $\theta_{(\wl)}$ along $\wl$ is known as the
...
Having introduced the concept of expansion of a null hypersurface,
let us make an attempt to distinguish a black hole event horizon from, let us
say, a future light cone.
