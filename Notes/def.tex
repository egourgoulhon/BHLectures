...
\ee
A first important property of $\Sp$ is to be spacelike, i.e. every vector
...
that $\w{p}$ is necessarily spacelike, which proves that $\Sp$ is a spacelike
submanifold.

Let us denote by $\w{q}$ the
\defin{metric induced on $\Sp$ by $\w{g}$}\index{induced!metric}\index{metric!induced --},
i.e. the bilinear form defined by
\be
...
\ee
In other words, $(\Sp, \w{q})$ is a Riemannian manifold.
...
$T_p^\perp\Sp$ which is a timelike plane and is such that
$T_p\M$ is the direct sum of $T_p\Sp$ and $T_p^\perp\Sp$ :
\be
    \forall p\in \Sp,\quad T_p\M = T_p\Sp \oplus T_p^\perp\Sp .
\ee
...
which can be seen as the two intersection of the null cone at $p$ with
the 2-plane $T_p^\perp\Sp$ (cf. Fig.~??).
Let us denote by $\w{k}$ a null vector in the null direction that is not
defined by $\wl$. By a proper rescaling $\w{k}\mapsto \alpha\w{k}$,
