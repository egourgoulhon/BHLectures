...
it must located at $x = 1/\alpha$, i.e. at $t/r = 1/\alpha - 1$,
or equivalently at
...
prolonged to the past in $\M_{\rm rad}$, i.e. to $x < 1/\alpha$,
...
observer $\Obs$, the partial Cauchy surface $\Sigma$ and its future Cauchy development $D^+(\Sigma)$ (hatched area, cf. Sec.~\ref{s:ker:Cauchy_hor} for the definition), which is bounded in the future by $\scri^+$ and the Cauchy horizon $\Hor_{\rm C}$).}
\end{figure}
