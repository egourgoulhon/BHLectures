...
as a coordinate system on $\M_{\rm rad}$. By construction $u$ is constant
along the outgoing radial null geodesics and $v$ is constant along the ingoing
ones. From Eq.~(\ref{e:vai:eq_out_v0_small}), we get
\[
    \frac{1}{r} \D r = - \D u + \derd{}{x}\left( \ln f_\alpha(x) \right) \, \D x
        = - \D u +  \frac{1 - \alpha x}{\alpha x^2 - x + 2} \D x .
\]
