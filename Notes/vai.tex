...
In the Schwarzschild region, the event horizon is the hypersurface
$r = 2m$. It is generated by null geodesics belonging to the family
of the outgoing radial ones. Let us consider one such geodesic, $\hat{\Li}$ say.
...
black curve). If one prolongs $\hat{\Li}$ into the radiation
...
If we extends further the radial null geodesic $\hat{\Li}$
to the past in the Minkowksi
region, it becomes the straight line of equation $t = r - 2 r_B$.
$\hat{\Li}$ thus reaches $r=0$ at some event $C$ of
coordinate\footnote{As in Chap.~\ref{s:lem}, the subscipt `hb' stands
for \emph{horizon birth} and will below justified below.}
$t = t_{\rm hb} = -2 r_B$.
...
starts as the future light cone of the point of IEF coordinates
$(t, r) = (t_{\rm hb}, 0)$, with
\be \label{e:vai:t_hb}
...
event horizon at $r=2m$ of Schwarzschid spacetime.
\end{greybox}

\begin{proof}
Let us now consider an outgoing radial null geodesic $\Li$
...
$\scri^+$. We conclude that the null hypersurface generated by
the geodesics $\hat{\Li}$ for all the values of $(\th,\ph)$ is a black hole
horizon.
\end{proof}
