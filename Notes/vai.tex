...
\subsection{Solution for $M(v)$ piecewise linear}
...
\be \label{e:vai:def_alpha}
    \alpha := \frac{2m}{v_0} .
\ee
It is immediately apparent that on (\ref{e:vai:self_similar_metric})
that for any $\mu > 0$, the homothety $H_\mu:\ (v, r) \mapsto (\mu v, \mu r)$
maps $\w{g}$ to $\mu^2 \w{g}$. Hence $H_\mu$ is a
conformal isometry\index{conformal!isometry}\index{isometry!conformal --} of
$\w{g}$ with a constant conformal factor $\mu^2$. The homotheties $(H_\mu)_{\mu\in \R_{>0}}$
form a 1-dimensional
group, the generator of which is obtained by considering infinitesimal transformations,
i.e. homotheties of factor $\mu = 1 + \D\lambda$ where $\D\lambda$ is
...
coordinates should not be surprising since the homethety $H_\mu$ has the
same expression in both coordinate systems:  $H_\mu:\ (t, r) \mapsto (\mu t, \mu r)$,
given that $\mu v = \mu(t + r) = \mu t + \mu r$.
...
In other words, except for $\mu=1$, the homotheties $H_\mu$ are not isometries,
...
They fulfill $\Lie{\xi} \w{g} = \lambda \w{g}$, where $\lambda$ is a scalar field.
Equation~(\ref{e:vai:Lie_xi_g}) constitutes the particular subcase $\lambda = 2$.
\end{remark}
...
\be
    x := \frac{v}{r} ,
\ee
which is invariant under the homotheties $H_\mu$.
...
quadratic polynomial $P_\alpha(x) := \alpha x^2 - x + 2$ admit some real roots
or not. Let us first focus on the case where $P_\alpha$ has not real root.
...
\be \label{e:vai:sol_r_x_v0_small}
 \encadre{   r = \frac{\sqrt{2}\,  r_0}{\sqrt{\alpha x^2 - x + 2}}
...
\ee
where the integration constant $r_0>0$ is the value of $r$ at $x=0$, or equivalently
at $v = 0$.
Hence:
\begin{greybox}
...
The equation of $\Li_{(r_0,\th,\ph)}$ in terms of the IEF coordinates
...
\ee
where the function $f(x)$ is given by the right-hand side of
Eq.~(\ref{e:vai:sol_r_x_v0_small}) and $x_{\rm max} = +\infty$ if $r$ reaches
$0$ along $\Li_{(r_0,\th,\ph)}$ for $v < v_0$
or $x_{\rm max}$ is the solution of $x_{\rm max} f(x_{\rm max}) = v_0$
(outer edge of the radiation shell).
\end{greybox}
...
In spherical symmetry, the inspection of the radial null geodesics give us
a direct access to the black hole event horizon.
It appears clearly on Fig.~\ref{f:vai:diag_S0} that the outgoing
radial null geodesics that cross $r=0$ for $t$ larger than a certain
threshold, $t_{\rm hb}$ say\footnote{As in Chap.~\ref{s:lem}, the subscipt `hb' stands
for \emph{horizon birth}.}, do not reach
arbitraly large values of $r$. They all eventually
fall back to the curvature singularity at $r=0$.
The value of $t_{\rm hb}$ can be determined by integrating Eq.~(\ref{e:vai:ODE_outgoing_null})
backward in time from the initial value $r = 2m$ at $t = t_{\rm init}$, with $t_{\rm init} > v_0 - 2 m$.
Indeed, $r=2m$ is the location of the event horizon
in the Schwarzschild region $t + r > v_0$, which is generated by null geodesics from the
outgoing family.
