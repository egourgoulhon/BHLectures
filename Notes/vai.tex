...
\ee
We shall discuss the case $v_0 \geq 16 \, m$ in Sec.~\ref{s:vai:naked_sing}.
Assuming (\ref{e:vai:v0_small}),
...
\subsection{Configurations with a naked singularity} \label{s:vai:naked_sing}

Still within the framework of $M(v)$ piecewise linear (homothetic radiation shell,
Sec.~\ref{s:vai:sol_M_linear}), let us consider the case
\be
    \alpha < \frac{1}{8} \iff v_0 > 16 \, m .
\ee
The polynomial $P_\alpha(x) := \alpha x^2 - x + 2$, which appears in the
numerator of the ODE (\ref{e:vai:ODE_outgoing_x}) ruling outgoing radial null geodesics,
admits then two real roots:
\be
...
Let us now turn to the generic outgoing radial null geodesics: we assume
$x\neq x_1$ and $x \neq x_2$ and rewrite Eq.~(\ref{e:vai:ODE_outgoing_x_naked}) as
\[
    \D \ln r = \frac{x_1 + x_2 - x}{(x - x_1)(x - x_2)} \, \D x .
\]
The generic solution is
\be \label{e:vai:sol_r_x_out_naked}
    r = r_* \frac{| x - x_2 | ^{x_1/(x_2 - x_1)}}{| x - x_1 | ^{x_2/(x_2 - x_1)}} ,
\ee
where $r_*$ is constant along a given geodesic.
The function $r(x)$ given by Eq.~(\ref{e:vai:sol_r_x_out_naked})
is plotted in Fig.~\ref{f:vai:r_x_naksing} for $\alpha=1/9$, which is
equivalent to $x_1 = 3$ and $x_2 = 6$.
...
$\alpha = 1/9$, which implies $x_1 = 3$ and $x_2 = 6$, so that
Eq.~(\ref{e:vai:sol_r_x_out_naked}) simplifies to
$r/r_* = |x - 6|/(x - 3)^2$. The graph of $r(x)$ admits a local
maximum for $x=1/\alpha$, i.e. $x=9$ in the present case, although this
is barely noticeable on the figure.
}
\end{figure}

Having determined the outgoing radial null geodesics, we get
\begin{greybox}
The infalling radiation shell described by the Vaidya metric with $M(v)$ piecewise linear
...
is naked\index{naked singularity}: it is connected to remote observers
by the outgoing radial null geodesics $\Li^{*1}_{(\th,\ph)}$, as well as
by the family of outgoing radial
null geodesics that crosses the outer edge of the radiation shell ($v=v_0$) with
...
point $A$ such that $x_A=1/\alpha$ (cf. Fig.~\ref{f:vai:diag_naked_S0}). When
...
$x$ such that $x_1 < x < 1/\alpha$, i.e. between the points $A$ and $C$
