...
\defin{future outer trapping horizon}\index{future!outer trapping horizon}\index{trapping horizon!future!outer --}. In the Schwarzschild region, Eq.~(\ref{e:vai:def_T}) reduces
...
$\mathscr{T}$ in $\M_{\rm rad}$ is vey simple: $ r = \alpha(t + r)$, or equivalently
$t = (\alpha^{-1} - 1) r$. For $v_0 = 3m$, as in Fig.~\ref{f:vai:diag_S0},
$\alpha = 2/3$ and we get $t = r / 2$. Hence, in $\M_{\rm rad}$,
$\mathscr{T}$ appears as the straight line segment of slope $1/2$
drawn in red in Fig.~\ref{f:vai:diag_S0}.
By considering the light cones delineated by the radial null geodesics, it appears
clearly from Fig.~\ref{f:vai:diag_S0} that $\mathscr{T}$ is a spacelike hypersurface in $\M_{\rm rad}$.
This is actually a generic feature of future outer trapping horizons in dynamical spacetimes,
...
region $\M_{\rm rad}$. This is acheived by considering $(u, v, \th,\ph)$
as a coordinate system on $\M_{\rm rad}$.
The relation between the coordinates $(u, v, \th,\ph)$ and
...
$u_\Hor = 2 u_0 - \ln \alpha - {\pi}/{\sqrt{8\alpha - 1}}$. Given expression
...
pale blue, yellow and grey.
Solid (resp. dashed) green lines are outgoing (resp. ingoing)
radial null geodesics. The hatched area is the black hole region,
delimited by the event horizon $\Hor$.
...
In view of this, one obtains the Carter-Penrose diagram shown in
...
is \defin{naked}\index{naked singulariy}\index{singularity!naked --}.
...
observer $\Obs$, the partial Cauchy surface $\Sigma$ and its future Cauchy development $D^+(\Sigma)$ (hatched area, cf. Sec.~\ref{s:ker:Cauchy_hor} for the definition), which is bounded in the future by
...
to remote observers. One may say that the
singularity $O_2 O_3$ is ``clothed'' by the event horizon $\Hor$.
\end{remark}

The remote observers see the naked singularity as soon as they cross
