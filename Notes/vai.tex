...
i.e. homotheties of ratio $\mu = 1 + \D\lambda$ where $\D\lambda$ is
...
coordinates should not be surprising since the homothety $H_\mu$ has the
...
Eq.~(\ref{e:vai:sol_r_x_v0_small}) and either $x_{\rm max} = +\infty$ if $r$ reaches
...
Eqs.~(\ref{e:vai:eq_out_v0_small})-(\ref{e:vai:sol_r_x_v0_small}).
It appears clearly on the figure that, in the radiation region, a homothety of center $(t, r) = (0,0)$
and of ratio $\mu$
transforms a geodesic $\Li_{(r_0,\th,\ph)}$ to the geodesic $\Li_{(\mu r_0,\th,\ph)}$ of
the same family, in agreement with the homothetic symmetry discussed
above.
...
Let us determine explicitely the location of the latter for the homothetic
shell collapse considered above, since we have at disposal the solution
(\ref{e:vai:sol_r_x_v0_small}) for the outgoing radial null geodesics.
