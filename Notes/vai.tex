...
case, $r$ increases monotically along $\Li_{(u,\th,\ph)}^{\rm out}$ in the radiation region,
...
If a geodesic $\Li_{(u,\th,\ph)}^{\rm out}$  has a $r$-turning point in the
...
In order to draw a conformal Carter-Penrose diagram of the Vaidya collapse,
we need first to introduce a double null coordinate system in the radiation
region $\M_{\rm rad}$. More precisely, let us use $(u, v, \th,\ph)$
as a coordinate system on $\M_{\rm rad}$.
The relation between the oordinates $(u, v, \th,\ph)$ and
$(v, r, \th, \ph)$ is provided by Eq.~(\ref{e:vai:eq_out_v0_small}):
\be \label{e:vai:u_lnr_v}
    u = - \ln\left(\frac{r}{m}\right) + \ln f_\alpha\left(\frac{v}{r}\right) .
\ee
By construction $u$ is constant
along the outgoing radial null geodesics and $v$ is constant along the ingoing
ones.
...
\]
By comparison with (\ref{e:vai:self_similar_metric}), we deduce immediately
...
nonzero in all the radiation region $\M_{\rm rad}$. This shows that
coordinates $(u,v,\th,\ph)$ are regular coordinates on $\M_{\rm rad}$
...
by $(u,\th,\ph) = \mathrm{const}$. Furthermore, the null vectors $\wpar_u$
...
\begin{remark}
The reader may have noticed a slight disymmetry between the coordinates $u$
...
On the boundary of $\M_{\rm rad}$, the location of the curvature singularity (cf. Sec.~\ref{s:vai:thin:sing}) is found by taking the limit $r \to 0$ in Eq.~(\ref{e:vai:u_lnr_v}).
One gets
\be
    u = - \ln\left( \frac{v}{m} \right) + K_\alpha, \qquad
    K_\alpha := \ln \sqrt{\frac{2}{\alpha}}
        + \frac{1}{\sqrt{8\alpha - 1}} \left[
    \frac{\pi}{2}
    + \arctan\left( \frac{1}{\sqrt{8\alpha - 1}} \right)\right] .
\ee
Along a given ingoing radial null geodesic,
...
In Sec.~\ref{s:vai:infall}, we have focussed on homothetic radiation shells
...
and that $\mathscr{N}_{\rm I} \cap \mathscr{N}_{\rm II}$ is the region
...
and $\alpha < 1/8$ (low energy density case). The points $A$, $B$ and $C$
are the same as in Fig.~\ref{f:vai:diag_naked_S0}, while the point $O$
of Fig.~\ref{f:vai:diag_naked_S0} correspond to the whole segment
$v=0, r=0$ depicted by the inclined zigzag line.
The Minkowski region $\M_{\rm rad}$, radiation region $\M_{\rm rad}$
and Schwarzschild region $\M_{\rm Sch}$ are depicted in respectively
pale blue, yellow and grey. Zigzag segments indicate curvature singularities.}
\end{figure}
...
(Fig.~\ref{f:vai:CPdiag_NI_NII}) with Carter-Penrose diagrams of the Minkowski
...
In view of this, one obtains a Carter-Penrose diagram as shown in
Fig.~\ref{f:vai:CPdiag_nk_sing}.

\subsection{Naked singularity and Cauchy horizon}
