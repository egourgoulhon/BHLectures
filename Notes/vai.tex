...
\begin{hist}
Vaidya metric has been actually first derived by Henri Mineur\index{Mineur, H.} in 1933 \cite{Mineu1933},
as the solution of the Einstein equation describing the exterior of a spherically symmetric body,
...
(cf. the unumbered equation at the top of p.~35 of Mineur's article \cite{Mineu1933}).
...
constant nonzero scalar curvature (negative for $\mathbb{H}^2$ and positive for $\SS^2$).}
...
replaced by a $-$ sign, which is done in order to get Eq.~(\ref{e:vai:Mineur_metric_modern}).}
...
Only 20 years later did Prahalad Chunnilal Vaidya\index{Vaidya, P.C.} publish
...
In the foreword of Vaidya's 1951 article \cite{Vaidy51a},
...
as it appears in \emph{Ref. \cite{Mineu1933}}. Our attention
...
\emph{The line element (5) \emph{[Eq.~(\ref{e:vai:Vaidya_metric_original})]}
...
(\ref{e:vai:Mineur_metric_modern}) the \emph{Mineur-Vaidya metric}, but
we keep here the standard name of \emph{Vaidya metric}.
...
cleary reduces to Minkowski metric (\ref{e:glo:Mink_metric_spher}) wherever $M(v)=0$.
...
$\M_{\rm rad}$ (the imploding shell) is painted in yellow, $\M_{\rm Min}$ lies below it and
...
\begin{greybox}
The imploding radiation shell described by the Vaidya metric with $M(v)$ piecewise linear
...
\begin{hist}
The imploding radiation shell with $M(v)$ piecewise linear has been extensively
...
in 1975 \cite{SteinmKL75}, but this regards bodies raidating away all their masses, the exterior of which
...
In order to add the future outer trapping horizon $\mathscr{T}$
...
provided a Carter-Penrose diagram similar to that Fig.~\ref{f:vai:CPdiag_nk_sing}
except that it regards the limiting case $\alpha \to 1/8$, for which
...
for the radiation region extending to $\scri^+$ (no pure Schwarzschild exterior but
$M'(v) \to 0$ for $v\to +\infty$) (Fig. 3 in Ref.~\cite{Kurod84}).
\end{hist}
...
the naked singularity\index{naked singulariy}\index{singularity!naked --} feature noticed in Sec.~\ref{s:vai:low_alpha}:
it appears clearly
...
\defin{locally naked}\index{naked singulariy!locally --}\index{singularity!locally naked --},
while $O_1 O_2$ is qualified as
\defin{globally naked}\index{naked singulariy!globally --}\index{singularity!globally naked --}.
