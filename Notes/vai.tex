...
A \defin{shell of ingoing radiation}\index{shell!of ingoing radiation}
is defined the choice of the function $M(v)$ such that
$M'(v) \neq 0$ only on a finite interval of $v$. By choosing properly
the origin of $v$, we may consider that this interval is $[0, v_0]$, with $v_0>0$
parametrizing the thickness of the shell. Furthermore, we choose $M(v)$
so that $M(v) = 0$ for $v < 0$. We have then $M(v)$ increasing from 0 to
a constant value, $m$ say, when $v$ varies from $0$ to $v_0$ and
$M(v) = m$ for $v > v_0$.
