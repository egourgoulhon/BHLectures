...
for the homethetic shell model ($M(v) = m v/v_0 = \alpha v /2$) with
$\alpha > 1/8$ ($v_0 < 16m$).
...
\begin{greybox}
The homothetic imploding radiation shell with $\alpha := 2m/v_0 < 1/8$
generates a black hole, the event horizon of which orginates
...
general functions
$M(v)$. In particular, the criterion for having a shell-focusing singularity\index{shell-focusing singulariy}\index{singularity!shell-focusing --}
...
It is worth to stress that in the litterature,
...
motivated either by the modelization of radiating bodies or the
study of black hole evaporation. Nonetheless, most results are applicable
to Vaidya collapse, which is based on the
\emph{ingoing} version of the metric, simply by performing some time reversal.
