...
The null vector corresponding to the first solution is nothing but
the vector $\w{k}$ introduced in Sec.~\ref{s:vai:general} [cf. Eq.~(\ref{e:vai:k_IEF})].
...
for the mass function $M(v)$ shown in Fig.~\ref{f:vai:mass_function}.
...
Those of $\w{k}$ are called the \defin{ingoing radial null geodesics}\index{ingoing!null geodesic},
while those of $\wl$ are called the \defin{outgoing radial null geodesics}\index{ingoing!null geodesic}.
\end{greybox}

\begin{proof}
The geodesic character is easy for the integral curves of $\w{k}$, since the defining property (\ref{e:vai:def_k}) of the latter
yields to
\[
    k^\mu \nabla_\mu k_\alpha = - k^\mu \underbrace{\nabla_\mu \nabla_\alpha v}_{\nabla_\alpha \nabla_\mu v}
    = - k^\mu \nabla_\alpha \nabla_\mu v =  k^\mu \nabla_\alpha k_\mu
    = \frac{1}{2} \nabla_\alpha (\underbrace{k_\mu k^\mu}_{0}) = 0 .
\]
Hence $\wnab_{\w{k}} \w{k} = 0$, i.e. $\w{k}$ is a geodesic vector field. Regarding
$\wl$, a direct computation shows that it is pregeodesic:
...
\end{proof}
Some radial null geodesics are drawn in green in Fig.~\ref{f:vai:diagram}.

\begin{remark}
As in the Schwarzschild case (cf. Remark~\ref{r:sch:outgoing_ingoing} on p.~\pageref{r:sch:outgoing_ingoing}),
...
In spherical symmetry, the consideration of the radial null geodesics give us
a direct access to the black hole event horizon. It appears clearly on
