...
\be \label{e:gis:Phib_prov}
    \Phi_b(u) =  \frac{\sqrt{2}}{\sqrt{u_{\rm a} - u_{\rm n}}} \int_{\phi(u,b)}^{\pi/2}
...
\be \label{e:gis:phi_u_b}
   \encadre{ \phi(u, b) := \arcsin\left( \sqrt{\frac{u - u_{\rm n}}{u_{\rm p} - u_{\rm n}}}\right) }
     \qquad (b > b_{\rm c}) .
\ee
...
\be \label{e:gis:Phib_elliptic_int}
    ...
    \left[ K(k) - F\left( \phi(u,b) ,\; k\right) \right] } \qquad (b > b_{\rm c}) ,
\ee
...
that
\be
    0 <  \arcsin\left( \sqrt{\frac{- u_{\rm n}}{u_{\rm p} - u_{\rm n}}} \right)
     < \phi(u, b) \leq \frac{\pi}{2} ,
\ee
...
\]
In view of (\ref{e:gis:def_incompl_elliptic_int}) and (\ref{e:gis:def_K_complete}),
the first integral is $F(\pi/2,k) = K(k)$, while the second is
$F(\phi(u,b), k)$.
We have thus
