...
$u_{\rm a}$ in terms of $b$.  Note that $t$ is a decreasing function of $u$
and takes values in the range
\be \label{e:gis:t_range_b_gt_bc}
    0 \leq t \leq \sqrt{\frac{1 - u_{\rm n} /u_{\rm a}}{1 - u_{\rm n} /u_{\rm p}}} < 1,
\ee
where the last inequality follows form $u_{\rm n} < 0$ and $u_{\rm p} < u_{\rm a}$
[cf. Eq.~(\ref{e:gis:order_u_zeros})].
From
\[
...
\be
    ...
    \lim_{b\to +\infty} k = 0 .
\ee
Given the range (\ref{e:gis:t_range_b_gt_bc}) of $t$, we have
\be
    0 \leq \phi(u, b) < \arcsin\sqrt{\frac{1 - u_{\rm n} /u_{\rm a}}{1 - u_{\rm n} /u_{\rm p}}}
    < \frac{\pi}{2} ,
\ee
with $\phi(u_{\rm p}, b) = 0$.
...
from Eqs.~(\ref{e:gis:Phib_elliptic_int})-(\ref{e:gis:phi_u_b}) that
\be
...
case. The only real zero of $P_b$ is $u_{\rm n}<0$, which is the function of $b$
...
\be \label{e:gis:Pb_factor_b_lt_bc}
    P_b(u) = 2 (u - u_{\rm n}) \left[
        (u - b)^2 + A^2 - (u_{\rm n} - b)^2 \right] ,
\ee
with
\be
    b := \frac{1}{4} - \frac{u_{\rm n}}{2} \qquad\mbox{and}\qquad
    A := \sqrt{u_{\rm n}(3 u_{\rm n} - 1)} .
\ee
\begin{remark}
From the above definitions and the range (\ref{e:gis:range_un_b_lt_bc}) of
$u_{\rm n}$, we have
\[
   A^2 - (u_{\rm n} - b)^2 = \frac{1}{16} \underbrace{(2 u_{\rm n} - 1)}_{<0}
        \underbrace{(6 u_{\rm n} + 1)}_{<0} > 0 .
\]
The term inside the square brackets in Eq.~(\ref{e:gis:Pb_factor_b_lt_bc}) is
then clearly positive, in agreement with $u_{\rm n}$ being the only real
...
where
\be
    u_* := A + u_{\rm n} = \sqrt{u_{\rm n}(3 u_{\rm n} - 1)} + u_{\rm n}
\ee
\be \label{e:gis:def_Phib_b_lt_bc}
   \encadre{ \Phi_b(u) := \int_u^{u_*} \frac{\D\bar{u}}{\sqrt{P_b(\bar{u})}}
    = \int_u^{u_*} \frac{\D\bar{u}}{\sqrt{2 \bar{u}^3 - \bar{u}^2 + (m/b)^2}} }
     \quad (b<b_{\rm c})
\ee
and $\ph_*$ is the value of $\ph$ at $u=u_*$. Note that $u_*$ is a function
of $b$, via the expression (\ref{e:gis:u_n_b_lt_bc}) of $u_{\rm n}$.
It is actually a decreasing function of $b$, with,
according to the limits (\ref{e:gis:range_un_b_lt_bc}),
\be
    \lim_{b\to 0} u_* = -\infty \qquad\mbox{and}\qquad
    \lim_{b\to b_{\rm c}^-} u_* = \frac{1}{3} .
\ee
Note that $\lim_{b\to b_{\rm c}^-} u_* = \lim_{b\to b_{\rm c}^+} u_{\rm p}$,
so that there is a kind of continuity of (\ref{e:gis:def_Phib_b_lt_bc})
...
for $P_b(u)$:
\[
    \Phi_b(u) = \int_u^{A+u_{\rm n}}
    \frac{\D\bar{u}}{\sqrt{ 2 (\bar{u} - u_{\rm n}) \left[
        (\bar{u} - b)^2 + A^2 - (u_{\rm n} - b)^2 \right]}} .
\]
Performing the change of variable
\be
    t := \frac{A + u_{\rm n} - \bar{u}}{A - u_{\rm n} + \bar{u}}
    \quad\iff\quad
    \bar{u} = A \frac{1-t}{1+t} + u_{\rm n}
\ee
yields
\be
    \Phi_b(u) = \int_0^{\frac{A + u_{\rm n} - u}{A - u_{\rm n} + u}}
    \frac{\D t}{(1-t^2)\sqrt{ A \frac{1+t^2}{1- t^2} + u_{\rm n} - b }} .
\ee
...
\be \label{e:gis:Dph_F_psi_k}
    \encadre{  \Delta\ph = \eps_L \frac{2 \sqrt{2}}{\sqrt{u_{\rm a} - u_{\rm n}}}
    \, F(\phi(0, b), k) } ,
\ee
where
\be \label{e:gis:def_psi}
   \phi(0, b) = \arcsin\left( \frac{1}{k} \sqrt{\frac{u_{\rm p}}{u_{\rm a}}} \right)  .
\ee
...
\be \label{e:gis:Dph_K_F_phi_k}
    \Delta\ph = \eps_L \frac{2 \sqrt{2}}{\sqrt{u_{\rm a} - u_{\rm n}}}
    \, \left[ K(k) - F(\psi, k) \right],
\ee
with
...
\be
    \lim_{\veps\to 0} \tan \psi =  \frac{1}{\sqrt{2}} ,
\ee
which implies
\be \label{e:gis:lim_eps_zero_phi}
    \lim_{\veps\to 0} \cos\psi = \sqrt{\frac{2}{3}} \qquad\mbox{and}\qquad
    \lim_{\veps\to 0} \sin\psi = \frac{1}{\sqrt{3}} .
\ee
Now, for any $\psi\in{}[0,\pi/2)$ , we have, according to the definition
(\ref{e:gis:def_incompl_elliptic_int}) of $F$,
\be
    F(\psi, 1) = \int_0^\psi \frac{\D\vartheta}{\sqrt{1 - \sin^2\vartheta}}
        = \int_0^\psi \frac{\D\vartheta}{\cos\vartheta}
        = \ln\left( \frac{1 + \sin\psi}{\cos\psi} \right) .
\ee
In view of (\ref{e:gis:lim_eps_zero_phi}), we conclude that in the present
case
\be
    \lim_{\veps\to 0} F(\psi, k) = \ln \left(\frac{\sqrt{3} + 1}{\sqrt{2}} \right) .
\ee
