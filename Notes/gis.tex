...
described by the polar coordinates $(b,\alpha)$ and there is a unique null geodesic
moving from the black hole region to $\Obs$ that
intersects the screen orthogonally at $(\tau,b,\alpha)$.
...
A null geodesic emitted by $\Obs_{\rm em}$ reaches the asymptotic observer $\Obs$, who is located at $\ph=0$, iff $\ph_\infty = 0 \mod 2\pi$, i.e. iff
...
(\ref{e:gis:ph_inf_ph_em_lim_b_gt_bc}), while in the second
...
\be
    A(r_{\rm em})  :=
    \frac{648(2\sqrt{3} - 3)(1 - 3 u_{\rm em})}{2 + 3 u_{\rm em}
    + \sqrt{3(1 + 6 u_{\rm em})}}, \qquad
    u_{\rm em} := \frac{m}{r_{\rm em}} .
\ee
...
\]
which we may write, denoting the solution with $\eps_L=+1$ by $b_n^+$
and that with $\eps_L=-1$ by $b_n^-$,
\be
b = b_n^+ := b_{\rm c} + m A(r_{\rm em})  \mathrm{e}^{\ph_{\rm em} - 2\pi n}
\quad\mbox{or}\quad
b = b_n^- := b_{\rm c} + m A(r_{\rm em})  \mathrm{e}^{- \ph_{\rm em} - 2\pi n},\quad
n\in \mathbb{N}^* .
\ee
Via Eq.~(\ref{e:gis:b_eta}), these two sequences of the impact parameter $b$
correspond to two sequences of the emission angle $\eta$:
\begin{subequations}
\begin{align}
&\sin \eta = \sin\eta_n^+ := - \sqrt{U(r_{\rm em})}
