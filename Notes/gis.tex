...
\be \label{e:gis:phi_u_b}
   \encadre{ \phi(u, b) := \arcsin\left( \frac{1}{k} \sqrt{\frac{u_{\rm p} - u}{u_{\rm a} - u}}\right) }
     \qquad (b > b_{\rm c})
\ee
and $F(\phi, k)$ is the
...
evaluated via the elliptic integral expression (\ref{e:gis:Phib_elliptic_int})-(\ref{e:gis:phi_u_b}),
...
The first case corresponds to $0 \leq t < \frac{u_*}{u_* - 2 u_{\rm n}}$ and we
perform the change of variable $t = \cos\theta$ in the integral
...
\be \label{e:gis:def_phi_u_b_lt_bc}
   \encadre{ \phi(u, b) := \arccos\left( \frac{|u_* - u|}{u_* + u - 2 u_{\rm n}} \right) }
   \qquad (b < b_{\rm c}).
\ee
...
\]
In view of (\ref{e:gis:def_incompl_elliptic_int}),
the first integral is $F(\pi/2,k)$, while the second is
$F(\phi(u,b), k)$. Actually $F(\pi/2,k)$ receives a special name: it is
called the
\defin{complete elliptic integral of the first kind}\index{complete!elliptic integral}\index{elliptic integral!complete --} and its standard notation is $K(k)$:
...
\be \label{e:gis:Phib_u_lt_us}
    ...
    \left[ K(k) - F \left( \phi(u, b),\, k \right) \right]}
...
we perform the change of variable $t = -\cos\theta$ in order to keep
$\theta\in(0,\pi/2)$. We obtain then
\[
...
\be \label{e:gis:Phib_u_gt_us}
    \encadre{ \Phi_b(u) =  \frac{1}{\sqrt{2(u_* - u_{\rm n})}}
    \left[ F \left( \phi(u, b),\, k \right) - K(k) \right]}
...
\ee
To get the behaviour of $F(\phi(0,b),\, k)$ when both of the above limits hold, we shall
...
kind\index{complete!elliptic integral}\index{elliptic integral!complete --}
