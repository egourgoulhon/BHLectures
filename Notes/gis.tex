...
We are going to refine (\ref{e:ges:Dph_infinite}), since in
what follows, we will need the diverging behaviour of $\Delta\ph$ in terms
of $b - b_{\rm crit}$. For this, we shall rely on some properties of
elliptic integrals. The starting point is the expression
(\ref{e:gis:Dph_F_alpha_k}) of $\Delta\ph$ involving the incomplete elliptic
integral of the first kind $F(\alpha, k)$.

For $b$ close to $b_{\rm crit}$, let us introduce
...
\be \label{e:ges:b_bc_eps}
    b-b_{\rm crit} = \frac{9\sqrt{3}}{2} m \veps^2 + O(\veps^3).
\ee
...
\be
    u_{\rm neg} = - \frac{1}{6} + \frac{2}{9} \veps^2 + O(\veps^3)
    \qquad\mbox{and}\qquad
    u_{\rm apo} = \frac{1}{3} \left( 1 + \veps - \frac{2}{3} \veps^2 \right)
     + O(\veps^3) .
\ee
Substituting these values in the expression (\ref{e:gis:def_k_modulus})
of the modulus $k$ and expanding to the first order in $\veps$ leads:
\be
    k = 1 - \frac{2}{3} \veps + O(\veps^2).
\ee
