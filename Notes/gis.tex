...
Fig.~\ref{f:gis:u_per_apo_neg}. Note that $u_{\rm n} < -1/6$ and
that for $b\to b_{\rm c}^-$,
there is a smooth connection
with the value for $b\geq b_{\rm c}$, which is given by
Eq.~(\ref{e:gis:u_n_b_gt_bc}). Finally, by expanding Eq.~(\ref{e:gis:u_n_b_lt_bc}),
we get $u_{\rm n} \sim - (2b_{\rm c}/b)^{2/3} / 6$ when $b\to 0$, hence
\be
    \lim_{b\to 0} u_{\rm n} = - \infty .
\ee
