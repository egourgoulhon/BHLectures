...
\subsection{The asymptotic observer}
...
$\th=0$ (upper left), $\th=\pi/6$ (upper right), $\th=\pi/3$ (lower left)
and $\th=3/2$ (lower right).}
\end{figure}

\subsection{Image of an accretion disk}

A realistic ``source of light'' in the vicinity of a black hole is an
accretion disk\index{accretion!disk} \cite{AbramF13}. Actually most of astronomical
...
The accretion disk is a thin disk model
developed by Novikov \& Thorne \cite{NovikT73} and Page \& Thorne \cite{PageT74},
lying in the equatorial plane $\th=\pi/2$. The inner radius of the disk
...
\textsf{Gyoto}\footnote{\url{https://gyoto.obspm.fr}}\index{Gyoto} \cite{VincePGP11}.
This code integrates the null geodesic equations backward, from the observer's
screen. For Fig.~\ref{f:gis:disk_images}, the observer is located at $r=1000 m$
and at various colatitudes, ranging from $\th=0$ (disk seen face-on) to
$\th=3/2$ (disk seen almost edge-on).
The colors in Fig.~\ref{f:gis:disk_images} encode the flux at a fixed wavelength.


On the face-on view (upper-left panel of Fig.~\ref{f:gis:disk_images}), one
may distinguish three images of the accretion disk:
\begin{itemize}
...
$\Delta\ph = \pm \pi/2$ in their trajectory plane; the inner boundary of the
primary image is the image of the ISCO under these geodesics;
\item the \emph{secondary image}, which is the bright yellow ring separated
from the inner boundary of the primary image by the thick black ring; this
...
the observer and have
$\Delta\ph = \pm 3\pi/2$ in their trajectory plane;
...
the observer and have
$\Delta\ph = \pm 5\pi/2$ in their trajectory plane.
\end{itemize}
As discussed in Sec.~\ref{s:gis:images_point}, there are actually an infinite
number of images of the accretion disk but they are fainter and fainter and
only three of them are visibile in Fig.~\ref{f:gis:disk_images}. Moreover
these images are formed by exponentially thiner rings and accumulate near the circle $b = b_{\rm c}$. In Fig.~\ref{f:gis:disk_images}, the tertiary image corresponds
to $n=2$ for $\epsilon_L = 1$ and $n=1$ for $\epsilon_L = -1$
in Eq.~(\ref{e:gis:ph_inf_2pi_n}) \footnote{This asymmetry is
due to $\ph_{\rm em}\in{[0,2\pi)}$ instead
of $\ph_{\rm em}\in{(-\pi, \pi]}$.}, so that Eqs.~(\ref{e:gis:bn_sequence})
...
for the tertiary image. So we may conclude that the interior of the tertiary image
corresponds to the black hole shadow as defined in Sec.~\ref{s:gis:shadow}, up to a relative discrepancy of $10^{-3}$.

For nonzero inclination angle, the images shown in Fig.~\ref{f:gis:disk_images}
...
Note that for the three views with $\th\not=0$, the primary, secondary and
...
by Karl Schwarzschild\index{Schwarzschild, K.} in January 1916 \cite{Schwa1916} and
Johannes Droste\index{Droste, J.} in May 1916 \cite{Drost1917}, but these two
...
\emph{null} geodesics is due to Ludwig Flamm\index{Flamm, L.} in September
...
The photon circular orbit at $r=3m$ has been exhibited by David Hilbert\index{Hilbert, D.}
in December 1916 \cite{Hilbe1917a,Hilbe1917b}. Hilbert has also discussed the
critical null geodesics ($b = b_{\rm c}$, cf. Sec.~\ref{s:gis:crit_geod}) and their spirals
...
by Max von Laue\index{von Laue, M.} published in 1921 \cite{Laue1921}.
In addition, von Laue noted that a sphere of radius $r_0$ such that $2m < r_0 < 3m$ will
...
Darwin \cite{Darwi59} (multiple images of a point source, with accumulation just outside the critical radius), Synge \cite{Synge66}, Bardeen \cite{Barde73}, Luminet \cite{Lumin79,Lumin19}, Marck \cite{Marck96}
