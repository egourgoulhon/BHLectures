...
We shall refine (\ref{e:ges:Dph_infinite}) in Sec.~??

\subsection{Total change in $\ph$ from some emission point}
...
considered above is then a special case: that for which the first point is
infinitely far from the black hole.

\subsubsection{Case $b > b_{\rm c}$}
...
to its value $\ph_{\rm em}$ at $\tlamb = \tlamb_{\rm em}$.
...
\be
   \encadre{ \ph_\infty = \ph_{\rm em} + \eps_L \frac{\sqrt{2}}{\sqrt{u_{\rm a} - u_{\rm n}}}
    \left[ F\left(\phi_b(u_{\rm em}), k\right)
           - F\left(\phi_b(0), k \right) \right] } \qquad (\tlamb_{\rm em} > \tlamb_{\rm p}) ,
\ee
...
integral of the first kind $F(\phi_b(0), k)$. As already noticed, we
