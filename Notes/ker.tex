...
implies that $\wpar_{\hat{r}}$ is a global null vector on $\M$.
We may then use it to set the time orientation of $(\M,\w{g})$ by demanding that
\be
    \w{k} := - \wpar_{\hat{r}}
\ee
is a future-directed null vector in all $\M$. The minus sign
...
The field lines
of $\wpar_{k}$ are future-directed null curves,
...
immediate consequence of $-r$ being a future-oriented parameter along $\Li^{\rm in}_{(v,\th,\tph)}$.
...
with respect to the ingoing family is that $r$ is a future-oriented affine
parameter along outgoing principal null geodesics only in regions
...
Let us show $\M_{\rm I}$ lies entirely in the causal past of the future null infinity, $J^{-}(\scri^+)$, i.e.
