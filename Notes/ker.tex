...
\subsection{The zero-angular-momentum observers (ZAMO)} \label{s:ker:ZAMO}
...
Sec.~\ref{s:ker:time_orientation}. Indeed, the latter is set by the global null
...
\[
    ...
    \ \mbox{on} \ \M_{\rm ZAMO} .
\]
From the lemma stated in Sec.~\ref{s:fra:time_orientation}, we conclude that
$\w{N}$ is a future-directed timelike vector everywhere on $\M_{\rm ZAMO}$.
...
\be
    \w{a} = \vw{\nabla}\ln N + (\w{\nabla}_{\w{n}} \ln N) \w{n} .
\ee
In view of expression (\ref{e:ker:lapse_ZAMO}) of $N$, we have $\w{a}\neq 0$. In other words, the
...
\subsection{The asymptotic inertial observers}
