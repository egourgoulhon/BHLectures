...
These two families correspond to \defin{stationary observers}\index{stationary!observer in Kerr spacetime}:
they evolve at fixed $r$ and $\th$, with a 4-velocity $\w{u}$ that is a
linear combination of the Killing vectors $\w{\xi}$ and $\w{\eta}$:
\be
    \w{u} = u^t(r,\th) \, \w{\xi} + u^\ph(r,\th) \, \w{\eta} .
\ee
Since $u^t(r,\th)$ and $u^\ph(r,\th)$ are constant along the worldline $\Li$
of a given observer,
it follows that the spacetime geometry as perceived by the observer does not evolve
(in other words, $\Li$ is an orbit of the isometry group $\R\times\mathrm{SO}(2)$ of Kerr spacetime),
hence the qualifier \emph{stationary} given to such observers.
...
means that $\Obs$'s 4-velocity $\w{n}$ is nothing but $\w{N}$ rescaled to form a unit vector,
namely
\be \label{e:ker:def_n_ZAMO}
    \w{n} := N \w{N} = - N \vw{\nabla} t ,
\ee
with\footnote{Note that $N$ is not defined as the (pseudo)norm of $\w{N}$, as the notation might suggest, but rather as the inverse of it.} $N := (-\w{N}\cdot\w{N})^{-1/2}$. In view of expression (\ref{e:ker:w_square}), we get
...
$(\w{e}^{(\alpha)})_{0\leq \alpha\leq 3}$ that constitutes, at each point $p\in\Li$, a dual basis of $(\w{e}_{(\alpha(p)})$, namely $(\w{e}^{(\alpha)})$ obeys
...
\be
    \encadre{ \omega := \frac{\D\ph}{\D t} = \frac{2 a m r}{\rho^2(r^2 + a^2) + 2 a^2 m r \sin^2\th} }.
\ee
...
defined}, we mean that the above definition of $L$  fully makes sense for a \emph{geodesic},
for which it leads to
a conserved quantity along the worldline (cf. Sec.~\ref{s:ges:fiom}), and we are going to see
that a ZAMO's worldline is not a geodesic. However, it shares with $L=0$ geodesics
the property $\D\ph/\D t= \omega$.

...

Actually, the
name \emph{locally nonrotating observer}, initially given to a ZAMO (cf. historical note below),
better conveys the specificity of such an observer: a ZAMO $\Obs$ does not feel an important
rotation effect, namely the \emph{Sagnac effect}\index{Sagnac effect}:
the proper time measured by $\Obs$ for a light signal to perform a full circuit on a circle
...
\ee
In view of expression (\ref{e:ker:lapse_ZAMO}) for $N$, we conclude that\footnote{An explicit expression of $\w{a}$
in terms of $(r,\th)$ can be found in Eqs.~(70)-(71) of Ref.~\cite{Semer93} or Eqs. (A.9) and (A.10) of Ref.~\cite{BiniGJ17}.} $\w{a}\neq 0$. In other words, the
