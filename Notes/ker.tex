...
\be \label{e:ker:J_Komar_partial_der}
    ...
    \sqrt{q}\, \D x^2 \, \D x^3 } .
\ee
As above, let us choose for $\Sp$ a 2-sphere $\{t=\mathrm{const},\; r=\mathrm{const}\}$.
In evaluating the terms $s^\mu \partial_\mu \eta_\nu \, n^\nu$
and $n^\mu \partial_\mu \eta_\nu \, s^\nu$ as $r\rightarrow+\infty$, we have
be a little more cautious than in Sec.~\ref{s:ker:Komar_mass}, since
the components $\eta_\alpha$ are diverging when $r\rightarrow+\infty$:
\[
...
$g^{t\ph} \sim - 2 a m / r^3$ when $r\rightarrow+\infty$, so that
\[
...
$\uu{s} = (\rho/\sqrt{\Delta})\, \dd r$, so that $\w{s}$ is normal to $\Sp$.
