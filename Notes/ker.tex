...
\ee
Along a given ZAMO worldline $\Li$, the coordinate $\ph$ evolves according to
$\D\ph/\D t |_\Li= {\D \ph}/{\D \tau} \times {\D \tau}/{\D t} = {n^\ph}/{n^t} =
...
\ee
Now, we have seen in Sec.~\ref{s:ges:circular_orbits} that $\omega$ defined in this way [cf. Eq.~(\ref{e:ges:def_Omega})] is nothing
but the angular velocity around the rotation axis as measured by an asymptotically distant static observer.
...
\ee
Note that on the $r>0$ side (resp. $r<0$ side), $\w{e}_{(r)} = (\sqrt{\Delta}{r})\wpar_r$ is oriented outward (resp. inward) with respect to the black hole. Hence, in the asymptotic region of $\M_{\rm I}$,
a ZAMO feels some radial Newtonian gravitational field $\w{g} = -\w{a}$ towards the black hole, while
in the asymptotic region of $\M_{\rm III}$, a ZAMO feels a gravitational field away from the black hole.
This is of course in agreement with the negative mass aspect of the Kerr metric in region $\M_{\rm III}$,
as discussed in Sec.~\ref{s:ker:basic_prop}.
...
hence the name \emph{zero-angular momentum observer}. Indeed, the observer's specific ``angular momentum'', losely defined by $\ell := \w{\eta}\cdot\w{n}$ [cf. Eq.~(\ref{e:ges:def_eps_ell})], is identically
...
gyroscopes axes. For the ZAMO observer, the 4-rotation turns out to be\footnote{See the notebook~\ref{s:sam:Kerr_ZAMO_frame}; see also
Eqs.~(73)-(74) of Ref.~\cite{Semer93}.}
\be
   \encadre{ \w{\omega}_{\rm rot} = - \frac{2a^3 m r\sqrt{\Delta} \cos\th\sin^2\th}{\rho^3[\rho^2(r^2 + a^2) + 2 a^2 m r \sin^2\th]}\, \w{e}_{(r)}
    + \frac{am[\rho^2(a^2 - r^2) - 2 r^2(r^2 + a^2)]\sin\th}{\rho^3[\rho^2(r^2 + a^2) + 2 a^2 m r \sin^2\th]}\, \w{e}_{(\th)} }.
\ee
Hence, as soon as $a\neq 0$, $\w{\omega}_{\rm rot} \neq 0$.
Far from the black hole, $\rho\sim |r|$ and we get
\be
     \w{\omega}_{\rm rot} \underset{r\to\pm\infty}{\sim} - \frac{3 a m \sin\th}{|r|^3} \, \w{e}_{(\th)} .
\ee
In particular, in the equatorial plane $(\th=\pi/2)$, $\w{\omega}_{\rm rot} \sim 3 a m/|r|^3 \, \w{e}_z$,
...
$\w{\Omega} = 0$ for the ZAMO congruence\footnote{Incidently, we also get $\uu{a} = \dd \ln N + \alpha \, \uu{n}$
and Eq.~(\ref{e:ker:acc_ZAMO}) shows that $\alpha$ is actually zero.}.
Physically, this means that if each ZAMO sets up a orthonomal spatial frame $(\w{e'}_i)$ with axes aligned along
