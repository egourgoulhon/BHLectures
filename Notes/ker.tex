...
\subsection{The zero-angular-momentum observers (ZAMO)}
...
\ee
\begin{proof}
The infinitesimal vector that relates the point of coordinate $t$ to that of coordinate $t+\D t$ along $\Obs$'s worldine is $\D \w{x} = \D\tau\,  \w{n}$ (by the very definition of the 4-velocity $\w{n}$, compare Eq.~(\ref{e:fra:def_u})). We have then, using Eq.~(\ref{e:ker:def_n_ZAMO}),
\[
