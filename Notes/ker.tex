...
Let us use the Komar mass expression~(\ref{e:sta:M_Komar_partial_der}) to compute it
for the Kerr spacetime. For this purpose, we shall consider the Boyer-Lindquist coordinates
...
$r\rightarrow +\infty$, $\sqrt{q} = r^2\sin\th$. Hence (\ref{e:sta:M_Komar_partial_der})
yields
\[
...
\begin{remark}
Besides the sign and the change $\w{\xi}\leftrightarrow\w{\eta}$, we notice a difference by a factor 2 between the r.h.s. of (\ref{e:sta:def_Komar_mass}) and (\ref{e:ker:def_Komar_J}).
