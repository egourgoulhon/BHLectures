...
gyroscopes axes. For the ZAMO observer, the 4-rotation turns out to be\footnote{See the notebook~\ref{s:sam:Kerr_ZAMO_frame} for the computation; see also
...
we note also that expression~(\ref{e:ker:Carter_frame_0}) for $\veps_{(0)}$ agrees with the
general form (\ref{e:ker:4vel_stationary_obs}) of a stationary observer's 4-velocity.

The algebraic dual (cf. Sec.~\ref{s:bas:linear_form} in Appendix~\ref{s:bas}) of the Carter frame,
called the \defin{Carter coframe}\index{Carter!coframe}
...
of Kerr spacetime (cf. Sec.~\ref{s:ker:principal_geod}). Indeed, by adding and subtracting
