...
\subsubsection{Ingoing principal null geodesics}
...
\subsubsection{Outgoing principal null geodesics}
...
from $\el_\alpha = g_{\alpha\mu} \el^\mu$, with $g_{\alpha\mu}$ given by
Eq.~(\ref{e:ker:metric_Kerr_3p1}):
...
Lemma~2 of Sec.~\ref{s:fra:time_orientation}: the scalar product of
...
it has indeed simple components in the 3+1 Kerr frame:
...
being nothing but $\Hor$ itself, we recover the fact that $\el$ is normal
to $\Hor$.
\end{remark}
...
\ee
i.e. the same Newtonian behaviour as for the ZAMO 4-acceleration [Eq.~(\ref{e:ker:acc_ZAMO_asymp})],
...
the Carter observer is more bound to the spacetime structure, being closely related to
