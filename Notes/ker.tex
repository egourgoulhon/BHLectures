...
$(t,r,\th,\ph)$. This is easily acheived
...
vector field $\w{k} := -\wpar_{\hat{r}}$, once expressed in terms of 3+1 Kerr
basis via (\ref{e:ker:frame_Kerr3p1_Kerr}), is
\be
...
and that the parameter associated with $\w{k}$, i.e. $-r$, is an
affine parameter of this geodesic. The curves $\Li^{\rm in}_{(v,\th,\tph)}$
...
immediate consequence of $-r$ being a parameter along $\Li^{\rm in}_{(v,\th,\tph)}$.
...
The ingoing principal null geodesics form a \emph{congruence}: through
...
congruence of principal null geodesics, called the \defin{outgoing principal null geodesics}\index{principal!null geodesic}. The fact that the Weyl tensor $\w{C}$
...
$\D u = 0$ and $\D\tilde{\tph}=0$, so that
\begin{align}
& \frac{\D \ti}{\D r} =   \frac{r^2 + 2m r + a^2}{\Delta}  \nonumber \\
& \frac{\D \tilde{\tph}}{\D r} = \frac{2a}{\Delta} . \nonumber
\end{align}
In order to have a parametrization regular on $\Hor$ and $\Hor_{\rm in}$,
where $\Delta=0$, let us introduce along $\Li^{\rm out}_{(u,\th,\tilde{\tph})}$
...
parameter $-r$ associated to $\w{k}$ is affine.
...
\ee
with [cf. Eq.~(\ref{e:ker:bar_kappa})]
\[
