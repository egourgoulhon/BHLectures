...
\subsection{Stationary observers}
...
in any coordinate system adapted to the spacetime symetries, like the Boyer-Lindquist ones
$(t,r,\th,\ph)$, the Kerr ones $(v,r,\th,\tph)$ or the 3+1 Kerr ones $(\ti, r, \th, \tph)$,
according to
...
and $u^\ph = \alpha\omega$. Hene $\D\ph/\D t |_\Li=\omega$.
The same demonstration applies to Kerr and 3+1 Kerr coordinates because
...
observer can exist only where $\w{\xi}$ is timelike, i.e. outside the ergoregion (cf. Sec.~\ref{s:ker:ergoregion}).

Since static observers are not very usefull for describing physical processes
...
The ZAMO rotation velocity seen at infinity $\omega$ is obtained
by comparing Eqs.~(\ref{e:ker:4vel_stat_obs_omega}) and Eq.~(\ref{e:ker:par_t_3p1})
rewritten as $\w{n} = N^{-1}(\wpar_t - \beta^\ph \, \wpar_\ph)$. We thus get
