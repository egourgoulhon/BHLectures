...
null curves. We shall therefore denote them by $\Li_{(v,\th,\tph)}$.
We shall see below that $\Li_{(v,\th,\tph)}$ is actually a null geodesic.
...
a very simple form (cf. Appendix~\ref{s:sam} for the computation):
...
Kerr coordinates (cf. Appendix~\ref{s:sam} for the computation):
...
extra non-diagonal terms: ${\tilde g}_{\ti r}$ and ${\tilde g}_{r \ti}$. However
...
\section{Event horizon and Cauchy horizon}
...
According to the discussion in Sec.~\ref{s:glo:strong_rigidity}, $\Omega_H$
is called the
