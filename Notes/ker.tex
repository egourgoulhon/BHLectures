...
congruence of principal null geodesics, called the \defin{outgoing principal null geodesics}\index{principal!null geodesic!outgoing --}\index{outgoing!principal null geodesic}. As $-r$ was a affine parameter along
...
These two families correspond to the more general concept of a
\defin{stationary observer}\index{stationary!observer in Kerr spacetime},
which is defined as an observer in Kerr spacetime whose 4-velocity $\w{u}$ is a linear
...
\ee
It follows that the worldline $\Li$ of a stationary observer is an
orbit of the isometry group $\R\times\mathrm{SO}(2)$ of Kerr spacetime. In physical terms,
this means that the spacetime geometry as perceived by the observer does not evolve.

A stationary observer moves necessarily at fixed values of non-ignorable coordinates
in a coordinate system adapted to the spacetime symmetries, like the Boyer-Lindquist or Kerr
coordinates. For instance, considering the Boyer-Lindquist coordinates $(t, r, \th, \ph)$,
...
- \beta^\ph$, where we have used (\ref{e:ker:par_t_3p1}) to get ${n^\ph}/{n^t} = - \beta^\ph$.
We have thus
...
but the angular velocity around the symmetry axis as measured by an asymptotically distant static observer.
...
It is related to the Boyer-Lindquist coordinate coframe $(\dd x^\alpha)$ by
\begin{subequations}
...
this makes the Carter frame well taylored to the computation of the Riemann's curvature tensor
via Cartan's formula (see Refs.~\cite{Carte73a,ONeil95}).

The angular velocity around the symmetry axis of a Carter observer
as measured by an asymptotically distant static observer\footnote{cf. the discussion for the ZAMO
case below Eq.~(\ref{e:ker:omega_ZAMO}).} is
\be \label{e:ker:omega_Carter}
    \encadre{ \omega_{\rm C} := \left. \frac{\D\ph}{\D t} \right| _\Li  = \frac{a}{r^2 + a^2} } .
\ee
\begin{proof}
We have $\D\ph/\D t |_\Li= {\D \ph}/{\D \tau} \times {\D \tau}/{\D t}$, with, by the
definition (\ref{e:fra:def_u}) of a 4-velocity, ${\D \ph}/{\D \tau} = \veps_{(0)}^\ph$
and ${\D t}/{\D \tau} = \veps_{(0)}^t$. Reading the components $\veps_{(0)}^t$ and
$\veps_{(0)}^\ph$ on Eq.~(\ref{e:ker:Carter_frame_0}) leads to the result.
\end{proof}
...
(Eqs.~(81)-(82) in Ref.~\cite{Carte68b}). The explicit form for Kerr spacetime can be found in Carter's lecture at the famous Les Houches School (1972) \cite{Carte73a} (Eqs. (5.19)-(5.22) and (5.31)-(5.34)).
The Carter frame has been popularized by Roman Znajek\index{Znajek, R.L.} in 1977 \cite{Znaje77} for
studying electromagnetic processes around a black hole. Carter frame is heavily used in
Barrett O'Neill's\index{O'Neill, B.} monograph about Kerr black holes \cite{ONeil95}, where it is called the
\emph{Boyer-Lindquist frame field}.
\end{hist}
