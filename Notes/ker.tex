...
By means of computer algebra (cf. the SageMath notebook~\ref{s:sam:Kerr_solution}),
one easily checks:
\begin{tprop}{Kerr metric as a solution to the vacuum Einstein equation}{}
The Kerr metric $\w{g}$ [Eq.~(\ref{e:ker:metric_BL})] is a solution to the vacuum
...
\begin{tprop}{asymptotic flatness of Kerr spacetime}{}
...
\begin{tprop}{stationarity and axisymmetry of Kerr spacetime}{}
...
is asymptotically timelike. Given Definition~\ref{d:sta:stationary}, we conclude that the Kerr spacetime is stationary.
...
Hence, according to Definition~\ref{d:sta:axisym}, the Kerr spacetime is axisymmetric.
\end{proof}

\begin{tprop}{non-staticity for $a\neq 0$}{}
For $a\neq 0$, as we have assumed in (\ref{e:ker:a_lower_m}), the
Kerr spacetime is not static (cf. Definition~\ref{d:sta:static}):
...
\begin{tprop}{Schwarzschild limit}{}
...
\begin{tprop}{the double flat disk at $r=0$}{}
...
\begin{tprop}{ergoregion of Kerr spacetime}{}
...
\begin{tprop}{Carter time machine}{}
...
\section{Extension of the spacetime manifold through \texorpdfstring{$\Delta=0$}{Δ=0}}
...
\begin{tprop}{time orientation of Kerr spacetime}{}
...
\begin{tprop}{$\wpar_r$ past-directed in $\M_{\rm II}$}{ker:partial_r_MII_past}
In region $\M_{\rm II}$, the vector $\wpar_r$ of Boyer-Lindquist coordinates
is a past-directed timelike vector.
\end{tprop}

\begin{proof}
Applying Lemma~\ref{p:fra:lem2} with $\w{u}=\w{k}$ and
...
\begin{tprop}{decreasing of $r$ in $\M_{\rm II}$}{ker:r_decreasing_M_II}
...
According to Property~\ref{p:ker:partial_r_MII_past}, $-\wpar_r$ is a future-directed timelike vector
...
Property~\ref{p:ker:r_decreasing_M_II}.
...
\begin{tprop}{outgoing principal null geodesics}{}
...
Using Lemma~\ref{p:fra:lem2} with $\w{u}=\w{k}$ and
$\w{v} = \wl$, we conclude
\begin{tprop}{$\wl$ future-directed}{ker:el_future_directed}
...
\begin{tprop}{causal type of the hypersurfaces of constant $r$}{}
\begin{itemize}
...
In view of Definition~\ref{d:neh:Killing_horizon}, we conclude:

\begin{tprop}{$\Hor$ as a Killing horizon}{ker:H_Killing}
...
\ee
thereby arriving at:
\begin{tprop}{$\Hor_{\rm in}$ as a Killing horizon}{}
...
\begin{tprop}{black hole region in Kerr spacetime}{}
...
(Property~\ref{p:sta:strong_rigidity_thm}): the
...
we get
\begin{tprop}{null generators of the event horizon}{}
...
Similarly, we have
\begin{tprop}{null generators of the inner horizon}{}
...
\begin{tprop}{non-degeneracy of the event horizon}{}
...
Example~\ref{x:def:Schw_hor3} [cf. Eq.~(\ref{e:def:kappa_Schw_hor})].
\end{remark}
...
area of any of its cross-sections (cf. Property~\ref{p:neh:invariance_area}).
To compute $A$, we shall not use Boyer-Lindquist
...
\be \label{e:ker:q_cross_section_H}
  ...
  (r_+^2 + a^2\cos^2\th) \, \dd \th^2
    ...
    \sin^2\th \, \dd \tph^2 .
\ee
The metric determinant is $q := \det (q_{ab}) = (r_+^2 + a^2)^2 \sin^2\th$, which yields
the following expression for the area of the cross-section $\Sp$:
\[
...
we may recast this result to let appear only $m$ and $a$:
...
\]
which, by virtue of Lemma~\ref{p:fra:lem2},
...
(cf. Definition~\ref{d:sta:axisym});
...
\begin{tprop}{non-degeneracy of the inner horizon}{}
...
\begin{tprop}{Cauchy horizon as part of a bifurcate Killing horizon}{}
...
(see Ref.~\cite{BurkoKZ16} for a more recent study of this instability).
