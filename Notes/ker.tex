...
Contrary to $\Obs$'s worldline, the curves of fixed $(r,\th, \ph)$, the tangent vector
...
\be
    \encadre{ \wpar_t = N \, \w{n} + \w{\beta} }.
\ee
By combining Eqs.~(\ref{e:ker:def_n_ZAMO}), (\ref{e:ker:lapse_ZAMO}) and (\ref{e:ker:normal_ZAMO}), we
get
\be
    \w{\beta} = - \frac{2 a m r}{\rho^2(r^2 + a^2) + 2 a^2 m r \sin^2\th}\, \wpar_\ph .
\ee

The family of \defin{zero-angular-momentum observers (ZAMO)}\index{zero-angular-momentum observer}\index{ZAMO}
is a congruence of observers filling $\M_{\rm ZAMO}$, whose worldlines are normal to the
hypersurfaces $\Sigma_t$ of constant Boyer-Lindquist time $t$ and whose orthonormal frame
$\w{e}_{(\alpha)}$ is
related to the Boyer-Lindquist coordinate frame $(\wpar_\alpha)$ by
\begin{subequations}
\begin{align}
  & \w{e}_{(0)} = \w{n} =
...
under the name of \defin{locally nonrotating observers}\index{locally!nonrotating observer}\index{nonrotating!observer} for generic stationary and axisymmetric spacetimes. The application
to the exterior part of Kerr spacetime has been performed by Bardeen, William H. Press and Saul A. Teukolsky
in 1972 \cite{BardePT72}. The name \emph{ZAMO} is due to  Kip S. Thorne and Douglas MacDonald
\cite{ThornM82} (1982).
\end{hist}
