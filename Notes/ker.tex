...
better conveys the specificity of such an observer: it does not feel any rotation effect.
For instance, he does not notice any Sagnac effect\index{Sagnac effect} (see Ref.~\cite{Barde70} for details), nor any
change of orientation of gyroscopes with respect to its frame. The geometrical translation of this
nonrotating feature is the fact
that the congruence of the ZAMOs' worldlines is hypersurface-orthogonal, being orthogonal to the
$\Sigma_t$ foliation at each point.

Even if it is not rotating, a ZAMO is not an inertial observer: it is not in free-fall, since its 4-acceleration $\w{a} := \w{\nabla}_{\w{n}}\, \w{n}$
...
can be found in Appendix~A of Ref.~\cite{BiniGJ17} [Eqs. (A.9) and (A.10)].} $\w{a}\neq 0$. In other words, the
