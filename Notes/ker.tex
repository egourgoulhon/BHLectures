...
largely because of the no-hair theorem (cf. Sec.~\ref{s:sta:no-hair}),
according to which
all stationary black holes in the Universe are Kerr black holes.
...
\be \label{e:ker:a_lower_m}
    0 < a < m ,
\ee
postponing the case $a=m$ to Chap.~\ref{s:ext}.
...
$v$ by $\delta v = 2m$. Note that some members of the outgoing principal null
family are tangent to the two horizons $\Hor$ and $\Hor_{\rm in}$; they are actually
generators of them (cf. Sec.~\ref{s:gek:null_gen_hor} and \ref{s:ker:Cauchy_hor}).
}
\end{figure}
...
$\uu{k}$ in terms of the Boyer-Lindquist coordinate coframe:
...
$\uu{k}$ in terms of the 3+1 Kerr coordinates [Eqs.~(\ref{e:ker:k_ti_tr})
and (\ref{e:ker:k_form_Kerr})] are perfectly regular on all $\M$.
\end{remark}
...
where it appears clearly that at $r=r_+$ the outgoing principal null geodesics are
tangent to $\Hor$.
...
\section{Observers in Kerr spacetime}

The concept of \emph{observer} has been recalled in Sec.~\ref{s:fra:measure}.
...
only where the hypersurface $\Sigma_t$ is spacelike, so that it has
a timelike normal (cf. Sec.~\ref{s:def:hor_as_null}). A normal 1-form to $\Sigma_t$ is
of course $\dd t$. The associated normal vector to $\Sigma_t$, $\w{N}$ say, is obtained
...
In components:
\[
   N^\alpha = - g^{\alpha\mu} (\dd t)_\mu = - g^{\alpha\mu} \delta^t_{\ \, \mu} = - g^{t\alpha} .
\]
...
\ee
The scalar square of the normal vector is
...
Indeed, up to a $\sin^2\th \geq 0$ factor, it coincides with
the expression (\ref{e:ker:eta_square}) of $\w{\eta}\cdot\w{\eta}$.
...
The observer $\Obs$ is chosen so that $\w{N}$ is tangent to its worldline
at all points.
The 4-velocity $\w{n}$ of $\Obs$ is then nothing but $\w{N}$ rescaled to a unit vector,
namely
\be \label{e:ker:def_n_ZAMO}
    \w{n} := N \w{N} = - N \vw{\nabla} t ,
\ee
with\footnote{Note that $N$ is not defined as the (pseudo)norm of $\w{N}$, but rather as the inverse of it.} $N := (-\w{N}\cdot\w{N})^{-1/2}$. In view of expression (\ref{e:ker:w_square}), we get
...
\ee
\begin{proof}
The infinitesimal vector that connects the point of proper time $\tau$ to that of proper time $\tau+\D\tau$ along $\Obs$'s worldine is $\D \w{x} = \D\tau\,  \w{n}$ (by the very definition of the 4-velocity $\w{n}$, compare Eq.~(\ref{e:fra:def_u})). The corresponding increment in the coordinate $t$ is given by
...
is an observer $\Obs$ of the above type, i.e. whose worldline is normal to the
...
$\w{e}_{(\alpha)}$ is an orthonormal basis.

Each ZAMO can be characterized by its coordinates $(r_0,\th_0,\ph_0)$ at some fixed
value $t_0$ of the Boyer-Lindquist time $t$. The ZAMO's form thus a 3-parameter family of observers
filling $\M_{\rm ZAMO}$.
The coordinates $(r,\th,\ph)$ span each hypersurface $\Sigma_t$.
Contrary to the ZAMO's worldlines, the curves of fixed $(r,\th, \ph)$, the tangent vector
of which is $\wpar_t = \w{\xi}$, are not orthogonal to $\Sigma_t$, except for $a=0$. The orthogonal
decomposition of $\wpar_t$ into a part along the normal $\w{n}$ and a part tangent to $\Sigma_t$
defines the \defin{shift vector}\index{shift vector} $\w{\beta}$:
\be \label{e:ker:par_t_3p1}
    \encadre{ \wpar_t = N \, \w{n} + \w{\beta} }, \qquad \w{n}\cdot\w{\beta} = 0 .
\ee
From Eq.~(\ref{e:ker:ZAMO_frame_0}), we get
\be
    \w{\beta} = - \frac{2 a m r}{\rho^2(r^2 + a^2) + 2 a^2 m r \sin^2\th}\, \wpar_\ph .
\ee
Along a given ZAMO worldline, the coordinate $\ph$ evolves according to
$\D\ph/\D t = {\D \ph}/{\D \tau} \times {\D \tau}/{\D t} = {n^\ph}/{n^t} =
- \beta^\ph$, where we have used (\ref{e:ker:par_t_3p1}) to write ${n^\ph}/{n^t} = - \beta^\phi$.
When seen from infinity,
the ZAMO is thus rotating around the black hole at the angular speed
\be
    \omega := \frac{\D\ph}{\D t} = \frac{2 a m r}{\rho^2(r^2 + a^2) + 2 a^2 m r \sin^2\th} .
\ee
...
At first glance, this rotating behaviour might seem contradictory with
the name \emph{zero-angular momentum observer}. This denomination arises because
the observer specific ``angular momentum'', losely defined by $L := \w{\eta}\cdot\w{n}$ [cf. Eq.~(\ref{e:ges:conserved_angu_mom})], is identically
zero, the observer's 4-velocity $\w{n}$ being orthogonal to $\w{\eta} = \wpar_\ph$. By \emph{losely
defined}, we mean that the above definition of $L$  makes fully sense for a geodesic, as it leads to
a conserved quantity along the worldline (cf. Sec.~\ref{s:ges:fiom}), and we are going to see
that a ZAMO's worldline is not geodesic. Actually, the
name \emph{locally nonrotating observer}, initially given to a ZAMO (cf. historical note below),
better conveys the specificity of such an observer: a ZAMO $\Obs$ does not feel some rotation effects.
For instance, $\Obs$ does not notice any \emph{Sagnac effect}\index{Sagnac effect},
namely the proper time measured by $\Obs$ for a light signal to perform a full circuit on a circle
at $r=\mathrm{const}$, $\th=\mathrm{const}$ is the same for a forward (increasing $\ph$) motion
of the signal as for a backward (decreasing $\ph$) one (see Ref.~\cite{Barde70} for details).
Another property of the ZAMOs, as a family, is to form a zero-vorticity congruence. This follows from
the property of being orthogonal to a family of hypersurfaces, namely $(\Sigma_t)_{t\in\R}$.

Even if it is ``locally nonrotating'', a ZAMO is not an inertial observer: it is not in free-fall, since its 4-acceleration $\w{a} := \w{\nabla}_{\w{n}}\, \w{n}$
is nonzero. Indeed, it can be shown that $\w{a}$ is the orthogonal projection onto $\Sigma_t$ of
the gradient of the logarithm of the lapse function (see e.g. Eq.~(4.19) in Ref.~\cite{Gourg12}):
\be
    \w{a} = \vw{\nabla}\ln N + (\w{\nabla}_{\w{n}} \ln N) \w{n} .
\ee
In view of expression (\ref{e:ker:lapse_ZAMO}) of $N$, we have\footnote{An explicit expression of $\w{a}$
can be found in Eqs.~(70)-(71) of Ref.~\cite{Semer93} or Eqs. (A.9) and (A.10) of Ref.~\cite{BiniGJ17}.} $\w{a}\neq 0$. In other words, the
ZAMO's worldline is not a geodesic (cf. Eq.~(\ref{e:geo:geod_eq_v}) in Appendix~\ref{s:geo}) and
if it does not feel any rotation, a ZAMO feels some acceleration (``weight''), the closer to the black hole, the stronger it is.
...
\subsection{Carter observers}

A \defin{Carter observer}\index{Carter!observer}\index{observer!Carter --}
is an observer $\Obs$ defined in the region $\M_{\rm I}\cup \M_{\rm III}$ of Kerr spacetime, whose
...
\end{align}
\end{subequations}
We note that the frame $(\w{e}_\alpha)$ is well defined because $\Delta > 0$ in $\M_{\rm I}\cup \M_{\rm III}$.
It is an easy exercice to check that $(\w{\veps}_\alpha)$ is an orthonormal frame.
...
that $\veps_{(0)}^\alpha = \D x^\alpha / \D\tau$, which proves that $\Li$ is
timelike and $\veps_{(0)}$ is the 4-velocity of observer $\Obs$.

The dual (cf. Sec.~\ref{s:bas:linear_form} in Appendix~\ref{s:bas}) of the Carter frame
is the basis 1-forms
\begin{subequations}
\begin{align}
...
the form of the dual coframe for a quite general class of spacetimes including the Kerr one
(Eqs.~(81)-(82) in Ref.~\cite{Carte68b}). The explicit form for Kerr spacetime can be found in Carter's famous lecture at Les Houches School (1972) \cite{Carte73a} (Eqs. (5.19)-(5.22) and (5.31)-(5.34)).
\end{hist}

\subsection{Asymptotic inertial observers}
